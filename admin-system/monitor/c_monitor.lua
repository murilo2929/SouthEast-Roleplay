...
function showMonitorWindow(content, ableToRemove)
	...
	bRemove = guiCreateButton(10,405,201,22,"REMOVER",false, wMonitor)
	bEdit = guiCreateButton(211,405,214,22,"EDITAR",false,wMonitor)
	bCancel = guiCreateButton(425,405,201,22,"CANCELAR",false,wMonitor)
...
function removePlayerFromList(button, state)
	if button == "left" and state == "up" then
		...
		if (row==-1) or (col==-1) then
			outputChatBox("Selecione um jogador primeiro!", 255, 0, 0)
		else
			...
			if player then
				...
				outputChatBox("Não existe tal jogador (mais)...", 255, 0, 0)
			end
		end
	end
end
		...
		if (row==-1) or (col==-1) then
			...
			if player then
				if setClipboard ( tostring(guiGridListGetItemText(monitorList, guiGridListGetSelectedItem(monitorList), 3)) ) then
					outputChatBox("Conteúdo do monitor copiado para a área de transferência.")
				end
			else
				outputChatBox("Não existe tal jogador (mais)...", 255, 0, 0)
			end
...
function showoMonitorAdd()
	...
	guiLogIn = guiCreateWindow ( X , Y , Width , Height , "Adicionar alguém ao sistema de monitoramento" , false )
	
	guiUserLabel  = guiCreateLabel(0.05, 0.15, 0.9, 0.2, "Usuario Exato:", true, guiLogIn)
	guiUserNameEdit = guiCreateEdit(0.05, 0.25, 0.9, 0.2, "", true, guiLogIn)
	guiPasswordLabel  = guiCreateLabel(0.05, 0.45, 0.9, 0.2, "Razão:", true, guiLogIn)
	guiPasswordEdit = guiCreateEdit(0.05, 0.55, 0.9, 0.2, "", true, guiLogIn)

	guiLogInBackButton = guiCreateButton(0.15, 0.8, 0.3, 0.2, "Cancelar", true, guiLogIn)
	guiLogInSubmitButton = guiCreateButton(0.55, 0.8, 0.3, 0.2, "Proximo", true, guiLogIn)
...
function showoMonitorAdd2()
	...
	guiUserLabel = guiCreateLabel(11,22,64,22,"Usuario:",false,guiLogIn)
	...
	guiLogInBackButton = guiCreateButton(11,50,109,20,"Cancelar",false,guiLogIn)
	guiLogInSubmitButton = guiCreateButton(120,50,111,20,"Proximo",false,guiLogIn)
