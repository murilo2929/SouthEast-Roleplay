function doMonitorList(sourcePlayer, command, targetPlayerName, ...)
	if (exports.integration:isPlayerTrialAdmin(sourcePlayer) or exports.integration:isPlayerSupporter(sourcePlayer)) then
		if not targetPlayerName then
			...
			if not ... then
				outputChatBox("SYNTAX: /" .. command .. " [player] [rasão]", sourcePlayer, 255, 194, 14)
			else
				...
				if targetPlayer then
					...
					if exports.mysql:query_free("UPDATE account_details SET monitored = '" .. exports.mysql:escape_string(reason) .. "' WHERE account_id = " .. exports.mysql:escape_string(accountID)) then
						...
						outputChatBox("Você adicionou " .. getPlayerName(targetPlayer):gsub("_", " ") .. " para a lista de monitores.", sourcePlayer, 0, 255, 0)
					end
				end
			end
		end
...
addEventHandler("monitor:onSaveEdittedMonitor", getRootElement( ),
	function (sourcePlayer, username, monitorContent, targetPlayerName1)
		...
		if exports.mysql:query_free("UPDATE account_details SET monitored = '" .. exports.mysql:escape_string(reason) .. "' WHERE account_id = '" .. exports.mysql:escape_string(exports.cache:getIdFromUsername(username)).."'") then
			exports.anticheat:changeProtectedElementDataEx(targetPlayer, "admin:monitor", reason, false)
			outputChatBox("[MONITOR] Você atualizou " .. username .. " para a lista de monitores.", sourcePlayer, 0, 255, 0)
			doMonitorList(sourcePlayer, "monitor") 
			local staffTitle = exports.global:getPlayerAdminTitle(sourcePlayer)
			exports.global:sendMessageToAdmins("[MONITOR] "..staffTitle.." "..staffUsername.." monitor modificado no jogador '"..targetPlayerName.."' ("..monitorContent..").")
			exports.global:sendMessageToSupporters("[MONITOR] "..staffTitle.." "..staffUsername.." monitor modificado no jogador '"..targetPlayerName.."' ("..monitorContent..").")
		else
			outputChatBox("[MONITOR] Falha ao atualizar " .. username .. " para a lista de monitores.", sourcePlayer, 255, 0, 0) 
		end
	end	
)
		...
		if not ... then
			...
			if name then
				...
				if exports.mysql:query_free("UPDATE account_details SET monitored = '" .. exports.mysql:escape_string(reason) .. "' WHERE account_id = " .. exports.mysql:escape_string(uid)) then
					exports.anticheat:changeProtectedElementDataEx(targetPlayer, "admin:monitor", reason, false)
					outputChatBox("Você adicionou " .. username .. " para a lista de monitores.", sourcePlayer, 0, 255, 0)
					local staffTitle = exports.global:getPlayerAdminTitle(sourcePlayer)
					exports.global:sendMessageToAdmins("[OMONITOR] "..staffTitle.." "..staffUsername.." adicionou um monitor offline no jogador '"..username.."' ("..reasonTemp..").")
					exports.global:sendMessageToSupporters("[OMONITOR] "..staffTitle.." "..staffUsername.." adicionou um monitor offline no jogador '"..username.."' ("..reasonTemp..").")
				end
			end
		end
	end
	...
	if (exports.integration:isPlayerTrialAdmin(sourcePlayer) or exports.integration:isPlayerSupporter(sourcePlayer)) then
		if not ... then
			...
			if name then
				...
				if exports.mysql:query_free("UPDATE account_details SET monitored = '" .. exports.mysql:escape_string(reason) .. "' WHERE account_id = " .. exports.mysql:escape_string(uid)) then
					exports.anticheat:changeProtectedElementDataEx(targetPlayer, "admin:monitor", reason, false)
					outputChatBox("Você adicionou " .. name["username"] .. " para a lista de monitores.", sourcePlayer, 0, 255, 0)
				end
			end
		end
	end
	...
	function( )
		if exports.integration:isPlayerTrialAdmin(client) or exports.integration:isPlayerSupporter(client) then
			...
			if exports.mysql:query_free("UPDATE account_details SET monitored = '' WHERE account_id = " .. exports.mysql:escape_string(accountID)) then
				...
				outputChatBox("Você removeu " .. getPlayerName(source):gsub("_", " ") .. " da lista de monitor.", client, 0, 255, 0)
				
				local staffTitle = exports.global:getPlayerAdminTitle(client)
				exports.global:sendMessageToAdmins("[MONITOR] "..staffTitle.." "..staffUsername.." monitor removido no jogador '"..playerUsername.."'.")
				exports.global:sendMessageToSupporters("[MONITOR] "..staffTitle.." "..staffUsername.." monitor removido no jogador '"..playerUsername.."'.")
