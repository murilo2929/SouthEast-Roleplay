		...
		if veh then
			...
			if intID then
				...
				if foundIntID and keyType then
					...
					outputChatBox(" Você não está em nenhum veículo ou possível interior.", thePlayer, 255,0 ,0 )
					return false
				end
			end
		end
...
function generateFakeIdentity(player, cmd)
	if exports.integration:isPlayerLeadAdmin(player) then
		if getElementData(player, "fakename") then
			...
			outputChatBox("Identidade falsa removida.",player)
			return false
		end
		...
		outputChatBox("Identidade falsa ativada.",player)
