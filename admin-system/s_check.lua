function doCheck(sourcePlayer, command, ...)
	if (exports.integration:isPlayerTrialAdmin(sourcePlayer) or exports.integration:isPlayerSupporter(sourcePlayer)) then
		if not (...) then
			outputChatBox("SYNTAX: /" .. command .. " [Nome Parcial Player / ID] ou [Usuario]", sourcePlayer, 255, 194, 14)
		else
			...
			if (checkTarget) then
				...
				if (logged==0) then
					outputChatBox("jogador não conectado.", sourcePlayer, 255, 0, 0)
				else
				...
				dbQuery(function(qh, offlineTarget)
					...
					if accountData_core then 
						...
						outputChatBox("Conta/Player '"..offlineTarget.."' não encontrado", sourcePlayer, 255, 0, 0)
					end
					dbFree(qh)
...
function savePlayerNote(dbid, username, text)
	if exports.integration:isPlayerTrialAdmin(client) or exports.integration:isPlayerSupporter(client) then
		if dbid then 
			...
			if result then
				outputChatBox( "Nota para o " .. username .. " Tem sido atualizado.", client, 0, 255, 0 )
			else
				outputChatBox( "A atualização da nota falhou.", client, 255, 0, 0 )
			end
		else
			outputChatBox( "Incapaz de obter ID da conta.", client, 255, 0, 0 )
		end
	end
end
...
function showOfflineAdminHistory( gameaccountid, name )
	if (exports.integration:isPlayerTrialAdmin( source ) or exports.integration:isPlayerSupporter( source )) and tonumber(gameaccountid) then
		...
		if result then
			...
			outputChatBox( "Falha ao recuperar o histórico.", source, 255, 0, 0)
		end
	end
end
...
function removeAdminHistoryLine(ID)
	...
	if sqlQuery then
		...
		if source then
			outputChatBox("Entrada de histórico de administração #"..ID.." removido", source, 0, 255, 0)
...
addCommandHandler( "historico",
	function( thePlayer, commandName, ... )
		if not (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
			if (...) then
				outputChatBox("Apenas administradores ou Supporters podem verificar o histórico de administração de outros jogadores.", thePlayer, 255, 0, 0)
				return false
			end
		end
