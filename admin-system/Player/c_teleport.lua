...
function openLocationManager(locationData)
    ...
    gui.manager = guiCreateWindow(0, 0, w, h, "Gerente de locais de teletransporte", false)
    ...
    gui.colValue = guiGridListAddColumn(gui.grid, "Valor", 0.2)
    gui.colDesc = guiGridListAddColumn(gui.grid, "Descrição", 0.4)
    gui.colCreator = guiGridListAddColumn(gui.grid, "Criador", 0.2)
    ...
    for v, k in pairs(data) do
        ...
        guiGridListSetItemText(a, b, gui.colCreator, exports.cache:getUsernameFromId(k["location_creator"]) or "Desconhecido", false, false)
    end

    gui.addNew = guiCreateButton(10, 205, 71, 35, "Add Novo", false, gui.manager)
    ...
    gui.deleteSelected = guiCreateButton(87, 205, 71, 35, "Deletar", false, gui.manager)
    ...
    addEventHandler("onClientGUIClick", gui.deleteSelected, function ()
            ...
            if r ~= -1 and c ~= -1 then
                ...
                exports.hud:sendBottomNotification(localPlayer, "Gerente de localização", "Selecione um local da lista primeiro.")
            end
    end, false)

    gui.closeWindow = guiCreateButton(299, 205, 91, 35, "Fechar", false, gui.manager)
...
function openNewLocation()
    ...
    if canManage(localPlayer) then
        ...
        local w, h = 400, 150
        local status = "Sua posição atual será usada para este marcador."
        local c1, c2, c3 = 255, 255, 255 --white
        local r = 1
        local failures = 0
        local fool = getElementData(localPlayer, "account:username")

        gui.newLocation = guiCreateWindow(0, 0, w, h, "Teleport Mark Criador", false)
        exports.global:centerWindow(gui.newLocation)

        gui.valueLabel = guiCreateLabel(10, 53, 54, 15, "Valor", false, gui.newLocation)
        gui.valueInput = guiCreateEdit(10, 28, 150, 25, "", false, gui.newLocation)
        checkElementUse(gui.valueInput)

        gui.descLabel = guiCreateLabel(164, 53, 118, 15, "Descrição da localização", false, gui.newLocation)
        ...
        gui.newCreate = guiCreateButton(240, 110, 150, 30, "Criar", false, gui.newLocation)
