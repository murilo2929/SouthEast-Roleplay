...
function openWeaponCreatorMain(commandName, category)
	if exports.integration:isPlayerHeadAdmin(localPlayer) and getElementData(localPlayer, "loggedin") == 1 then
		...
		wWeaponsMain = guiCreateWindow(wWeaponsMain_x,wWeaponsMain_y,560,230,"Admin Weapon Creator v2.0 ",false)
			guiWindowSetSizable(wWeaponsMain,false)
			lCredit1 = guiCreateLabel(10,28,300,18,"Criador de arma de administrador",false,wWeaponsMain)
			...
			GUIEditor_Label[2] = guiCreateLabel(30,114,102,18,"Armas",false,wWeaponsMain)
				guiSetFont(GUIEditor_Label[2],"default-bold-small")
			boxWeapons =  guiCreateComboBox(100,55,116,21,"Nenhum",false,wWeaponsMain)
			...
			lAmmo = guiCreateLabel(300,114,102,18,"Munição",false,wWeaponsMain)
				guiSetFont(lAmmo,"default-bold-small")
			boxAmmo = guiCreateComboBox(360,55,116,21,"Nenhum",false,wWeaponsMain)
				guiComboBoxAdjustHeight (  boxAmmo, 30 )
				guiComboBoxAddItem(boxAmmo, "Nenhum")
			...
			eAmmoPerClip = guiCreateEdit(450,90,85,17,"-1 (Cheio)",false,wWeaponsMain)
			...
			guiCreateLabel(13,153,326,28,"Jogador para o qual você deseja gerar a arma (nome ou ID parcial):",false,wWeaponsMain)
			...
			bSpawnc = guiCreateButton(285,190,100,30,"SPAWN & FECHAR",false,wWeaponsMain)
				guiSetFont(bSpawnc,"default-bold-small")
				guiSetEnabled(bSpawnc, false)
			bClose = guiCreateButton(395,190,100,30,"CANCELAR",false,wWeaponsMain)
...
function switchToFullMode()
	...
	eWeaponBox = guiCreateEdit(100,55,116,21,"Clique duas vezes com o botão esquerdo na lista",false,wWeaponsMain)
		guiEditSetReadOnly(eWeaponBox,true)
	eAmmoBox = guiCreateEdit(360,55,116,21,"Clique duas vezes com o botão direito na lista abaixo",false,wWeaponsMain)
	...
	local lhelp = guiCreateLabel(30,185,250,50,"- Clique duas vezes com o botão esquerdo para gerar a arma\n- Double right click to spawn ammunition\n- Clique com o botão do meio para copiar as informações da arma",false,wWeaponsMain)
	...
	local columnName = guiGridListAddColumn( gunChart1, "Nome", 0.3)
	local columnAmmoPerClip = guiGridListAddColumn( gunChart1, "Ammo/Clip", 0.1)
	local columnDamage = guiGridListAddColumn( gunChart1, "Dano", 0.1)
	local columnRange = guiGridListAddColumn( gunChart1, "Range", 0.1)
	local columnAccuracy = guiGridListAddColumn( gunChart1, "Precisão", 0.1)
	local columnFiringSpeed = guiGridListAddColumn( gunChart1, "Cadencia de Tiro", 0.15)
			...
			if button == "middle" then
				...
				if row ~= -1 and col ~= -1 then
					...
					if setClipboard (tmp) then
						--outputChatBox( "[WEAPON CREATOR] Informação da arma copiada para a área de transferência.")
						guiSetText(lCoolDown, "Informação da arma copiada para a área de transferência.")
					...
					--outputChatBox( "[WEAPON CREATOR] Você precisa selecionar uma arma primeiro.", 255, 0, 0 )
					guiSetText(lCoolDown, "Você precisa selecionar uma arma primeiro.")
			...
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					...
					if not targetPlayer then 
						guiSetText(lCoolDown, "Insira o jogador alvo.")
						guiLabelSetColor(lCoolDown, 255,255,0)
						setTimer(function ()
							guiSetText(lCoolDown, "")
						end,2000,1)
					elseif not tonumber(guiGetText( eWeapons)) then
						guiSetText(lCoolDown, "Quantidade inválida de armas.")
						guiLabelSetColor(lCoolDown, 255,255,0)
						setTimer(function ()
							guiSetText(lCoolDown, "")
						end,2000,1)
					...
					outputChatBox( "[WEAPON CREATOR] Você precisa escolher uma arma", 255, 0, 0 )
				end
			end
			...
			if button == "right" then
				...
				if row ~= -1 and col ~= -1 then
					...
					elseif not tonumber(guiGetText( eAmmo)) then
						guiSetText(lCoolDown, "Quantidade inválida de ammopacks.")
						guiLabelSetColor(lCoolDown, 255,255,0)
						setTimer(function ()
							guiSetText(lCoolDown, "")
						end,2000,1)
...
function showGunChart()
	if (exports.integration:isPlayerTrialAdmin(getLocalPlayer())) then
		showCursor(true)
		if wWeaponsMain then 
			outputChatBox( "[WEAPON CREATOR] Por favor, feche o Criador de Arma primeiro.", 255,0,0)
		elseif wWeaponsChart then
			...
			wWeaponsChart = guiCreateWindow(wWeaponsMain_x,wWeaponsMain_y,560,450,"Tabela de armas de administrador",false)
				guiWindowSetSizable(wWeaponsChart,false)
			bCloseChart = guiCreateButton(500,20,70,20,"FEchar",false,wWeaponsChart)
				guiSetFont(bCloseChart,"default-bold-small")
				...
				function copyToClipBoard( button )
					if button == "left" then
						...
						if row ~= -1 and col ~= -1 then
							...
							if weapDamage ~= "" then
								tmp = tmp..", Dano: "..weapDamage
							end
							if weapAccuracy ~= "" then
								tmp = tmp..", Precisão: "..weapAccuracy
							end
							if weapFiringSpeed ~= "" then
								tmp = tmp..", Cadencia de Tiro: "..weapFiringSpeed.."."
							end
							if setClipboard (tmp) then
								outputChatBox( "[WEAPON CREATOR] Informação da arma copiada para a área de transferência.")
								-- guiSetText(lCoolDown, "Informação da arma copiada para a área de transferência.")
...
function checkNameExists(theEditBox)
	...
	if (count>1) then
		...
		guiSetText(lError, "Múltiplos encontrados.")
		guiLabelSetColor(lError, 255, 255, 0)
	elseif (count==1) then
		isTargetPlayerValid = true
		targetPlayerName = getPlayerName(found)
		guiSetText(lError, "Player Encontrado: " .. getPlayerName(found) .. " (ID #" .. getElementData(found, "playerid") .. ")")
...
function validateWeaponAmount(theEditBox)
	if tonumber(guiGetText(theEditBox)) == nil then
		guiSetText(lErrorWeap, "Deve ser númerico!")
...
function validateAmmoAmount(theEditBox)
	if tonumber(guiGetText(theEditBox)) == nil then
		guiSetText(lErrorAmmo, "Deve ser númerico!")
...
function validateAmmoPerClip(theEditBox)
	if tonumber(guiGetText(theEditBox)) == nil then
		guiSetText(lErroreAmmoPerClip, "Deve ser númerico!")
