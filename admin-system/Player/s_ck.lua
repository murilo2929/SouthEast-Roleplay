...
function ckPlayer(thePlayer, commandName, targetPlayer, ...)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) or not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador / ID] [Causa da Morte]", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					...
					outputChatBox("Seu personagem tomou CK por " .. getPlayerName(thePlayer) .. ".", targetPlayer, 255, 194, 14)
					showChat(targetPlayer, false)
					outputChatBox("Você deu CK em ".. targetPlayerName ..".", thePlayer, 255, 194, 1)
					exports.logs:dbLog(thePlayer, 4, targetPlayer, "CK razão: "..mysql:escape_string(tostring(info)))
...
function unckPlayer(thePlayer, commandName, ...)
	if (exports.integration:isPlayerAdmin(thePlayer)) then
		if not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Completo Jogador]", thePlayer, 255, 194, 14)
		else
			...
			if (mysql:num_rows(result)>1) then
				outputChatBox("Muitos resultados - Insira um nome mais exato.", thePlayer, 255, 0, 0)
			elseif (mysql:num_rows(result)==0) then
				outputChatBox("Jogador não existe ou não levou CK.", thePlayer, 255, 0, 0)
			else
				...
				outputChatBox(targetPlayer .. " não esta mais com CK.", thePlayer, 0, 255, 0)
...
addCommandHandler("removerck", unckPlayer)
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (...) then
			...
			if (mysql:num_rows(result)>1) then
				outputChatBox("Muitos resultados - Insira um nome mais exato.", thePlayer, 255, 0, 0)
			elseif (mysql:num_rows(result)==0) then
				outputChatBox("Jogador não existe.", thePlayer, 255, 0, 0)
			else
				local row = mysql:fetch_assoc(result)
				local dbid = tonumber(row["id"]) or 0
				local cked = tonumber(row["cked"]) or 0
				if cked == 0 then
					outputChatBox("Jogador não esta com CK.", thePlayer, 255, 0, 0)
				elseif cked == 2 then
					outputChatBox("O jogador já está enterrado.", thePlayer, 255, 0, 0)
				else
					...
					outputChatBox(targetPlayer .. " foi enterrado.", thePlayer, 0, 255, 0)
		...
		if not (...) then
			...
			elseif (mysql:num_rows(result)==0) then
				...
				elseif cked == 2 then
					...
					triggerEvent('sendAme', thePlayer, "coloca ".. tostring(targetPlayer) .." para descançar.")
...
addCommandHandler("enterrar", buryPlayer)
		...
		if not (...) then
			...
			if (mysql:num_rows(result1)==0) then
				...
				if cked == 0 then
					outputChatBox("Jogador não esta com CK.", thePlayer, 255, 0, 0)
				elseif cked == 2 then
					outputChatBox("Você não pode mover alguém que já está enterrado.", thePlayer, 255, 0, 0)
				else
					...
					if isIn and not exports.integration:isPlayerTrialAdmin(thePlayer) then
						triggerEvent('sendAme', thePlayer, "move o corpo de ".. tostring(targetPlayer) ..".")
						outputChatBox(targetPlayer .. " foi movido.", thePlayer, 0, 255, 0)
...
addCommandHandler("movercorpo", movePlayer)
