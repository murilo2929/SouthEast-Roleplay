...
function reconPlayer(commandName, targetPlayer)
	...
	if getElementData(localPlayer, "loggedin") == 1 and (exports.integration:isPlayerTrialAdmin(localPlayer)) then
		...
		if not (targetPlayer) then
			if not reconx then
				return outputChatBox("SYNTAX: /" .. commandName .. " [Nick Parcial Player]", 255, 194, 14)
			end
			if toggleRecon(false) then
				reconTargets = {}
				pointer = 0
				outputChatBox("Recon desligado.", 0, 255, 0)
			end
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(localPlayer, targetPlayer)
			if not targetPlayer then
				return outputChatBox("Player não encontrado.", 255, 194, 14)
			end
			...
			if isElement(newtarget) then
				...
				outputChatBox("A pessoa que você tentou assistir está reconectando "..targetPlayerName..", seu pedido foi transferido.", 255, 194, 14)
			end

			if toggleRecon(true, targetPlayer) then
				outputChatBox("Agora assistindo " .. targetPlayerName .. ".", 0, 255, 0)
...
addCommandHandler("spec", reconPlayer)
...
function getTarget(order)
	if #reconTargets < 2 then
		return false, "Por favor /spec mais jogadores para poder trocar entre eles."
	end
	if order == "arrow_r" then
		...
		if not reconTargets[pointer] then
			...
			if not target then
				...
				return false, "Este jogador acabou de sair."
			else
...
function displayReconInfo()
    ...
    dxDrawText("Alvo: "..exports.global:getPlayerFullIdentity(reconTarget,3,true), 517+xo, 406+yo, 887+xo, 423+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    ...
    dxDrawText("Arma: "..weapon, 517+xo, 440+yo, 706+xo, 457+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    dxDrawText("Colete: "..math.floor( getPedArmor( reconTarget ) ), 706+xo, 423+yo, 887+xo, 440+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    ...
    dxDrawText("Dinheiro: $"..exports.global:formatMoney(getElementData(reconTarget, "money")), 517+xo, 457+yo, 706+xo, 474+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    dxDrawText("Banco: $"..exports.global:formatMoney(getElementData(reconTarget, "bankmoney")), 706+xo, 457+yo, 887+xo, 474+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    local veh = "Pressione a seta para a esquerda/direita para alternar entre os alvos ("..tableToString(reconTargets)..")"
    local vehicle = getPedOccupiedVehicle( reconTarget )
    if vehicle then
    	veh = "Veículo: " .. exports.global:getVehicleName( vehicle ) .. " (" ..getVehicleName( vehicle ).." - ID #"..getElementData( vehicle, "dbid" ) .. " - HP: "..math.floor( getElementHealth( vehicle ))..")"
    end
	...
	if dim > 0 then
		loc = "Dentro do interior ID #"..dim
	end
    dxDrawText("Facção: "..text, 887+xo, 406+yo, 1257+xo, 423+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, true, true, false, false)
    dxDrawText("Local: "..loc, 1076+xo, 440+yo, 1257+xo, 440+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    ...
    dxDrawText("Dimensão: "..dim, 1076+xo, 457+yo, 1257+xo, 457+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
    local hoursplayed = getElementData(reconTarget, "hoursplayed")
    hoursplayed = tonumber(hoursplayed) or "Desconhecido"
    dxDrawText("Horas jogadas: "..hoursplayed, 887+xo, 474+yo, 1076+xo, 474+yo, tocolor(255, 255, 255, 255), 1.00, "default", "left", "top", false, false, true, false, false)
