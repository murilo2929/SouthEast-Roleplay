...
function deleteLocation(thePlayer, id)
    if canManageTPS(thePlayer) then
        ...
        if not loc or not loc["location_value"] or not checkLocationDuplication(loc["location_value"]) then
            exports.hud:sendBottomNotification(thePlayer, "Location Manager", "A linha selecionada não existe no banco de dados ou já foi excluída.")
            return nil
        end

        if exports.mysql:query_free("DELETE FROM `admin_teleports` WHERE `id`='"..id.."'") then
            exports.hud:sendBottomNotification(thePlayer, "Location Manager", "Você excluiu a localização "..loc["location_value"].." com sucesso.")
            updateLocationData()
        else
...
function teleportToLocation(thePlayer, commandName, target, optionalPlayer)
    if (exports.integration:isPlayerTrialAdmin(thePlayer) or
        ...
        if not (target) then
			outputChatBox("SYNTAX: /" .. commandName .. " [lugar] [jogador para se teletransportar (opcional)]", thePlayer, 255, 194, 14)
            outputChatBox("Todos locais podem ser vistos usando /tps.", thePlayer, 255, 194, 14)
		...
		elseif not optionalPlayer and target then
            ...
            if locs[target] then
                ...
                outputChatBox("Você se teletransportou para " .. tostring(locVal) .. ".", thePlayer, 0, 255, 0)
				...
				outputChatBox("Local inválido inserido, consulte os locais em /tps.", thePlayer, 255, 0, 0)
			end
		elseif optionalPlayer and target then
			...
			if targetPlayer then
				...
				if (logged==0) then
					outputChatBox("Jogador não conectado!", thePlayer, 255, 0 , 0)
				elseif locs[target] then
                    ...
                    locInt = locs[target]["int"]
                    locDim = locs[target]["dim"]
					outputChatBox("Você foi teletransportado para "..tostring(locVal).." por "..exports.global:getPlayerFullIdentity(thePlayer)..".", targetPlayer, 255, 194, 14)
					outputChatBox("Você teletransportou "..exports.global:getPlayerFullIdentity(targetPlayer).." para "..tostring(locVal)..".", thePlayer, 255, 194, 14)
					...
					outputChatBox("Local inválido inserido, consulte /places para ajuda.", thePlayer, 255, 0, 0)
				end
			end
		else
...
function gotoPlayer(thePlayer, commandName, target)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerVCTMember(thePlayer) or exports.integration:isPlayerMappingTeamMember(thePlayer) then
		if commandName:lower() == "goto" or commandName:lower() == "tp" then
			if not (target) then
				...
				if targetPlayer then
					...
					if (logged==0) then
						...
						if not (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerVCTMember(thePlayer)) and (exports.integration:isPlayerMappingTeamMember(thePlayer)) then
                            ...
                            if isPedInVehicle(thePlayer) then
                                outputChatBox("Você não pode estar em um veículo ao usar este comando.", thePlayer, 255, 0, 0)
                                return 
                            end

                            if targetPlayer ~= reporter then
                                outputChatBox("Você só pode se teletransportar para o jogador que está reportando.", thePlayer, 255, 0, 0)
                                return
                            end
                            ...
                            outputChatBox(username.." se teletransportou para você.", targetPlayer)
                            return 
                        end
						...
						outputChatBox(" Você se teletransportou para o jogador " .. targetPlayerName .. ".", thePlayer)
						
						if exports.integration:isPlayerSupporter(thePlayer) then
							outputChatBox(" Supporter " .. username .. " se teletransportou para você. ", targetPlayer)
						else
							local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
							if hiddenAdmin == 0 then
								outputChatBox(" Admin " .. username .. " se teletransportou para você. ", targetPlayer)
							end
						end
					end
				end
			...
			if (logged==0) then
				...
				outputChatBox(" Você aceitou a solicitação de teletransporte de " .. getPlayerName(target):gsub("_", " ") .. ".", thePlayer)
				
				if exports.integration:isPlayerSupporter(thePlayer) then
					outputChatBox(" Supporter " .. username .. " aceitou seu pedido de teletransporte. ", target)
				else
					outputChatBox(" Admin " .. username .. " aceitou seu pedido de teletransporte.", target)
				end
			end
		end
	end
...
addCommandHandler("tp", gotoPlayer, false, false)
...
function getPlayer(thePlayer, commandName, from, to)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerVCTMember(thePlayer) then
		if(not from or not to) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Jogador] [Para Jogador]", thePlayer, 255, 194, 14)
		else
			...
			if(fromplayer and toplayer) then
				...
				if(not logged1 or not logged2) then
					outputChatBox("Pelo menos um dos jogadores não está logado.", thePlayer, 255, 0 , 0)
				else
					...
					if (playerAdmLvl < targetAdmLvl) then
						outputChatBox("Enviando "..targetPlayerName.." solicitação de teletransporte, pois eles têm uma classificação mais alta do que você.", thePlayer, 255, 194, 14)
						outputChatBox(getPlayerName(thePlayer):gsub("_", " ").." quer te teletransportar para eles. /atp para aceitar, /dtp para recusar.", targetPlayer, 255, 194, 14)
					...
					outputChatBox(" Você teletransportou o jogador " .. targetPlayerName1:gsub("_"," ") .. " para " .. targetPlayerName2:gsub("_"," ") .. ".", thePlayer)

					local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
					if hiddenAdmin == 0 then
						outputChatBox(" Um administrador " .. admin .. " teletransportou você para " .. targetPlayerName2:gsub("_"," ") .. ". ", fromplayer)
						outputChatBox(" Um administrador " .. admin .. " teletransportou " .. targetPlayerName1:gsub("_"," ") .. " até você.", toplayer)
					else
						outputChatBox(" Um administrador oculto teletransportou você para " .. targetPlayerName2:gsub("_"," ") .. ". ", fromplayer)
						outputChatBox(" Um administrador oculto teletransportou " .. targetPlayerName1:gsub("_"," ") .. " até você.", toplayer)
					end
				end
			end
		end
...
addCommandHandler("enviarjogador", getPlayer, false, false)
...
function getPlayer(thePlayer, commandName, target)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerScripter(thePlayer) then
		if not target then
			outputChatBox("SYNTAX: /" .. commandName .. " /trazer [Nome Parcial Jogador]", thePlayer, 255, 194, 14)
		else
			local username = getPlayerName(thePlayer)
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, target)
			if targetPlayer then
				...
				if (logged==0) then
					...
					outputChatBox(" Você teletransportou o jogador " .. targetPlayerName .. " até você.", thePlayer)

					if exports.integration:isPlayerSupporter(thePlayer) then
						outputChatBox(" Supporter " .. username .. " teletransportou você para ele. ", targetPlayer)
					else
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
						if hiddenAdmin == 0 then
							outputChatBox(" Admin " .. username .. " teletransportou você para ele. ", targetPlayer)
						end
					end
...
addCommandHandler("trazer", getPlayer, false, false)

function acceptTeleport(thePlayer)
	local targetPlayer = false
	targetPlayer = getElementData(thePlayer, "teleport:targetPlayer")
	if not targetPlayer then
		outputChatBox("Você não tem nenhum pedido de teletransporte pendente.",thePlayer,255,0,0)
	else
...
function denyTeleport(thePlayer)
	...
	if not targetPlayer then
		...
		outputChatBox(" Você negou a solicitação de teletransporte de " .. getPlayerName(targetPlayer):gsub("_", " ").. ".", thePlayer)
		outputChatBox(" "..getPlayerName(thePlayer):gsub("_", " ").." negou seu pedido de teletransporte.", targetPlayer)
		removeElementData(thePlayer, "teleport:targetPlayer")
	end
end
...
function osendtoLS(thePlayer, commandName, ...)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerScripter(thePlayer) then
		if not (...) then
            ...
            outputChatBox("Jogador offline enviado para Pershing Square.", thePlayer, 0, 255, 0)
            else
...
addCommandHandler("enviarls", osendtoLS, false, false)