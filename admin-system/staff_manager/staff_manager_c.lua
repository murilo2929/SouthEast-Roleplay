...
function openStaffManager(staffInfo1, staffTeams1, globalChangelogs1) --/ MAXIME
	if canPlayerAccessStaffManager(localPlayer) then
		...
		if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
	        ...
	        GUIEditor.tab.home = guiCreateTab("Gerenciar STAFF", GUIEditor.tabpanel[1])

	        GUIEditor.label[1] = guiCreateLabel(10, 16, 56, 28, "Procurar:", false, GUIEditor.tab.home)
	        ...
	        addEventHandler("onClientGUIChanged", GUIEditor.edit[1], function()
	        	if source == GUIEditor.edit[1] then
	        		...
	        		if string.len(text) >= 3 then
		        		local count = 0
		        		guiSetText(GUIEditor.label[2], "Procurando..")
		        		...
		        		timer.username = setTimer(function()
		        			...
		        			if username then
		        				guiSetText(GUIEditor.label[2], "Encontrado: "..username..". Solicitando mais informações do servidor..")
		        				guiLabelSetColor ( GUIEditor.label[2], 0,255,0 )
		        				killTimerIfExisted(timer.username)
		        				triggerServerEvent("staff:getStaffInfo", localPlayer, username)
		        			else
		        				if count > 4 then
		        					guiSetText(GUIEditor.label[2], "Usuario não encontrado.")
	        ...
	        GUIEditor.label[2] = guiCreateLabel(10, 44, 306, 23, "Por Favor digite o usuario exato", false, GUIEditor.tab.home)
	        ...
	        GUIEditor.label[12] = guiCreateLabel(10, 197+yExtend, 74, 22, "Logs de Mudanças:", false, GUIEditor.tab.home)
	        ...
	        GUIEditor.gridcol.date = guiGridListAddColumn(GUIEditor.gridlist[1], "Data", 0.25)
	        GUIEditor.gridcol.type = guiGridListAddColumn(GUIEditor.gridlist[1], "Tipo", 0.1)
	        GUIEditor.gridcol.from = guiGridListAddColumn(GUIEditor.gridlist[1], "De", 0.2)
	        GUIEditor.gridcol.to = guiGridListAddColumn(GUIEditor.gridlist[1], "Para", 0.2)
	        GUIEditor.gridcol.by = guiGridListAddColumn(GUIEditor.gridlist[1], "Por", 0.1)
	        GUIEditor.gridcol.details = guiGridListAddColumn(GUIEditor.gridlist[1], "Detalhes", 0.1)

	        GUIEditor.button[1] = guiCreateButton(411, 16, 122, 28, "Enviar", false, GUIEditor.tab.home)
	        ...
	        for i, team in ipairs(staffTitles) do
		        ...
		        GUIEditor.gridcolStaff[i].reports = guiGridListAddColumn(GUIEditor.gridlistStaff[i], "Contagem de Reports", 0.1)
		        ...
		        GUIEditor.labelStaff[i] = guiCreateLabel(0, 0, 1, 1, "Carregando..", true, GUIEditor.tabStaff[i])
		        ...
		        addEventHandler( "onClientGUIDoubleClick", GUIEditor.gridlistStaff[i], function(button, state)
		            ...
		            elseif button == "right" then
		               ...
		               if setClipboard(rank.." "..username.." "..reports) then
		               		outputChatBox("Copiado '"..rank.." "..username.." "..reports.."' para a área de transferência. Clique duas vezes com o botão direito para editar essse staff.")
		               end
	        ...
	        GUIEditor.gridcolChangeLogs.date = guiGridListAddColumn(GUIEditor.gridlist.changelogs, "Data", 0.22)
	        GUIEditor.gridcolChangeLogs.type = guiGridListAddColumn(GUIEditor.gridlist.changelogs, "Tipo", 0.1)
	        GUIEditor.gridcolChangeLogs.username = guiGridListAddColumn(GUIEditor.gridlist.changelogs, "Username", 0.1)
	        GUIEditor.gridcolChangeLogs.from = guiGridListAddColumn(GUIEditor.gridlist.changelogs, "De", 0.18)
	        GUIEditor.gridcolChangeLogs.to = guiGridListAddColumn(GUIEditor.gridlist.changelogs, "Para", 0.18)
	        GUIEditor.gridcolChangeLogs.by = guiGridListAddColumn(GUIEditor.gridlist.changelogs, "Por", 0.1)
	        GUIEditor.gridcolChangeLogs.details = guiGridListAddColumn(GUIEditor.gridlist.changelogs, "Detalhes", 0.08)
	        guiSetVisible(GUIEditor.gridlist.changelogs, false)
	        GUIEditor.label.cloading = guiCreateLabel(0, 0, 1, 1, "Carregando..", true, GUIEditor.tab.changelogs)
	        ...
	        addEventHandler( "onClientGUIDoubleClick", GUIEditor.gridlist.changelogs, function(button, state)
	            if button == "left" then
	               ...
	               if setClipboard(text) then
	               		outputChatBox("Copiado '"..text.."' para a área de transferência.")
	               end
	            end
	        end, false )

	        GUIEditor.button[2] = guiCreateButton(10, 425+yExtend, 700+xExtend, 26, "Fechar", false, GUIEditor.window[1])
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) and info and type(info) == "table" then
		...
		if user then
			if user.username then
				...
				guiSetText(GUIEditor.label[2], info.error or "Informações obtidas sobre '"..user.username.."' do servidor.")
				guiLabelSetColor ( GUIEditor.label[2], 0,255,0 )
				if info.error then
					clearStatusText()
				end
		...
		if changelogs then
			...
			for i=1, #changelogs do
				...
				guiGridListSetItemText(GUIEditor.gridlist[1], row, GUIEditor.gridcol.type, changelogs[i].promoted == "1" and "Promoção" or "Rebaixamento", false, false)
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) and info and type(info) == "table" and #info > 0 then
		...
		if changelogs then
			...
			for i=1, #changelogs do
				...
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.type, changelogs[i].promoted == "1" and "Promoção" or "Rebaixamento", false, false)
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.username, changelogs[i].userid or "[deletado]", false, false)
...
function guiStaffChange()
	...
	if source == GUIEditor.combobox.admin then
		...
		if (not exports.integration:isPlayerLeadAdmin(localPlayer) or myLevel < admin or myLevel < currentPos[1]) and not exports.integration:isPlayerLeadScripter(localPlayer) then
			guiSetText(GUIEditor.label[7], "Você não tem permissões suficientes.")
			guiComboBoxSetSelected ( GUIEditor.combobox.admin, currentPos[1])
		elseif admin > currentPos[1] then
			guiSetText(GUIEditor.label[7], "Promovendo para "..staffTitles[1][admin])
			futurePos[1] = admin
		elseif admin < currentPos[1] then
			guiSetText(GUIEditor.label[7], "Rebaixamento para "..staffTitles[1][admin])
			futurePos[1] = admin
		else
	...
	elseif source == GUIEditor.combobox.sup then
		...
		if not exports.integration:isPlayerLeadAdmin(localPlayer) and not exports.integration:isPlayerSupportManager(localPlayer) then
			guiSetText(GUIEditor.label[8], "Você não tem permissões suficientes.")
			guiComboBoxSetSelected ( GUIEditor.combobox.sup, currentPos[2])
		elseif sup > currentPos[2] then
			guiSetText(GUIEditor.label[8], "Promovendo para "..staffTitles[2][sup])
			futurePos[2] = sup
		elseif sup < currentPos[2] then
			guiSetText(GUIEditor.label[8], "Rebaixamento para "..staffTitles[2][sup])
			futurePos[2] = sup
		else
	...
	elseif source == GUIEditor.combobox.vct then
		...
		if not exports.integration:isPlayerLeadAdmin(localPlayer) and (myLevel <= vct or myLevel <= currentPos[3]) then
			guiSetText(GUIEditor.label[9], "Você não tem permissões suficientes.")
			guiComboBoxSetSelected ( GUIEditor.combobox.vct, currentPos[3])
		elseif vct > currentPos[3] then
			guiSetText(GUIEditor.label[9], "Promovendo para "..staffTitles[3][vct])
			futurePos[3] = vct
		elseif vct < currentPos[3] then
			guiSetText(GUIEditor.label[9], "Rebaixamento para "..staffTitles[3][vct])
			futurePos[3] = vct
		else
	...
	elseif source == GUIEditor.combobox.scripter then
		...
		if not exports.integration:isPlayerLeadScripter(localPlayer) then
			guiSetText(GUIEditor.label[10], "Você não tem permissões suficientes.")
			guiComboBoxSetSelected ( GUIEditor.combobox.scripter, currentPos[4])
		elseif scripter > currentPos[4] then
			guiSetText(GUIEditor.label[10], "Promovendo para "..staffTitles[4][scripter])
			futurePos[4] = scripter
		elseif scripter < currentPos[4] then
			guiSetText(GUIEditor.label[10], "Rebaixamento para "..staffTitles[4][scripter])
			futurePos[4] = scripter
		else
	...
	elseif source == GUIEditor.combobox.mapper then
		...
		if not exports.integration:isPlayerLeadScripter(localPlayer) and (myLevel <= mapper or myLevel <= currentPos[5]) then
			guiSetText(GUIEditor.label[11], "Você não tem permissões suficientes.")
			guiComboBoxSetSelected ( GUIEditor.combobox.mapper, currentPos[5])
		elseif mapper > currentPos[5] then
			guiSetText(GUIEditor.label[11], "Promovendo para "..staffTitles[5][mapper])
			futurePos[5] = mapper
		elseif mapper < currentPos[5] then
			guiSetText(GUIEditor.label[11], "Rebaixamento para "..staffTitles[5][mapper])
			futurePos[5] = mapper
		else
	...
	elseif source == GUIEditor.combobox.fmt then
		...
		if not exports.integration:isPlayerLeadAdmin(localPlayer) and not exports.integration:isPlayerFMTLeader(localPlayer) then
			guiSetText(GUIEditor.label[15], "Você não tem permissões suficientes.")
			guiComboBoxSetSelected ( GUIEditor.combobox.fmt, currentPos[6])
		elseif fmt > currentPos[6] then
			guiSetText(GUIEditor.label[15], "Promovendo para "..staffTitles[6][fmt])
			futurePos[6] = fmt
		elseif fmt < currentPos[6] then
			guiSetText(GUIEditor.label[15], "Rebaixamento para "..staffTitles[6][fmt])
			futurePos[6] = fmt
		else
...
function showConfirmEditStaff()
    ...
    GUIEditor.window[2] = guiCreateWindow(687, 334, 420, 171, "Confirmação", false)
    guiWindowSetSizable(GUIEditor.window[2], false)
    exports.global:centerWindow(GUIEditor.window[2])
    GUIEditor.label[100] = guiCreateLabel(11, 27, 394, 63, "Você está prestes a atualizar as permissões da equipe para o usuário '"..staffInfo.user.username.."'.\n\nPor favor, insira os motivos ou detalhes (se houver) sobre esta mudança de equipe na caixa de texto abaixo:", false, GUIEditor.window[2])
    ...
    GUIEditor.button[100] = guiCreateButton(12, 136, 198, 25, "Continuar", false, GUIEditor.window[2])
    ...
    GUIEditor.button[101] = guiCreateButton(212, 136, 198, 25, "Cancelar", false, GUIEditor.window[2])
