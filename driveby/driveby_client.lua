...
function toggleDriveby()
	...
	if equipedWeapon == 0 then
		...
		if not lastSlotAmmo or lastSlotAmmo == 0 or getSlotFromWeapon(getPedWeapon (localPlayer,lastSlot)) == 0 then
			for key,weaponID in ipairs(weaponsTable) do
				...
				if weapon == weaponID then
					local totalAmmoPerWeapon = getPedTotalAmmo ( localPlayer, slot ) - 1
					if totalAmmoPerWeapon > 0 then
		...
		if prevw and nextw then
			...
			helpText:text( "Pressione '"..prevw.."' ou '"..nextw.."' para mudar de arma" )
			fadeInHelp()
...
function pressKey ( controlName )
	setPedControlState ( localPlayer, controlName, true )
	setTimer ( setPedControlState, 150, 1, localPlayer, controlName, false )
end
