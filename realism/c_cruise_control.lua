...
function deactivateCruiseControl()
    ...
    setPedControlState(localPlayer, "accelerate",false)
    ccEnabled = false
    exports.hud:sendBottomNotification(localPlayer, "Cruise Control", "Cruise Control desativado.")
end
				...
				if (ccEnabled) then
					...
					if targetSpeed > 10 then
						if (getVehicleType(theVehicle) == "Automobile" or getVehicleType(theVehicle) == "Bike" or getVehicleType(theVehicle) =="Boat" or getVehicleType(theVehicle) == "Train" or getVehicleType(theVehicle) == "Plane" or getVehicleType(theVehicle) == "Helicopter") or specialVehicles[getElementModel(theVehicle)] then
							exports.hud:sendBottomNotification(localPlayer, "Cruise Control Ativado", "Use - e + para ajustar a velocidade.")
							activateCruiseControl()
						end
					else
                        exports.hud:sendBottomNotification(localPlayer, "Cruise Control", "Cruise Control pode ser usado para manter a velocidade, não puxar para cima.")
					end
				end
            else
                exports.hud:sendBottomNotification(localPlayer, "Cruise Control", "O motor não está ligado!")
			end
		end
	end
end
