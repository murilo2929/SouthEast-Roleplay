			...
			if isElementStreamedIn(value) and (isElementOnScreen(value)) and (tV~=value) then
				...
				if (isLineOfSightClear(px, py, pz, x, y, z, true, false, false, true, false, false, true, tV)) then
					...
					if (tx) then
						...
						local plate = getElementData(value, "show_plate") == 1 and ("Placa: "..getElementData(value,"plate").." | ") or ""
		...
		for key, value in ipairs(getElementsByType("player")) do
			if isElementStreamedIn(value) and (isElementOnScreen(value)) and (localPlayer~=value) and tV ~= getPedOccupiedVehicle(value) then
				...
				if (isLineOfSightClear(px, py, pz, x, y, z, true, false, false, true, false, false, true, tV)) then
					...
					-- needs fixing
					if (blackMales[skin]) then text = "Homem Negro"
					elseif (whiteMales[skin]) then text = "Homem Branco"
					elseif (asianMales[skin]) then text = "Homem Asiatico"
					elseif (blackFemales[skin]) then text = "Mulher Negra"
					elseif (whiteFemales[skin]) then text = "Mulher Branca"
					elseif (asianFemales[skin]) then text = "Mulher Asiatica"
					else text = "Homem Branco"
					end
