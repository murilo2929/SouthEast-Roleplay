	...
	if (handbrake == 0) then
		if getVehicleType(vehicle) == 'BMX' or getVehicleType(vehicle) == 'Bike' then
			if not kickstand then
				outputChatBox('Este veículo não tem freio de mão.', player, 255, 0, 0)
			elseif not isVehicleOnGround(vehicle) and not forceOnGround then
				outputChatBox('Você precisa estar no terreno para que isso funcione.', player, 255, 0, 0)
			elseif math.floor(exports.global:getVehicleVelocity(vehicle)) > 2 then
				outputChatBox("Isso não funciona durante a condução...", player, 255, 0, 0)
			else
		...
		elseif (isVehicleOnGround(vehicle) or forceOnGround) or getVehicleType(vehicle) == "Boat" or getVehicleType(vehicle) == "Helicopter" or someExceptions[getElementModel(vehicle)] then
			if kickstand then
				outputChatBox('Este veículo não tem suporte.', player, 255, 0, 0)
				return false
			end
		...
		if getVehicleType(vehicle) == 'BMX' or getVehicleType(vehicle) == 'Bike' then
			if not kickstand then
				outputChatBox('Este veículo não possui freio de mão.', player, 255, 0, 0)
				return
			end
		else
			if kickstand then
