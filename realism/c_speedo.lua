...
function drawSpeedo()
	if getElementData(localPlayer,"speedo") ~= "0" and not isPlayerMapVisible() and exports.hud:isActive() then
		...
		if (vehicle) then
			...
			dxDrawText( "Quilometragem: "..tostring(math.floor(getDistanceTraveled()/1000)), x - 153, y - 215, x-70, 5, tocolor (255,255,255, 200), 1, 'default', 'center' )
			dxDrawText( "Bateria: "..tostring(math.floor( getElementData( vehicle, 'battery' ) or 100 )).."%", x - 150, y - 145, x-70, 5, tocolor (255,255,255, 200), 1, 'default', 'center' )
		end
	end
end
