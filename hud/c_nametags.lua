...
function moneyUpdateFX(state, amount, toEachother)
	if amount and tonumber(amount) and tonumber(amount) > 0  then
		local info = {{"Atualização de finanças pessoais"},{""}}
		...
		if state then
			...
			table.insert(info, {"   - Dinheiro: $"..exports.global:formatMoney(money).." ("..moneyFloat["text"]..")"})
			if toEachother then
				table.insert(info, {"   - Dinheiro no Banco: $"..exports.global:formatMoney(bankmoney-amount)})
			else
								...
								if not (collision) or aimsSniper() or (reconx) then
									...
									if (sx) and (sy) then
										...
										if (sx) and (sy) and oldsy then
											...
											dxDrawText(name, sx-offset+2, sy+2, (sx-offset)+140 / distance, sy+120 / distance, tocolor(0, 0, 0, 255), scale, font, "center", "center", false, false, false, false, false)
											dxDrawText(name, sx-offset, sy, (sx-offset)+140 / distance, sy+120 / distance, tocolor(r, g, b, 255), scale, font, "center", "center", false, false, false, false, false)
