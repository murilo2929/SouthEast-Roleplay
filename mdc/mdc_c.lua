...
function openLogin (fromComputer)
	if fromComputer or hasMDCPermissions() then
		...
		outputChatBox( "Você não está perto de um computador móvel de dados.", 255, 155, 155 )
	end
end
addEvent("mdc:loginW")
...
function mdc_errorWin ( text )
	...
	window.dialog.closeButton = guiCreateButton( 10, 60, width - 20, 40, "Fechar", false, window.dialog.window )
...
function mdc_confirmWin ( text )
	...
	local x = SCREEN_X / 2 - width / 2
	local y = SCREEN_Y / 2 - height / 2
	window.confirmWin.window = guiCreateWindow( x, y, width, height, "MDC Confirmar", false )
	...
	window.confirmWin.bYes = guiCreateButton( 10, 80, 110, 40, "Sim", false, window.confirmWin.window )
	window.confirmWin.bNo = guiCreateButton( 130, 80, 110, 40, "Não", false, window.confirmWin.window )
