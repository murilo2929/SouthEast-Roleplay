...
function display_vehicle ( id, model, color1, color2, color3, color4, plate, faction, owner, owner_type, impound, stolen, crimes )
	...
	vehW.window = guiCreateWindow( x, y, width, height, "Mobile Data Computer - MDC Pesquisa - Veículo: ".. plate, false )

	vehW.nameLabel	= guiCreateLabel( 10, 30, 180, 20, "Veículo: ", false, vehW.window )
	vehW.plateLabel 	= guiCreateLabel( 10, 50, 180, 20, "Placa: ", false, vehW.window )
	vehW.primaryLabel	= guiCreateLabel( 10, 70, 180, 20, "Cor Primaria: ", false, vehW.window )
	vehW.secondLabel	= guiCreateLabel( 10, 90, 180, 20, "Cor Secundaria: ", false, vehW.window )
	vehW.vinLabel		= guiCreateLabel( 10, 110, 180, 20, "Numero VIN: ", false, vehW.window )
	vehW.ownerLabel	= guiCreateLabel( 10, 130, 220, 20, "Dono: ", false, vehW.window )
	vehW.impoundLabel	= guiCreateLabel( 10, 150, 220, 20, "Apreendido: ", false, vehW.window )
	vehW.stolenLabel	= guiCreateLabel( 10, 170, 220, 20, "Roubado: ", false, vehW.window )
	...
	local impoundText = "Não"
	if impound[1] ~=0 then
		if impound[2] == 1 then
			impoundText = "LSPD | Liberar"
		elseif impound[2] == 59 then
			impoundText = "SCoSA | Liberar"
		else
			impoundText = "RT"
		end
	end
	vehW.impoundButton_release	= guiCreateButton( 105, 150, 220, 20, impoundText, false, vehW.window )
	vehW.stolen2Button	= guiCreateButton( 105, 170, 220, 20, ( tonumber( stolen ) == 1 and "Sim" or "Não" ), false, vehW.window )
	if impound[1] == 0 then
	...
	addEventHandler( 'onClientGUIClick', vehW.window,
		function ()
			if source == vehW.stolen2Button then
				...
				guiSetText( vehW.stolen2Button, ( tonumber( stolen ) == 1 and "Sim" or "Não" ) )
			elseif source == vehW.impoundButton_release then
				...
				if can and org == impound[2] then
					...
					outputChatBox("Sua conta não está autorizada a liberar este veículo.", 255,0,0)
	...
	vehW.crimesTab	= guiCreateTab( "Violações por Excesso de Velocidade", vehW.mainPanel )
	vehW.crimesTable	= guiCreateGridList ( 10, 10, width - 40, 230, false, vehW.crimesTab )
	vehW.dateCol		= guiGridListAddColumn( vehW.crimesTable, "Data", 0.3 )
	vehW.speedCol		= guiGridListAddColumn( vehW.crimesTable, "Velocidade", 0.14 )
	vehW.locCol		= guiGridListAddColumn( vehW.crimesTable, "Local", 0.24)
	vehW.personCol	= guiGridListAddColumn( vehW.crimesTable, "Pessoa", 0.28 )
	...
	local function drawImpReportDetail(text)
		if text then
			if vehW.imp_reports_list and isElement(vehW.imp_reports_list) then
				...
				vehW.back = guiCreateButton( 10, 220, width - 40, btnH, "Voltar" ,false, vehW.imp_reports_tab )
	...
	if impound[3] and #impound[3] > 0 then
		vehW.imp_reports_tab	= guiCreateTab( "Relatórios de apreensão", vehW.mainPanel )
		...
		vehW.imp_reports_list_id		= guiGridListAddColumn( vehW.imp_reports_list, "Report Nu.", 0.3 )
		vehW.imp_reports_list_reporter		= guiGridListAddColumn( vehW.imp_reports_list, "Relator", 0.3 )
		vehW.imp_reports_list_date		= guiGridListAddColumn( vehW.imp_reports_list, "Data", 0.3 )
	...
	if ( #crimes > 0 ) then
		...
		local row = guiGridListAddRow ( vehW.crimesTable )
		guiGridListSetItemText ( vehW.crimesTable, row, vehW.dateCol, "Sem Violações de Velocidade", false, false )
	end


	vehW.closeButton = guiCreateButton( 10, height - 50, width - 20, 40, "Fechar", false, vehW.window )
