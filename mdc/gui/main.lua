...
function main ( warrants, apb, impounds, calls, anpr )
	...
	mainW.window = guiCreateWindow( x, y, width, height, (org_name and (org_name.." -") or "").. "MDC Principal", false )
	...
	--Search Button
	mainW.searchButton = guiCreateButton( button.x, button.y, button.width, button.height, "Procurar DB", false, mainW.window )
	...
	if can then
		...
		mainW.addButton = guiCreateButton( button.x, button.y, button.width, button.height, "Emitir APB", false, mainW.window )
	...
	if level == 2 then
		button.x = button.x + button.width + spacer
		mainW.accountButton = guiCreateButton( button.x, button.y, button.width, button.height, "ADM Sistema", false, mainW.window )
	...
	if can then
		button.x = button.x + button.width + spacer
		mainW.tollsButton = guiCreateButton( button.x, button.y, button.width, button.height, "Pedágios", false, mainW.window )
	...
	mainW.personCol	= guiGridListAddColumn( mainW.apbTable, "Pessoa", 0.25 )
	mainW.wantedCol	= guiGridListAddColumn( mainW.apbTable, "APB", 0.5 )
	mainW.issuedByCol	= guiGridListAddColumn( mainW.apbTable, "Emitido Por", 0.25 )
	mainW.timeCol	= guiGridListAddColumn( mainW.apbTable, "Data", 0.15 )
	...
	if ( #apb > 0 ) then
		for i = 1, #apb, 1 do
			...
			if(apb[ i ][ 5 ] == apb[ i ][ 3 ]) then
				...
				issuedByText = (apb[ i ][ 5 ] or 'Desconhecido') .. " (" .. (apb[ i ][ 3 ] or 'Desconhecido') .. ")"
			end
		...
		local row = guiGridListAddRow ( mainW.apbTable )
		guiGridListSetItemText ( mainW.apbTable, row, mainW.personCol, "Sem APBs", false, false )
	end
	
	if canAccess( localPlayer, 'canSeeWarrants' ) then
		mainW.warrantTab	= guiCreateTab( "Mandados", mainW.mainPanel )
		mainW.warrantTable	= guiCreateGridList ( 10, 10, width - 35, height - 190, false, mainW.warrantTab )
		mainW.charCol		= guiGridListAddColumn( mainW.warrantTable, "Suspeito", 0.25 )
		mainW.warrantCol	= guiGridListAddColumn( mainW.warrantTable, "Mandado", 0.45 )
		mainW.issuedCol	= guiGridListAddColumn( mainW.warrantTable, "Emitido Por", 0.25 )
		...
		if ( #warrants > 0 ) then
			...
			local row = guiGridListAddRow ( mainW.warrantTable )
			guiGridListSetItemText ( mainW.warrantTable, row, mainW.charCol, "Sem Mandados", false, false )
		end
	end
	...
	--[[if canAccess( localPlayer, 'canSeeWarrants' ) then
		...
		mainW.plateCol	= guiGridListAddColumn( mainW.anprTable, "Placa do Veículo", 0.25 )
		mainW.descCol	= guiGridListAddColumn( mainW.anprTable, "Procurado por", 0.5 )
		mainW.issuedByColumn	= guiGridListAddColumn( mainW.anprTable, "Emitido Por", 0.25 )
		if ( #anpr > 0 ) then
			for i = 1, #anpr, 1 do
			...
			local row = guiGridListAddRow ( mainW.anprTable )
			guiGridListSetItemText ( mainW.anprTable, row, mainW.plateCol, "Sem ANPRs", false, false )
		end
	end	]]

	--Impounds / Maxime / 2015.2.1
	mainW.imps_lots	= guiCreateTab( "Apreensões", mainW.mainPanel )
	mainW.imps_lots_list	= guiCreateGridList ( 10, 10, width - 35, height - 190, false, mainW.imps_lots )
	mainW.imps_lots_list_col_dep = guiGridListAddColumn( mainW.imps_lots_list, "Departamento", 0.1 )
	mainW.imps_lots_list_col_lane = guiGridListAddColumn( mainW.imps_lots_list, "Faixa", 0.05 )
	mainW.imps_lots_list_col_days = guiGridListAddColumn( mainW.imps_lots_list, "Data da Liberação", 0.2 )
	mainW.imps_lots_list_col_fine = guiGridListAddColumn( mainW.imps_lots_list, "Multa ($)", 0.1 )
	mainW.imps_lots_list_col_model = guiGridListAddColumn( mainW.imps_lots_list, "Modelo", 0.4 )

	mainW.imps_lots_list_col_plate = guiGridListAddColumn( mainW.imps_lots_list, "Placa", 0.1 )
	mainW.imps_lots_list_col_vin = guiGridListAddColumn( mainW.imps_lots_list, "VIN", 0.1 )
	
	mainW.imps_lots_list_col_id = guiGridListAddColumn( mainW.imps_lots_list, "((Veículo ID))", 0.08 )
	...
	if canAccess( localPlayer, 'canSeeCalls' ) then
		mainW.callsTab			= guiCreateTab( "Ligações 911", mainW.mainPanel )
		mainW.callsTable		= guiCreateGridList( 10, 10, width - 35, height - 190, false, mainW.callsTab )
		mainW.callerCol		= guiGridListAddColumn( mainW.callsTable, "Chamador", 0.2 )
		mainW.phoneCol			= guiGridListAddColumn( mainW.callsTable, "Número de Telefone", 0.12 )
		mainW.convoCol			= guiGridListAddColumn( mainW.callsTable, "Descrição", 0.5 )
		mainW.timeCol			= guiGridListAddColumn( mainW.callsTable, "Tempo", 0.1 )
		...
		if ( #calls > 0 ) then
			...
			addEventHandler("onClientGUIClick", mainW.callsTable,
				function (button)
					if button == "left" then
						local rcMenu = exports.rightclick:create("Ação...")
						local row = {}
						
						row.viewDetails = exports.rightclick:addRow("Ver detalhes do 911")
						...
						if characterName ~= "N/A" then
							row.viewMdc = exports.rightclick:addRow("Ver arquivo MDC")
						...
						addEventHandler("onClientGUIClick", row.viewDetails, 
							function ()
								...
								exports.hud:sendBottomNotification(localPlayer, "Mobile Data Computer", "Chamador: "..characterName.." - Número de telefone: "..phonenumber.." - Descrição: "..description.." - Tempo:"..time )
							end
						, false)
					end
				end
			...
			local row = guiGridListAddRow ( mainW.callsTable )
			guiGridListSetItemText ( mainW.callsTable, row, mainW.callerCol, "Sem chamadas para o 911", false, false )
		end
	end
	
	
	mainW.closeButton = guiCreateButton( 10, height - 50, width - 20, 40, "Fechar", false, mainW.window )
