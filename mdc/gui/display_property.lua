...
function display_property ( id, interiorType, owner, cost, name, address, district, dim, ownerClickable )
	...
	window = { }
	window.window = guiCreateWindow( x, y, width, height, "MDC Pesquisa - Propriedade: ".. id, false )
	
	window.zipLabel		= guiCreateLabel( 10, 30, 180, 20, "Código Postal (ID): ", false, window.window )
	window.typeLabel 	= guiCreateLabel( 10, 50, 180, 20, "Tipo: ", false, window.window )
	window.ownerLabel	= guiCreateLabel( 10, 70, 180, 20, "Dono: ", false, window.window )
	window.costLabel	= guiCreateLabel( 10, 90, 180, 20, "Custo: ", false, window.window )
	window.nameLabel	= guiCreateLabel( 10, 110, 180, 20, "Nome: ", false, window.window )
	window.addressLabel	= guiCreateLabel( 10, 130, 180, 20, "Endereço: ", false, window.window) --MS: Added new Label for Address
	window.districtLabel= guiCreateLabel( 10, 150, 220, 20, "Distrito: ", false, window.window )
	...
	if dim > 0 then --MS: Sets the address if it is in the main world, or directs you to the interior it's inside of. Should account for sub-ints.
		window.addressLabel	= guiCreateLabel( 105, 130, 180, 20, "(( Dentro: " ..dim.. " ))", false, window.window) 
	else
		if not address then
			address = 'N/A'
		end
