...
function system_admin ( users, count )
	...
	window.window = guiCreateWindow( x, y, width, height, "MDC Administrador do Sistema", false )

	window.usersTable	= guiCreateGridList ( 10, 90, width - 20, height - 90 - 60, false, window.window )
	window.userCol		= guiGridListAddColumn( window.usersTable, "Usuario", 0.45 )
	...
	window.createButton = guiCreateButton( 10, 30, ( width / 3 ) - 15, 50, "Criar Conta", false, window.window )
	...
	window.editButton = guiCreateButton( width / 3 + 5, 30, ( width / 3 ) - 10, 50, "Editar Conta", false, window.window )
	...
	window.deleteButton = guiCreateButton( ( width / 3 * 2 ) + 5, 30, ( width / 3 ) - 15, 50, "Deletar Contar", false, window.window )
	addEventHandler( "onClientGUIClick", window.deleteButton,
		function( )
			...
			if accountID then
				local user =  guiGridListGetItemText ( window.usersTable, row, window.userCol )
				local cfWin, cfYes, cfNo, cfLbl = mdc_confirmWin("Você quer EXCLUIR o usuário '"..tostring(user).."'?")
...
function create( )
	...
	local x = SCREEN_X / 2 - width / 2
	local y = SCREEN_Y / 2 - height / 2
	window.window = guiCreateWindow( x, y, width, height, "MDC Criar Conta", false )

	window.userLabel	= guiCreateLabel( 10, 37, 100, 20, "Nome do Personagem:", false, window.window )
	...
	guiComboBoxAddItem( window.levelCombo, "Administrador" )
	local org = exports.factions:getCurrentFactionDuty( localPlayer )

	window.editButton = guiCreateButton( 10, height - 100, width - 20, 40, "Criar!", false, window.window )
	addEventHandler( "onClientGUIClick", window.editButton,
		function ()
...
function edit( accountID, user, level )
	...
	local x = SCREEN_X / 2 - width / 2
	local y = SCREEN_Y / 2 - height / 2
	window.window = guiCreateWindow( x, y, width, height, "MDC Editar Conta: "..user, false )
	...
	window.editButton = guiCreateButton( 10, height - 100, width - 20, 40, "Editar!", false, window.window )
	addEventHandler( "onClientGUIClick", window.editButton,
		function ()
			local level = guiComboBoxGetSelected ( window.levelCombo )
			guiSetVisible( window.window, false )
