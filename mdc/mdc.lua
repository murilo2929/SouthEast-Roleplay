local possibleLicenses = {
	--[database field] = text
	gun_license = 'Porte de Arma - Tier 1',
	gun2_license = 'Porte de Arma - Tier 2',
	car_license = 'Habilitação - Carro',
	bike_license = 'Habilitação - Moto',
	--pilot_license = 'San Andreas Pilot Certificate',
	fish_license = 'Permição Pesca',
	boat_license = 'Habilitação - Barco',
}
...
function cacheOnStart()
	...
	if results and num_affected_rows > 0 then
		for i, row in ipairs( results ) do
			...
			res[2] = row.description or "Desconhecido"
			res[3] = getCharacterNameFromID( row.charactername, 2 )
			res[4] = row.id
			res[5] = row.organization or "Desconhecido"
			res[6] = row.time
...
function getMDCNameFromID( id )
	...
	if exports.mysql:num_rows( result ) > 0 then
		...
		local playerName = firstLetter and (firstLetter .. playerName[ 2 ]) or 'Desconhecido'
		return playerName
	else
		return false
	end
...
function login( charid, silent )
	...
	if res and nums > 0 then
		...
		dbFree( accountQuery )
		if not silent then
			outputChatBox( "Você não tem uma conta com acesso suficiente para usar o MDC.", source, 255, 155, 155 )
		end
	end
end
	...
	if canAccess( source, 'canSeeWarrants' ) then
		...
		if ( warrantResult ) then
			...
			while true do
				...
				warrants[count][3] = account and getCharacterNameFromID(account.charid, thisOrg) or "Desconhecido"
				warrants[count][4] = account and account.organization or "Desconhecido"
		...
		for i, lane in pairs(lanes) do
			...
			if tonumber(lane.veh) ~= 0 then
				...
				if veh and getElementData(veh, "impounder") == oneLane.impounder and getElementData(veh, "Impounded") ~= 0 then
					...
					oneLane.vin = getElementData(veh, "show_vin") == 1 and getElementData(veh, "dbid") or "Sem VIN"
					oneLane.plate = getElementData(veh, "show_plate") == 1 and getElementData(veh, "plate") or "Sem Placa"
...
function search( query, queryType )
	...
	elseif queryType == "Pessoa" or tonumber(queryType) == 0 then --Person
		local character = nil
		local criminal = nil
	...
	elseif (queryType == "Placa do Veículo" or tonumber(queryType) == 1) or (queryType == "VIN do Veículo (( ID ))" or tonumber(queryType) == 3) then --Vehicle
		local q = ""
		if queryType == "Placa do Veículo" or tonumber(queryType) == 1 then
			q = "v.plate = '".. exports.mysql:escape_string( query ) .. "'"
		elseif queryType == "VIN do Veículo (( ID ))" or tonumber(queryType) == 3 then
		...
		if vehicle and vehicle.id ~= mysql_null() then
			...
			if ( result2 ) then
				...
				while true do
					...
					crimes[count][4] = exports.cache:getCharacterName(row["personVisible"]) or "Não visivel"
					count = count + 1
			...
			if tonumber( vehicle.owner ) ~= -1 then
				...
				vehicle.owner = owner and (owner.charactername or "Desconhecido") or "Desconhecido"
	...
	elseif queryType == "CEP da Propiedade (( ID ))" or tonumber(queryType) == 2 then --Property
		...
		if exports.mysql:num_rows( result ) > 0 then
			...
			elseif tonumber( interior.type ) == 1 then
				interior.type = "Negócio"
			elseif tonumber( interior.type ) == 2 then
				interior.type = "Governo"
			else
				interior.type = "Apartamento"
			end
...
function remove_pilot_license( charid, charactername, license_uid, licensetext )
	...
	if query then
		...
		add_pilot_event( charid, charactername, "Licença Revogada (MDC)", tostring(licensetext) )
		--[[
		local targetPlayer = exports.global:getPlayerFromCharacterID(charid)
		if targetPlayer then
			getPlayerPilotLicenses(targetPlayer, true)
...
function create_account( charid, level, organization )
	...
	if not charid then
		outputChatBox("Personagem não localizado.", source, 255, 0, 0)
		return
	end
	...
	if res and nums > 0 then
		outputChatBox( "A conta MDC para este usuário já existe para esta organização.", source, 255, 0, 0 )
	else
		dbExec( exports.mysql:getConn('mta'), "INSERT INTO `mdc_users` ( `charid`, `level`, `organization` ) VALUES ( ?, ?, ? ) ", charid, level + 1, organization )
		outputChatBox( "A conta MDC foi criada com sucesso.", source, 255, 0, 0 )
	end
					...
					if tonumber(row.character) == charID then
						...
						if licenseID then
							...
							if licenseID == 7 then --typerating
								...
								if vehName then
									licenseText = "Digitando: "..tostring(vehName)
								end
							else
...
function refreshPilotLicensesCache(thePlayer, commandName)
	...
	if (faction and leader) or exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) then
		outputChatBox("Atualizando o cache de licenças piloto...", thePlayer, 255, 194, 14)
		local startTime = getTickCount()
		--mdc_faa_licenses
		local result = exports.mysql:query("SELECT * FROM `mdc_faa_licenses`")
		if result then
			...
			outputChatBox("ERROR! Falha ao carregar licenças piloto!", thePlayer, 255, 0, 0)
			outputDebugString("mdc-system/mdc.lua: Failed to load pilot licenses! (/"..tostring(commandName)..")",2)
			return
		end
		outputChatBox("Carregado "..tostring(#mdc_faa_licenses).." licenças de piloto em"..tostring(math.ceil(getTickCount()-startTime)).."ms.", thePlayer, 255, 194, 14)
	end
end
...
function makeMdcAccount(player, cmd, target, faction, level)
	if exports.integration:isPlayerScripter( player ) or exports.integration:isPlayerHeadAdmin( player ) then
		local function printTip( p )
			outputChatBox( "SYNTAX: /"..cmd.." [Nome Parcial Jogador ou ID] [facção ID] [level; 0=deletar conta, 1=normal, 2=admin] ", p )
		end
		...
		if not targetPlayer then
			printTip( player )
			return not outputChatBox( "Player não encontrado.", player, 255,0,0 )
		end

		if not exports.factions:getFactionFromID( faction ) then
			printTip( player )
			return not outputChatBox( "Facção ID "..faction.." não encontrado. Veja /showfactions ", player, 255, 0, 0 )
		elseif tonumber(level) ~= 0 and tonumber(level) ~= 1 and tonumber(level) ~= 2 then
			printTip( player )
			return not outputChatBox( "Level deve ser 1 ou 2.", player, 255, 0, 0 )
		else
			...
			if res and nums > 0 then
				if tonumber(level) == 0 then
					...
					return outputChatBox("Conta MDC de "..targetPlayerName.." fou excluida.", player, 0,255,0 )
				elseif res[1].level == tonumber(level) then
					return not outputChatBox( "Este jogador já teve uma conta MDC para esta facção com o mesmo nível de acesso que você tentou fazer. ", player, 255,0,0 )
				else
					dbExec( exports.mysql:getConn('mta'), "UPDATE mdc_users SET level=? WHERE id=?", level, res[1].id )
					outputChatBox("Nível de acesso para conta MDC "..targetPlayerName.." fou atualizada de "..(res[1].level).." para "..level..".", player, 0,255,0 )
				end
			else
				if tonumber(level) == 0 then
					outputChatBox("Conta MDC de "..targetPlayerName.." não foi encontrada.", player, 0,255,0 )
				else
					dbExec( exports.mysql:getConn('mta'), "INSERT INTO mdc_users SET charid=?, level=?, organization=?", charid, level, faction )
					outputChatBox("Conta MDC de "..targetPlayerName.." foi criada com sucesso.", player, 0,255,0 )
				end
			end
		end
	end
...
addCommandHandler("criarcontamdc", makeMdcAccount)
