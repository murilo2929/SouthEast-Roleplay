		...
		if fileExists(path) then
			...
			if data then --shader exist
				...
				if isPreview then
					...
					outputChatBox("Sua visualização terminará em 30 segundos ou /cancelartextura.", 255, 194, 14)
					textureName = texName 
					veh = element
				end	
			else
...
function validateFileFromURL(url)
	...
	if fileExists(path) then --file already exists, so we can simply check filesize
		...
		if size > maxFileSize then
			local text = "O tamanho do arquivo excede o tamanho máximo permitido para texturas de itens ("..tostring(math.floor(maxFileSize/1000)).."kb)."
...
function cancelPreview()
	if isTimer(previewTimer) then
		...
		textureName, veh, previewTimer = nil
		outputChatBox("Você cancelou sua visualização.", 255, 194, 14)
	else
		outputChatBox("No momento, você não está visualizando uma textura de veículo.", 255, 194, 14)
	end
end	
addCommandHandler("cancelartextura", cancelPreview)