...
addEventHandler("show_diagnose_window", getLocalPlayer(),
	function(int_tbl,ext_tbl, patient)
		...
		if gui['window'] then 
			...
			gui['window'] = guiCreateWindow( (x/2)-(sw/2), (y/2)-(sh/2), sw, sh-80, "Mostrando paciente: "..gsub(name, "_", " "), false)
			...
			gui.tab[1] = guiCreateTab('Externo', gui['w_tab'])
			gui.tab[2] = guiCreateTab('Interno', gui['w_tab'])
			--gui.tab[3] = guiCreateTab('Status', gui['w_tab'])
			gui.tab[4] = guiCreateTab('Legenda', gui['w_tab'])
