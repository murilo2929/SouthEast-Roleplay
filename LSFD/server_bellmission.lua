...
function fdfire()
    ...
    local fX, fY, fZ = fireTable[randomfire][1],fireTable[randomfire][2],fireTable[randomfire][3]
            ...
            for k, v in ipairs (playersOnFireTeam) do
                outputChatBox("[RADIO] Recebemos uma chamada anônima sobre um grande incidente.",v,245, 40, 135)
				outputChatBox("[RADIO] O incidente é o seguinte: "..fireTable[randomfire][5],v,245, 40, 135)
                outputChatBox("[RADIO] Local: "..fireTable[randomfire][4].." Por favor, va até lá. Nós adicionamos um blip no seu GPS.",v,245, 40, 135)
            end
...
function randomfire (thePlayer)
	if ( exports.integration:isPlayerTrialAdmin ( thePlayer ) ) then
		...
		if (isFireOn) then
			outputChatBox ("AdmCMD: Já existe um incêndio. Use /cancelfire ou espere 30 minutos.", thePlayer,255,0,0)
		else
			fdfire()
			outputChatBox ("AdmCMD: Você acionou aleatoriamente um incêndio para FD!", thePlayer,255,0,0)
			outputChatBox ("AdmCMD: Digite /cancelarincendio para cancelar!", thePlayer,255,0,0)
		end
	end
end
...
function cancelrandomfire (thePlayer)
	if ( exports.integration:isPlayerTrialAdmin ( thePlayer ) ) then
		...
		if (isFireOn) then
			...
			outputChatBox ("AdmCMD: Você parou o fogo aleatório para FD!", thePlayer,255,0,0)
			outputChatBox ("AdmCMD: Pode demorar até cinco minutos antes de ser totalmente cancelado.", thePlayer,255,0,0)
			restartResource(thisResource)
			isFireOn = false
		else
			outputChatBox ("AdmCMD: Não há fogo iniciado. Use /randomfire para iniciar.", thePlayer,255,0,0)
		end
	end
end
addCommandHandler("cancelarincendio", cancelrandomfire)
