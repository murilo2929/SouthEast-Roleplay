...
function drawIBIS()
	...
	ibis.Window = guiCreateWindow(x, y, 689, 168, "IBIS Painel de Controle", false)
	...
	ibis.bNextAnn = guiCreateButton(76, 118, 59, 36, "Anunciar", false, ibis.Window)
	...
	ibis.bSetLine = guiCreateButton(250, 119, 70, 35, "Linha", false, ibis.Window)
	addEventHandler("onClientGUIClick", ibis.bSetLine, updateLine, false)
	ibis.bSetRoute = guiCreateButton(330, 119, 70, 35, "Rota", false, ibis.Window)
	addEventHandler("onClientGUIClick", ibis.bSetRoute, updateRoute, false)
	ibis.bSetDestination = guiCreateButton(410, 119, 70, 35, "Destino", false, ibis.Window)
	...
	ibis.lLine = guiCreateLabel(26, 89, 24, 14, "Linha", false, ibis.Window)
	ibis.lRoute = guiCreateLabel(60, 89, 35, 14, "Rota", false, ibis.Window)
	ibis.lDestination = guiCreateLabel(105, 89, 35, 14, "Dest.", false, ibis.Window)
	ibis.lCurrentStop = guiCreateLabel(145, 89, 53, 14, "Para atual", false, ibis.Window)
	ibis.lTotalStops = guiCreateLabel(208, 89, 62, 14, "Total de paradass", false, ibis.Window)
end
	...
	if lineEdit then
		if guiGetText(source) ~= "OK" then
			if keypadClicks == 0 then
				...
				updateScreen("Definir linha: " .. tempVariable)
...
function checkDataClient(typeOfData, dataCorrect, otherValues)
	...
	if (typeOfData == 0) then
		if lineEdit and dataCorrect == 0 then
			updateScreen("Definir linha: 000\nLinha não existe, tente outra!")
			tempVariable = nil
			keypadClicks = 0
	...
	elseif (typeOfData == 1) then
		if routeEdit and dataCorrect == 0 then
			updateScreen("Line set: " .. line .. "\nDefinir rota: 000\nA rota não existe na linha definida, tente outra!")
			tempVariable = nil
			keypadClicks = 0
	...
	elseif (typeOfData == 2) then
		if destinationEdit and dataCorrect == 0 then
			updateScreen("Set destination: 000\nO destino não existe, tente outro!")
			tempVariable = nil
			keypadClicks = 0
...
function updateRoute()
	if routeEdit == false and destinationEdit == false and lineEdit == false then
		if (line == "000") then
			updateScreen("Selecione uma linha primeiro")
			return
		end
		updateScreen("Line set: " .. line .. "\nDefinir rota: 000")
		routeEdit = true
