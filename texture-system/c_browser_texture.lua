    ...
    if canModify then
        ...
        addEventHandler("onClientGUIAccepted", eURL, function()
            ...
            if text:sub(1, 7) == "http://" or text:sub(1, 8) == "https://" then
                ...
                outputChatBox("URL precisa começar com h http:// or https://.", 255, 0, 0)
            end
        end)
...
function showPresentationControls(browser)
    ...
    wPresentationControls = guiCreateWindow((screen_width - width) / 2, 20, width, height, "Apresentação", false)
