...
function checkInt(thePlayer, commandName, intID)
	if exports.integration:isPlayerTrialAdmin( thePlayer ) or exports.integration:isPlayerSupporter(thePlayer) then
		if not tonumber(intID) or (tonumber(intID) <= 0 ) or (tonumber(intID) % 1 ~= 0 ) then
			...
			if intID == 0 then
				outputChatBox( "Você precisa estar dentro de um interior.", thePlayer, 255, 194, 14)
				outputChatBox("Ou usar SYNTAX: /"..commandName.." [Interior ID]", thePlayer, 255, 194, 14)
				return false
			end
		end
		...
		if mQuery1 then
			...
			mysql:free_result(mQuery1)
			if not row then
				outputChatBox("Este ID de interior não existe!", thePlayer, 255, 0, 0)
				return
			end -- 			
...
function setInteriorFaction(thePlayer, cmd, ...)
	if exports.integration:isPlayerAdmin(thePlayer) then
		...
		if not (...) then
			outputChatBox("SYNTAX: /" .. cmd .. " [Nome da facção ou ID da facção]", thePlayer, 255, 194, 14 )
			return
		end

		local dim = getElementDimension(thePlayer)
		if dim < 1 then
			outputChatBox("Você precisa estar dentro de um interior to perform this action.", thePlayer, 255, 0, 0 )
			return
		end
		...
		if not theFaction then
			outputChatBox("Facção não encontrada.", thePlayer, 255, 0, 0 )
			return
		end
		...
		if not isElement(interiorElement) then
			outputChatBox("Nenhum interior encontrado aqui.", thePlayer, 255, 0, 0 )
			return
		end
...
function setInteriorToMyFaction(thePlayer, cmd, fID)
	...
	if not fID then
		outputChatBox("SYNTAX: /" .. cmd .. " [ID Facção]", thePlayer, 255, 194, 14)
		return
	end
	...
	if not faction or not leader then
		outputChatBox("Você deve ser um líder de facção para realizar esta ação.", thePlayer, 255, 0, 0 )
		return
	end

	local dim = getElementDimension(thePlayer)
	if dim < 1 then
		outputChatBox("Você precisa estar dentro de um interior para realizar esta ação.", thePlayer, 255, 0, 0 )
		return
	end
	...
	local dbid, entrance, exit, interiorType, interiorElement = exports.interior_system:findProperty( thePlayer )
	if not isElement(interiorElement) then
		outputChatBox("Interior não encontrado.", thePlayer, 255, 0, 0 )
		return
	end
	...
	if intStatus.owner ~= charId then
		outputChatBox("Você deve possuir este interior para executar esta ação.", thePlayer, 255, 0, 0 )
		return
	end
