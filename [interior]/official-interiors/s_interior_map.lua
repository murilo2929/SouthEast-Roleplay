...
interiors = {
	...
	[175] = { 3, 1025.4520263672, 1215.1385498047, 1485.0085449219, 60 , "Interior Prisão"  },
	[176] = { 10, 1564.1539306641, 1799.4028320313, 2083.4116210938, 0 , "Interior Hospital"  }
}
...
function printInteriorsList(thePlayer, commandName,  from, to )
	...
	if not from or not to or from > to then
		outputChatBox("SYNTAX: /" .. commandName .. " [Do ID] [Até ID]", thePlayer, 255, 194, 15)
		return false
	end
	local count = 0
	local linkForum = "https:breve"
	for _,interior in pairs(interiors) do
		count = count + 1
	end

	outputChatBox("Lista de "..count.." interiores oficiais de "..from.." para "..to..":", thePlayer, 170,170,170)
