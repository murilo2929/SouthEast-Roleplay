	...
	elseif name then
		if (inttype==3) then -- Interior name and Owner for rented
			...
			if (exports.integration:isPlayerTrialAdmin(localPlayer) and getElementData(localPlayer, "duty_admin") == 1) or exports.global:hasItem(localPlayer, 4, ID) then
				gOwnerName = guiCreateLabel(0.0, 0.90, 1.0, 0.3, "Alugado por: " .. tostring(ownerName), true)
			...
			if (exports.integration:isPlayerTrialAdmin(localPlayer) and getElementData(localPlayer, "duty_admin") == 1) or exports.global:hasItem(localPlayer, 4, ID) or exports.global:hasItem(localPlayer, 5, ID) then
				gOwnerName = guiCreateLabel(0.0, 0.90, 1.0, 0.3, "Proprietario: " .. tostring(ownerName), true)
				guiSetFont(gOwnerName, "default")
				guiLabelSetHorizontalAlign(gOwnerName, "center", true)
				guiSetAlpha(gOwnerName, 0.0)
			end
		...
		if (ownerName=="None") and (inttype==3) then -- Unowned type 3 (rentable)
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Aperte F para alugar por $" .. tostring(exports.global:formatMoney(cost)) .. ".", true)
		...
		elseif (ownerName=="None") and (inttype<2) then -- Unowned any other type
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Aperte F para comprar por $" .. tostring(exports.global:formatMoney(cost)) .. ".", true)
			guiSetFont(gBuyMessage, "default")
			guiLabelSetHorizontalAlign(gBuyMessage, "center", true)
			guiSetAlpha(gBuyMessage, 0.0)
		else
			local msg = "Aperte F para entrar."
...
function showHouseMenu( absX, absY )
	...
	row.lock = rightclick:addRow("Trancar/Destrancar")
	addEventHandler("onClientGUIClick", row.lock, lockUnlockHouse, false)

	row.knock = rightclick:addRow("Bater na porta")
	...
	if getElementType(house) == "interior" then
		if hasKey(houseID, true) then
			row.note = rightclick:addRow("Editar mensagem de boas vindas")
			addEventHandler("onClientGUIClick", row.note, function()
				...
				wBizNote = guiCreateWindow(posX,posY,width,height,"Edit Mensagem de boas vindas - "..(getElementData(house, "name") or ("Interior ID #"..tostring( houseID ))),false)
				local eBizNote = guiCreateEdit(9,22,488,40,"",false,wBizNote)
				local bRemove = guiCreateButton(9,68,163,28,"Remover",false,wBizNote)
				local bSave = guiCreateButton(172,68,163,28,"Salvar",false,wBizNote)
...
function knockHouse()
	...
	if tick - lastKnocked > 5000 then
		...
		outputChatBox("Aguarde um pouco para bater novamente.", 255, 0, 0)
	end
end
...
addEventHandler( "setPlayerInsideInterior2", getRootElement( ),
	function( targetLocation, targetInterior, furniture)
		...
		if false and targetInterior then
			...
			if string.sub(tostring(adminnote),1,8) ~= "userdata" and adminnote ~= "\n" and getElementData(localPlayer, "duty_admin") == 1 then
				outputChatBox("[INT MONITOR]: "..adminnote:gsub("\n", " ").."[..]", 255,0,0)
				outputChatBox("'/checkint "..getElementData(targetInterior, "dbid").." 'para detalhes.",255,255,0)
			end
		end
	end
)
...
function showLoadingProgress(stats_numberOfInts, delayTime)
	...
	local title = stats_numberOfInts.." interiores(ETA: "..string.sub(tostring((tonumber(delayTime)-5000)/(60*1000)), 1, 3).." minutos) estão sendo carregados. "
...
function purchasePropertyGUI(interior, cost, isHouse, isRentable, neighborhood)
	...
	purchaseProperty.window[1] = guiCreateWindow(607, 396, 499, 210, "Comprar Propiedade", false)
	...
	local btnTextSet = {"Comprar usando Dinheiro", "Comprar usando Banco"}
	if exports.global:hasItem(localPlayer, 262) and (cost <= 40000) and isHouse and not isRentable then
		btnTextSet = {"Comprar usando Token", "Comprar usando Banco"}
		exports.hud:sendBottomNotification(localPlayer, "Tip", "Você ta usando um personagem novo, e por isso você tem um token que pode ser usado para comprar essa casa!")
	end
	if exports.global:countTable(fTable) > 0 then
		btnTextSet = {"Comprar \npara uso própio", "Comprar \npara facção"}
	end
	...
	btnPosX = btnPosX + btnW + margin/2
	purchaseProperty.button[4] = guiCreateButton(btnPosX, 156, btnW, 43, "Ver Interior", false, purchaseProperty.window[1])
	guiSetProperty(purchaseProperty.button[4], "NormalTextColour", "FFAAAAAA")
	btnPosX = btnPosX + btnW + margin/2
	purchaseProperty.button[3] = guiCreateButton(btnPosX, 156, btnW, 43, "Fechar", false, purchaseProperty.window[1])
	guiSetProperty(purchaseProperty.button[3], "NormalTextColour", "FFAAAAAA")

	purchaseProperty.label[2] = guiCreateLabel(110, 44, 315, 20, "Você pode então selecionar o seu método de pagamento.", false, purchaseProperty.window[1])
	purchaseProperty.label[3] = guiCreateLabel(20, 70, 88, 15, "Nome do Interior:", false, purchaseProperty.window[1])
	purchaseProperty.label[6] = guiCreateLabel(20, 90, 93, 15, "Vizinhança:", false, purchaseProperty.window[1])
	purchaseProperty.label[4] = guiCreateLabel(20, 110, 100, 15, "Preço:", false, purchaseProperty.window[1])
	purchaseProperty.label[5] = guiCreateLabel(250, 110, 73, 15, "Taxa:", false, purchaseProperty.window[1])
	purchaseProperty.label[11] = guiCreateLabel(20, 130, 315, 15, "Você gostaria que os móveis fossem habilitados?", false, purchaseProperty.window[1]) -- Furniture
    ...
    purchaseProperty.rad[1] = guiCreateRadioButton(245, 128, 50, 20, "Sim", false, purchaseProperty.window[1])
    purchaseProperty.rad[2] = guiCreateRadioButton(295, 128, 50, 20, "Não", false, purchaseProperty.window[1])
	...
	function()
		...
		if btnText == "Comprar usando Dinheiro" then
			...
			closePropertyGUI()
		elseif btnText == "Comprar usando Token" then
			triggerServerEvent("buypropertywithtoken", localPlayer, interior, guiRadioButtonGetSelected(purchaseProperty.rad[1]))
			closePropertyGUI()
		else
			btnTextSet = {"Comprar usando Dinheiro", "Comprar usando Banco", "Comprar usando Token"}
			...
			if exports.global:hasItem(localPlayer, 262) and (cost <= 40000) and isHouse and not isRentable then
				exports.hud:sendBottomNotification(localPlayer, "Tip", "Você está usando um novo personagem, por isso você tem um token em seu personagem que pode ser usado para comprar esta casa!")
	...
	function()
		...
		if btnText == "Comprar usando Banco" then
			...
			if isRentable then
				outputChatBox("As facções não podem possuir propriedades alugáveis no momento.", 255, 0, 0)
			else
	...
	if isHouse then
		...
		purchaseProperty.label[1] = guiCreateLabel(50, 26, 419, 18, "Confirme as seguintes informações sobre esta propriedade.", false, purchaseProperty.window[1])
	...
	elseif isRentable then
		guiSetText(purchaseProperty.window[1], "Alugar Propiedade")
		purchaseProperty.label[1] = guiCreateLabel(50, 26, 419, 18, "Por favor, confirme as seguintes informações sobre este imóvel alugável.", false, purchaseProperty.window[1])
		guiLabelSetHorizontalAlign(purchaseProperty.label[1], "center", false)
		guiSetVisible(purchaseProperty.label[5], false)
		guiSetText(purchaseProperty.label[4], "Custo por Payday:")
	else
		local theTax = exports.payday:getPropertyTaxRate(1)
		guiSetText(purchaseProperty.window[1], "Comprar Negócio")
		purchaseProperty.label[1] = guiCreateLabel(50, 26, 419, 18, "Confirme as seguintes informações sobre esta propriedade comercial.", false, purchaseProperty.window[1])
		guiLabelSetHorizontalAlign(purchaseProperty.label[1], "center", false)
		taxtax = cost * theTax
		guiSetText(purchaseProperty.label[10], "$"..exports.global:formatMoney(taxtax).."")
	end
...
function startBuyingForFaction(interior, cost, isHouse, furniture)
    ...
    factionBuyGUI.window[1] = guiCreateWindow(766, 385, 399, 121, "Selecione a facção para este interior", false)
    ...
    factionBuyGUI.combobox[1] = guiCreateComboBox(13, 35, 366, 113, "Selecione uma facção para comprar", false, factionBuyGUI.window[1])
    factionBuyGUI.button[2] = guiCreateButton(268, 64, 111, 42, "Aceitar", false, factionBuyGUI.window[1])
    ...
    addEventHandler("onClientGUIClick", factionBuyGUI.button[2], function()
    	...
    	if name ~= "Selecione uma facção para comprar" then
    		...
    		outputChatBox("Por favor selecione uma facção.", 255, 0, 0)
    	end
    end, false)
