...
function sellProperty(thePlayer, commandName, bla)
	if bla then
		outputChatBox("Utilize /venderpropiedade para vender essa propiedade para outro jogador.", thePlayer, 255, 0, 0)
		return
	end
	...
	if dbid > 0 then
		if interiorType == 2 then
			outputChatBox("Você não pode vender uma propiedade governamental.", thePlayer, 255, 0, 0)
		elseif interiorType ~= 3 and commandName == "unrent" then
			outputChatBox("Você não aluga esta propriedade.", thePlayer, 255, 0, 0)
		else
			...
			if interiorStatus.owner == getElementData(thePlayer, "dbid") or ( leader and faction ) then
				...
				outputChatBox("Você não possui esta propiedade.", thePlayer, 255, 0, 0)
			end
		end
	else
		outputChatBox("Você não esta em uma propiedade.", thePlayer, 255, 0, 0)
	end
end
addCommandHandler("sellproperty", sellProperty, false, false)
addCommandHandler("venderpropiedade", sellProperty, false, false)
addCommandHandler("unrent", sellProperty, false, false)
addCommandHandler("desalugar", sellProperty, false, false)
...
function publicSellProperty(thePlayer, dbid, showmessages, givemoney, CLEANUP)
	...
	if query then
		...
		if interiorType == 0 or interiorType == 1 then
			...
			if interiorStatus.owner == getElementData(thePlayer, "dbid") then
				...
				if givemoney then
					...
					if exports.global:takeMoney(gov, money, true) then
						exports.bank:addBankTransactionLog(-getElementData(gov, "id"), interiorStatus.owner, money, 3 , "Interior vendido para o Governo", getElementData(interiorElement, "name").." (ID: "..getElementData(interiorElement, "dbid")..")" )
					end
				end

				if showmessages then
					if CLEANUP == "FORCESELL" then
						exports.global:sendMessageToAdmins("[INTERIOR]: "..exports.global:getPlayerFullIdentity(thePlayer).." forçou a venda do interior #"..dbid.." ("..getElementData(interiorElement,"name")..").")
					...
					elseif givemoney then
						outputChatBox("Você vendeu sua propiedade por " .. exports.global:formatMoney(money) .. "$.", thePlayer, 0, 255, 0)
					else -- Token
						outputChatBox("Você vendeu sua propriedade em troca de $ 0 por ter sido comprado anteriormente por um token de interior.", thePlayer, 0, 255, 0)
					end
				end
			...
			elseif exports.factions:isPlayerInFaction(thePlayer, interiorStatus.faction) then
				...
				if givemoney and faction then
					...
					if exports.global:takeMoney(gov, money, true) then
						exports.bank:addBankTransactionLog(-getElementData(gov, "id"), -interiorStatus.faction, money, 3 , "Interior vendido para o Governo", getElementData(interiorElement, "name").." (ID: "..getElementData(interiorElement, "dbid")..")" )
					end
				end
				...
				if showmessages then
					...
					elseif faction then
						if givemoney then
							outputChatBox("Você vendeu sua propiedade por " .. exports.global:formatMoney(money) .. "$ (Transferido para o Banco de '"..getTeamName(faction).."')", thePlayer, 0, 255, 0)
						else
							outputChatBox("Você vendeu sua propriedade em troca de $ 0 por ter sido comprado anteriormente por um token de interior.", thePlayer, 0, 255, 0)
						end
					end
			...
			if showmessages then
				if CLEANUP == "FORCESELL" then
					...
					outputChatBox("Você não esta mais alugando essa propiedade.", thePlayer, 0, 255, 0)
				end
			end
			-- take all keys
			call( getResourceFromName( "item-system" ), "deleteAll", interiorType == 0 and 4 or 5, dbid )
		...
		outputChatBox("Error 504914 - Report no DISCORD", thePlayer, 255, 0, 0)
	end
end
...
function unownProperty(intid, reason) --This function is meant to be used by the system or to be triggered from UCP remotely. Works similarly to the publicSellProperty however, it's simplier. / MAXIME / 2015.1.11
	if intid and tonumber(intid) then
		...
		return false, "Interior invalido"
	end
	...
	if int and int.id ~= mysql_null() then
		...
		return false, "Interior não existe na DB."
	end
	...
	if interiorElement then
		...
		return true, "Interior não esta carregado no servidor."
	end
end
...
function sellTo(thePlayer, commandName, targetPlayerName)
	...
	if dbid > 0 and not isPedInVehicle( thePlayer ) then
		...
		if interiorStatus.type == 2 then
			outputChatBox("Você não pode vender uma propiedade do governo.", thePlayer, 255, 0, 0)
		elseif not targetPlayerName then
			outputChatBox("SYNTAX: /" .. commandName .. " [nome parcial / id]", thePlayer, 255, 194, 14)
			outputChatBox("Vende a propiedade que você esta para o jogador.", thePlayer, 255, 194, 14)
			outputChatBox("Fale para o comprador usar /pagar para receber o dinheiro pela propriedade.", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 10 and getElementDimension(targetPlayer) == getElementDimension(thePlayer) then
					if not exports.global:canPlayerBuyInterior(targetPlayer) then
						outputChatBox(targetPlayerName .. " já possui muitos interiores.", thePlayer, 255, 0, 0)
						outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " tentei vender um interior para você, mas você já tem muitos interiores.", targetPlayer, 255, 0, 0)
						return false
					end
					if interiorStatus.tokenUsed then
						outputChatBox("Este interior foi comprado por meio de um token e, portanto, não pode ser vendido a outros jogadores. Use /venderpropiedade.", thePlayer, 255, 0, 0)
						return
					end
						...
						if getElementData(targetPlayer, "dbid") ~= interiorStatus.owner then
							if exports.global:hasSpaceForItem(targetPlayer, 4, dbid) then
								...
								if query then
									...
									if interiorType == 0 or interiorType == 1 then
										outputChatBox("Você vendeu com sucesso sua propiedade para " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
										outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " vendeu para você esta propiedade.", targetPlayer, 0, 255, 0)
									else
										outputChatBox(targetPlayerName .. " assumiu o seu contrato de aluguel.", thePlayer, 0, 255, 0)
										outputChatBox("Você assumiu o contrato de aluguel de" .. getPlayerName(thePlayer):gsub("_", " "),  targetPlayer, 0, 255, 0)
									end
								...
								outputChatBox(targetPlayerName .. " não tem espaço para as chaves da propiedade.", thePlayer, 255, 0, 0)
							end
						else
							outputChatBox("você não pode vender para você mesmo.", thePlayer, 255, 0, 0)
						end
					else
						outputChatBox("Esta propiedade não é sua.", thePlayer, 255, 0, 0)
					end
				else
...
addCommandHandler("vender", sellTo)
...
function buyInterior(player, pickup, cost, isHouse, isRentable)
	if not exports.global:canPlayerBuyInterior(player) then
		outputChatBox("Você já tem muitos interiores.", player, 255, 0, 0)
		return false
	end
	...
	if isRentable then
		...
		if result then
			...
			if count ~= 0 then
				outputChatBox("Você já aluga outra casa.", player, 255, 0, 0)
				return
			end
		end
	elseif not exports.global:hasSpaceForItem(player, 4, 1) then
		outputChatBox("Você não tem espaço para as chaves.", player, 255, 0, 0)
		return
	end
	...
	if exports.global:takeMoney(player, cost) then
		...
		outputChatBox("Desculpa, você não pode pagar por isso.", player, 255, 194, 14)
	end
end
...
function buypropertyForFaction(interior, cost, isHouse, furniture, factionName)
	...
	outputChatBox("Parabéns! Você acabou de comprar esta propiedade para sua facção '"..factionName.."' por $" .. exports.global:formatMoney(cost) .. ".", source, 255, 194, 14)
...
function buyInteriorCash(pickup, cost, isHouse, isRentable, furniture)
	if not exports.global:canPlayerBuyInterior(client) then
		outputChatBox("Você já atingiu o número máximo de interiores. Você pode estender seus interiores máximos em F10 -> Recursos Premium.", client, 255, 0, 0)
		return
	end
	...
	if isRentable then
		...
		if result then
			local count = tonumber(result['cntval'])
			if count ~= 0 then
				outputChatBox("Você já esta alugando uma outra casa.", client, 255, 0, 0)
				return
			end
		end
	elseif not exports.global:hasSpaceForItem(client, 4, 1) then
		outputChatBox("Você não tem espaço para as chaves.", client, 255, 0, 0)
		return
	end
	...
	if exports.global:takeMoney(client, cost) then
		...
		outputChatBox("Desculpe, você não pode comprar esta propriedade.", client, 255, 194, 14)
	end
end
...
function buyInteriorBank(pickup, cost, isHouse, isRentable, furniture)
	if not exports.global:canPlayerBuyInterior(client) then
		outputChatBox("YoVocê já atingiu o número máximo de interiores. Você pode estender seus interiores máximos em F10 -> Recursos Premium.", client, 255, 0, 0)
		return
	end
	...
	if not exports.bank:takeBankMoney(client, cost) then
		outputChatBox( "Você não tem dinheiro em seu banco para comprar este imóvel", client, 255, 0, 0 )
	else
		local charid = getElementData(client, "dbid")
		local pickupid = getElementData(pickup, "dbid")
		local gov = getTeamFromName("Los Santos City Government")
...
function buyInteriorToken(pickup, furniture)
	...
	if not exports.global:takeItem(client, 262) then
		outputChatBox( "Você não tem um token para comprar esta propriedade", client, 255, 0, 0 )
	else
		local charid = getElementData(client, "dbid")
		local pickupid = getElementData(pickup, "dbid")
		outputChatBox("Parabéns! Você acabou de usar um token para comprar esta casa, lembre-se que esta casa não tem valor em dinheiro e você não pode vendê-la a amigos.", client, 255, 194, 14)
...
function moveSafe ( thePlayer, commandName )
	...
	if (dbid < 19000 and (exports.global:hasItem( thePlayer, 5, dbid ) or exports.global:hasItem( thePlayer, 4, dbid))) or (dbid >= 20000 and exports.global:hasItem(thePlayer, 3, dbid - 20000)) then
		if getPedContactElement(thePlayer) == safeTable[dbid] or getPedContactElement(thePlayer) == exports.vehicle:getSafe(dbid-20000) then
            outputChatBox("Mova para uma nova posição antes de reposicionar um cofre.", thePlayer, 255, 0, 0)
        else
			z = z - 0.5
			rotz = rotz + 180
			...
			elseif dbid > 0 and getSafe( dbid ) then
				if not updateSafe( dbid, { x, y, z }, rotz ) then
					outputChatBox("Ocorreram erros durante a movimentação do cofre.", thePlayer, 255, 0, 0)
				end
			else
				outputChatBox("Você precisa de um cofre primeiro!", thePlayer, 255, 0, 0)
			end
		end
	else
		outputChatBox("Você precisa das chaves deste interior para mover o Cofre.", thePlayer, 255, 0, 0)
	end
	outputChatBox("WARNING: Esses tipos de cofres estão obsoletos. Você pode comprar um Cofre de Armazenamento em qualquer loja de eletrônicos.", thePlayer, 255, 0, 0)
end
addCommandHandler("movesafe", moveSafe)
addCommandHandler("movercofre", moveSafe)
	...
	if found and itemValue then
		...
		if getElementData(interiorElement, "keypad_lock") then
			if not (exports.integration:isPlayerTrialAdmin(source) and getElementData(source, "duty_admin") == 1) then
				exports.hud:sendBottomNotification(source, "Fechadura digital sem chave", "Esta porta não tem chave, você deve usar o teclado para acessá-la.")
				return false
			end
		end
		...
		if locked == 0 then
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					...
					exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") desbloqueou o interior ID #"..itemValue.." sem chave.")
					exports.global:sendLocalText(source, " * A porta agora deve estar aberta. *", 255, 51, 102, 30, {}, true)
					exports["interior-manager"]:addInteriorLogs(itemValue, "unlock sem a chave", source)
				end
			else
				triggerEvent('sendAme', source, "coloca a chave na porta para destrancar.")
			end
			...
			if byAdmin then
				if getElementData(source, "hiddenadmin") == 0 then
					local adminTitle = exports.global:getPlayerAdminTitle(source)
					local adminUsername = getElementData(source, "account:username")
					exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") trancou o interior ID #"..itemValue.." sem chave.")
					exports.global:sendLocalText(source, " * A porta agora deve estar trancada. *", 255, 51, 102, 30, {}, true)
					exports["interior-manager"]:addInteriorLogs(itemValue, "lock sem a chave", source)
				end
			else
	...
	function( id, usingKeypad, playSoundAt )
		...
		if id and tonumber(id) and (hasKey1 or usingKeypad) then
			...
			if not usingKeypad then
				...
				if locked == 0 then
					if byAdmin then
						if getElementData(source, "hiddenadmin") == 0 then
							local adminTitle = exports.global:getPlayerAdminTitle(source)
							local adminUsername = getElementData(source, "account:username")
							exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") desbloqueou o interior ID #"..id.." sem a chave.")
							exports.global:sendLocalText(source, " * A porta agora deve estar aberta. *", 255, 51, 102, 30, {}, true)
							exports["interior-manager"]:addInteriorLogs(id, "unlock sem a chave", source)
						end
					else
						triggerEvent('sendAme', source, "coloca a chave na porta para destrancar.")
					end
					...
					if byAdmin then
						if getElementData(source, "hiddenadmin") == 0 then
							local adminTitle = exports.global:getPlayerAdminTitle(source)
							local adminUsername = getElementData(source, "account:username")
							exports.global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(source):gsub("_", " ").. " ("..adminUsername..") trancou o interior ID #"..id.." sem a chave.")
							exports.global:sendLocalText(source, " * A porta agora deve estar trancada. *", 255, 51, 102, 30, {}, true)
							exports["interior-manager"]:addInteriorLogs(id, "lock sem a chave", source)
						end
					else
						triggerEvent('sendAme', source, "coloca a chave na porta para trancá-la.")
					end
				...
				if locked == 0 then
					triggerEvent('sendAme', source, "destranca a porta.")
					exports.logs:dbLog(source, 31, {  "in"..tostring(id) }, "UNLOCK INTERIOR")
				else
					newRealLockedValue = true
					triggerEvent('sendAme', source, "tranca a porta.")
					exports.logs:dbLog(source, 31, {  "in"..tostring(id) }, "LOCK INTERIOR")
				end
...
function timedInteriorView(houseID)
	...
	if entrance then
		...
		outputChatBox( "Agora você está visualizando esta propriedade. Você não poderá largar nenhum item. Você pode sair de sua visualização saindo do interior ou aguardar o cronômetro de 60 segundos.", client, 0, 255, 0)
		...
		if getElementDimension(client) > 0 then
			...
			outputChatBox( "Se você quer de ver o interior de mais ângulos, use /freecam.", client, 0, 255, 0 )
		end
		...
		outputChatBox( "Casa Invalida.", client, 255, 0, 0 )
	end
end
...
function endTimedInteriorView(thePlayer, changedCharacter )
	...
	if info and pos then
		...
		if entrance then
			...
			outputChatBox( "Seu tempo acabou.", thePlayer, 0, 255, 0 )
		else
