...
local function makeFonts()
    intNameFont = intNameFont or dxCreateFont( ":resources/Anton.ttf", 30 ) or "default-bold"
    BizNoteFont = BizNoteFont or dxCreateFont( ":resources/Anton.ttf", 21 ) or "default-bold"
end
...
function renderInteriorName()
    if hitPickup and isElement(hitPickup) then
        ...
        makeFonts()
        local intInst = "Pressione F para entrar"
        local intStatus = getElementData(theInterior, "status")
        --Draw int name
        local intName = "" --ELEVADOR
        ...
        if isLastSourceInterior then
			...
			if intAddress and intAddress ~= "" then
				local intAddress = "Endereço: "..intAddress
            ...
            if canPlayerKnowInteriorOwner(theInterior) then -- House or Biz
                ...
                if intStatus.owner > 0 then
                    ...
                    if intType == 3 then
                        intOwner = "Rented by "..(ownerName or "..Carregando..")
                        intInst = "Pressione F para entrar"
                    elseif intType ~= 2 then
                        intOwner = "Propriedade de "..(ownerName or "..Carregando..")
                        intInst = "Pressione F para entrar"
                    end
                elseif intStatus.faction > 0 then
                    ...
                    if intType == 2 then
                        --intOwner = "Propriedade de no-one"
                        intInst = "Pressione F para entrar"
                    elseif intType == 3 then
                        local intPrice = exports.global:formatMoney(intStatus.cost)
                        intOwner = "Para aluguel: $"..intPrice
                        intInst = "Pressione F para alugar"
                    else
                        local intPrice = exports.global:formatMoney(intStatus.cost)
        ...
        dxDrawText ( intName or "Interior Desconhecido", n_l+textShadowDistance , n_t+textShadowDistance , n_r+textShadowDistance, n_b+textShadowDistance, tocolor(0,0,0,255),
                    1, intNameFont, "center", "center", false, true )
        dxDrawText ( intName or "Interior Desconhecido", n_l , n_t , n_r, n_b, textColor,
                    1, intNameFont, "center", "center", false, true )
