	...
	if (logged==1) then
		...
		if results then
			for _, res in pairs( results ) do
				local tab = {}
				tab.id = res.txn_id
				tab.details = "PAYPAL - DE "..(res.donor or 'DESCONHECIDO').." ("..res.payer_email..") to "..(res.donated_for or 'DESCONHECIDO')
		...
		if results then
			for _, res in pairs( results ) do
				...
				tab.details = "MOBILE - DE "..res.sender_phone.." ("..res.operator.." - "..res.country..")"
...
function activateDonationPerk(thePerk, ...)
	...
	if gameAccountID and gameAccountID > 0 then
		...
		if (mResult1) then
			...
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Falha ao recuperar seus dados do servidor.")
			showDonationGUI(client)
			return false
		end
	else
		triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Falha ao recuperar o ID da sua conta.")
		showDonationGUI(client)
		return false
	end
	...
	if thePerk == 13 then -- GC transfer
		...
		if (data.total <= donPoints) then
			...
			if not result then
				triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Não foi possível transferir GC. Razão: "..message)
				showDonationGUI(client)
				return false
			end

			addPurchaseHistory(client, "Game Coins transferidas ("..data.amount.." GCs para "..data.target..")", -data.total)
			showDonationGUI(client)
			...
			if dbExec(exports.mysql:getConn("core"), "UPDATE `accounts` SET `credits`=`credits`+? WHERE `username`=? ", data.amount, data.target) then
				for i, player in pairs(getElementsByType("player")) do
					if getElementData(player, "account:username") == data.target then
						...
						triggerClientEvent(player, "displayAchievement", player, "GAME COINS RECEBIDAS!", exports.global:getPlayerFullIdentity(client, 1).." presenteou você "..data.amount.." GCs.", data.amount)
						addPurchaseHistory(player, "Game Coins recebidas de "..exports.global:getPlayerFullIdentity(client, 1).." ("..data.amount.." GCs)", data.amount)
						return true
					end
				end
			end
			addPurchaseHistory(data.target, "Game Coins recebidas de "..exports.global:getPlayerFullIdentity(client, 1).." ("..data.amount.." GCs)", data.amount)
		else
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Você não tem GCs para transferir.")
			showDonationGUI(client)
			return false
		end
	elseif thePerk == 14 then -- maxints
		if (tonumber(perkDetails[2])*(tonumber(getElementData(client, "maxinteriors"))-9)*2 <= donPoints) then
			...
			if not result then
				triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação: "..message)
				showDonationGUI(client)
				return false
			else
				showDonationGUI(client)
			...
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Você não tem pontos suficientes para ativar '"..perkDetails[1] .."'")
			showDonationGUI(client)
			return false
		end
	...
	elseif thePerk == 17 then -- keypad door lock
		...
		if not data or not data[1] or not data[2] then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação: This interior is defected.")
			showDonationGUI(client)
			return false
		end
		...
		if not canPlayerCarryLocks() then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação: Your inventory is full!")
			showDonationGUI(client)
			return false
		end
		...
		if not takeGC then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação: "..takeGcError)
			showDonationGUI(client)
			return false
		end

		if exports.global:giveItem(client, 169, data[1]) and exports.global:giveItem(client, 169, data[1]) then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Perk ativado! Você recebeu um par de fechaduras com teclado digital sem chave.")
			addPurchaseHistory(client, perkDetails[1].." (For "..data[2]..", ID #"..data[1]..")", -perkDetails[2])
			showDonationGUI(client)
			return true
		else
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação, please take screenshot and make a GC refund request on forums.")
			showDonationGUI(client)
			return false
		end
	elseif thePerk == 28 then
		if (perkDetails[2] <= donPoints) then
			...
			if not result then
				...
				showDonationGUI(client)
				triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Perk ativado! Mude para a guia 'Benefícios ativados' para configurar sua estação.")
				return true
			end
		else
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Você não tem pontos suficientes para ativar '"..perkDetails[1] .."'")
	...
	elseif thePerk == 32 then
		...
		if currentHoursPlayed >= 15 then
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação: You've already had 15 or more hours played on this character.")
			showDonationGUI(client)
			return false
		end
		...
		showDonationGUI(client)
		triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Seu personagem recebeu instantâneo 15 horas jogadas!")
	...
	elseif thePerk == 34 then --learn language
		...
		if language > 0 and language <= exports["language-system"]:getLanguageCount() then
			...
			if langName == "<Invalid/Bugged Language>" then
				triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação: Invalid language!")
				return false
			end
				...
				if skill < 100 then
					...
					if result then
						...
						if not takeGC then
							...
							showDonationGUI(client)
							triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Seu personagem aprendeu "..tostring(langName).."!")
							addPurchaseHistory(client, perkDetails[1], -perkDetails[2])
							return true
						end
					else
						triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação: "..tostring(resultMsg))
						return false
					end
				else
					triggerClientEvent(client, "donation-system:getResponseFromServer", client, 1, "Erro ao ativar o benefício de doação: Seu personagem já aprendeu totalmente "..tostring(langName).."!")
				end
			else
				local result, resultMsg = exports["language-system"]:learnLanguage(client, language, false, 100)
				if result then
...
function removePerk(perkID)
	if not tonumber(perkID) then
		triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Falha ao enviar dados para o servidor.")
		showDonationGUI(client)
		return false
	else
	...
	if (hasPlayerPerk(client, perkID)) then
		...
		if gameAccountID and gameAccountID > 0 then
			...
			loadAllPerks(client)
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Perk removido.")
			showDonationGUI(client)
			return true
		else
			triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Falha ao recuperar o ID da sua conta.")
			showDonationGUI(client)
			return false
		end
	else
		triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Você não tem mais esse benefício.")
		showDonationGUI(client)
		return false
	end

	triggerClientEvent(client, "donation-system:getResponseFromServer", client, 2, "Algo deu errado.")
	showDonationGUI(client)
	return false
end
