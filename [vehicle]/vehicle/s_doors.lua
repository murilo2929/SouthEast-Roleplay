...
function controlRamp(theVehicle)
	...
	if not (exports['item-system']:hasItem(theVehicle, 117)) then
		outputChatBox("Você precisa do item no inventário do carro antes de fazer isso!", client, 255, 0, 0)
		return
	end

	if not (getElementData(theVehicle, "handbrake") == 1) then
		outputChatBox("Você precisa usar o freio de mão do veículo antes de implantar a rampa!", client, 255, 0, 0)
		return
	end
