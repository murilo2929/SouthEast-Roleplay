...
local function hasCreatePermission( player )
	return exports.integration:isPlayerLeadAdmin(player) or exports.integration:isPlayerLeadScripter(player) or exports.integration:isPlayerVehicleConsultant(player)
end]]

-- makeveh helpers
local function hasCreatePermission( player )
	return exports.integration:isPlayerScripter(player) or exports.integration:isPlayerHeadAdmin(player)
end

local function printVehicleCreateSyntax( player, command )
	outputChatBox("SYNTAX: /" .. command .. " [ID Da Veh Lib] [color1] [color2] [Dono] [Veículo Facção (1/0)] [-1=preço loja] [Insulfilm] ", player, 255, 194, 14)
	outputChatBox("NOTE: Se for um veículo de facção, a propriedade será dada ao ID especificado no campo do proprietário.", player, 255, 194, 14)
	outputChatBox("NOTE: Se for um veículo de facção, o custo é retirado do fundo da facção, ao invés do jogador.", player, 255, 194, 14)
end
...
function createPermanentVehicle(player, command, ...)
	if hasCreatePermission( player ) then
		...
		if not vehShopData then
			outputChatBox('Especificado ID de veículo inválido, use um ID de veículo de /vehlib.', player, 255, 100, 100)
			return
		end
		...
		if not id then -- the vehicle doesn't have a proper ID
			outputChatBox('O veículo não tem um modelo MTA definido, defina um usando /vehlib.', player, 255, 100, 100)
			return
		end
		...
		if isFactionOwned then
			...
			if not theTeam then
				outputChatBox("Não foi possível encontrar a facção com o ID: " .. owner .. ".", player, 255, 100, 100 )
				return
			end
			factionID = tonumber(owner)
			owner = -1
			if not exports.global:takeMoney(theTeam, cost) then
				outputChatBox("[MAKEVEH] Esta facção não pode pagar este veículo.", player, 255, 100, 100)
				outputChatBox("Essa facção não pode pagar este veículo.", player, 255, 100, 100)
				return
			end
		else
			...
			if other then
				...
				if not exports.global:canPlayerBuyVehicle(other) then
					outputChatBox("[MAKEVEH] Este jogador tem muitos carros.", player, 255, 0, 0)
					outputChatBox("Voce tem muitos carros.", other, 255, 0, 0)
					return
				elseif not exports.global:takeMoney(other, cost) then
					outputChatBox("[MAKEVEH] Este jogador não pode pagar este veículo.", player, 255, 0, 0)
					outputChatBox("Você não pode pagar este veículo.", other, 255, 0, 0)
					return
				end
			else
		...
		if (result) then
			...
			dbExec( exports.mysql:getConn('mta'), "INSERT INTO `vehicle_logs` (`vehID`, `action`, `actor`) VALUES (?, ?, ?) ", last_insert_id, command.." "..vehicleName.." ($"..cost.." - para "..owner..")" , adminID )

			if (hiddenAdmin==0) then
				exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(player) .. " ("..adminUsername..") spawnou um(a) "..vehicleName .. " (ID #" .. last_insert_id .. ") para "..owner.." por $"..cost..".")
				if not isFactionOwned then
					outputChatBox(tostring(adminTitle) .. " " .. getPlayerName(player) .. " spawnou um(a) "..vehicleName .. " (ID #" .. last_insert_id .. ") para "..owner.." por $"..cost..".", targetPlayer, 255, 194, 14)
				end
			else
				exports.global:sendMessageToAdmins("AdmCmd: Um admin oculto spawnou um(a) "..vehicleName .. " (ID #" .. last_insert_id .. ") para "..owner.." por $"..cost..".")
				if not isFactionOwned then
					outputChatBox("Um admin oculto spawnou um(a) "..vehicleName .. " (ID #" .. last_insert_id .. ") para "..owner.." por $"..cost..".", targetPlayer, 255, 194, 14)
				end
			end
			outputChatBox("[MAKEVEH] "..vehicleName .. " (ID #" .. last_insert_id .. ") spawnou com sucesso para "..owner..".", player, 0, 255, 0)

			if not isFactionOwned then
				outputChatBox("[MAKEVEH] $"..cost.." foi retirado do inventario do jogador.", player, 0, 255, 0)
				outputChatBox("$"..cost.." foi retirado de seu inventario.", targetPlayer, 0, 255, 0)
			else
				outputChatBox("[MAKEVEH] $"..cost.." foi retirado do baco da facção do jogador.", player, 0, 255, 0)
			end
				...
				--if (owner ~= -1) then
					if ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
						...
						if (locked) then
							...
							triggerEvent('sendAme', source, "destranca as portas do veículo.")
						else
							...
							triggerEvent('sendAme', source, "tranca as portas do veículo.")
						end
					else
...
function checkLock(thePlayer, seat, jacked)
	...
	if (locked) and not (jacked) then
		cancelEvent()
		outputChatBox("A porta esta trancada.", thePlayer)
	elseif getElementData(thePlayer, "restrain") == 1 then
		cancelEvent()
	end
end
...
function toggleLights(source, key, keystate)
	...
	if veh then
		if ( getElementData( veh, 'battery' ) or 100 ) > 0 then
			...
			exports.hud:sendBottomNotification( source, exports.global:getVehicleName(veh), "Bateria acabou." )
		end
	end
end
...
-- bike lock fix
function checkBikeLock(thePlayer, seat)
	if (isVehicleLocked(source)) and (getVehicleType(source)=="Bike" or getVehicleType(source)=="Boat" or getVehicleType(source)=="BMX" or getVehicleType(source)=="Quad" or getElementModel(source)==568 or getElementModel(source)==571 or getElementModel(source)==572 or getElementModel(source)==424 or getElementModel(source)==431 or getElementModel(source)==437) then
		...
		cancelEvent()
	elseif seat == 0 and getElementData(thePlayer, "restrain") == 1 then
		outputChatBox("Você não pode entrar no veículo algemado.", thePlayer, 255, 194, 15)
		cancelEvent()
	end
end
...
function(player, seat)
	local portaberta3 = getVehicleDoorOpenRatio (source, 3) --Frontal Esquerda
	local portaberta4 = getVehicleDoorOpenRatio (source, 4) --Traseira Esquerda
	local portaberta5 = getVehicleDoorOpenRatio (source, 5) -- Traseira Direita
	if getElementData(player,'restrain') == 1 then
		if (portaberta3 == 1) or (portaberta4 == 1) or (portaberta5 == 1) then
			if seat ~= 0 then
				 --outputChatBox("DIFERENTE 0.", player, 255, 194, 15)
			...
			cancelEvent()
			outputChatBox("PORTA FECHADA.", player, 255, 194, 15)
		end
	else
		outputChatBox("TA PRESO.", player, 255, 194, 15)
	end
end)
...
function(player, seat)
	if getElementData(source, "restrain") == 1 then
		if seat == 1 then
			setVehicleDoorOpenRatio (source, 3, 0)
		elseif seat == 2 then
			setVehicleDoorOpenRatio (source, 4, 0)
		elseif seat == 3 then
			setVehicleDoorOpenRatio (source, 5, 0)
		end
	end
end)]]
...
function setRealInVehicle(thePlayer)
	if isVehicleLocked(source) then
		...
		if owner < 0 and faction == -1 then
			exports.hud:sendBottomNotification( thePlayer, vehName, "(( Este "..vehName.." é um veículo civil. ))" )
		elseif (faction==-1) and (owner>0) then
			local ownerName = exports['cache']:getCharacterName(owner)
			if ownerName and exports.integration:isPlayerLeadAdmin( thePlayer, true ) then
				exports.hud:sendBottomNotification( thePlayer, vehName, "(( Este "..vehName.." pertence a " .. ownerName .. " ))" )
			end
		elseif (faction~=-1) then
			local factionName = exports.cache:getFactionNameFromId(faction) or "Facção Desconhecida"
			exports.hud:sendBottomNotification( thePlayer, vehName, "(( Este veículo pertence a " .. factionName .. ". ))" )
		end
	end
end
...
function removeFromFactionVehicle(thePlayer)
	...
	if (vfaction~=-1) then
		...
		local factionName = "Nenhuma (para ser deletado)"
...
function sellVehicle(thePlayer, commandName, targetPlayerName, itemValue)
	...
	if isPedInVehicle(thePlayer) then
		if not targetPlayerName then
			...
			outputChatBox("Vende o veículo em que você está para aquele jogador.", thePlayer, 255, 194, 14)
			outputChatBox("Peça ao comprador para usar /pagar para receber o dinheiro do veículo.", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer and getElementData(targetPlayer, "dbid") then
				...
				if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) < 20 then
						...
						if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") or isLeader or (exports.integration:isPlayerAdmin(thePlayer, true)) then
							if getElementData(targetPlayer, "dbid") ~= getElementData(theVehicle, "owner") then
								if getElementData(theVehicle, "token") then
									outputChatBox("Você não pode vender este veículo para outro jogador, pois você o comprou com um token.", thePlayer, 255, 0, 0)
									return
								end
								if exports.global:hasSpaceForItem(targetPlayer, 3, vehicleID) then
									if exports.global:canPlayerBuyVehicle(targetPlayer) then
										--if exports.integration:isPlayerLeadAdmin(thePlayer) --[[or exports['carshop-system']:isForSale(theVehicle)]] then
											...
											if query then
												...
												outputChatBox("Você vendeu com sucesso o seu " .. exports.global:getVehicleName(theVehicle) .. " para " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
												outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " vendeu a você  " .. exports.global:getVehicleName(theVehicle) .. ".", targetPlayer, 0, 255, 0)
												outputChatBox("Lembre-se de /park seu " .. exports.global:getVehicleName(theVehicle) .. ".", targetPlayer, 255, 255, 0)
										...
										outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " tentou vender um carro para você, mas você já tem carros demais.", targetPlayer, 255, 0, 0)
									end
								else
									outputChatBox(targetPlayerName .. " não tem espaço para as chaves do veículo.", thePlayer, 255, 0, 0)
									outputChatBox((getPlayerName(thePlayer):gsub("_", " ")) .. " tentou vender um carro para você, mas você não tem espaço para as chaves.", targetPlayer, 255, 0, 0)
								end
							else
					...
					outputChatBox("Você esta muito longe de " .. targetPlayerName .. ".", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function toggleSellExceptions (thePlayer, commandName, player)
	if (exports.integration:isPlayerLeadAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) and player then
	...
	elseif not player and (exports.integration:isPlayerLeadAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
...
function AdminVehicleSale(thePlayer, commandName, args)
	if isPedInVehicle(thePlayer) then
		...
		if ( vehType == ("Plane" or "Helicopter" or "Boat") or (getElementData(thePlayer, "temporarySell") == true ) or (exports.integration:isPlayerLeadAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) ) and not args then
			outputChatBox("SYNTAX: /" .. commandName .. " [nome parcial / id]", thePlayer, 255, 194, 14)
			outputChatBox("Vende o veículo em que você está para aquele jogador.", thePlayer, 255, 194, 14)
			outputChatBox("Peça ao comprador para usar /pagar para receber o dinheiro do veículo.", thePlayer, 255, 194, 14)
		elseif ( vehType == ("Plane" or "Helicopter" or "Boat") or (getElementData(thePlayer, "temporarySell") == true ) or (exports.integration:isPlayerLeadAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) ) and args then
			triggerEvent("sellVehicle", getResourceRootElement(), thePlayer, "sell", args)
		end
	end
end
addCommandHandler("vender", AdminVehicleSale)
				...
				if (getElementData(source, "realinvehicle") == 1) and not bikes[model] then
					...
					if seat == 0 or seat == 1 or exports.global:hasItem( source, 3, dbid ) then
						...
						if (locked) then
							setVehicleLocked(vehicle, false)
							triggerEvent('sendAme', source, "destranca as portas do veículo.")
							exports.logs:dbLog(source, 31, {  vehicle }, "DESTRANCADO DE DENTRO")
						else
							setVehicleLocked(vehicle, true)
							local doors = getDoorsFor(getElementModel(vehicle), -1)
							for index, doorEntry in ipairs(doors) do
							...
							triggerEvent('sendAme', source, "tranca as portas do veículo.")
							exports.logs:dbLog(source, 31, {  vehicle }, "TRANCADO DE DENTRO")
						end
					end
				end
			end
...
function lockUnlockOutside(vehicle)
	if (not source or exports.integration:isPlayerLeadAdmin(source)) or ( getElementData(vehicle, "Impounded") or 0 ) == 0 then
		local dbid = getElementData(vehicle, "dbid")
		...
		if (isVehicleLocked(vehicle)) then
			setVehicleLocked(vehicle, false)
			triggerEvent('sendAme', source, "pressiona a chave para destravar o veículo. ((" .. getVehicleName(vehicle) .. "))")
			exports.logs:dbLog(source, 31, {  vehicle }, "UNLOCK FROM OUTSIDE")
		else
			setVehicleLocked(vehicle, true)
			triggerEvent('sendAme', source, "pressiona a chave para trancar o veículo. ((" .. getVehicleName(vehicle) .. "))")
			local doors = getDoorsFor(getElementModel(vehicle), -1)
			for index, doorEntry in ipairs(doors) do
				setVehicleDoorOpenRatio(vehicle, doorEntry[2], 0, 0.5)
			end
...
function removeNOS(theVehicle)
	...
	triggerEvent('sendAme', source, "remove NITRO do " .. getVehicleName(theVehicle) .. ".")
...
local function canParkHere( thePlayer, veh )
	if call( getResourceFromName("tow-system"), "cannotVehpos", thePlayer, veh ) and not exports.integration:isPlayerLeadAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Não é possível estacionar seu veículo aqui.", thePlayer, 255, 0, 0)
	elseif isElementWithinColShape( thePlayer, HospitalCol ) and not exports.factions:isPlayerInFaction(thePlayer, 2) and not exports.integration:isPlayerLeadAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Somente o Serviço de Emergência Los Santos está autorizado a estacionar seus veículos em frente ao Hospital.", thePlayer, 255, 0, 0)
	elseif isElementWithinColShape( thePlayer, PershingSquareCol ) and not exports.factions:isPlayerInFaction(thePlayer, 1) and not exports.integration:isPlayerLeadAdmin(thePlayer, true) and not exports.integration:isPlayerSupporter(thePlayer, true) then
		return not outputChatBox("Apenas o Departamento de Polícia de Los Santos tem permissão para estacionar seus veículos na Praça Pershing.", thePlayer, 255, 0, 0)
	end
	return true
end
...
local function parkVeh( thePlayer, veh, commandName )
	if canParkHere( thePlayer, veh ) then
		...
		if (owner==playerid and TowingReturn == 0) or exports.global:hasItem(thePlayer, 3, dbid) or TowingReturn == 2 or exports.integration:isPlayerSupporter(thePlayer, true) or exports.integration:isPlayerLeadAdmin(thePlayer, true)  then
			if (dbid<0) then
				...
				outputChatBox("Posição de spawn do veículo definida.", thePlayer)
				-- logs
				...
				if ( getElementData(veh, "Impounded") or 0 ) > 0 then
					...
					if isElement( owner ) and exports.global:hasItem( owner, 2 ) then
						outputChatBox("((SFT&R)) #5555 [SMS]: Seu " .. getVehicleName(veh) .. " foi apreendido. Vá até o depósito para liberá-lo.", owner, 120, 255, 80)
					end
				end
			end
		end
...
addCommandHandler("estacionar", setVehiclePosition, false, false)
...
function setVehiclePosition2(thePlayer, commandName, vehicleID)
	if exports.integration:isPlayerTrialAdmin( thePlayer ) then
		...
		if not vehicleID or vehicleID < 0 then
			outputChatBox( "SYNTAX: /" .. commandName .. " [id veículo]", thePlayer, 255, 194, 14 )
		else
			...
			if veh then
				...
				exports.anticheat:changeProtectedElementDataEx(veh, "interior", interior)
				exports.anticheat:changeProtectedElementDataEx(veh, "dimension", dimension)
				outputChatBox("Posição de spawn do veículo id #" .. vehicleID .. " definida.", thePlayer)
				exports.logs:dbLog(thePlayer, 6, {  veh }, "PARK")
				for key, value in ipairs(destroyTimers) do
...
addCommandHandler("aestacionar", setVehiclePosition2, false, false)

function setVehiclePosition3(veh)
	if call( getResourceFromName("tow-system"), "cannotVehpos", source ) then
		outputChatBox("Apenas Los Santos Towing & Recovery tem permissão para estacionar seus veículos no estacionamento..", source, 255, 0, 0)
	elseif isElementWithinColShape( source, HospitalCol ) and not exports.factions:isPlayerInFaction(source, 2) and not exports.integration:isPlayerLeadAdmin(source) then
		outputChatBox("Somente o Serviço de Emergência Los Santos está autorizado a estacionar seus veículos em frente ao Hospital.", source, 255, 0, 0)
	elseif isElementWithinColShape( source, PershingSquareCol ) and not exports.factions:isPlayerInFaction(source, 1) and not exports.integration:isPlayerLeadAdmin(source) then
		...
		if (owner==playerid and TowingReturn == 0) or (exports.global:hasItem(source, 3, dbid)) or (TowingReturn == 2) or (exports.integration:isPlayerLeadAdmin(source)) then
			if (dbid<0) then
				...
				exports.anticheat:changeProtectedElementDataEx(veh, "interior", interior)
				exports.anticheat:changeProtectedElementDataEx(veh, "dimension", dimension)
				outputChatBox("Posição definida.", source)
				exports.logs:dbLog(thePlayer, 6, {  veh }, "PARK")
				for key, value in ipairs(destroyTimers) do
					if (tonumber(destroyTimers[key][2]) == dbid) then
						local timer = destroyTimers[key][1]
			...
			outputChatBox( "Você não pode estacionar aqui.", source, 255, 0, 0 )
		end
	end
end
...
function setVehiclePosition4(thePlayer, commandName, vehicle)
	...
	if not veh or commandName and not vehicle and getElementData(thePlayer, "realinvehicle") == 0 then
		...
		if (leader) and (f) then
			...
			exports.anticheat:changeProtectedElementDataEx(veh, "interior", interior)
			exports.anticheat:changeProtectedElementDataEx(veh, "dimension", dimension)
			outputChatBox("Posição de spawn do veículo id #" .. dbid .. " definida.", thePlayer)
			exports.logs:dbLog(thePlayer, 6, {  veh }, "PARK")
...
function detachVehicle(thePlayer)
	if isPedInVehicle(thePlayer) and getPedOccupiedVehicleSeat(thePlayer) == 0 then
		...
		if getVehicleTowedByVehicle(veh) then
			...
			outputChatBox("O trailer foi desconectado.", thePlayer, 0, 255, 0)
		else
			outputChatBox("Não tem um trailer...", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("detach", detachVehicle)
addCommandHandler("detachtrailer", detachVehicle)
