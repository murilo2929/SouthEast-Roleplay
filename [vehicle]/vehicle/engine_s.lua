...
function startEngine( players, veh )
	if not startings[ veh ] or getTickCount() - startings[ veh ] > 5000 then
		...
		if ( ( getElementData( veh, 'battery' ) or 100 ) > 0 ) or batteryExceptions( veh ) then
			...
			exports.hud:sendBottomNotification( client, exports.global:getVehicleName( veh ), "Bateria morreu." )
			triggerEvent('sendAme', client, "tenta ligar o motor, mas falha.")
			status = 'nobatt'
			return
		end
		...
		elseif status == 'hotwired' then 
			...
			triggerEvent('sendAme', client, "desce, acionando alguns cabos.")

			-- logs
			exports.vehicle_manager:addVehicleLogs( vid , "O mecanismo foi iniciado através do método hotwire devido a ele já ter sido configurado como hotwire por um administrador.", client )
			...
			setTimer(function (client)
				...
				elseif status == 'enginebroke' then
					exports.hud:sendBottomNotification( client, exports.global:getVehicleName( veh ), "Motor está quebrado." )
					triggerEvent('sendAme', client, "tenta ligar o motor, mas falha.")
				elseif status == 'nofuel' then
					exports.hud:sendBottomNotification( client, exports.global:getVehicleName( veh ), "O tanque de combustível está vazio." )
					triggerEvent('sendAme', client, "tenta ligar o motor, mas falha.")
				end
			end, 1500, 1, client)
		end
