...
function openRadioManager(defaultStations1, donorStations1)
	if true then--canAccessManager() then
		...
		if gui.main and isElement(gui.main) then
			guiSetText(gui.main, "Estação de Radio")
			...
			gui.main = guiCreateWindow(x,y,w,h,"Gerente de estação de rádio | Carregando..",false)
			guiWindowSetSizable(gui.main, false)

			gui.tabpanel = guiCreateTabPanel(0.0122,0.0401,0.9757,0.87,true,gui.main)
			gui.defaultStations = guiCreateTab("Estações padrão do servidor",gui.tabpanel)
			gui.donorStations = guiCreateTab("Estações de doadores",gui.tabpanel)
			...
			col.name = guiGridListAddColumn(grid.defaultStations,"Nome Estação",0.18)
			col.ip = guiGridListAddColumn(grid.defaultStations,"IP Estação",0.3)
			col.status = guiGridListAddColumn(grid.defaultStations,"Status",0.1)
			col.owner = guiGridListAddColumn(grid.defaultStations,"Dono Estação",0.1)
			col.expireDate = guiGridListAddColumn(grid.defaultStations,"Data Expira",0.16)
			...
			col2.name = guiGridListAddColumn(grid.donorStations,"Nome Estação",0.18)
			col2.ip = guiGridListAddColumn(grid.donorStations,"IP Estação",0.3)
			col2.status = guiGridListAddColumn(grid.donorStations,"Status",0.1)
			col2.owner = guiGridListAddColumn(grid.donorStations,"Dono Estação",0.1)
			col2.expireDate = guiGridListAddColumn(grid.donorStations,"Data Expira",0.16)
			col2.order = guiGridListAddColumn(grid.donorStations,"Order",0.08)

			gui.addNew = guiCreateButton(0.0135,0.9135,0.32476,0.0675,"Criar nova estação",true,gui.main)
			...
			gui.buyNew = guiCreateButton(0.0135,0.9135,0.32476,0.0675,"Comprar nova estação ("..perk[2].." GC)",true,gui.main)
			...
			gui.renew = guiCreateButton(0.0135,0.9135,0.32476,0.0675,"Renovar estação",true,gui.main)
			...
			gui.refresh = guiCreateButton(0.0135+0.32476,0.9135,0.32476,0.0675,"Sync estação para todos os clientes",true,gui.main)
			...
			gui.bClose = guiCreateButton(0.0135+0.32476*2,0.9135,0.32476,0.0675,"Fechar",true,gui.main)
...
function openNewStation(id, name, ip, state, order, buyNew)
	...
	local lineH = 25
	local col1 = 85
	gui.l1 = guiCreateLabel(margin, margin, w-margin*2, lineH, "NOVA ESTAÇÂO", false, gui.wNewStation)
	...
	gui.l2 = guiCreateLabel(margin, margin+lineH, col1, lineH, "Nome Estação:", false, gui.wNewStation)
	...
	gui.l3 = guiCreateLabel(margin, margin+lineH*2, col1, lineH, "Estação IP:", false, gui.wNewStation)
	...
	gui.bMoveUp = guiCreateButton(margin, margin+lineH/2+lineH*3, buttonW , lineH, buyNew and "Subir\n("..moveCost.." GCs)" or "Subir",false,gui.wNewStation)
	...
	gui.bMoveDown = guiCreateButton(margin+buttonW, margin+lineH/2+lineH*3, buttonW , lineH, buyNew and "Descer\n("..moveCost.." GCs)" or "Descer",false,gui.wNewStation)
	...
	gui.bSubmit = guiCreateButton(margin+buttonW*2, margin+lineH/2+lineH*3, buttonW , lineH, "Criar",false,gui.wNewStation)
	...
	addEventHandler("onClientGUIClick", gui.bSubmit, function()
		if source == gui.bSubmit then
			...
			if string.len(sName) < 1 or string.len(sIP) < 1 then
				...
				if guiGetText(gui.bSubmit) == "Salvar" then
					triggerServerEvent("editStation", localPlayer, id, sName, sIP)
					exports.global:playSoundCreate()
				elseif guiGetText(gui.bSubmit) == "Criar" then
					triggerServerEvent("createNewStation", localPlayer, sName, sIP)
					exports.global:playSoundCreate()
				elseif guiGetText(gui.bSubmit) == "Comprar" then
	...
	gui.bDelete = guiCreateButton(margin+buttonW*3, margin+lineH/2+lineH*3, buttonW , lineH, "Deletar",false,gui.wNewStation)
	...
	gui.bClose1 = guiCreateButton(margin+buttonW*4, margin+lineH/2+lineH*3, buttonW , lineH, "Fechar",false,gui.wNewStation)
	...
	if id then
		...
		guiSetText(gui.l1, "EDITAR ESTAÇÂO #"..id)
	else
		guiSetEnabled(gui.bMoveDown, false)
		guiSetEnabled(gui.bMoveUp, false)
		guiSetEnabled(gui.bDelete, false)
		if buyNew then
			guiSetText(gui.l1, "COMPRAR NOVA ESTAÇÂO")
		else
			guiSetText(gui.l1, "NOVA ESTAÇÂO")
		end
	end

	if state == "Ativado" then
		guiSetText(gui.bSubmit, "Desativado")
	elseif state == "Desativado" then
		guiSetText(gui.bSubmit, "Ativado")
	else
		if buyNew then
			guiSetText(gui.bSubmit, "Comprar")
		else
			guiSetText(gui.bSubmit, "Criar")
		end
	end

	local changeSubmitBtn = function ()
		if id and (source == gui.eName or source == gui.eIP) then
			guiSetText(gui.bSubmit, "Salvar")
		end
	end
...
function renewStation(station)
	...
	gui.l1 = guiCreateLabel(margin, margin, w-margin*2, lineH, "RENOVAR ESTAÇÂO", false, gui.wRenewStation)
	guiSetFont(gui.l1, "default-bold-small")
	guiLabelSetHorizontalAlign(gui.l1, "center", true)
	guiLabelSetVerticalAlign(gui.l1, "center", true)

	gui.l2 = guiCreateLabel(margin, margin+lineH, col1, lineH, "Você está prestes a renovar a estação de rádio ID#"..station[1]..":", false, gui.wRenewStation)
	--guiSetFont(gui.l2, "default-bold-small")
	guiLabelSetVerticalAlign(gui.l2, "center", true)

	gui.l3 = guiCreateLabel(margin, margin+lineH*2, col1, lineH, "Nome Estação: "..station[2], false, gui.wRenewStation)
	guiSetFont(gui.l3, "default-bold-small")
	guiLabelSetVerticalAlign(gui.l3, "center", true)
	...
	gui.l5 = guiCreateLabel(margin, margin+lineH*4, col1, lineH, "Dono: "..station[4], false, gui.wRenewStation)
	guiSetFont(gui.l5, "default-bold-small")
	guiLabelSetVerticalAlign(gui.l3, "center", true)

	gui.l6 = guiCreateLabel(margin, margin+lineH*5, col1, lineH, "Data de validade: "..station[5], false, gui.wRenewStation)
	...
	gui.b7days = guiCreateButton(margin, margin+lineH/2+lineH*6, buttonW , bH, "7 dias\n("..cost7.." GCs)",false,gui.wRenewStation)
	...
	gui.b30days = guiCreateButton(margin+buttonW, margin+lineH/2+lineH*6, buttonW , bH, "30 dias\n("..cost30.." GCs)",false,gui.wRenewStation)
	...
	gui.b90days = guiCreateButton(margin+buttonW*2, margin+lineH/2+lineH*6, buttonW , bH, "90 dias\n("..cost90.." GCs)",false,gui.wRenewStation)
	...
	gui.bClose1 = guiCreateButton(margin+buttonW*3, margin+lineH/2+lineH*6, buttonW , bH, "fechar",false,gui.wRenewStation)
	--guiSetFont(gui.bClose1, "default-bold-small")
	addEventHandler("onClientGUIClick", gui.bClose1, function()
		if source == gui.bClose1 then
			closeRenewStation()
