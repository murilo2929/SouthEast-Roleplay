...
function refreshCarShops(thePlayer, _, fromvehmang)
	if exports.integration:isPlayerLeadAdmin(thePlayer) then
		...
		exports.global:sendMessageToAdmins("[CARSHOPS] "..staffTitle.." "..staffUsername.." Renovou manualmente as concesssionarias.")
		exports.logs:dbLog(thePlayer, 4, thePlayer, "RESETCARSHOP")
	else
		outputChatBox("Agora você deve ser um administrador líder para usar esta função.", thePlayer, 255, 0, 0)
	end
end
...
function carshop_buyVehicle(paymentMethod)
	...
	if not exports.global:canPlayerBuyVehicle(client) then
		if isOverlayDisabled then
			outputChatBox("Você já atingiu o número máximo de veículos", client, 0, 255, 0)
		else
			exports.hud:sendBottomNotification(client, "Limite máximo de veículos", "Você já atingiu o número máximo de veículos. /stats para detalhes.")
		end
		return false
	end
	...
	if (paymentMethod == "cash") then
		if not exports.global:hasMoney(client, costCar) or costCar == 0 then
			if isOverlayDisabled then
				outputChatBox("Você não tem dinheiro suficiente para isso amigo..", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "Dinheiro é sempre um problema..", "Você não tem dinheiro suficiente para isso amigo..")
			end
			return false
		else
	...
	elseif (paymentMethod == "bank") then
		...
		if money < 0 or costCar == 0 then
			if isOverlayDisabled then
				outputChatBox("Você não tem dinheiro suficiente em sua conta bancária para isso amigo..", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "Dinheiro é sempre um problema..", "Você não tem dinheiro suficiente em sua conta bancária para isso amigo..")
			end
			return false
		else
	...
	elseif (paymentMethod == "token") then
		if costCar <= 35000 then
			if not exports.global:takeItem(client, 263) then
				outputChatBox("Houve um problema ao processar seu pedido.", client, 255, 0 ,0)
			end
		else
			outputChatBox("Você não pode usar um token em um carro que vale mais que $35,000.", client, 255, 0, 0)
		end
	else
		if isOverlayDisabled then
			outputChatBox("Não.", client, 0, 255, 0)
		else
			exports.hud:sendBottomNotification(client, "Não!", "Apenas não...")
		end
		return false
	end
	...
	if result then
		if tonumber(result["stock"]) < 1 then
			if isOverlayDisabled then
				outputChatBox("Desculpe, parece que este está em uso.", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "Estoque", "Desculpe, parece que este modelo está fora de estoque.")
			end
			return false
		end
	end
	...
	local license = (getElementData(client, "license.car") == 1) and "" or "Você não tem carteira de motorista. É melhor você não dirigir isso na rua."

	if isOverlayDisabled then
		outputChatBox("Parabéns, você acabou de comprar um veículo!", client)
		outputChatBox("Certifique-se de usar /estacionar no local que você deseja estacionar dentro de uma hora,", client)
		outputChatBox("caso contrário, o veículo será excluído, não recuperável.", client)
		outputChatBox(license, client)
		outputChatBox("Edite a descrição do veículo usando /ed.", client)
	else
		local content = {}
		table.insert(content, {"Certifique-se de usar /estacionar no local que você deseja estacionar dentro de uma hora ou o veículo será excluído!"})
		table.insert(content, {license})
		table.insert(content, {"Edite a descrição do veículo usando /ed."})

		exports.hud:sendBottomNotification(client, "Parabéns, você acabou de comprar um veículo!", content, 30)
	end
		...
		if shopname and shopnicename and vehicle_shop_id and vehYear and vehBrand and vehModel and vehPrice then
			...
			if orderedVehID and tonumber(orderedVehID) == tonumber(vehicle_shop_id) then
				if exports.global:hasItem(player, 2) then
					...
					exports.global:sendLocalText(player, "*"..playerName.." recebe uma mensagem de texto.", 255, 51, 102, 30, {}, true)
					outputChatBox("["..languagename.."] SMS de "..shopnicename..": Olá! Como você pediu, agora temos um "..itemName.." em estoque por $"..exports.global:formatMoney(vehPrice)..". Venha checar!" , player, 120, 255, 80)
				end
			end
		end
	end
