...
function pedTalk(state)
	if (state == 1) then
		--exports.global:sendLocalText(source, "Gabrielle McCoy fala: Welcome! Would you be unregistering, registering or changing your vehice plates today?", 255, 255, 255, 10)
		--outputChatBox("The fee is $".. exports.global:formatMoney(serverRegFee) .. " per vehicle.", source, 200, 200, 200)
	elseif (state == 2) then
		--exports.global:sendLocalText(source, "[English] Gabrielle McCoy fala: Sorry but the fee to register new plates is $" .. exports.global:formatMoney(serverRegFee) .. ". Please come back once you have the money.", 255, 255, 255, 10)
		outputChatBox(source, "Você não tem GCs para ativar este recurso.", 255,0,0)
	elseif (state == 3) then
		--exports.global:sendLocalText(source, "[English] Gabrielle McCoy fala: That is great! Lets get everything set up for you in our system.", 255, 255, 255, 10)
	elseif (state == 4) then
		exports.global:sendLocalText(source, "[English] Gabrielle McCoy fala: Não? Bem, espero que você mude de ideia mais tarde. Tenha um bom dia!", 255, 255, 255, 10)
	elseif (state == 5) then
		exports.global:sendLocalText(source, " *Gabrielle McCoy começa a inserir as informações em seu computador.", 255, 51, 102)
		exports.global:sendLocalText(source, "[English] Gabrielle McCoy fala: Tudo bem, você deve estar pronto para ir, eu enviarei alguém para lidar com a placa do seu veículo. Tenha um bom dia!", 255, 255, 255, 10)
	elseif (state == 6) then
		exports.global:sendLocalText(source, "[English] Gabrielle McCoy fala: Hmmm. De acordo com nossos registros, essa já é uma placa registrada.", 255, 255, 255, 10)
	elseif (state == 7) then
		exports.global:sendLocalText(source, "[English] Gabrielle McCoy fala: Bem, sinto muito, mas seu veículo não requer uma placa ou documentos registrados.", 255, 255, 255, 10)
	elseif (state == 8) then
		exports.global:sendLocalText(source, "[English] Gabrielle McCoy fala:Sinto muito, mas você é o proprietário deste veículo em papéis?", 255, 255, 255, 10)
	end
end
...
function setNewInfo(data, car)
	...
	if exports.donators:takeGC(client, perk[2]) then
		exports.donators:addPurchaseHistory(client, perk[1].. " (Placa: '"..data.."' para '"..exports.global:getVehicleName(tvehicle).."')", -perk[2])
...
function setNewReg(car)
	...
	if not exports.global:takeMoney(source, data == 1 and 175 or 50) then
		exports.global:sendLocalText(source, "[English] Gabrielle McCoy fala: Vai custar $"..(data == 1 and 175 or 50)..".", 255, 255, 255, 10)
	end
