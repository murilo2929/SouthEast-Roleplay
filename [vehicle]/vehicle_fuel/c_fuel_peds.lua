...
function requestATMInterfacePIN(theATM, atmLocationName)
	...
	if not (wPIN) then
		...
		wPIN = guiCreateWindow(x, y, width, height, "POS Machine at "..(atmLocationName or "Area Desconhecida"), false)
			guiWindowSetSizable(wPIN, false)

		local tabPanel = guiCreateTabPanel(0.05, 0.05, 0.9, 0.85, true, wPIN)
		local tabPersonal = guiCreateTab("Por favor, insira o codigo PIN", tabPanel)
...
function respondToATMInterfacePIN(Label_Error_Msg, r,g,b, action, otherInfo)
	...
	if action == "cardRemoved" then
		exports.hud:sendBottomNotification(localPlayer, "Máquina de Ponto de Venda", "A máquina POS não está funcionando corretamente porque o cartão do caixa eletrônico foi removido.")
		setTimer(closeATMInterfacePIN, 2000, 1)
	elseif action == "failedLessThan3" then
		exports.hud:sendBottomNotification(localPlayer, "Máquina de Ponto de Venda", "Acesso negado. PIN incorreto inserido.")
		guiSetEnabled(bEnter, true)
	elseif action == "locked" then
		exports.hud:sendBottomNotification(localPlayer, "Máquina de Ponto de Venda", "Número do cartão ATM '"..otherInfo.."' está bloqueado e não pode ser usado, entre em contato com o Banco.")
		guiSetEnabled(bEnter, true)
	elseif action == "success" then
		exports.hud:sendBottomNotification(localPlayer, "Máquina de Ponto de Venda", "Acesso permitido!")
		closeATMInterfacePIN()
		local option = 2
