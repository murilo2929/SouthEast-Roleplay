...
function checkPINCode(enteredCode)
	...
	if not foundAnATMCard then
		exports.hud:sendBottomNotification(source,"A máquina ATM não está funcionando corretamente!", "Isso é muito estranho, o cartão que você acabou de inserir, desapereceu magicamente!")
		return false
	end
	...
	if not check or not check["card_number"] then
			triggerClientEvent(source, "fuel:respondToATMInterfacePIN", source, "Acesso negado", 255,0,0, "failedLessThan3")
		return false
	end
	
	if check["card_locked"] == "1" then
		triggerClientEvent(source, "fuel:respondToATMInterfacePIN", source, "ERROR: Este cartão ATM não pode ser usado.", 255,0,0, "locked", cardNumber )
		return false
	end
	
	triggerClientEvent(source, "fuel:respondToATMInterfacePIN", source, "Acesso Permitido!", 70,255,14, "success" )
