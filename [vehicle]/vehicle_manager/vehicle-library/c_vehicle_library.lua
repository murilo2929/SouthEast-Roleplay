...
local function updateLibraryGrid(vehs)
	...
	for i = 1, #vehs do
		...
		guiGridListSetItemText(VehLibGrid, row, col.createdby, (accountNameBuilder(vehs[i].createdby) or "Ninguem") , false, false)
		guiGridListSetItemText(VehLibGrid, row, col.createdate, (vehs[i].createdate or "Ninguem"), false, true)
		guiGridListSetItemText(VehLibGrid, row, col.updatedby, (accountNameBuilder(vehs[i].updatedby) or "Ninguem"), false, false)
...
function showLibrary(vehs, thePed)
	...
	vehLib = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,"Sistema de veículos personalizados - Biblioteca de veículos",false)
	...
	col.enabled = guiGridListAddColumn(VehLibGrid,"Habilitado",0.06)
	col.mtamodel = guiGridListAddColumn(VehLibGrid,"MTA Model",0.15)
	col.brand = guiGridListAddColumn(VehLibGrid,"Marca",0.15)
	col.model = guiGridListAddColumn(VehLibGrid,"Modelo",0.15)
	col.year = guiGridListAddColumn(VehLibGrid,"Ano",0.1)
	col.price = guiGridListAddColumn(VehLibGrid,"Preço",0.1)
	col.tax = guiGridListAddColumn(VehLibGrid,"Imposto",0.1)
	col.updatedby = guiGridListAddColumn(VehLibGrid,"Atualizado Por",0.15)
	col.updatedate = guiGridListAddColumn(VehLibGrid,"Data Atualização",0.2)
	col.createdby = guiGridListAddColumn(VehLibGrid,"Criado Por",0.15)
	col.createdate = guiGridListAddColumn(VehLibGrid,"Data Criação",0.2)
	col.notes = guiGridListAddColumn(VehLibGrid,"Notas",0.5)
	...
	if thePed and isElement(thePed) and getElementData(thePed, "carshop") then
			...
			function( button )
				if button == "left" then
					...
					if row ~= -1 and col ~= -1 then
						...
						guiSetText(vehLib, "Você precisa selecionar um veículo da lista acima primeiro.")
						playError()
		...
		buttons["ordervehicle"] = guiCreateButton(0.148,0.9181,0.1237,0.0587,"Pedir Veículo",true,vehLib)
		...
		if playerOrderedFromShop then
			...
			buttons["cancelorder"] = guiCreateButton(0.148,0.9181,0.1237,0.0587,"Cancelar Pedido",true,vehLib)
			...
			addEventHandler( "onClientGUIClick", buttons["cancelorder"],
				function( button )
					if button == "left" then
						...
						triggerServerEvent("shop:storeKeeperSay", localPlayer, localPlayer, "Claro!" , getElementData(thePed, "name"))
						closeLibrary()
						playSuccess()
					end
				end,
		...
		buttons[1] = guiCreateButton(0.0115,0.9181,0.1237,0.0587,"Criar",true,vehLib)
		...
		buttons[2] = guiCreateButton(0.148,0.9181,0.1237,0.0587,"Ver/Modificar",true,vehLib)
			...
			function( button )
				if button == "left" then
					...
					if row ~= -1 and col ~= -1 then
						...
						guiSetText(vehLib, "Você precisa selecionar um registro da lista acima primeiro.")
						playError()
					end
				end
			end,
				...
				if button == "left" then
					...
					if row ~= -1 and col ~= -1 then
						...
						if createdby ~= getElementData(localPlayer, "account:username") and not exports.integration:isPlayerVehicleConsultant(localPlayer)  then
							guiSetText(vehLib, "Você só pode excluir os veículos que você adicionou. Notificar "..createdby.." se este veículo não for apropriado.")
							playError()
						else
		...
		buttons[5] = guiCreateButton(0.5574,0.9181,0.1237,0.0587,"Atualizar",true,vehLib)
		...
		buttons[13] = guiCreateButton(0.8304,0.8500,0.1569,0.0587, "Proxima Pagina",true,vehLib)
		...
		buttons[14] = guiCreateButton(0.6654,0.8500,0.1569,0.0587, "Pagina Anterior",true,vehLib)
		...
		buttons[15] = guiCreateButton(0.5000,0.8500,0.1569,0.0587, "Procurar",true,vehLib)
		...
		edits[10] = guiCreateEdit(0.0135,0.8500,0.4750,0.0580,"Marca, Modelo, Ano, Modelo MTA",true,vehLib)
	...
	buttons[6] = guiCreateButton(0.8304,0.9181,0.1569,0.0587,"Fechar",true,vehLib)
...
function searchLibrary(text)
	...
	if text == "Marca, Modelo, Ano, Modelo MTA" then 
		return false
	end
...
function addNewVehicle(veh)
	...
	addVehWindow = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,(veh.update and "Editar Veículo" or "Add novo veículo"),false)
	guiSetProperty(addVehWindow,"AlwaysOnTop","true")
	guiSetProperty(addVehWindow,"SizingEnabled","false")

	labels[1] = guiCreateLabel(0.0251,0.06,0.4292,0.0459,"Modelo Veículo MTA (Nome ou ID):",true,addVehWindow)
	...
	addEventHandler("onClientGUIComboBoxAccepted", gui["spawnto"], function()
		if guiComboBoxGetSelected(gui["spawnto"]) <= 0 then
			...
			if not isElement(edits[7]) then
				labels["stock"] = guiCreateLabel(0.0251,0.54,0.4292,0.0459,"Estoque Total:",true,addVehWindow)
	...
	labels["doortype"] = guiCreateLabel(0.516,0.435,0.4292,0.0459,"Portas:",true,addVehWindow)
	...
	labels[7] = guiCreateLabel(0.0251,0.5383+0.1046+0.015,0.4292,0.0459,"Nota(s):",true,addVehWindow)
	guiSetFont(labels[7],"default-bold-small")

	memos[1] = guiCreateMemo(0.0388,0.6224+0.07,0.9178,0.15,(veh.note or ""),true,addVehWindow)


	checkboxes[1] = guiCreateCheckBox(0.8,0.435,0.15,0.0459,"Habilitado",false,true,addVehWindow) --0.5383
	if veh.enabled and tonumber(veh.enabled) == 1 then
		guiCheckBoxSetSelected(checkboxes[1], true)
	end

	if veh.update then
		checkboxes[2] = guiCreateCheckBox(0.8,0.495,0.151,0.0459,"Copiar",false,true,addVehWindow)
	end


	buttons[8] = guiCreateButton(0.0388,0.8622,0.4475,0.0944,"Cancelar",true,addVehWindow)
	...
	buttons[9] = guiCreateButton(0.516,0.8622,0.4406,0.0944,"Validar",true,addVehWindow)
...
function editFuel(fuel)
	...
	editFuelWindow = guiCreateWindow((sx-w)/2,(sy-h)/2,w,h,"Configurações de combustível",false)
	guiSetProperty(editFuelWindow,"AlwaysOnTop","true")
	guiSetProperty(editFuelWindow,"SizingEnabled","false")

	labels["engine"] = guiCreateLabel(0.0251,0.0867,0.4292,0.0459,"Tipo de motor:",true,editFuelWindow)
	guiSetFont(labels["doortype"],"default-bold-small")

	gui["engine"] = guiCreateComboBox( 0.0388,0.1327,0.2,0.0791, "Gasolina", true, editFuelWindow)
	...
	labels[8] = guiCreateLabel(0.0251,0.185,0.4292,0.0459,"Consumo (litros/quilômetro):",true,editFuelWindow)
	guiSetFont(labels[8],"default-bold-small")

	edits[8] = guiCreateEdit(0.0388,0.235,0.4155,0.06,(fuel.con or ""),true,editFuelWindow)

	labels[9] = guiCreateLabel(0.516,0.185,0.4292,0.0459,"Capacidade (litros):",true,editFuelWindow)
	guiSetFont(labels[9],"default-bold-small")

	edits[9] = guiCreateEdit(0.5388,0.235,0.4178,0.06,(fuel.cap or ""),true,editFuelWindow)

	--shit
	buttons[11] = guiCreateButton(0.0388,0.8622,0.4475,0.0944,"Cancelar",true,editFuelWindow)
	...
	buttons[12] = guiCreateButton(0.516,0.8622,0.4406,0.0944,"Atualizar",true,editFuelWindow)
...
function validateCreateVehicle(data)
	if guiGetText(buttons[9]) == "Criar" or guiGetText(buttons[9]) == "Atualizar" then
		...
		if string.len(guiGetText(edits[2])) > 0 then
			guiSetText(labels[2], "Marca (OK!):")
			guiLabelSetColor(labels[2], 0, 255,0)
		else
			guiSetText(labels[2], "Marca (Invalido!):")
		...
		if string.len(guiGetText(edits[3])) > 0 then
			guiSetText(labels[3], "Modelo (OK!):")
			guiLabelSetColor(labels[3], 0, 255,0)
		else
			guiSetText(labels[3], "Modelo (Invalido!):")
		...
		if string.len(input) > 0 and tonumber(input) and tonumber(input) > 1000 and tonumber(input) < 3000 then
			guiSetText(labels[4], "Ano (OK!):")
			guiLabelSetColor(labels[4], 0, 255,0)
		else
			guiSetText(labels[4], "Ano (Invalido!):")
		...
		if string.len(input) > 0 and tonumber(input) and tonumber(input) > 0 then
			guiSetText(labels[5], "Preço (OK!):")
			guiLabelSetColor(labels[5], 0, 255,0)
		else
			guiSetText(labels[5], "Preço (Invalido!):")
		...
		if string.len(input) > 0 and tonumber(input) and tonumber(input) >= 0 then
			guiSetText(labels[6], "Imposto (OK!):")
			guiLabelSetColor(labels[6], 0, 255,0)
		else
			guiSetText(labels[6], "Imposto (Invalido!):")
		...
		if isElement(edits[7]) then
			...
			if string.len(input) > 0 and tonumber(input) and math.floor(tonumber(input)) >= 1 then
				guiSetText(labels[7], "Estoque Total (OK!):")
				guiLabelSetColor(labels[7], 0, 255,0)
			else
				guiSetText(labels[7], "Estoque Total (Invalido!):")
			...
			if string.len(input) > 0 and tonumber(input) and math.floor(tonumber(input)) >= 1 and math.floor(tonumber(input)) <= 100 then
				guiSetText(labels[8], "Taxa de Spawn (OK!):")
				guiLabelSetColor(labels[8], 0, 255,0)
			else
				guiSetText(labels[8], "Taxa de Spawn (Invalido!):")
		...
		if allGood then
			if data and data.update then
				guiSetText(buttons[9], "Atualizar")
			else
				guiSetText(buttons[9], "Criar")
			end
			playSuccess()
		else
			guiSetText(buttons[9], "Validar")
			playError()
		end
	end
end
...
function showConfirmDelete(id, brand, model, createdby)
	...
	labels[8] = guiCreateLabel(0.0254,0.2072,0.9645,0.1982,"Tem certeza que deseja deletar o veículo #"..id.."("..brand.." "..model..")?",true,editFuelWindow)
	guiLabelSetHorizontalAlign(labels[8],"center",false)
	labels[9] = guiCreateLabel(0.0254,0.4054,0.9492,0.2162,"Esta ação não pode ser desfeita!",true,editFuelWindow)
	guiLabelSetHorizontalAlign(labels[9],"center",false)
	buttons[10] = guiCreateButton(0.0254,0.6577,0.4695,0.2613,"Cancelar",true,editFuelWindow)
	addEventHandler( "onClientGUIClick", buttons[10], function()
		if source == buttons[10] then
			closeConfirmDelete()
		end
	end)
	buttons[11] = guiCreateButton(0.5051,0.6577,0.4695,0.2613,"Confirmar",true,editFuelWindow)
	addEventHandler( "onClientGUIClick", buttons[11], function()
		if source == buttons[11] then
