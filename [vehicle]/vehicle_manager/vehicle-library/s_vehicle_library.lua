...
function createVehicleRecord(data)
	...
	if not data.update then
		...
		if not mQuery1 then
			...
			outputChatBox("[VEHICLE MANAGER] Falha ao criar novo veículo #"..mQuery1.." na biblioteca.", client, 255,0,0)
			return false
		end
		sendLibraryToClient(client)
		outputChatBox("[VEHICLE MANAGER] Novo veículo #"..mQuery1.." criado na biblioteca.", client, 0,255,0)
		exports.logs:dbLog(client, 6, { client }, " Criou Novo veículo #"..mQuery1.." na biblioteca.")
		--exports.global:sendMessageToAdmins("[VEHICLE-MANAGER]: "..getElementData(client, "account:username").." has created Novo veículo #"..mQuery1.." na biblioteca.")
		return true
	else
		if data.copy then
			local mQuery1 = mysql:query_insert_free("INSERT INTO vehicles_shop SET vehmtamodel='"..toSQL(data["mtaModel"]).."', vehbrand='"..toSQL(data["brand"]).."', vehmodel='"..toSQL(data["model"]).."', vehyear='"..toSQL(data["year"]).."', vehprice='"..toSQL(data["price"]).."', vehtax='"..toSQL(data["tax"]).."', createdby='"..toSQL(getElementData(client, "account:id")).."', notes='"..toSQL(data["note"]).."', enabled='"..toSQL(enabled).."', `spawnto`='"..toSQL(data["spawnto"]).."', `doortype` = " .. data.doortype.. ", `stock`="..data.stock..", `spawn_rate`="..data.rate)
			...
			sendLibraryToClient(client)
			outputChatBox("[VEHICLE MANAGER] Novo veículo #"..mQuery1.." criado na biblioteca.", client, 0,255,0)
			exports.logs:dbLog(client, 6, { client }, " Created Novo veículo #"..mQuery1.." na biblioteca.")
			--exports.global:sendMessageToAdmins("[VEHICLE-MANAGER]: "..getElementData(client, "account:username").." has created Novo veículo #"..mQuery1.." na biblioteca.")
			return true
		else
			...
			if not mQuery1 then
				...
				outputChatBox("[VEHICLE MANAGER] Atualizar veículo #"..data.id.." da biblioteca de veículos falhou.", client, 255,0,0)
				return false
			end
			sendLibraryToClient(client)
			outputChatBox("[VEHICLE MANAGER] Você atualizou o veículo #"..data.id.." da biblioteca de veículos.", client, 0,255,0)
			exports.logs:dbLog(client, 6, { client }, " Atualizou veículo #"..data.id.." da biblioteca.")
			--exports.global:sendMessageToAdmins("[VEHICLE-MANAGER]: "..getElementData(client, "account:username").." has updated vehicle #"..data.id.." na biblioteca.")
			return true
		end
	end
end
...
function deleteVehicleFromLibraby(id)
	...
	if not mQuery1 then
		...
		outputChatBox("[VEHICLE MANAGER] Tentou deletar o veículo #"..id.." da biblioteca de veículos mas falhou.", client, 255,0,0)
		return false
	end
	outputChatBox("[VEHICLE MANAGER] Você deletou o veículo #"..id.." da biblioteca de veículos.", client, 0,255,0)
	sendLibraryToClient(client)
	return true
end
