...
local startDriveTestConvo = {
	...
	"Não tenha vergonha, pode testar.",
	"Eu aposto que você vai gosstar!"
}
local finishDriveTestConvo = {
	"Então, o que achou?",
	"Nada mal, em?",
	"Você definitivamente deve comprar esse!",
	"Então..?",
	"Então, acha que vale o investimento?"
}
local orderVehicleConvo = {
	"Tudo bem, vamos te notificar assim que estivermos com estoque.",
}
...
function createDealerNPCs(which)
	...
	if which == getThisResource() or which == "Jacob Jones" then
		--outputDebugString("Created")
		dealershipPed[5] = createPed( 143, -1656.0673828125, 1207.712890625, 7.25 )
	...
	if which == getThisResource() or which == "James Thomas" then
		--outputDebugString("Created")
		dealershipPed[6] = createPed( 23, -1658.7646484375, 1208.1982421875, 7.25 )
...
function createTestVehicle(vehShopID, thePed, fromVehLib)
	...
	local veh = createVehicle(400, x, y, z , 0, 0, 180, plate)
	if not (veh) then
