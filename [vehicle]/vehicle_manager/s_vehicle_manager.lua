...
function gotoVeh(thePlayer, vehID )
	executeCommandHandler ( "tpveiculo", thePlayer, vehID )
end
	...
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerVehicleConsultant( thePlayer ) then
		if not tonumber(vehID) or (tonumber(vehID) <= 0 ) or (tonumber(vehID) % 1 ~= 0 ) then
			...
			if not vehID then
				...
				outputChatBox("Or use SYNTAX: /"..commandName.." [ID Veículo]", thePlayer, 255, 194, 14)
				return false
			elseif vehID <= 0 then
				outputChatBox( "Você não pode usar /checarveiculo em veículo temporario.", thePlayer, 255, 0, 0)
				return false
			end
		end
		...
		if mQuery1 then
			...
			mysql:free_result(mQuery1)
			if not row then
				outputChatBox("ID Veículo #"..vehID.." não existe!", thePlayer, 255, 0, 0)
				return false
			end

			if row["vjob"] ~= "-1" then
				outputChatBox("Você não pode usar /checarveiculo em um veículo de trabalho da prefeitura.", thePlayer, 255, 0, 0)
				return false
			end
...
addCommandHandler("checarveiculo", checkVeh)
...
function saveAdminNote(vehID, adminNote, noteId)
	...
	if string.len(adminNote) > 500 then
		outputChatBox("Não foi possível adicionar a nota do administrador. Motivo: ultrapassou 500 caracteres.", source, 255, 0, 0)
		return false
	end
	...
	if noteId then
		if mysql:query_free("UPDATE vehicle_notes SET note='"..mysql:escape_string(adminNote).."', creator="..getElementData(source, "account:id").." WHERE id ="..noteId.." AND vehid="..vehID) then
			outputChatBox("Você atualizou com sucesso a entrada de nota do administrador #"..noteId.." no veículo #"..vehID..".", source, 0, 255,0)
		...
		if insertedId then
			outputChatBox("Você adicionou com sucesso uma nova entrada de nota de administrador #"..insertedId.." para o veículo #"..vehID..".", source, 0, 255,0)
...
function getVehicleWeight (thePlayer, commandName, specifiedVehicle)
	if exports.integration:isPlayerVehicleConsultant(thePlayer) or exports.integration:isPlayerAdmin(thePlayer) or exports.integration:isPlayerVCTMember(thePlayer) then
		if not specifiedVehicle then
			...
			if not vehicle then
				outputChatBox("Você precisa estar sentado em um veículo para usar este comando, ou você pode usar /checarpesoveiculo <veh ID>.", thePlayer, 255, 194, 14)
				return false
			else
				local mass = getVehicleHandling(vehicle).mass or false
				if not mass then
					outputChatBox("ERROR: Contate um scripter or a VCT member. Code: 824jS", thePlayer, 255, 0, 0)
					return false
				else
					outputChatBox("Peso do veículo: #7CFC00"..mass.."kg. #FFFFFFCálculo automatizado (massa x 3) para chopshop: #01C5BB"..(tonumber(mass) * 3).."$", thePlayer, 255, 255, 255, true)
				end
			end
		elseif tonumber(specifiedVehicle) and tonumber(specifiedVehicle) > 0 then
			...
			if not exists then
				outputChatBox("Este ID do veículo não existe.", thePlayer, 255, 194, 14)
				return false
			else
				local mass = getVehicleHandling(exists).mass or false
				if not mass then
					outputChatBox("ERROR: Contate um scripter or a VCT member. Code: 823jS", thePlayer, 255, 0, 0)
					return false
				else
					outputChatBox("Peso do veículo: #7CFC00"..mass.."kg. #FFFFFFCálculo automatizado (massa x 3) para chopshop: #01C5BB"..(tonumber(mass) * 3).."$", thePlayer, 255, 255, 255, true)
				end
			...
			outputChatBox("ERROR: Contate um scripter. Code: 822jS", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("getvehweight", getVehicleWeight)
addCommandHandler("checarpesoveiculo", getVehicleWeight)
...
function systemDeleteVehicle(vehid, reason) --This function is meant to be used by the system or to be triggered from UCP remotely. Works similarly to the delveh however, it's simplier and doesn't use mta elements. / MAXIME / 2015.1.11
	if vehid and tonumber(vehid) then
		...
		return false, "id do veh em falta ou inválido"
	end
	...
	if veh and veh.id ~= mysql_null() then
		...
		return false, "veículo não existe na database."
	end
	...
	if theVehicle then
		...
		return true, "o veículo não é carregado no jogo, então apenas limpo no banco de dados."
	end
end
