...
function makeTagObject(cx, cy, cz, rot, interior, dimension)
	...
	if (tag~=9) then
		...
		exports.global:sendLocalMeAction(source, "picha a parede.")
		...
		outputChatBox("Você marcou a parede!", source, 255, 194, 14)
	else
		local distance = 5
		...
		if (object) then
			...
			outputChatBox("Você removeu a tag. Você ganhou $30 por fazer isso.", source, 255, 194, 14)
...
function clearNearbyTag(thePlayer)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if (object) then
			...
			destroyElement(object)
			local query = mysql:query_free("DELETE FROM tags WHERE id='" .. mysql:escape_string(id) .. "'")
			outputChatBox("Tag excluída com ID #" .. id .. ".", thePlayer, 0, 255, 0)
		else
			outputChatBox("Você não está perto de nenhuma tag.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("delnearbytag", clearNearbyTag, false, false)
addCommandHandler("removertagproxima", clearNearbyTag, false, false)

function showNearbyTags(thePlayer)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		local x, y, z = getElementPosition(thePlayer)
		local count = 0
		outputChatBox("Tags proximas:", thePlayer, 255, 126, 0)
		for key, value in ipairs(exports.global:getNearbyElements(thePlayer, "object")) do
			local objtype = getElementData(value, "type")
			if (objtype=="tag") then
...
addCommandHandler("tagsproximas", showNearbyTags, false, false)
...
function setTag(thePlayer, commandName, newTag)
	...
	elseif getElementData(thePlayer, "tag") == 9 then
		outputChatBox("Você não pode definir sua tag enquanto estiver em City Maintenance.", thePlayer, 255, 0, 0)
	else
		...
		if (newTag>0) and (newTag<9) then
			...
			--else
				...
				outputChatBox("Tag alterada para #" .. newTag .. ".", thePlayer, 0, 255, 0)
			...
			outputChatBox("Valor inválido, insira um valor entre 1 e 8.", thePlayer, 255, 0, 0)
		end
	end
end
