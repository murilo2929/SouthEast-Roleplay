				...
				if getDistanceBetweenPoints3D(cx,cy,cz,x,y,z) <= viewDistance then --Within radius viewDistance
					...
					if px and isLineOfSightClear(cx, cy, cz, x, y, z, true, false, false, true, true, false, false) then				
						...
						if getElementData(theVehicle, "carshop") then
							...
							toBeShowed = toBeShowed.."Preço: $"..exports.global:formatMoney(price).."\n Impostos: $"..exports.global:formatMoney(taxes)
							lines = lines+ 2
						else
							...
							if job == 1 then
								descToBeShown = "RS Haul - Vamos despejar sua carga."
								lines = lines + 1
							elseif job == 2 then
								descToBeShown = "Yellow Cab Co.\nLigue #8294 para uma volta!"
								lines = lines + 2
							elseif job == 3 then
								...
								for j = 1, 5 do
									local desc = getElementData(theVehicle, "descrição:"..j)
							...
							if not noPlateVehs[getElementModel(theVehicle)] then
								if getElementData(theVehicle, "show_plate") == 0 then
									if getElementData(localPlayer, "duty_admin") == 1 then
										toBeShowed = toBeShowed.."((Placa: "..plate.."))\n"
										lines = lines + 1
									else
							...
							if getElementData(theVehicle, "show_vin") == 0 then
								if getElementData(localPlayer, "duty_admin") == 1 or getElementData(localPlayer, "duty_supporter") then
									toBeShowed = toBeShowed.."((VIN(ID): "..vin.."))"
									lines = lines + 1
								else
							...
							if vowner == getElementData(localPlayer, "dbid") or exports.global:isStaffOnDuty(localPlayer) or exports.integration:isPlayerScripter(localPlayer) or exports.integration:isPlayerVCTMember(localPlayer) then
								toBeShowed = toBeShowed.."\nShop ID: "..(getElementData(theVehicle, "vehicle_shop_id") or "N/A")
								lines = lines + 1
								local ownerName = nil
								if vowner > 0 then
								...
								if vowner > 0 then 
									...
									if protected then
										...
										protectedText = "[Proteção de inatividade restante: "..details.."]"
										local toBeAdded = "\n"..protectedText
										...
										if active and (powner == getElementData(localPlayer, "dbid") or exports.integration:isPlayerStaff(localPlayer)) then
											...
											if owner_last_login and tonumber(owner_last_login) then
												...
												inactiveText = inactiveText.." Proprietário visto pela última vez é irrelevante | "
											end
	...
	if getPedOccupiedVehicle(getLocalPlayer()) then
		...
		if exports.global:hasItem(getLocalPlayer(), 3, dbid) or owner == playerId or exports.global:hasItem(theVehicle, 3, dbid) or exports.factions:isPlayerInFaction(getLocalPlayer(), factionid) or exports.integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1 then
			if dbid > 0 then
				...
				showCursor(true)
				wEditDescription = guiCreateWindow(x,y,441,230,"Editar descrição do veículo",false)
				...
				bClose = guiCreateButton(220,198,210,75,"Fechar",false,wEditDescription)
				...
				exports.hud:sendBottomNotification(getLocalPlayer(), "Descrição do veículo", "Você não pode definir descrições em veículos temporários.")
			end
		else
			exports.hud:sendBottomNotification(getLocalPlayer(), "Descrição do veículo", "Você não tem as chaves deste veículo.")
		end
	else
		exports.hud:sendBottomNotification(getLocalPlayer(), "Descrição do veículo", "Você deve estar no veículo do qual deseja alterar a descrição.")
	end
end
