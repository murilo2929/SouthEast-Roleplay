...
function delNearbyGeneralshops(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		outputChatBox("Excluindo loja próxima NPC(s):", thePlayer, 255, 126, 0)
		local count = 0
		...
		if (count==0) then
			outputChatBox("   Nada Deletado.", thePlayer, 255, 126, 0)
		else
			outputChatBox("   Deletado "..count.." Nenhum.", thePlayer, 255, 126, 0)
		end
	end
end
...
function createGeneralshop(thePlayer, commandName, shoptype, skin, ...)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if not shoptype or not g_shops[shoptype] then
			outputChatBox("SYNTAX: /" .. commandName .. " [tipo loja] [skin, -1 = random] [Firstname Lastname, -1 = random]", thePlayer, 255, 194, 14)
		...
		if shoptype == 17 then
			if dimension == 0 and interior == 0 then
				outputChatBox("Deve ser criado em um interior de um negócio.", thePlayer, 255, 0, 0)
				return false
			end
		end
...
function getNearbyGeneralshops(thePlayer, commandName)
	...
	if (found and status.owner == getElementData(thePlayer, "dbid")) or exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) then
			...
			if (pedType) then
				if (pedType=="shop") then
					...
					if (distance<=10) and (dimension==cdimension) then
						local dbid = getElementData(thePed, "dbid")
						local shoptype = getElementData(thePed, "shoptype")
						local pedName = getElementData(thePed, "name") or "Sem Nome"
						outputChatBox("   Shop ID #" .. dbid .. ", tipo "..shoptype..", nome: "..tostring(pedName):gsub("_", " "), thePlayer, 255, 126, 0)
						count = count + 1
					end
				end
			end
		...
		outputChatBox("Membros que não fazem parte da equipe só podem usar este comando em seus próprios interiores.", thePlayer, 255, 194, 14)
	end
end
...
function moveNPCshop(thePlayer, commandName, value)
	...
	elseif getElementDimension(foundShop) ~= dim then
		outputChatBox("A loja deve estar no mesmo interior em que você está tentando movê-la.", thePlayer, 255, 0, 0)
		return
	end
...
function gotoShop(thePlayer, commandName, shopID)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not tonumber(shopID) then
			...
			if not foundShop then
				outputChatBox("Nenhuma loja fundada com ID #"..shopID, thePlayer, 255, 0, 0)
				return false
			end
...
function reloadGeneralShop(thePlayer, commandName, id)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if loadOneShop(id) then
			outputChatBox("Loja recarregada ID#"..id..".",thePlayer, 0,255,0)
		else
...
function clickStoreKeeper()
	...
	if not success then
		outputChatBox(currentUser.." está usando este NPC, por favor, aguarde um momento.", client, 255,0,0)
		return false
	end
...
addEventHandler( "shop:buy", resourceRoot, function( index )
	...
	_G['shop'] = shop
	if not shop then
		outputChatBox("Error " .. error .. "-NE, reporte no discord", client, 255, 0, 0 )
		return
	end
	...
	if not item then
		outputChatBox("Error " .. error .. "-NEI-" .. index .. ", report at discord", client, 255, 0, 0 )
		return
	end
	...
	if not exports.global:hasSpaceForItem( client, item.itemID, item.itemValue ) then
		outputChatBox("Seu inventario esta cheio.", client, 255, 0, 0)
		return
	end

	--checking age
	if item.minimum_age and getElementData(client, "age") < item.minimum_age then
		outputChatBox( "Você precisa ter " .. item.minimum_age .. " anos ou ser mais velho para comprar isso.", client, 255, 0, 0 )
		return
	end

	-- check for money
	if not exports.global:hasMoney( client, price ) then
		outputChatBox( "Você não tem dinheiro para comprar isso " .. item.name .. ".", client, 255, 0, 0 )
		return
	end
	...
	elseif itemID == 68 then -- Lottery Tickets / Check and
		outputChatBox( "Este item está temporariamente desativado.", client, 255, 0, 0 )
	...
	elseif itemID == 115 or itemID == 116 then
		if item.license and getElementData( client, "license.gun" ) ~= 1 then
			outputChatBox( "Você não tem licença de arma.", client, 255, 0, 0 )
			return
		else
	...
	if exports.global:takeMoney( client, price ) and exports.global:giveItem( client, itemID, itemValue, item.metadata ) then
		outputChatBox( "Você comprou um(a) " .. item.name .. " por $" .. exports.global:formatMoney( price ) .. ".", client, 0, 255, 0 )
	else
	...
	if itemID == 2 then
		...
		outputChatBox("Seu numero é #" .. itemValue .. ".", client, 255, 194, 14 )
	elseif itemID == 114 then -- vehicle mods
		outputChatBox("Para adicionar este item a qualquer veículo, vá até uma garagem e clique duas vezes no item enquanto está sentado dentro.", client, 255, 194, 14 )
		...
		if setShopStats(source, 'sPendingWage', 0) and setShopStats(source, 'sIncome', 0) then
			if faction then
				if exports.bank:giveBankMoney(faction, profit) then
					exports.bank:addBankTransactionLog(0, -getElementData(faction, 'id'), profit, 13, 'Renda do lojista '..tostring(getPedName(source)), getPedName(source))
					playCollectMoneySound(source)
					outputChatBox("Total $"..exports.global:formatMoney(profit).." o lucro líquido foi transferido para o banco de "..getTeamName(faction)..".", client, 0, 255, 0)
...
function checkSupplies(thePlayer)
    ...
    if (dbid==0) then
        outputChatBox("Você não está em um negócio.", thePlayer, 255, 0, 0)
    else
        ...
        if exports.integration:isPlayerTrialAdmin(thePlayer) or tonumber(owner)==getElementData(thePlayer, "dbid") or exports.global:hasItem(thePlayer, 4, dbid) or exports.global:hasItem(thePlayer, 5, dbid) then
            outputChatBox("Você pode verificar o abastecimento de uma empresa abrindo a respectiva GUI da loja.", thePlayer, 255, 194, 14)
        else
            outputChatBox("Você não está em um negócio ou é o proprietário da empresa.", thePlayer, 255, 0, 0)
        end
    end
end
addCommandHandler("checksupplies", checkSupplies, false, false)
addCommandHandler("versuprimentos", checkSupplies, false, false)
...
function canIUseThisName(pedName)
	...
	if row3 then
		return false, "O personagem de outro jogador já usou este nome '"..pedName.."'."
	end
	...
	if row33 then
		return false, "Outra loja já usou este nome '"..pedName.."'."
	end
	return true, "Esse nome é legal"
end
...
function updateShopSupplies(interiorID, supplies, thePlayer, cost, pedName)
	...
	if success then
		...
		if query then
			...
			exports.bank:addBankTransactionLog(getElementData(thePlayer, "dbid"), nil, cost, 0, "SHOP RESTOCK", "Comprou "..tostring(cost).."$ no valor de suprimentos para ID comercial"..tostring(interiorID), nil, nil)
			storeKeeperSay(thePlayer, "Nós armazenamos metade do seu pedido em estoque para sua empresa. A outra metade foi enviada para os motoristas de entrega RS Haul.", pedName)
		end
	else
		outputChatBox(why, thePlayer, 255, 0, 0)
	end
