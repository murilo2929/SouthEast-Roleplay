...
function hasSupplies(int, supplies)
	if int and getElementType(int) == 'interior' then
		...
		if interiorType == 1 then -- business
			...
			return false, "Este interior is not a business thus far no transaction can be made here."
		end
	else
		return false, "Nenhuma transação pode ser feita aqui."
	end
end
	...
	--if (getElementData(thePlayer, "scripter_level") or 0) >= 2 then
		if not supply then
			...
			if interior then
				...
				outputChatBox("Este interior (" .. getElementData(interior, "dbid") .. ") tem " .. tostring(supplies[tostring(supply)]) .. " suprimentos de '" .. supply .. "'.", thePlayer)
			else
				outputChatBox("Você não está dentro de um interior.", thePlayer, 255, 0, 0)
			end
		end
	--end
end
addCommandHandler("checksupply", checkSupply)
addCommandHandler("versuprimentos", checkSupply)
...
function giveProfit(interior, ped, player, item, price)
	if isProfitable(player, interior) then
		...
		if playerGender == 0 then
			triggerEvent('sendAme', player, "tira algumas notas de dólar de sua carteira e entrega para "..pedName)
		else
			triggerEvent('sendAme', player, "tira algumas notas de dólar de sua carteira e entrega para "..pedName)
		end
		local r = getRealTime()
		local timeString = ("%02d/%02d/%04d %02d:%02d"):format(r.monthday, r.month + 1, r.year+1900, r.hour, r.minute)
		local ownerNoti = "Um cliente comprou um(a) "..item.name.." por $"..exports.global:formatMoney(price).."."
		local logString = "- "..timeString.." : Um cliente comprou um(a) "..item.name.." por $"..exports.global:formatMoney(price)..".\n"

		triggerEvent("sendAme", ped, "deu a "..playerName.." um(a) "..item.name..".")
		storeKeeperSay(player, "Olha Você aqui. E..", pedName)
		if playerGender == 0 then
			storeKeeperSay(player, "Obrigado, senhor. Tenha um bom dia!", pedName)
		else
			storeKeeperSay(player, "Obrigada Senhora. Tenha um bom dia!", pedName)
		end
...
function canPlayerCollectProfit(player, ped, int)
	...
	if not is then -- if not a business then noone can collect.
		return false, "Você não pode obter lucros de uma propriedade não comercial."
	end
	...
	if faction_id > 0 then
		...
		if exports.factions:isPlayerInFaction(player, faction_id) then
			...
			return false, "Você não é membro da "..getTeamName(faction).."."
		end
	...
	-- exceptions
	return false, "Você não tem permissões suficientes para realizar esta ação."
end
...
function updateShopSalary(thePlayer)
	...
	if thePlayer then
		...
		outputChatBox("------------COMECE A ATUALIZAR OS SALÁRIOS DA LOJA------------", thePlayer)
	end
	local count = 0
			...
			if isActiveBusiness(business) then
				...
				if locked == 0 then
					...
					if (sProfit) >= (0-settings.limitDebtAmount) then
						...
						if update then
							outputDebugString("Shop ID#"..shopID.." Salário dos funcionarios atualizado "..sPendingWage.." -> "..sNewPendingWage)
							if thePlayer then
								outputChatBox("Shop ID#"..shopID.." Salário dos funcionarios atualizado "..sPendingWage.." -> "..sNewPendingWage, thePlayer)
							end
						else
							outputDebugString("Shop ID#"..shopID.." Salário dos funcionarios atualizado Failed.")
							if thePlayer then
								outputChatBox("Shop ID#"..shopID.." Salário dos funcionarios atualizado Failed.", thePlayer)
							end
							count = count - 1
						end
					else
						...
						if delete and delete2 then
							...
							outputDebugString("Shop ID#"..shopID.." Excluiu-se devido à dívida excedente $"..exports.global:formatMoney(settings.limitDebtAmount)..".")
							if thePlayer then
								outputChatBox("Shop ID#"..shopID.." Excluiu-se devido à dívida excedente $"..exports.global:formatMoney(settings.limitDebtAmount)..".", thePlayer)
							end
							destroyElement(shop)
							exports.global:sendMessageToAdmins("[BIZ-SYSTEM] Shop ID#"..shopID.." Excluiu-se devido ao débito excedido $"..exports.global:formatMoney(settings.limitDebtAmount)..".")
						else
							outputDebugString("Shop ID#"..shopID.." Falha ao excluir loja mesmo que a dívida seja excedida $"..exports.global:formatMoney(settings.limitDebtAmount)..".")
							if thePlayer then
								outputChatBox("Shop ID#"..shopID.." Falha ao excluir loja mesmo que a dívida seja excedida $"..exports.global:formatMoney(settings.limitDebtAmount)..".", thePlayer)
							end
							count = count - 1
						end
					end
	...
	if thePlayer then
		...
		if hiddenAdmin == 0 then
			exports.global:sendMessageToAdmins("[BIZ-SYSTEM]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") forçou "..count.." lojas personalizadas para receber o salário.")
		else
			exports.global:sendMessageToAdmins("[BIZ-SYSTEM]: Um administrador oculto forçou "..count.." lojas personalizadas para receber o salário.")
		end
	end
end
