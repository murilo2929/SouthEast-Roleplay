...
function showCreateFactionDropItem(npcID)
	...
	gui.lItem = guiCreateLabel(43,39,117,24,"Arma:",false,gui.main)
	guiLabelSetVerticalAlign(gui.lItem,"center")
	guiSetFont(gui.lItem,"default-bold-small")

	gui.cItem = guiCreateComboBox(170,39,213,24,"Nenhum",false,gui.main)
	local comboItemIndex = {}
	comboItemIndex[0] = {nil, nil, "Nenhum"}
	guiComboBoxAddItem(gui.cItem, "Nenhum")
	local weaponList = exports.Arma:getFactionNpcItems()
	...
	gui.lCaliber = guiCreateLabel(43,73,117,24,"Calibre:",false,gui.main)
	...
	gui.lItemDesc = guiCreateLabel(43,107,117,24,"Nome Customizado:",false,gui.main)
	...
	local xOffset1 = 70
	gui.lPrice = guiCreateLabel(43,141,117,24,"Preço:",false,gui.main)
	...
	local xOffset2 = 203
	gui.lQuan = guiCreateLabel(43+xOffset2,141,117,24,"Quantidade:",false,gui.main)
	...
	gui.lRetock = guiCreateLabel(43,175,117,24,"Reabastecimento automático:",false,gui.main)
	...
	gui.lExplain = guiCreateLabel(170,202,233,27,"Dias após o item ser reabastecido com a quantidade pré-definida completa.\nInsira 0 para desativar o reabastecimento automático.",false,gui.main)
	...
	gui.bCreate = guiCreateButton(220,239,191,37,"Criar",false,gui.main)
	...
	addEventHandler( "onClientGUIClick", gui.bCreate,function()
		if gui.main and isElement(gui.main) then
			...
			guiSetText(gui.bCreate, "Enviando solicitação ao servidor..")
		end
...
function updateGUIs()
	...
	if not itemID then
		...
		if itemID == 115 then --gun
			guiSetText(gui.lItemDesc, "Nome Customizado:")
			--guiSetEnabled(gui.eCaliber, true)
			guiSetText(gui.eItemDesc, "")
		elseif itemID == 116 then --ammo
			guiSetText(gui.lItemDesc, "Balas/Mag:")
			--guiSetEnabled(gui.eCaliber, false)
			local pack = exports.Arma:getAmmo(weaponList[index][2])
...
function factionDropWeaponBuy(products, proID, shopElement)
	...
	wfactionDropWeaponBuy = guiCreateWindow( X, Y, Width, Height,"Compra de Produto",false)
	exports.global:playSoundSuccess()
	...
	local lProductName = guiCreateLabel(147,27,280,18,"Nome do Produdo: "..itemName or ""..".",false,wfactionDropWeaponBuy)
	local lAmount = guiCreateLabel(147,45,280,18,"Detalhes: "..itemValue or ""..".",false,wfactionDropWeaponBuy)
	local lPrice = guiCreateLabel(147,63,42,18,"Preço: $",false,wfactionDropWeaponBuy)
	local ePrice = guiCreateLabel(189,62,238,20,exports.global:formatMoney(itemPrice),false,wfactionDropWeaponBuy)
	local lDesc = guiCreateLabel(147,81,250,18,"Motivo no personagem para comprar este item: ",false,wfactionDropWeaponBuy)
	...
	bRemove = guiCreateButton(9,161,142,27,"Tirar",false,wfactionDropWeaponBuy)
	...
	if canPlayerAdminShop(localPlayer) then
		addEventHandler( "onClientGUIClick", bRemove, function ()
			...
			guiSetText(bRemove, "Retirando o item..")
	...
	local bPayByCash = guiCreateButton(151,161,142,27,"Pagar com Dinheiro",false,wfactionDropWeaponBuy)
	...
	local bCancel = guiCreateButton(291,161,136,27,"Cancelar",false,wfactionDropWeaponBuy)
