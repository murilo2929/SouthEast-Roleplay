...
function adminEditPedGui(element)
	if exports.integration:isPlayerTrialAdmin(localPlayer) or exports.integration:isPlayerScripter(localPlayer) then
		...
		local type = getElementData(element, "rpp.npc.type") or "[geral]"
		...
		local createdAt = getElementData(element, "rpp.npc.createdAt") or "Unknown"
	        ...
	        adminEditWindow = guiCreateWindow(50, 150, 290, 450, "Editar Ped", false)
		...
		local guiY = 25
	        ...
	        GUIEditor.label[2] = guiCreateLabel(65, guiY, 185, 23, "", false, GUIEditor.scrollpane[1])
	        	if id > 0 then
	        		...
	        		guiSetText(GUIEditor.label[2], tostring(id).." [PED TEMPORARIO]")
	        	end
	        guiY = guiY + 23
	        GUIEditor.label[3] = guiCreateLabel(10, 48, 55, 23, "Tipo", false, GUIEditor.scrollpane[1])
	        GUIEditor.edit[1] = guiCreateEdit(65, 48, 129, 23, tostring(type), false, GUIEditor.scrollpane[1])
	        GUIEditor.button[2] = guiCreateButton(195, 48, 56, 23, "Procurar", false, GUIEditor.scrollpane[1])
	        GUIEditor.label[4] = guiCreateLabel(10, 71, 55, 23, "Comportamento", false, GUIEditor.scrollpane[1])
	        GUIEditor.edit[2] = guiCreateEdit(65, 71, 93, 23, tostring(behav), false, GUIEditor.scrollpane[1])
	        GUIEditor.button[3] = guiCreateButton(158, 71, 93, 23, "Procurar", false, GUIEditor.scrollpane[1])
	        GUIEditor.edit[3] = guiCreateEdit(65, 94, 93, 23, tostring(skin), false, GUIEditor.scrollpane[1])
	        GUIEditor.button[4] = guiCreateButton(158, 94, 93, 23, "Procurar", false, GUIEditor.scrollpane[1])
	        ...
	        GUIEditor.label[9] = guiCreateLabel(10, 122, 55, 69, "Posição", false, GUIEditor.scrollpane[1])
	        ...
	        GUIEditor.label[12] = guiCreateLabel(10, 252, 55, 23, "dimensão", false, GUIEditor.scrollpane[1])
	        GUIEditor.label[13] = guiCreateLabel(10, 280, 55, 23, "Congelado", false, GUIEditor.scrollpane[1])
	        ...
	        GUIEditor.label[14] = guiCreateLabel(10, 308, 55, 23, "Nome", false, GUIEditor.scrollpane[1]) --+28
	        ...
	        GUIEditor.label[17] = guiCreateLabel(10, 392, 55, 23, "Animação", false, GUIEditor.scrollpane[1])
	  		GUIEditor.edit[11] = guiCreateEdit(65, 392, 129, 23, tostring(animation), false, GUIEditor.scrollpane[1])
	 		GUIEditor.button[5] = guiCreateButton(195, 392, 56, 23, "Procurar", false, GUIEditor.scrollpane[1])
	        GUIEditor.label[18] = guiCreateLabel(10, 420, 55, 23, "Comentario", false, GUIEditor.scrollpane[1])
	        	GUIEditor.edit[12] = guiCreateEdit(65, 420, 185, 23, tostring(comment), false, GUIEditor.scrollpane[1])
	        GUIEditor.label[19] = guiCreateLabel(10, 448, 60, 23, "Criado by", false, GUIEditor.scrollpane[1])
	        	GUIEditor.label[20] = guiCreateLabel(75, 448, 180, 23, tostring(createdByUsername), false, GUIEditor.scrollpane[1])
	        GUIEditor.label[21] = guiCreateLabel(10, 471, 60, 23, "Criado em", false, GUIEditor.scrollpane[1])
	        	GUIEditor.label[22] = guiCreateLabel(75, 471, 180, 23, tostring(createdAt), false, GUIEditor.scrollpane[1])

	        bDelete = guiCreateButton(10, 420, 80, 20, "Deletar", false, adminEditWindow)
	        bSave = guiCreateButton(100, 420, 80, 20, "Salvar", false, adminEditWindow)
	        bCancel = guiCreateButton(190, 420, 80, 20, "Cancelar", false, adminEditWindow)
		...
		addEventHandler("onClientGUIClick", bDelete,
			function ()
				...
				wDialog = guiCreateWindow(0.37, 0.36, 0.23, 0.16, "Confirmar Exclusão", true)
				guiWindowSetSizable(wDialog, false)

				dialog_label = guiCreateLabel(0.05, 0.20, 0.89, 0.44, "Tem certeza de que deseja excluir permanentemente este ped? Essa ação não pode ser desfeita.", true, wDialog)
				guiLabelSetHorizontalAlign(dialog_label, "left", true)
				dialog_btnYes = guiCreateButton(0.05, 0.71, 0.43, 0.22, "Sim", true, wDialog)
				dialog_btnNo = guiCreateButton(0.51, 0.71, 0.43, 0.22, "Não", true, wDialog)
...
function selectPedTypeWindow()
	...
	winSelPedType = guiCreateWindow(x, y, width, height, "Selecionar Tipo Ped", false)
	...
	local colName = guiGridListAddColumn(gridList, "Nome", 0.5)
	local colAdmin = guiGridListAddColumn(gridList, "Admin", 0.15)

	typeSel_b1 = guiCreateButton(0.05, 0.85, 0.45, 0.1, "Selecionar", true, winSelPedType)
	typeSel_bCancel = guiCreateButton(0.5, 0.85, 0.45, 0.1, "Cancelar", true, winSelPedType)
...
function selectPedBehavWindow()
	...
	local colName = guiGridListAddColumn(gridList, "Nome", 0.35)
	...
	behavSel_bCancel = guiCreateButton(0.5, 0.85, 0.45, 0.1, "Cancelar", true, winSelPedBehav)
