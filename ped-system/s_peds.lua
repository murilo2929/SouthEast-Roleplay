...
function healPed(player, ped)
	if(getElementType(ped) == "ped") then
		if not (exports.factions:isInFactionType(player, 4)) then
			outputChatBox("Você não tem habilidades médicas básicas, entre em contato com o ES.", player, 255, 0, 0)
		else
			...
			if foundkit then
				if isPedDead(ped) then
					...
					local message
					if pedName then
						message = "abre seu kit médico e trata "..tostring(pedName)
					else
						message = "abre seu kit médico e trata a pessoa"
					end
					...
					if itemValue > 1 then
						...
						if not exports.global:hasItem(thePlayer, 70) then
							outputChatBox("Aviso, você está sem kits de primeiros socorros. pega mais alguns no armario.", player, 255, 0, 0)
						end
					end
					...
					if pedName then
						outputChatBox("Você tratou " ..pedName.. " (NPC).", player, 0, 255, 0)
					else
						outputChatBox("Você tratou um NPC.", player, 0, 255, 0)
					end
					exports.logs:dbLog(player, 35, ped, "HEAL NPC FOR $" .. price)
				end
			else
				outputChatBox("Você precisa de um kit de primeiros socorros para tratar pessoas.", player, 255, 0, 0)
			end
		end
	end
end
	...
	if(getElementType(ped) == "ped") then
		if exports.integration:isPlayerAdmin(player) then
			...
			if dbid > 0 then
				...
				destroyElement(ped)
				exports.global:sendMessageToAdmins("[Peds] Temp NPC '"..tostring(pedName).."' foi deletado por " .. getPlayerName(player) .. ".")
			end
		end
	end
end
