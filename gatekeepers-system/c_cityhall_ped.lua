...
function popupJesPedMenu()
	...
	local l1 = guiCreateLabel(0, 0.08, 1, 0.25, "Com o que posso ajudar?", true, vmOptionMenu)
	guiLabelSetHorizontalAlign(l1, "center")
	local bJob = guiCreateButton(0.05, 0.3, 0.87, 0.18, "Eu quero me inscrever para um emprego.", true, vmOptionMenu)
	addEventHandler("onClientGUIClick", bJob, bJobF, false)

	local bID = guiCreateButton(0.05, 0.5, 0.87, 0.18, "Preciso de um novo cartão de identificação. ($5)", true, vmOptionMenu)
	addEventHandler("onClientGUIClick", bID, newIDCard, false)

	local bSomethingElse = guiCreateButton(0.05, 0.7, 0.87, 0.18, "Nada, muito obrigado.", true, vmOptionMenu)
