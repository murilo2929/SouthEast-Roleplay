	...
	if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") and (sent==false) and (element~=getLocalPlayer()) then
		...
		if (gatekeeper) then
			...
			if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=3) then
				...
				bTalkToPed = guiCreateButton(0.05, 0.3, 0.87, 0.25, "Conversar", true, wPedRightClick)
				addEventHandler("onClientGUIClick", bTalkToPed,  function (button, state)
					if(button == "left" and state == "up") then
						...
						elseif (ped=="Melina Dupont") then -- Custom clothes NPC at Dupont HQ.
							...
							exports.hud:sendBottomNotification(getLocalPlayer(),(ped or "NPC").." diz:", "        'Olá!'")
						end
					end
				end, false)

				bClosePedMenu = guiCreateButton(0.05, 0.6, 0.87, 0.25, "Fechar Menu", true, wPedRightClick)
