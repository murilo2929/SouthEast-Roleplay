...
function itemPropertiesGUI(object)
	...
	if canEditItemProperties(localPlayer, object) then
		...
		local x = (sx/2)-(w/2)
		local y = (sy/2)-(h/2)
		propertiesWindow = guiCreateWindow(x, y, w, h, "Propriedades do Item", false)
		...
		if not itemValue then
			...
			if exports["item-system"]:getItemHideItemValue(itemID) then
				if exports.integration:isPlayerSupporter(localPlayer) and exports.global:isStaffOnDuty(localPlayer) or exports.integration:isPlayerTrialAdmin(localPlayer) and exports.global:isStaffOnDuty(localPlayer) or exports.integration:isPlayerScripter(localPlayer) and exports.global:isStaffOnDuty(localPlayer) then
					itemValueText = tostring(itemValue).." (oculto)"
				else
					itemValueText = "(oculto)"
				end
			else
		...
		GUIEditor.button[4] = guiCreateButton(158, 354, 164, 27, "Salvar", false, propertiesWindow)
		GUIEditor.button[5] = guiCreateButton(9, 354, 147, 27, "Cancelar", false, propertiesWindow)
		...
		GUIEditor.label[11] = guiCreateLabel(7, 53, 299, 21, "Valor item: "..itemValueText, false, GUIEditor.tab[1])

		GUIEditor.label[2] = guiCreateLabel(7, 74, 299, 21, "Colocado por: "..creatorName, false, GUIEditor.tab[1])
		GUIEditor.label[3] = guiCreateLabel(7, 95, 299, 21, "Data de Colocação: "..createdDateText, false, GUIEditor.tab[1])
		GUIEditor.label[4] = guiCreateLabel(7, 116, 299, 21, "Protegido: "..protectedText, false, GUIEditor.tab[1])
		GUIEditor.checkbox[1] = guiCreateCheckBox(7, 137, 299, 21, "Use a Posição Exata", useExactValues, false, GUIEditor.tab[1])

		GUIEditor.tab[2] = guiCreateTab("Permissões", GUIEditor.tabpanel[1])

		GUIEditor.label[5] = guiCreateLabel(9, 11, 46, 18, "Usar:", false, GUIEditor.tab[2])

		GUIEditor.button[1] = guiCreateButton(62, 35, 240, 25, "Definir", false, GUIEditor.tab[2])
		...
		GUIEditor.button[2] = guiCreateButton(62, 99, 240, 25, "Definir", false, GUIEditor.tab[2])
		...
		GUIEditor.button[3] = guiCreateButton(62, 166, 240, 25, "Definir", false, GUIEditor.tab[2])
		...
		GUIEditor.label[6] = guiCreateLabel(9, 75, 46, 18, "Mover:", false, GUIEditor.tab[2])
		GUIEditor.label[7] = guiCreateLabel(9, 140, 46, 18, "Pegar:", false, GUIEditor.tab[2])
		GUIEditor.label[8] = guiCreateLabel(9, 239, 296, 56, "Essas configurações definem quem pode usar, mover e pegar este item. O personagem que largou o item, os administradores e qualquer pessoa com a chave para o interior podem editar essas configurações. Os itens definidos como 'protegidos' por um administrador não podem ser movidos ou retirados antes que o item seja desprotegido por um administrador.", false, GUIEditor.tab[2])
		guiSetFont(GUIEditor.label[8], "default-small")
		guiLabelSetHorizontalAlign(GUIEditor.label[8], "left", true)
		GUIEditor.label[9] = guiCreateLabel(9, 204, 112, 18, "Protegido: "..protectedText, false, GUIEditor.tab[2])
		...
		if newCreatorName then
			guiSetText(GUIEditor.label[2], "Colocado por: "..newCreatorName)
		end
...
function itemPropertiesGUIFillData(object, creatorName, createdDate, protected)
	if currentObject == object then
		if creatorName and isElement(GUIEditor.label[2]) then
			guiSetText(GUIEditor.label[2], "Colocado por: "..creatorName:gsub("_", " "))
		end
		if isElement(GUIEditor.label[3]) then
			if createdDate then
				createdDate = tostring(createdDate)
			else	
				createdDate = "Desconhecido"
			end
			guiSetText(GUIEditor.label[3], "Data da Colocação: "..createdDate)
		end
		if isElement(GUIEditor.label[4]) then
			...
			guiSetText(GUIEditor.label[4], "Protegido: "..protected)
			guiSetText(GUIEditor.label[9], "Protegido: "..protected)
		end		
	end
end
...
function showDataSet(type, action, oldData)
	...
	if type == 4 then --factions
		...
		local x = (sx/2)-(w/2)
		local y = (sy/2)-(h/2)
		dataWindow = guiCreateWindow(x, y, w, h, "Selecione as Facções", false)
		...
		feedbackLabel = guiCreateLabel(12, 26, 361, 22, "Insira o nome ou ID de uma facção para adicionar.", false, dataWindow)
		inputGridlist = guiCreateGridList(9, 88, 367, 265, false, dataWindow)
			guiGridListAddColumn(inputGridlist, "ID", 0.1)
			guiGridListAddColumn(inputGridlist, "Facção", 0.8)
		
		local btnSave = guiCreateButton(10, 401, 366, 31, "Definir data", false, dataWindow)
		local btnRemove = guiCreateButton(9, 357, 116, 30, "Remover selecionado", false, dataWindow)
		local btnRemoveAll = guiCreateButton(130, 357, 116, 30, "Remover tudo", false, dataWindow)
			...
			function ()
				...
				if faction then
					...
					if factionID and factionName then
						...
						if rows > 50 then
							guiSetText(feedbackLabel, "Muitas entradas.")
							...
							if alreadyAdded then
								...
								guiSetText(feedbackLabel, "Insira o nome ou ID de uma facção para adicionar.")
						...
						guiSetText(feedbackLabel, "Não encontrado.")
						guiLabelSetColor(feedbackLabel, 255, 0, 0)
					end
				else
					guiSetText(feedbackLabel, "Insira o nome ou ID de uma facção para adicionar.")
	...
	elseif type == 5 then --characters
		...
		local x = (sx/2)-(w/2)
		local y = (sy/2)-(h/2)
		dataWindow = guiCreateWindow(x, y, w, h, "Selecione Personagens", false)
		guiWindowSetSizable(dataWindow, false)

		inputCharName = guiCreateEdit(9, 50, 276, 27, "", false, dataWindow)
		local btnAdd = guiCreateButton(288, 52, 88, 25, "Add", false, dataWindow)
		feedbackLabel = guiCreateLabel(12, 26, 361, 22, "Digite o nome de um personagem para adicionar.", false, dataWindow)
		inputGridlist = guiCreateGridList(9, 88, 367, 265, false, dataWindow)
			guiGridListAddColumn(inputGridlist, "Nome do Personagem", 0.9)
			...
			function ()
				...
				if charname then
					...
					if rows > 50 then
						...
						if alreadyAdded then
							guiSetText(inputCharName, "")
							guiSetText(feedbackLabel, "Digite o nome de um personagem para adicionar.")
							guiLabelSetColor(feedbackLabel, 255, 255, 255)
						else
							local row = guiGridListAddRow(inputGridlist)
	...
	elseif type == 8 then --query string
        ...
        local btn = guiCreateButton(16, 126, 392, 32, "Definir Data", false, dataWindow)
