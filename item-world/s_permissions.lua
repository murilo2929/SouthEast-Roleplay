...
function saveItemProperties(object, use, useData, move, moveData, pickup, pickupData, exactValues)
	...
	if getElementParent(getElementParent(object)) == getResourceRootElement(getResourceFromName("item-world")) then
		...
		if id then
			...
			if result then
				outputChatBox("Propriedades salvas para '"..tostring(itemName).."'.", client, 0,255,0)
			else
				outputChatBox("Falha ao salvar propriedades para '"..tostring(itemName).."'.", client, 255,0,0)
				outputChatBox("Se o problema persistir, faça um relatório de bug.", client, 255,0,0)
			end
		end
	end
end
