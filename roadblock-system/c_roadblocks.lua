...
function enableRoadblockGUI(parameter)
	...
	if not (rbWindow) and faction then
		...
		rbWindow = guiCreateWindow ( x, y, width, height, "Criar Roadblocks", false)
		...
		bClose = guiCreateButton(0.5, 0.85, 0.45, 0.1, "Fechar", true, rbWindow)
		addEventHandler("onClientGUIClick", bClose, cancelRoadblockGUI, false)
	
		outputChatBox("Selecione um roadblock, coloque-o no local que você deseja.", 0, 255, 0)
		outputChatBox("Use -, =, [, ], pgup e pgdn para girar seu objeto", 0, 255, 0)
		outputChatBox("Pressione [ESPAÇO] para salvar o objeto.", 0, 255, 0)
...
function selectRoadblockGUI(button, state)
	if (source==bUse) and (button=="left") or (source==rbList) and (button=="left") then
		...
		if (row==-1) or (col==-1) then
			outputChatBox("Selecione um tipo primeiro!", 255, 0, 0)
		else
			cleanupRoadblock()
