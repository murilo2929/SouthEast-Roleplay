...
function roadblockCreateWorldObject(objectid, tempObjectPosX, tempObjectPosY, tempObjectPosZ, tempObjectPosRot, tempObjectRotX, temptObjectRotY)
	if(isRoadblockEditAllowed(client)) then
		...
		if not (slot == 0) then
			outputChatBox("Roadblock gerado com ID #" .. slot.. ".", client, 0, 255, 0)
			exports['logs']:dbLog(client, 28, client, "Spawned Roadblock ID "..tostring(slot))
		else
			outputChatBox("Muitos roadblocks já foram gerados, remova alguns.", client, 0, 255, 0)
		end
	end
end
...
function getNearbyRoadblocks(thePlayer, commandName)
	if(isRoadblockEditAllowed(thePlayer)) then
		...
		if not (found) then
			...
			outputChatBox("Use /delrb [id] para remover.", thePlayer, 255, 126, 0)
		end
	end
end
addCommandHandler("nearbyrb", getNearbyRoadblocks, false, false)
addCommandHandler("nearbyrbs", getNearbyRoadblocks, false, false)
addCommandHandler("rbsproximos", getNearbyRoadblocks, false, false)
	...
	if(isRoadblockEditAllowed(thePlayer)) then
		if not (id) then
			...
			if (roadblocksarr[id]==nil) then
				outputChatBox("Nenhum roadblock foi encontrado com este ID.", thePlayer, 255, 0, 0)
			else
				...
				outputChatBox("Roadblock removido com ID #" .. id .. ".", thePlayer, 0, 255, 0)
			end
		end
	end
end
...
addCommandHandler("removerrb", removeRoadblock, false, false)
...
function removeAllRoadblocks(thePlayer, commandName)
	if(exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		outputChatBox("Todos os roadblocks foram removidos.", thePlayer, 0, 255, 0)
	end
end
...
addCommandHandler("removertodosrbs", removeAllRoadblocks, false, false)
