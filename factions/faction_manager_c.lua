...
function showFactionList( factions )
	...
	if not fGUI.window[1] and not isElement( fGUI.window[1] ) then
		...
		fGUI.button[1] = guiCreateButton(674, 552, 116, 38, "Fechar", false, fGUI.window[1])
		fGUI.button[2] = guiCreateButton(10, 552, 116, 38, "Criar Facção", false, fGUI.window[1])
		fGUI.button[3] = guiCreateButton(136, 552, 116, 38, "Editar Facção", false, fGUI.window[1])
		fGUI.button[4] = guiCreateButton(262, 552, 116, 38, "Lista Membros", false, fGUI.window[1])
		fGUI.button[5] = guiCreateButton(388, 552, 116, 38, "Deletar Facção", false, fGUI.window[1])
		...
		addEventHandler( 'onClientGUIClick', fGUI.window[1], function()
			...
			elseif source ==  fGUI.button[3] and fGUI.gridlist[1]  then
				...
				if row ~= -1 and col ~= -1 then
					...
					outputChatBox( "Você precisa selecionar um item da lista primeiro.", 255, 0, 0 )
				end
	...
	if not factions then
		if not fGUI.label[1] then
			...
			fGUI.label[1] = guiCreateLabel ( 0, 0, 1, 1, "Buscando informações do servidor...", true, fGUI.window[1] )
		...
		fGUI.gridlist.colName = guiGridListAddColumn(fGUI.gridlist[1], "Nome Facção", 0.23)
		fGUI.gridlist.colPlayers = guiGridListAddColumn(fGUI.gridlist[1], "Membros Online", 0.1)
		fGUI.gridlist.colType = guiGridListAddColumn(fGUI.gridlist[1], "Tipo", 0.1)
		fGUI.gridlist.colInts = guiGridListAddColumn(fGUI.gridlist[1], "Interiores", 0.07 )
		fGUI.gridlist.colVehs = guiGridListAddColumn(fGUI.gridlist[1], "Veículos", 0.07 )
		fGUI.gridlist.colIntPerk = guiGridListAddColumn(fGUI.gridlist[1], "Interiores Custom Gratis", 0.08)
		fGUI.gridlist.colSkinPerk = guiGridListAddColumn(fGUI.gridlist[1], "Skins Custom Gratis", 0.08)
		fGUI.gridlist.colBeforeTax = guiGridListAddColumn(fGUI.gridlist[1], "Valor Antes do Imposto", 0.14)
		fGUI.gridlist.colFreeWage = guiGridListAddColumn(fGUI.gridlist[1], "Salário Antes da Cobrança da Facção", 0.20)
		...
		for _, value in ipairs(factions) do
			...
			guiGridListSetItemText(fGUI.gridlist[1], row, fGUI.gridlist.colIntPerk, value.free_custom_ints == 1 and 'Sim' or 'Não', false, false)
			guiGridListSetItemText(fGUI.gridlist[1], row, fGUI.gridlist.colSkinPerk, value.free_custom_skins == 1 and 'Sim' or 'Não', false, false)
...
addCommandHandler( "faccoes", showFactionListCmd, false, false )
...
function editFaction( fact_id )
    ...
    editFact.window[1] = guiCreateWindow(155, 453, 385, 300, fact_id and "Editar Facção" or "Criar nova facção", false)
    guiWindowSetSizable(editFact.window[1], false)
    exports.global:centerWindow( editFact.window[1] )
    togWindow( fGUI.window[1], false )

    editFact.label[1] = guiCreateLabel(20, 26, 97, 29, "Nome Facção:", false, editFact.window[1])
    ...
    editFact.label[2] = guiCreateLabel(20, 65, 97, 29, "ID Facção:", false, editFact.window[1])
	guiLabelSetVerticalAlign(editFact.label[2], "center")

	editFact.label[6] = guiCreateLabel(20, 130, 140, 29, "Valor do ativo antes de impostos:", false, editFact.window[1])
	...
	editFact.label[7] = guiCreateLabel(20, 162, 140, 29, "Valor do salário antes\nfacção ser cobrada:", false, editFact.window[1])
    ...
    editFact.combobox[1] = guiCreateComboBox(120, 100, 245, 29, "Selecione um tipo de facção", false, editFact.window[1])
    ...
    editFact.label[3] = guiCreateLabel(20, 100, 97, 29, "Tipo Facção:", false, editFact.window[1])
    guiLabelSetVerticalAlign(editFact.label[3], "center")
    editFact.label[4] = guiCreateLabel(20, 205, 97, 29, "Interiores Max:", false, editFact.window[1])
    ...
    editFact.label[5] = guiCreateLabel(204, 205, 97, 29, "Veículos Max:", false, editFact.window[1])
    ...
    editFact.checkbox[1] = guiCreateCheckBox(20, 238, 144, 29, "Interiores Custom Gratis", data and data.free_custom_ints == 1 or false, false, editFact.window[1])
    editFact.checkbox[2] = guiCreateCheckBox(20, 260, 144, 29, "Skins Custom Gratis", data and data.free_custom_skins == 1 or false, false, editFact.window[1])
    editFact.button[1] = guiCreateButton(179, 250, 88, 40, "Cancelar", false, editFact.window[1])
    editFact.button[2] = guiCreateButton(277, 250, 88, 40, "Enviar", false, editFact.window[1])
    addEventHandler( 'onClientGUIClick', editFact.window[1], function()
    	...
    	elseif source == editFact.button[2] then
    		...
    		if string.len( submit_data.name ) < 3 then
    			exports.global:playSoundError()
    			return not outputChatBox( "O nome da facção deve ter 3 caracteres ou mais.", 255, 0, 0 )
    		elseif not submit_data.type then
    			exports.global:playSoundError()
    			return not outputChatBox( "Tipo de facção inválido.", 255, 0, 0 )
    		elseif not submit_data.max_interiors or submit_data.max_interiors < 0 then 
    			exports.global:playSoundError()
    			return not outputChatBox( "O máximo de interiores deve ser positivo. ", 255, 0, 0 )
    		elseif not submit_data.max_vehicles or submit_data.max_vehicles < 0 then
    			exports.global:playSoundError()
				return not outputChatBox( "O máximo de veículos deve ser positivo. ", 255, 0, 0 )
    		else
...
function delConfirm( fact_id )
    ...
    delGUI.window[1] = guiCreateWindow(429, 298, 437, 206, "Deletar Facção", false)
    guiWindowSetSizable(delGUI.window[1], false)
    exports.global:centerWindow( delGUI.window[1] )

    delGUI.label[1] = guiCreateLabel(15, 43, 412, 110, "Você está prestes a deletar a facção ID #" .. fact.id .. " (" .. fact.name .. ").\n\nTudo associado à facção, incluindo interiores, veículos, itens, etc. de propriedade da facção também será destruído.\nEssa ação não pode ser desfeita.\n\nVocê tem certeza que quer continuar?", false, delGUI.window[1])
    guiLabelSetHorizontalAlign(delGUI.label[1], "left", true)
    delGUI.button[1] = guiCreateButton(17, 158, 200, 33, "Cancelar", false, delGUI.window[1])
    delGUI.button[2] = guiCreateButton(223, 158, 200, 33, "Continuar", false, delGUI.window[1])    
...
function listMember( fact_id, response, data )
	...
	togWindow( fGUI.window[1], false )
	local wExtend = 45
    listMemberGUI.window[1] = guiCreateWindow(519, 255, 555+wExtend, 372, "Listando membros da facção", false)
    ...
    if data then
	    ...
	    listMemberGUI.col.charactername = guiGridListAddColumn(listMemberGUI.gridlist[1], "Membro", 0.27)
		listMemberGUI.col.username = guiGridListAddColumn(listMemberGUI.gridlist[1], "Conta", 0.15)
	    ...
	    for _, member in ipairs( data ) do
			...
			guiGridListSetItemText(listMemberGUI.gridlist[1], row, listMemberGUI.col.faction_leader, member.faction_leader == 1 and 'Sim' or 'Não', false, false)
			...
			guiGridListSetItemText(listMemberGUI.gridlist[1], row, listMemberGUI.col.duty, member.duty and "Em serviço" or "Folga", false, false)
		...
		if response then
	    	...
	    	listMemberGUI.label[1] = guiCreateLabel ( 0, 0, 1, 1, "Buscando informações do servidor...", true, listMemberGUI.window[1] )
    ...
    listMemberGUI.button[1] = guiCreateButton(451+wExtend, 332, 94, 30, "Fechar", false, listMemberGUI.window[1])
...
addCommandHandler( 'showfactionplayers', function ( cmd, fact_id )
	if canAccessFactionManager( localPlayer ) then
		if not fact_id or not tonumber(fact_id) or tonumber( fact_id ) < 1 then
			return not outputChatBox( "SYNTAX: /"..cmd.." [ID Facção]" )
		end
		listMember( fact_id )
	end
end, false, false)
...
if not (wFactionList) then
		...
		local colName = guiGridListAddColumn(fGUI.gridlist[1], "Nome Facção", 0.6)
		local colPlayers = guiGridListAddColumn(fGUI.gridlist[1], "Players", 0.14)
		local colType = guiGridListAddColumn(fGUI.gridlist[1], "Tipo", 0.14)
		...
		bFactionListClose = guiCreateButton(0.025, 0.9, 0.95, 0.1, "Fechar", true, wFactionList)
