...
addCommandHandler("faccao", showFactionMenu)
	...
	if (logged == 1) then
		...
		if (menuVisible == 0) then
			...
			if (factionID) then
					...
					if hasMemberPermissionTo(source, factionID, "respawn_vehs") then
						...
						if vehResult then
							...
							for _, row in ipairs(vehResult) do
								...
								vehicleLocations[j] = exports.global:hasItem(veh, 139) and exports.global:getElementZoneName(veh) or "Desconhecido"
								j = j + 1
							end
						else
							dbFree(vehicleQuery)
				...
				outputChatBox("Você não esta em uma fação.", source)
			end
		else
...
function callbackRespawnVehicles(factionID)
	...
	if not hasMemberPermissionTo(client, factionID, "respawn_vehs") then
		outputChatBox("Não permitido desculpe.", client)
		return
	end
	...
	if not (factionCooldown) then
		...
		for k, v in ipairs(teamPlayers) do
			outputChatBox(username:gsub("_", " ") .. " restaurou todos os veículos de facção desocupados.", v, 255, 194, 14)
		end

		setTimer(resetFactionCooldown, 60000, 1, theTeam)
		exports.anticheat:changeProtectedElementDataEx(theTeam, "cooldown", true, false)
	else
		outputChatBox("No momento, você não pode respawnar os veículos de sua facção, por favor, espere um pouco.", source, 255, 0, 0)
	end
end
...
function callbackRespawnOneVehicle(vehicleID, factionID)
	...
	if not hasMemberPermissionTo(client, factionID, "respawn_vehs") then
		outputChatBox("Não permitido desculpe.", source, 255, 0, 0)
		return
	end
	if theVehicle then
		...
		if (factionID == theVehicleID and not getVehicleOccupant(theVehicle, 0) and not getVehicleOccupant(theVehicle, 1) and not getVehicleOccupant(theVehicle, 2) and not getVehicleOccupant(theVehicle, 3) and not getVehicleTowingVehicle(theVehicle)) then
			...
			for k, v in ipairs(teamPlayers) do
				outputChatBox(playerName:gsub("_", " ") .. " respawnou o veículo ID " .. vehicleID .. ".", v, 255, 194, 14)
			end
		else
			outputChatBox("Esse veículo está atualmente ocupado.", source, 255, 0, 0)
		end
	else
		outputChatBox("Selecione um veículo que deseja respawnar.", source, 255, 0, 0)
	end
end
...
function callbackUpdateMOTD(motd, factionID)
	...
	if (factionID ~= -1) then
		if dbExec(mysql:getConn("mta"), "UPDATE factions SET motd=? WHERE id=?", motd, factionID) then
			outputChatBox("Você mudou o MOTD de sua facção para '" .. motd .. "'", client, 0, 255, 0)
...
function callbackUpdateNote(note, factionID)
	...
	if (factionID ~= -1) then
		if dbExec(mysql:getConn("mta"), "UPDATE factions SET note=? WHERE id=?", note, factionID) then
			outputChatBox("Você mudou com sucesso a nota de líder de sua facção.", client, 0, 255, 0)
...
function callbackUpdateFNote(fnote, factionID)
	...
	if (factionID ~= -1) then
		if dbExec(mysql:getConn("mta"), "UPDATE factions SET fnote=? WHERE id=?", fnote, factionID) then
			outputChatBox("Você mudou com sucesso a nota de toda a facção.", client, 0, 255, 0)
...
function callbackRemovePlayer(removedPlayerName, factionID)
	...
	if removedPlayer and not factionDetails[factionID] then
		outputChatBox("Newp, não vai acontecer, desculpe.", client)
		return
	end
	...
	if dbExec(mysql:getConn("mta"), "DELETE FROM characters_faction WHERE character_id=(SELECT id FROM characters WHERE charactername=?) AND faction_id=?", removedPlayerName, factionID) then
		...
		outputChatBox(username:gsub("_", " ") .. " removeu você da facção '" .. tostring(theTeamName) .. "'", removedPlayer, 255, 0, 0)
		local organizedTable = {}
		...
		-- Send message to everyone in the faction
		exports.factions:sendNotiToAllFactionMembers(factionID, removedPlayerName:gsub("_", " ") .. " foi removido da facção '" .. tostring(theTeamName) .. "'.", "Removido por " .. username:gsub("_", " ") .. ".")
	else
		outputChatBox("Error ao remover " .. removedPlayerName:gsub("_", " ") .. " da facção, Contate um administrador.", source, 255, 0, 0)
	end
end
...
function callbackPerkEdit(perkIDTable, playerName, factionID)
	...
	if dbExec(mysql:getConn("mta"), "UPDATE `characters_faction` SET `faction_perks`=? WHERE character_id=(SELECT id FROM characters WHERE charactername=?) AND faction_id=?", jsonPerkIDTable, playerName, factionID) then
		outputChatBox(" Duty perks atualizador para " .. playerName:gsub("_", " ") .. ".", client, 255, 0, 0)
		if targetPlayer then
			factionInfo[factionID].perks = perkIDTable
			setElementData(targetPlayer, "faction", factionInfo)

			outputChatBox(" Seus perks foram atualizados por " .. getPlayerName(client):gsub("_", " ") .. ".", targetPlayer, 255, 0, 0)
		end
	end
end
...
function callbackQuitFaction(factionID)
	...
	if dbExec(mysql:getConn("mta"), "DELETE FROM characters_faction WHERE character_id=? AND faction_id=?", getElementData(client, "dbid"), factionID) then
		outputChatBox("Você saiu da facção '" .. theTeamName .. "'.", client)
		...
		-- Send message to everyone in the faction
		exports.factions:sendNotiToAllFactionMembers(factionID, username:gsub("_", " ") .. " saiu da facção '" .. theTeamName .. "'.")
	else
		outputChatBox("Falha ao sair da facção, entre em contato com um administrador.", client, 255, 0, 0)
	end
end
...
function callbackInvitePlayer(invitedPlayer, factionID)
	...
	if count >= 3 then
		outputChatBox("O jogador se juntou ao número máximo de facções permitidas.", client, 255, 0, 0)
		return
	elseif isPlayerInFaction(invitedPlayer, factionID) then
		outputChatBox("O jogador já é membro dessa facção.", client, 255, 0, 0)
		return
	end
	...
	if dbExec(mysql:getConn("mta"), "INSERT INTO characters_faction SET faction_leader = 0, faction_id = ?, faction_rank = ?, character_id = ?", factionID, defaultRank, dbid) then
		...
		outputChatBox("Player " .. invitedPlayerNick:gsub("_", " ") .. " agora é um membro da facção '" .. tostring(theTeamName) .. "'.", client, 0, 255, 0)
		exports.factions:sendNotiToAllFactionMembers(factionID, invitedPlayerNick:gsub("_", " ") .. " juntou-se como um novo membro de sua facção '" .. tostring(theTeamName) .. "'.")
		triggerEvent("onPlayerJoinFaction", invitedPlayer, theTeam)
		outputChatBox("Você foi definido para Faction '" .. tostring(theTeamName) .. "'.", invitedPlayer, 255, 194, 14)
	else
		outputChatBox("O jogador já está em uma facção.", client, 255, 0, 0)
	end
end
...
function addDuty(dutyItems, finalLocations, dutyNewSkins, name, factionID, dutyID)
	...
	if dutyID == 0 then
		...
		refreshClient("> " .. getPlayerName(client):gsub("_", " ") .. ": Serviço adicionnado '" .. name .. "'.", factionID, false)
...
function addLocation(x, y, z, r, i, d, name, factionID, index)
	...
	if interiorElement then
		...
		if tonumber(interiorF) == tonumber(factionID) or d == 0 then
			...
			refreshClient("> " .. getPlayerName(client):gsub("_", " ") .. ": Local adicionado '" .. name .. "'.", factionID, false)
			exports.logs:dbLog(client, 35, "fa" .. tostring(factionID), "Added location, Name:" .. name .. " Database ID:" .. newIndex .. " x:" .. x .. " y:" .. y .. " z:" .. z .. " raio:" .. r .. " interior:" .. i .. " dimensão:" .. d)
			else
				...
				refreshClient("> " .. getPlayerName(client):gsub("_", " ") .. ": Localização revisada ID #" .. index .. ".", factionID, false)
				exports.logs:dbLog(client, 35, "fa" .. tostring(factionID), "Revised location ID #" .. index .. " x:" .. x .. " y:" .. y .. " z:" .. z .. " radius:" .. r .. " interior:" .. i .. " dimension:" .. d)
			end
		else
			outputChatBox("O interior que você inseriu deve ser propriedade da facção para ser adicionado como um local de serviço.", client, 255, 0, 0)
		end
	else
		outputChatBox("O servidor não conseguiu encontrar o interior que você digitou!", client, 255, 0, 0)
	end
end
...
function addVehicle(vehicleID, factionID)
	...
	if element then
		if getElementData(element, "faction") == factionID then
			...
			locations[tonumber(factionID)][newIndex] = { newIndex, "VEICULO", nil, nil, nil, nil, nil, nil, tonumber(vehicleID), getElementModel(element) }
			refreshClient("> " .. getPlayerName(client):gsub("_", " ") .. ": Adicionou veículo #" .. vehicleID .. ".", factionID, false)
			exports.logs:dbLog(client, 35, "fa" .. tostring(factionID), "Adicionou veículo #" .. vehicleID .. " Database ID:" .. newIndex)
			--outputChatBox("Added vehicle "..vehicleID.." successfully.", client, 0, 255, 0)
		else
			outputChatBox("Você só pode adicionar veículos de facção como locais de equipamentos.", client, 255, 0, 0)
		end
	else
		outputChatBox("Erro ao encontrar seu veículo, você digitou a ID correta?", client, 255, 0, 0)
	end
end
...
function removeLocation(removeID, factionID)
	...
	refreshClient("> " .. getPlayerName(client):gsub("_", " ") .. ": local removido com ID " .. removeID .. ".", factionID, client)
end
...
function removeDuty(removeID, factionID)
	...
	refreshClient("> " .. getPlayerName(client):gsub("_", " ") .. ": local de se equipar removido " .. removeID .. ".", factionID, client)
end
...
addEventHandler("faction-system.saveNewRank", root, 
	function(playerName, oldRank, newRank, factionID)
		...
		if (oldRank == newRank) then
			outputChatBox("Você não pode mudar o rank de uma pessoa para o mesmo.", client, 255, 125, 0)
			return
		end
		...
		local rankName = getRankName(newRank)
		exports.factions:sendNotiToAllFactionMembers(fID, playerName:gsub("_", " ") .. " foi "..highOrLow.." de '" .. getRankName(oldRank) .. "' para '" .. rankName .. "' por "..getPlayerName(client):gsub("_", " "))
	end
)