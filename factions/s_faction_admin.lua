...
function adminSetPlayerFaction(thePlayer, commandName, partialNick, factionID)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerFMTLeader(thePlayer) then
		...
		if not (partialNick) or not (factionID) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador/ID] [ID Facção]", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				if not theTeam then
					outputChatBox("ID Facção Invalido.", thePlayer, 255, 0, 0)
					return
				elseif isPlayerInFaction(targetPlayer, factionID) then
					outputChatBox("Jogador já faz parte desta facção.", thePlayer, 255, 0, 0)
					return
				end 
				...
				if num_factions >= 5 then
					outputChatBox("Este jogador já está na quantidade máxima de facções.", thePlayer, 255, 0, 0)
					return
				end
				...
				if dbExec(exports.mysql:getConn("mta"), "INSERT INTO characters_faction SET faction_leader = 0, faction_id = ?, faction_rank = ?, faction_phone = NULL, character_id=?", factionID, defaultRank, getElementData(targetPlayer, "dbid")) then
					...
					outputChatBox("Player " .. targetPlayerNick .. " agora é um membro da facção '" .. getTeamName(theTeam) .. "' (#" .. factionID .. ").", thePlayer, 0, 255, 0)
					triggerEvent("onPlayerJoinFaction", targetPlayer, theTeam)
					outputChatBox("Você foi definido para a facção '" .. getTeamName(theTeam) .. "'.", targetPlayer, 255, 194, 14)
...
addCommandHandler("adicionarfaccao", adminSetPlayerFaction, false, false)
		...
		if not (partialNick) or not (factionID) then
			...
			if targetPlayer then
				...
				if dbExec(exports.mysql:getConn("mta"), "DELETE FROM characters_faction WHERE faction_id=? AND character_id=?", factionID, getElementData(targetPlayer, "dbid")) then
					...
					outputChatBox("Player " .. targetPlayerNick .. " foi removido da facção "..factionID, thePlayer, 0, 255, 0)
					outputChatBox("Você foi removido da facção #"..factionID, targetPlayer, 255, 0, 0)
...
addCommandHandler("removerfaccao", adminRemovePlayerFaction, false, false)
...
function adminSetFactionLeader(thePlayer, commandName, partialNick, factionID)
	if exports.integration:isPlayerAdmin(thePlayer) or exports.integration:isPlayerFMTLeader(thePlayer) then
		factionID = tonumber(factionID)
		if not (partialNick) or not (factionID)  then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador] [ID Facção]", thePlayer, 255, 194, 14)
		elseif factionID > 0 then
			...
			if targetPlayer then
				...
				if not theTeam then
					...
					return
				elseif isPlayerInFaction(targetPlayer, factionID) then
					outputChatBox("Jogador já faz parte desta facção, este comando os coloca na facção como o líder.", thePlayer, 255, 0, 0)
					return
				end
				...
				if dbExec(exports.mysql:getConn("mta"), "INSERT INTO characters_faction SET faction_leader = 1, faction_id = ?, faction_rank = ?, faction_phone = NULL, character_id=?", factionID, theRank, getElementData(targetPlayer, "dbid")) then
					...
					outputChatBox("Player " .. targetPlayerNick .. " agora é o lider da facção '" .. getTeamName(theTeam) .. "' (#" .. factionID .. ").", thePlayer, 0, 255, 0)

					triggerEvent("onPlayerJoinFaction", targetPlayer, theTeam)
					outputChatBox("Você foi definido como lider da facção '" .. getTeamName(theTeam) .. "'.", targetPlayer, 255, 194, 14)

					exports.logs:dbLog(thePlayer, 4, { targetPlayer, theTeam }, "SET INTO FACTION LEADER")
					exports.factions:sendNotiToAllFactionMembers(factionID, targetPlayerNick .. " agora é o lider da sua facção '" .. getTeamName(theTeam) .. "'", "Definido por "..exports.global:getPlayerFullIdentity(thePlayer))
				else
					outputChatBox("ID Facção Invalido.", thePlayer, 255, 0, 0)
				end
			end
...
addCommandHandler("adicionarliderfaccao", adminSetFactionLeader, false, false)
...
function adminSetFactionRank(thePlayer, commandName, partialNick, factionID, ...)
	if (exports.integration:isPlayerAdmin(thePlayer)) then
		...
		if not (partialNick) or not (...)  then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador] [ID Facção] [Rank Facção, 1-20]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerNick = exports.global:findPlayerByPartialNick(thePlayer, partialNick)
			...
			if targetPlayer then
				...
				if not isPlayerInFaction(targetPlayer, factionID) then
					outputChatBox("Jogador não esta nesta facção.", thePlayer, 255, 0, 0)
					return
				end
				...
				if dbExec(exports.mysql:getConn("mta"), "UPDATE characters_faction SET faction_rank =? WHERE character_id = ? AND faction_id=?", rankID, getElementData(targetPlayer, "dbid"), factionID) then
					...
					outputChatBox("Player " .. targetPlayerNick .. " agora é rank " .. rankName .. ".", thePlayer, 0, 255, 0)
					outputChatBox("Admin " .. getPlayerName(thePlayer):gsub("_"," ") .. " definiu você para o rank " .. rankName .. " na facção #"..factionID..".", targetPlayer, 0, 255, 0)
...
addCommandHandler("difinirrankfaccao", adminSetFactionRank, false, false)
...
function setFactionMoney(thePlayer, commandName, factionID, amount)
	if (exports.integration:isPlayerScripter(thePlayer)) then
		if not (factionID) or not (amount)  then
			outputChatBox("SYNTAX: /" .. commandName .. " [ID Facção] [Dinheiro]", thePlayer, 255, 194, 14)
		else
			factionID = tonumber(factionID)
			if factionID and factionID > 0 then
				...
				if amount and amount > 500000*2 then
					outputChatBox("Por motivos de segurança, você não tem permissão para definir mais de $ 1.000.000 de uma vez para uma facção.", thePlayer, 255, 0, 0)
					return false
				end

				if (theTeam) then
					if exports.global:setMoney(theTeam, amount) then
						outputChatBox("Definiu para a facção '" .. getTeamName(theTeam) .. "'s uma quantia de " .. amount .. " $.", thePlayer, 255, 194, 14)
						exports.factions:sendNotiToAllFactionMembers(factionID, "'"..getTeamName(theTeam).."' banco da facção atualizado", exports.global:getPlayerFullIdentity(thePlayer, 1).." definiu o banco para $"..exports.global:formatMoney(amount)..".", nil, true)
					else
						outputChatBox("Não foi possível definir dinheiro para essa facção.", thePlayer, 255, 194, 14)
					end
				else
					outputChatBox("ID Facção Invalido.", thePlayer, 255, 194, 14)
				end
			else
				outputChatBox("ID Facção Invalido.", thePlayer, 255, 194, 14)
			end
...
addCommandHandler("definirdinheirofaccao", setFactionMoney, false, false)
...
function getTax(thePlayer)
	loadWelfare( )
	outputChatBox( "Auxilo Bem Estar: $" .. exports.global:formatMoney(unemployedPay), thePlayer, 255, 194, 14 )
	outputChatBox( "Imposto: " .. ( exports.global:getTaxAmount(thePlayer) * 100 ) .. "%", thePlayer, 255, 194, 14 )
	outputChatBox( "Imposto de Renda: " .. ( exports.global:getIncomeTaxAmount(thePlayer) * 100 ) .. "%", thePlayer, 255, 194, 14 )
end
addCommandHandler("gettax", getTax, false, false)
addCommandHandler("verimpostos", getTax, false, false)
			...
			if factionID and factionID > 0 then
				...
				if (theTeam) then
					if getElementData(theTeam, "type") >= 2 and getElementData(theTeam, "type") <= 6 then
						if exports.global:takeMoney(getFactionFromName("Los Santos City Government"), amount) then
							exports.global:giveMoney(theTeam, amount)
							outputChatBox("Você adicionou $" .. exports.global:formatMoney(amount) .. " para o orçamento da facção '" .. getTeamName(theTeam) .. "' (Total: " .. exports.global:getMoney(theTeam) .. ").", thePlayer, 255, 194, 14)
							mysql:query_free( "INSERT INTO wiretransfers (`from`, `to`, `amount`, `reason`, `type`) VALUES (" .. -3 .. ", " .. -getElementData(theTeam, "id") .. ", " .. amount .. ", '', 8)" )
						else
							outputChatBox("Você não pode pagar isso.", thePlayer, 255, 194, 14)
						end
					else
						outputChatBox("Você não pode definir um orçamento para essa facção.", thePlayer, 255, 194, 14)
					end
				else
					outputChatBox("ID Facção Invalido.", thePlayer, 255, 194, 14)
				end
...
--addCommandHandler("definirorcamento", setFactionBudget, false, false)
...
function setTax(thePlayer, commandName, amount)
	...
	if isInFaction and rank >= 15 then
		...
		if not amount or amount < 0 or amount > 30 then
			...
			outputChatBox("Novo imposto é " .. amount .. "%", thePlayer, 0, 255, 0)
		end
	end
end
addCommandHandler("settax", setTax, false, false)
addCommandHandler("definirimposto", setTax, false, false)
...
function setIncomeTax(thePlayer, commandName, amount)
	...
	if isInFaction and rank >= 15 then
		...
		if not amount or amount < 0 or amount > 25 then
			...
			outputChatBox("Novo imposto de renda é " .. amount .. "%", thePlayer, 0, 255, 0)
		end
	end
end
addCommandHandler("setincometax", setIncomeTax, false, false)
addCommandHandler("definirimpostoderenda", setIncomeTax, false, false)
...
function setWelfare(thePlayer, commandName, amount)
	...
	if isInFaction and rank >= 15 then
		...
		if not amount or amount <= 0 then
			outputChatBox("SYNTAX: /" .. commandName .. " [Dinheiro]", thePlayer, 255, 194, 14)
		elseif mysql:query_free( "UPDATE settings SET value = " .. unemployedPay .. " WHERE name = 'welfare'" ) then
			unemployedPay = amount
			outputChatBox("Novo Auxilio Bem-Estar é $" .. exports.global:formatMoney(unemployedPay) .. "/payday", thePlayer, 0, 255, 0)
		else
...
addCommandHandler("definirauxilobemestar", setWelfare, false, false)

function issueGovLicense(thePlayer, commandName, type, ...)
	local licenseTypes = {"Alvará de funcionamento - Regular", "Alvará de funcionamento - Premium", "Licença de entretenimento adulto", "Licença de Jogo", "Licença de bebida"}
	local isInFaction, rank = isPlayerInFaction(thePlayer, 3)
	if isInFaction and rank >= 3 then
		local type = tonumber(type)
		if not type or not licenseTypes[type] or not ... then
			outputChatBox("SYNTAX: /" .. commandName .. " [tipo] [nome negócio]", thePlayer, 255, 194, 14)
			...
			if success then
				outputChatBox("Criou um " .. text .. ".", thePlayer, 0, 255, 0)
			else
...
addCommandHandler("licencagov", issueGovLicense, false, false)
		...
		if (factionID) and (factionID > 0) then
			...
			if (theTeam) then
				...
				for k,v in ipairs(getPlayersInFaction(factionID)) do
					outputChatBox((hiddenAdmin == 0 and adminTitle .. " " .. username or "Admin Oculto") .. " restaurou todos os veículos de facção desocupados.", v)
				end

				exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. username .. " restaurou todos os veículos de facção desocupados para a facção ID " .. factionID .. ".")
				exports.logs:dbLog(thePlayer, 4, theTeam, "FACTION RESPAWN for " .. factionID)
			else
				outputChatBox("ID Facção Invalido.", thePlayer, 255, 0, 0, false)
			end
		else
			outputChatBox("SYNTAX: /" .. commandName .. " [ID Facção]", thePlayer, 255, 194, 14, false)
		end
	end
end
addCommandHandler("respawnfaction", respawnFactionVehicles, false, false)
addCommandHandler("respawnfaccao", respawnFactionVehicles, false, false)
...
function adminDuty(thePlayer)
	if (exports.integration:isPlayerHeadAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerFMTLeader(thePlayer)) then
		...
		elseif type(dutyAllow) ~= "table" then
			outputChatBox("Houve um problema com o cache de inicialização deste recurso. Contate um Scripter.", thePlayer, 255, 0, 0)
		else
			outputChatBox("Ops! Alguém já está editando as permissões de serviço. Desculpa!", thePlayer, 255, 0, 0) -- No time to set up proper syncing + kinda not needed.
		end
	end
end
