...
function showFactionMenu(motd, memberUsernames, memberRanks, memberPerks, memberLeaders, memberOnline, memberLastLogin, --[[memberLocation,]] factionRanks, factionWages, factionTheTeam, note, fnote, vehicleIDs, vehicleModels, vehiclePlates, vehicleLocations, memberOnDuty, towstats, phone, membersPhone, fromShowF, factionID, properties, factionRankID, rankOrder)
	if (gFactionWindow==nil) then
		...
		gFactionWindow = guiCreateWindow(0.1, 0.25, 0.85, 0.525, "Menu Facção", true)
		...
		colName = guiGridListAddColumn(gMemberGrid, "Nome", 0.20)
		colRank = guiGridListAddColumn(gMemberGrid, "Rank", 0.20)
		colOnline = guiGridListAddColumn(gMemberGrid, "Status", 0.115)
		colLastLogin = guiGridListAddColumn(gMemberGrid, "Ultimo Login", 0.13)
		...
		if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) or (factionType==7) then -- Added Mechanic type \ Adams
			--colLocation = guiGridListAddColumn(gMemberGrid, "Localização", 0.12)
			colWage = guiGridListAddColumn(gMemberGrid, "Salário ($)", 0.06)
		--else
			--colLocation = guiGridListAddColumn(gMemberGrid, "Localização", 0.1)
		end

		if phone then
			colPhone = guiGridListAddColumn(gMemberGrid, "Celular Nu.", 0.08)
		end
				...
				if rID == y then
					...
					if (not memberLastLogin[x]) then
						...
						elseif (memberLastLogin[x]==1) then
							login = tostring(memberLastLogin[x]) .. " dia atrás"
						else
							login = tostring(memberLastLogin[x]) .. " dias atrás"
						end
					end
					...
					if colDuty then
						if memberOnDuty[x] then
							guiGridListSetItemText(gMemberGrid, row, colDuty, "Em serviço", false, false)
							guiGridListSetItemColor(gMemberGrid, row, colDuty, 0, 255, 0)
						else
							guiGridListSetItemText(gMemberGrid, row, colDuty, "Folga", false, false)
		...
		guiSetText(ftab[factionID], tostring(teamName) .. " (" .. counterOnline .. " de " .. (counterOnline+counterOffline) .. " Membros Online)")

		-- Make the buttons
		if (hasMemberPermissionTo(localPlayer, factionID, "del_member")) then
			gButtonKick = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Demitir Membro", true, tabOverview)
		...
		if (hasMemberPermissionTo(localPlayer, factionID, "change_member_rank")) then
			gButtonPromote = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Promover/Rebaixar Membro", true, tabOverview)
		...
		if (factionType==2) or (factionType==3) or (factionType==4) or (factionType==5) or (factionType==6) or (factionType==7) then -- Added Mechanic type \ Adams
			if (hasMemberPermissionTo(localPlayer, factionID, "modify_ranks")) then
				gButtonEditRanks = guiCreateButton(0.825, 0.2292, 0.16, 0.06, "Editar Ranks e Salários", true, tabOverview)
				addEventHandler("onClientGUIClick", gButtonEditRanks, btEditRanks, false)
			end	
		else
			if (hasMemberPermissionTo(localPlayer, factionID, "modify_ranks")) then
				gButtonEditRanks = guiCreateButton(0.825, 0.2292, 0.16, 0.06, "Editar Ranks", true, tabOverview)
				addEventHandler("onClientGUIClick", gButtonEditRanks, btEditRanks, false)
			end	
		end
		...
		if (hasMemberPermissionTo(localPlayer, factionID, "add_member")) then
			gButtonInvite = guiCreateButton(0.825, 0.3824, 0.16, 0.06, "Convidar Membro", true, tabOverview)
		...
		if (hasMemberPermissionTo(localPlayer, factionID, "respawn_vehs")) then
			gButtonRespawnui = guiCreateButton(0.825, 0.459, 0.16, 0.06, "Respawn Veículos", true, tabOverview)
			addEventHandler("onClientGUIClick", gButtonRespawnui, showrespawn, false)
		end

			local _y = 0.5356
			if phone then
				gAssignPhone = guiCreateButton(0.825, _y, 0.16, 0.06, "Celular Nu.", true, tabOverview)
			...
			if factionType >= 2 then 
				if (hasMemberPermissionTo(localPlayer, factionID, "set_member_duty")) then
					gButtonPerk = guiCreateButton(0.825, _y, 0.16, 0.06, "Gerenciar Duty Perks", true, tabOverview)
			...
			if (hasMemberPermissionTo(localPlayer, factionID, "respawn_vehs")) then
				tabVehicles = guiCreateTab("(Líder) Veículos", tabs)
				...
				colVehLocation = guiGridListAddColumn(gVehicleGrid, "Localização", 0.4)
				gButtonVehRespawn = guiCreateButton(0.825, 0.076, 0.16, 0.06, "Respawn Veículo", true, tabVehicles)
				gButtonAllVehRespawn = guiCreateButton(0.825, 0.1526, 0.16, 0.06, "Respawn Todos os Veículos", true, tabVehicles)
			...
			if (hasMemberPermissionTo(localPlayer, factionID, "manage_interiors")) then
				tabProperties = guiCreateTab("(Lider) Propriedades", tabs)
				...
				colName = guiGridListAddColumn(gPropertyGrid, "Nome", 0.30)
				colProLocation = guiGridListAddColumn(gPropertyGrid, "Localização", 0.4)
			...
			if (hasMemberPermissionTo(localPlayer, factionID, "modify_factionl_note")) then
				...
				gButtonSaveNote = guiCreateButton(0.79, 0.90, 0.2, 0.08, "Salvar", true, tabNote)
			...
			if towstats then
				if (hasMemberPermissionTo(localPlayer, factionID, "see_towstats")) then
					tabTowstats = guiCreateTab("(Líder) Towstats", tabs)
					gTowGrid = guiCreateGridList(0.01, 0.015, 0.8, 0.905, true, tabTowstats)
					local totals = {[0] = 0, [-1] = 0, [-2] = 0, [-3] = 0, [-4] = 0}
					local colName = guiGridListAddColumn(gTowGrid, 'Nome', 0.2)
					local colRank = guiGridListAddColumn(gTowGrid, 'Rank', 0.2)
					local cols = {
						[0] = guiGridListAddColumn(gTowGrid, 'esta semana', 0.1),
						[-1] = guiGridListAddColumn(gTowGrid, 'semana passada', 0.1),
						[-2] = guiGridListAddColumn(gTowGrid, '2 semanas passada', 0.1),
						[-3] = guiGridListAddColumn(gTowGrid, '3 semanas passada', 0.1),
						[-4] = guiGridListAddColumn(gTowGrid, '4 semanas passada', 0.1)
					}
			...
			if hasMemberPermissionTo(localPlayer, factionID, "modify_faction_note") then
				...
				gButtonSaveFNote = guiCreateButton(0.79, 0.90, 0.2, 0.08, "Salvar", true, tabFNote)
			...
			if hasMemberPermissionTo(localPlayer, factionID, "manage_finance") then
				tabFinance = guiCreateTab("(Lider) Finança", tabs)
			...
			if factionType >= 2 then
				if (hasMemberPermissionTo(localPlayer, factionID, "modify_duty_settings")) then	
					tabDuty = guiCreateTab("(Lider) Configurações Duty", tabs)
			...
			gButtonQuit = guiCreateButton(0.825, 0.7834, 0.16, 0.06, "Sair Facção", true, tabOverview)
			gButtonExit = guiCreateButton(0.825, 0.86, 0.16, 0.06, "Sair Menu", true, tabOverview)
...
function showrespawn()
	...
	showrespawnUI = guiCreateWindow(sx/2 - 150,sy/2 - 50,300,100,"Respawn veículo", false)
	local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Você tem certeza que deseja respawnar os veículos da facção?",true,showrespawnUI)
	guiLabelSetHorizontalAlign (lQuestion,"center",true)
	gButtonRespawn = guiCreateButton(0.1,0.65,0.37,0.23,"Sim",true,showrespawnUI)
	gButtonNo = guiCreateButton(0.53,0.65,0.37,0.23,"Não",true,showrespawnUI)
...
--[[function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		bRanksClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Fechar", true, wRanks)
...
function btEditMOTD(button, state)
	if (source==gButtonEditMOTD) and (button=="left") and (state=="up") then
		if not (wMOTD) then
			...
			wMOTD = guiCreateWindow(x, y, width, height, "Mensagem do Dia", false)
			tMOTD = guiCreateEdit(0.1, 0.2, 0.85, 0.1, tostring(theMotd), true, wMOTD)

			guiSetInputEnabled(true)

			bUpdate = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Atualizar!", true, wMOTD)
			addEventHandler("onClientGUIClick", bUpdate, sendMOTD, false)

			bMOTDClose= guiCreateButton(0.1, 0.775, 0.85, 0.15, "Fechar Janela", true, wMOTD)
...
function sendMOTD(button, state)
	if (source==bUpdate) and (button=="left") and (state=="up") then
		...
		if (found1) or (found2) then
			outputChatBox("Sua mensagem contém caracteres inválidos.", 255, 0, 0)
		else
...
function btInvitePlayer(button, state)
	if (source==gButtonInvite) and (button=="left") and (state=="up") then
		if not (wInvite) then
			...
			wInvite = guiCreateWindow(x, y, width, height, "Convidar Jogador", false)
			tInvite = guiCreateEdit(0.1, 0.2, 0.85, 0.1, "Nome Parcial Jogador", true, wInvite)
			addEventHandler("onClientGUIChanged", tInvite, checkNameExists)

			lNameCheck = guiCreateLabel(0.1, 0.325, 0.8, 0.3, "Jogador não encontrado ou multiplos encontrados.", true, wInvite)
			...
			bInvite = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Convidar!", true, wInvite)
			guiSetEnabled(bInvite, false)
			addEventHandler("onClientGUIClick", bInvite, sendInvite, false)

			bCloseInvite = guiCreateButton(0.1, 0.775, 0.85, 0.15, "Fechar Janela", true, wInvite)
...
function checkNameExists(theEditBox)
	...
	if (count>1) then
		...
		guiSetEnabled(bInvite, false)
	elseif (count==1) then
		guiSetText(lNameCheck, "Jogador Encontrado. ("..foundstr..")")
		guiLabelSetColor(lNameCheck, 0, 255, 0)
		invitedPlayer = found
		guiSetEnabled(bInvite, true)
	elseif (count==0) then
		guiSetText(lNameCheck, "Jogador não encontrado ou multiplos encontrados.")
		guiLabelSetColor(lNameCheck, 255, 0, 0)
		guiSetEnabled(bInvite, false)
	end
	guiLabelSetHorizontalAlign(lNameCheck, "center")
...
function btQuitFaction(button, state)
	if (button=="left") and (state=="up") and (source==gButtonQuit) then
		...
		--else
			local sx, sy = guiGetScreenSize()
			wConfirmQuit = guiCreateWindow(sx/2 - 125,sy/2 - 50,250,100,"Confirmação de Saida", false)
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Você realmente deseja sair da facção " .. getTeamName(theTeam) .. "?",true,wConfirmQuit)
			guiLabelSetHorizontalAlign (lQuestion,"center",true)
			local bYes = guiCreateButton(0.1,0.65,0.37,0.23,"Sim",true,wConfirmQuit)
			local bNo = guiCreateButton(0.53,0.65,0.37,0.23,"Não",true,wConfirmQuit)
...
function btKickPlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonKick) then
		...
		--[[else]]if (playerName~="") then
			...
			outputChatBox("Você removeu " .. playerName:gsub("_", " ") .. " da facção '" .. tostring(theTeamName) .. "'.", 0, 255, 0)
			triggerServerEvent("cguiKickPlayer", getLocalPlayer(), playerName, faction_tab)
		else
			outputChatBox("Por favor selecione um jogador para remover.")
		end
	end
end
...
function btButtonPerk(button, state)
	if (button=="left") and (state=="up") and (source==gButtonPerk) then
		...
		if (playerName == "") then
			outputChatBox("Selecione um membro para gerenciar.")
			return
		end
...
function gotPackages(factionPackages)
	...
	wPerkWindow = guiCreateWindow(x, y, width, height, "Perks da facção para "..playerName, false)
	...
	if not factionPerks then
		outputChatBox("Falha ao carregar os perks de "..playerName.. "")
		factionPerks = { }
	end
	...
	bPerkSave = guiCreateButton(0.05, 0.900, 0.9, 0.045, "Salvar", true, wPerkWindow)
	bPerkClose = guiCreateButton(0.05, 0.950, 0.9, 0.045, "Fechar", true, wPerkWindow)
...
function btRespawnOneVehicle(button, state)
	if button == "left" and state == "up" then
		...
		if vehID then
			...
			outputChatBox("Selecione um veículo para respawnar.", 255, 0, 0)
		end
	end
end
	...
	if (button=="left") and (state=="up") and (source==gAssignPhone) then
		...
		if playerName ~= "" then
			...
			if not (wPhone) then
				...
				wPhone = guiCreateWindow(x, y, width, height, "Número de Telefone", false)
				...
				guiCreateLabel(0.1, 0.2, 0.8, 0.08, "Número de telefone para " .. playerName .. ":", true, wPhone)

				guiSetInputEnabled(true)

				bSet = guiCreateButton(0.1, 0.6, 0.85, 0.15, "Atribuir número de telefone", true, wPhone)
				addEventHandler("onClientGUIClick", bSet, setPhoneNumber, false)

				bClosePhone = guiCreateButton(0.1, 0.775, 0.85, 0.15, "Fechar Janela", true, wPhone)
			...
			outputChatBox("Selecione um membro para ativar o líder.")
		end
	end
end
...
function btPromotePlayer(button, state)
	if (button=="left") and (state=="up") and (source==gButtonPromote) then
		...
		if (playerName == "") then
			outputChatBox("Selecione o jogador que deseja alterar o rank primeiro.", 255, 125, 0)
			return
		end
...
function setPromotionRanks(rankTbl, rankName, playerName)
	...
	if (playerName~="") then
		...
		wPromotions = guiCreateWindow(sX, sY, wX, wY, "Mudar Rank Facção", false)
		guiWindowSetSizable(wPromotions, false)
		-- Labels
		lPromotions1 = guiCreateLabel(14, 26, 150, 15, "Membro Selecionado:", false, wPromotions)
		...
		lPromotions3 = guiCreateLabel(14, 65, 89, 15, "Rank Atual:", false, wPromotions)
		...
		guiGridListAddColumn(promotionsGridlist, "Lista Rank", 0.9)
		-- Button
		bPromotionsUpdate = guiCreateButton(9, 280, 93, 27, "Atualizar Rank", false, wPromotions)
		...
		outputChatBox("Selecione um membro para promover / rebaixar.", 255, 0, 0)
	end
end
...
function saveNewRank()
	...
	if (not row or row == -1) then
		outputChatBox("Selecione um rank para a qual deseja definir está pessoa.", 255, 125, 0)
		return
	end
...
function fillFinance(factionID, bankThisWeek, bankPrevWeek, bankmoney, vehiclesvalue, propertiesvalue)
	...
	tabWeeklyStatement = guiCreateTab("Extrato Semanal", financeTabs)
		...
		statementColLast = guiGridListAddColumn(weeklyStatementGridlist, "Semana passada", 0.25)
		statementColThis = guiGridListAddColumn(weeklyStatementGridlist, "Está semana", 0.25)

		assetsGridlist = guiCreateGridList(639, 11, 395, 298, false, tabWeeklyStatement)
		guiGridListAddColumn(assetsGridlist, "Ativos", 0.65)
		guiGridListAddColumn(assetsGridlist, "Valor", 0.25)

		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Conta Bancaria", false, false)
		...
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Veículos", false, false)
		...
		local row = guiGridListAddRow(assetsGridlist)
		guiGridListSetItemText(assetsGridlist, row, 1, "Propriedades", false, false)
        ...
        local label1 = guiCreateLabel(413, 10, 156, 30, "As informações de finanças da facção datam de no máximo 2 semanas atrás.", false, tabWeeklyStatement)
        	guiSetFont(label1, "default-small")
        	guiLabelSetHorizontalAlign(label1, "left", true)

        local label2 = guiCreateLabel(413, 292, 156, 15, "Clique duas vezes em uma linha para mostrar os detalhes.", false, tabWeeklyStatement)
        	guiSetFont(label2, "default-small")

	tabTransactions = guiCreateTab("Transações", financeTabs)
...
--[[
	...
	local row = guiGridListAddRow(weeklyStatementGridlist)
	guiGridListSetItemText(weeklyStatementGridlist, row, statementColText, "Lucro", false, false)
	...
	addEventHandler("onClientGUIDoubleClick", weeklyStatementGridlist, function()
		...
		elseif(rowName == "Profit") then
			...
			tabStatementDetails = guiCreateTab("Detalhes: "..tostring(rowName), financeTabs)
...
function beginLoad()
	...
	guiGridListSetItemText(Duty.gridlist[1], 0, 2, "Carregando", false, false)

	guiGridListAddRow(Duty.gridlist[2])
	guiGridListSetItemText(Duty.gridlist[2], 0, 1, "Carregando", false, false)

	guiGridListAddRow(Duty.gridlist[3])
	guiGridListSetItemText(Duty.gridlist[3], 0, 1, "Carregando", false, false)
...
function createDutyMain()
    ...
    guiGridListAddColumn(Duty.gridlist[1], "Nome", 0.2)
    guiGridListAddColumn(Duty.gridlist[1], "Raio", 0.1)
    guiGridListAddColumn(Duty.gridlist[1], "Interior", 0.1)
    guiGridListAddColumn(Duty.gridlist[1], "Dimensão", 0.12)
    ...
    Duty.button[1] = guiCreateButton(0.005, 0.939, 0.09, 0.0504, "Adicionar local", true, tabDuty)
    ...
    Duty.label[1] = guiCreateLabel(0.0059, 0.0076, 0.2625, 0.03, "Local Duty", true, tabDuty)
    guiLabelSetHorizontalAlign(Duty.label[1], "center", false)
    Duty.button[2] = guiCreateButton(0.1, 0.939, 0.099, 0.0504, "Remover local", true, tabDuty)
    ...
    Duty.button[3] = guiCreateButton(0.205, 0.939, 0.099, 0.0504, "Editar Local Duty", true, tabDuty)
    ...
    guiGridListAddColumn(Duty.gridlist[2], "Nome", 0.3)
    guiGridListAddColumn(Duty.gridlist[2], "Locais", 0.4)
    ...
    Duty.button[4] = guiCreateButton(0.66, 0.939, 0.09, 0.0504, "Adicionar Duty Novo", true, tabDuty)
    guiSetProperty(Duty.button[4], "NormalTextColour", "FFAAAAAA")
    addEventHandler("onClientGUIClick", Duty.button[4], createDuty, false)

    Duty.button[5] = guiCreateButton(0.765, 0.939, 0.09, 0.0504, "Remover Duty", true, tabDuty)
    guiSetProperty(Duty.button[5], "NormalTextColour", "FFAAAAAA")
    addEventHandler("onClientGUIClick", Duty.button[5], removeDuty, false)

    Duty.button[6] = guiCreateButton(0.869, 0.939, 0.09, 0.0504, "Editar Duty Perks", true, tabDuty)
    ...
    Duty.label[3] = guiCreateLabel(0.325, 0.0076, 0.2886, 0.03, "Localizações Veículos Duty", true, tabDuty)
    guiLabelSetHorizontalAlign(Duty.label[3], "center", false)
    Duty.button[8] = guiCreateButton(0.3355, 0.5304, 0.1, 0.0504, "Adicionar Veículo Duty", true, tabDuty)
    ...
    Duty.button[9] = guiCreateButton(0.5177, 0.5304, 0.1, 0.0504, "Remover Veículo Duty", true, tabDuty)
   ...
   --[[Duty.gridlist[4] = guiCreateGridList(0.3355, 0.6, 0.282, 0.35, true, tabDuty) Was going to be for logs but meh UCP.
    ...
    guiGridListAddColumn(Duty.gridlist[4], "Nome", 0.3)
...
function grabDetails(dutyID)
	...
	guiGridListSetItemText(DutyCreate.gridlist[1], 0, 2, "Carregando", false, false)

	guiGridListAddRow(DutyCreate.gridlist[2])
	guiGridListSetItemText(DutyCreate.gridlist[2], 0, 2, "Carregando", false, false)

	guiGridListAddRow(DutyCreate.gridlist[3])
	guiGridListSetItemText(DutyCreate.gridlist[3], 0, 1, "Carregando", false, false)

	guiGridListAddRow(DutyCreate.gridlist[4])
	guiGridListSetItemText(DutyCreate.gridlist[4], 0, 1, "Carregando", false, false)
end
		...
		for k,v in pairs(customg[customEditID][5]) do
			if tonumber(v[2]) >= 0 then -- Items
				...
				if tonumber(v[2]) == -100 then
					guiGridListSetItemText(DutyCreate.gridlist[3], row, 1, "Colete", false, false) -- Weapon Name
	...
	for k,v in pairs(allowList) do
		if tonumber(v[2]) >= 0 then -- Items
			...
			if customEditID == 0 or (customEditID ~= 0 and not customg[customEditID][5][tostring(v[1])]) then
				...
				if tonumber(v[2]) == -100 then
					guiGridListSetItemText(DutyCreate.gridlist[1], row, 1, "Colete", false, false)
...
function checkAmmo()
	...
	if r >= 0 then
		if tonumber(guiGetText(DutyCreate.edit[2])) then
			if tonumber(guiGridListGetItemText(DutyCreate.gridlist[1], r, 2)) >= tonumber(guiGetText( DutyCreate.edit[2] )) then
				...
				guiSetText(DutyCreate.label[2], "Valido")
	...
	guiSetText(DutyCreate.label[2], "Invalido")
...
function createDuty()
    ...
    DutyCreate.window[1] = guiCreateWindow(450, 310, 768, 566, "Janela Edição Duty - Principal", false)
    guiWindowSetSizable(DutyCreate.window[1], false)
    centerWindow(DutyCreate.window[1])

    DutyCreate.button[1] = guiCreateButton(600, 512, 158, 44, "Cancelar", false, DutyCreate.window[1])
    ...
    DutyCreate.button[2] = guiCreateButton(454, 512, 138, 44, "Salvar", false, DutyCreate.window[1])
    ...
    guiGridListAddColumn(DutyCreate.gridlist[1], "Nome Arma", 0.5)
    guiGridListAddColumn(DutyCreate.gridlist[1], "Quantidade máxima de munição", 0.5)
   ...
   --guiGridListAddColumn(DutyCreate.gridlist[2], "ID", 0.1)
    guiGridListAddColumn(DutyCreate.gridlist[2], "Nome Item", 0.3)
    guiGridListAddColumn(DutyCreate.gridlist[2], "Descrição", 0.7)
    ...
    guiGridListAddColumn(DutyCreate.gridlist[3], "Arma", 0.5)
    guiGridListAddColumn(DutyCreate.gridlist[3], "Munição", 0.3)
    ...
    DutyCreate.edit[2] = guiCreateEdit(445, 81, 127, 27, "Quantidade de munição", false, DutyCreate.window[1])
    DutyCreate.label[2] = guiCreateLabel(444, 108, 128, 77, "Invalido", false, DutyCreate.window[1])
    guiLabelSetColor(DutyCreate.label[2], 255, 0, 0)
    ...
    DutyCreate.button[8] = guiCreateButton(160, 512, 138, 44, "Locais", false, DutyCreate.window[1])
    ...
    DutyCreate.label[3] = guiCreateLabel(57, 19, 319, 15, "Armas Disponiveis", false, DutyCreate.window[1])
    guiLabelSetHorizontalAlign(DutyCreate.label[3], "center", false)
    DutyCreate.label[4] = guiCreateLabel(582, 17, 176, 17, "Armas de Trabalho", false, DutyCreate.window[1])
    guiLabelSetHorizontalAlign(DutyCreate.label[4], "center", false)
    DutyCreate.label[5] = guiCreateLabel(57, 228, 319, 15, "Itens Disponiveis", false, DutyCreate.window[1])
    guiLabelSetHorizontalAlign(DutyCreate.label[5], "center", false)
    DutyCreate.label[6] = guiCreateLabel(583, 227, 175, 21, "Itens de Trabalho", false, DutyCreate.window[1])
    guiLabelSetHorizontalAlign(DutyCreate.label[6], "center", false)
    DutyCreate.label[7] = guiCreateLabel(14, 463, 88, 32, "Nome Duty:", false, DutyCreate.window[1])
...
function createLocations()
    ...
    DutyLocations.window[1] = guiCreateWindow(573, 285, 520, 423, "Janela Edição Duty - Locais", false)
    ...
    guiGridListAddColumn(DutyLocations.gridlist[1], "Nome", 0.9)
    ...
    guiGridListAddColumn(DutyLocations.gridlist[2], "Nome", 0.9)
    ...
    DutyLocations.label[1] = guiCreateLabel(10, 19, 233, 17, "Todos os locais", false, DutyLocations.window[1])
    guiLabelSetHorizontalAlign(DutyLocations.label[1], "center", false)
    DutyLocations.label[2] = guiCreateLabel(270, 19, 233, 17, "Locais Trabalho", false, DutyLocations.window[1])
    guiLabelSetHorizontalAlign(DutyLocations.label[2], "center", false)
    DutyLocations.button[3] = guiCreateButton(270, 367, 146, 36, "Salvar", false, DutyLocations.window[1])
    ...
    DutyLocations.button[4] = guiCreateButton(103, 367, 146, 36, "Cancelar", false, DutyLocations.window[1])
	...
	if string.find(raw, ":") then
		...
		if tonumber(howAboutIt[1]) and tonumber(howAboutIt[2]) then
			if not skinAlreadyExists(tonumber(howAboutIt[1]), tonumber(howAboutIt[2])) then
				...
				outputChatBox("Você não pode adicionar a mesma skin duas vezes.", 255, 0, 0)
			end
		else
			outputChatBox("Por favor, use apenas números.", 255, 0, 0)
		end
	else
...
function createSkins(customSkins)
	if DutySkins.window[1] and isElement(DutySkins.window[1]) and customSkins and type(customSkins)=='table' then
	    ...
	    DutySkins.label[1] = guiCreateLabel(10, 80, 132, 138, "Carregando...", false, DutySkins.window[1])
	    ...
	    guiGridListAddColumn(DutySkins.gridlist[2], "Adicionada", 0.85)
	    if customEditID == 0 then
	    ...
	    DutySkins.button[1] = guiCreateButton(10, 228, 87, 25, "Cancelar", false, DutySkins.window[1])
	    DutySkins.button[2] = guiCreateButton(103, 228, 87, 25, "Adicionar", false, DutySkins.window[1])
	    guiSetVisible(DutySkins.button[2], false)
	    DutySkins.button[3] = guiCreateButton(197, 228, 87, 25, "Salvar", false, DutySkins.window[1])
	    ...
	    addEventHandler('onClientGUIClick', DutySkins.window[1], function()
	    	...
	    	if source == DutySkins.gridlist[1] then
				...
				if r >= 0 then
					...
					guiSetText(DutySkins.button[2], "Adicionar")
					guiSetText(DutySkins.edit[1], "")
				end
	    	...
	    	elseif source == DutySkins.gridlist[2] then
				...
				if r >= 0 then
					guiSetVisible(DutySkins.button[2], true)
					guiSetText(DutySkins.button[2], "Remover")
				end
			...
			elseif source == DutySkins.button[2] then
				if guiGetText(DutySkins.button[2]) == "Remover" then
				...
				elseif guiGetText(DutySkins.button[2]) == 'Adicionar' then
					r, c = guiGridListGetSelectedItem ( DutySkins.gridlist[1] )
					if r >= 0 then
...
function createVehicleAdd()
    ...
    DutyVehicleAdd.window[1] = guiCreateWindow(685, 338, 335, 85, "Adicionar Novo Veículo de Trabalho", false)
    guiWindowSetSizable(DutyVehicleAdd.window[1], false)
    centerWindow(DutyVehicleAdd.window[1])

    DutyVehicleAdd.edit[1] = guiCreateEdit(9, 26, 181, 40, "Veículo ID", false, DutyVehicleAdd.window[1])
    ...
    DutyVehicleAdd.button[2] = guiCreateButton(263, 26, 62, 40, "Fechar", false, DutyVehicleAdd.window[1])
...
function createDutyLocationMaker(x, y, z, r, i, d, name)
    ...
    DutyLocationMaker.window[1] = guiCreateWindow(638, 285, 488, 198, "Adicionar local de serviço", false)
    guiWindowSetSizable(DutyLocationMaker.window[1], false)
    centerWindow(DutyLocationMaker.window[1])

    DutyLocationMaker.label[1] = guiCreateLabel(8, 24, 44, 19, "Valor X:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[1] = guiCreateEdit(56, 24, 135, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[2] = guiCreateLabel(201, 24, 53, 19, "Valor Y:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[2] = guiCreateEdit(253, 23, 88, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[3] = guiCreateLabel(355, 25, 52, 18, "Valor Z:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[3] = guiCreateEdit(406, 23, 71, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[4] = guiCreateLabel(8, 60, 49, 18, "Raio:", false, DutyLocationMaker.window[1])
    ...
    DutyLocationMaker.label[6] = guiCreateLabel(336, 60, 60, 18, "Dimensão:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.edit[6] = guiCreateEdit(402, 58, 75, 20, "", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[7] = guiCreateLabel(9, 92, 57, 21, "Nome:", false, DutyLocationMaker.window[1])
    DutyLocationMaker.label[8] = guiCreateLabel(10, 119, 467, 28, "O nome do local é usado estritamente para sua identificação.", false, DutyLocationMaker.window[1])
    ...
    DutyLocationMaker.button[1] = guiCreateButton(10, 149, 115, 37, "Inserir posição atual", false, DutyLocationMaker.window[1])
    ...
    DutyLocationMaker.button[2] = guiCreateButton(184, 149, 115, 37, "Fechar", false, DutyLocationMaker.window[1])
    ...
    DutyLocationMaker.button[3] = guiCreateButton(357, 149, 115, 37, "Salvar", false, DutyLocationMaker.window[1])
...
function saveGUI()
	if source == DutyCreate.button[2] then -- Main
		...
		if name ~= "" then
			...
			outputChatBox("Por favor, insira um nome para este dever.", 255, 0, 0)
			return
		end
		destroyElement(DutyCreate.window[1])
	...
	elseif source == DutyVehicleAdd.button[1] then -- Vehicle Add
		...
		if not duplicateVeh("Localização", id, factionIDg) then
			triggerServerEvent("Duty:AddVehicle", resourceRoot, tonumber(id), factionIDg)
			destroyElement(DutyVehicleAdd.window[1])
		else
			outputChatBox("Este veículo já foi adicionado.", 255, 0, 0)
		end
		...
		if (x and y and z and r and i and d and name) then
			if r >= 1 and r <=10 then
				if string.len(name) > 0 then
					...
					outputChatBox("Você deve inserir um nome.", 255, 0, 0)
					return
				end
			else
				outputChatBox("O raio deve estar entre 1 e 10", 255, 0, 0)
				return
			end
		else
			outputChatBox("Por favor, insira todas as informações corretamente.", 255, 0, 0)
			return
		end
		locationEditID = 0
		destroyElement(DutyLocationMaker.window[1])
