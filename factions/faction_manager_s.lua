...
addEventHandler( 'factions:editFaction', resourceRoot, function( data, old_id )
	...
	if res then
		if nums > 0 and res[1].name == data.name then
			return not triggerClientEvent( client, 'factions:editFaction:callback', resourceRoot, 'O nome da facção já está em uso.' )
		end
		...
		if old_id then
			...
			if nums and nums > 0 then
				...
				if team then
					...
					outputDebugString( "[FACÇÂO] factions:editFaction / Unable to allocate pool faction element id ".. old_id.. ". Data sample: "..tostring(team) )
				end
				exports.global:sendMessageToAdmins( "[FACÇÂO] " .. exports.global:getPlayerFullIdentity( client ).." modificou a facção '" .. data.name .. "'." )
				exports.factions:sendNotiToAllFactionMembers( old_id, "Sua facção '" .. data.name .. "' foi modificada.", "Modificada por "..exports.global:getPlayerFullIdentity( client, 1, true )..".")
				...
				return not triggerClientEvent( client, 'factions:editFaction:callback', resourceRoot, 'O código de erro 61 ocorreu durante o processo. A modificação da facção falhou. ' )
			end
		-- if creating new faction.
			...
			if id and tonumber(id) then
				...
				exports.global:sendMessageToAdmins( "[FACÇÂO] " .. exports.global:getPlayerFullIdentity( client ).." criou a facção '" .. data.name .. "'." )
...
addEventHandler( 'factions:delete', resourceRoot, function( factionID )
	...
	--if mysql:query_free("UPDATE vehicles SET deleted="..getElementData(thePlayer, "account:id").." WHERE faction=" .. factionID) then
		for i, veh in pairs(getElementsByType("vehicle")) do
			if veh and isElement(veh) and getElementData(veh, "faction") == factionID then
				...
				executeCommandHandler("delveh", client, vehid)
				exports.vehicle_manager:addVehicleLogs(vehid, "Veículo destruído após a exclusão da facção ("..(theTeam and getTeamName( theTeam ) or "N/A")..").", client)
	...
	exports.global:sendMessageToAdmins("[FACÇÂO] "..exports.global:getPlayerFullIdentity(client, 1, true).." deletou a facção '"..(theTeam and getTeamName( theTeam ) or "N/A").."'. "..ints.." interior(es) e "..vehs.." veículo(s) também foram excluidos.")
	exports.factions:sendNotiToAllFactionMembers(factionID, "Sua facção '"..(theTeam and getTeamName( theTeam ) or "N/A").."' was deleted", "Processed by "..exports.global:getPlayerFullIdentity(client, 1, true)..".")
	if theTeam then
			...
			if nums > 0 then
				...
				if res2 and nums2 > 0 then
					...
					for _, member in ipairs( res ) do
						...
						member.username = exports.cache:getUsernameFromId(member.account) or "Desconhecido"
				...
				return triggerClientEvent( client, 'factions:listMember', resourceRoot, fact_id, 'Esta facção atualmente não tem nenhum membro.' )
			end
		else
			dbFree( qh )
			return not triggerClientEvent( client, 'factions:listMember', resourceRoot, fact_id, 'Errors occurred while fetching information from server.' )
