...
function createAdminDuty(factionT, dutyT)
    ...
    DutyAllow.window[1] = guiCreateWindow(562, 250, 564, 351, "Admin Duty Configurações", false)
    ...
    guiGridListAddColumn(DutyAllow.gridlist[1], "Nome", 0.3)
    guiGridListAddColumn(DutyAllow.gridlist[1], "Descrição", 0.6)
    ...
    DutyAllow.label[2] = guiCreateLabel(11, 59, 543, 16, "Itens Permitidos", false, DutyAllow.window[1])
    ...
    DutyAllow.label[4] = guiCreateLabel(163, 308, 68, 20, "Valor Item:", false, DutyAllow.window[1])
    guiLabelSetVerticalAlign(DutyAllow.label[4], "center")
    DutyAllow.edit[2] = guiCreateEdit(230, 308, 78, 21, "1", false, DutyAllow.window[1])
    DutyAllow.combobox[1] = guiCreateComboBox(1, 25, 242, 998, "Selecionar Facção.", false, DutyAllow.window[1])
    exports.global:guiComboBoxAdjustHeight(DutyAllow.combobox[1], #factionT)
    DutyAllow.button[3] = guiCreateButton(442, 25, 102, 35, "Terminar", false, DutyAllow.window[1])
    ...
    guiGridListSetItemText ( DutyAllow.gridlist[1], row, 2, "Selecione uma Facção...", false, false )
		...
		for k,v in pairs(factionTable[key][3]) do
			if tonumber(v[2]) < 0 then
				...
				if tonumber(v[2]) == -100 then
					guiGridListSetItemText(DutyAllow.gridlist[1], row, 1, v[2], false, true)
					guiGridListSetItemText(DutyAllow.gridlist[1], row, 2, "Colete", false, false)
...
function toggleView()
	...
	if item == 1 then -- Weapons
        ...
        guiGridListSetColumnTitle(DutyAllow.gridlist[1], 2, "Nome")
        guiGridListSetColumnTitle(DutyAllow.gridlist[1], 3, "Max Munição")

        guiSetText(DutyAllow.label[1], "Arma ID:")
        guiSetText(DutyAllow.label[4], "Max Munição:")
	elseif item == 0 then -- Items
		guiSetText(DutyAllow.label[2], "Itens Permitidos")

        guiGridListSetColumnTitle(DutyAllow.gridlist[1], 2, "Nome")
        guiGridListSetColumnTitle(DutyAllow.gridlist[1], 3, "Descrição")

        guiSetText(DutyAllow.label[1], "Item ID:")
        guiSetText(DutyAllow.label[4], "Valor Item:")
	end
...
function allowItem()
	...
	if not exports['item-system']:isItem(itemID) and selection == 0 then
		outputChatBox("Isso não é um item...", 255, 0, 0)
		return
	elseif tonumber(itemID) == 16 then
		outputChatBox("As skins devem ser carregadas pelos líderes da facção enquanto estão em serviço na Star HQ.", 255, 0, 0)
		return
	elseif not getWeaponNameFromID(itemID) and selection == 1 and tonumber(itemID) ~= 100 then
		outputChatBox("Isso não é uma arma...", 255, 0, 0)
		return
	end
			...
			elseif selection == 1 then -- Weapon
				if tonumber(itemValue) then
					if not weapBanList[-tonumber(itemID)] then -- Check if its banned.
						...
						outputChatBox("Esta arma esta banida e não pode ser adicionada.", 255, 0, 0)
					end
				else
					outputChatBox("A munição maxima deve ser um número!", 255, 0, 0)
				end
			end
			populateList(faction)
		else
			outputChatBox("O ID do item deve ser um número!", 255, 0, 0)
		end
	else
		outputChatBox("Por favor faça uma seleção primeiro.", 255, 0, 0)
	end
end
