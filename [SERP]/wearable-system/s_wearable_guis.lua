...
function savePosition(table)
	...
	if exports.mysql:query_free("UPDATE characters SET witems = '" .. exports.mysql:escape_string( toJSON( witems ) ) .. "' WHERE id = " .. exports.mysql:escape_string( getElementData( source, "dbid" ) ) ) then
		outputChatBox( "You successfully updated your positioning!", source, 0, 255, 0 )
		exports.anticheat:changeProtectedElementDataEx( source, "wearableitems", witems, false)
	else
		outputChatBox( "Failed to set position. (SQL Error)", source, 255, 0, 0 )
	end
end
...
function requestPosition(itemID, itemName)
	...
	exports.global:sendMessageToAdmins("[SCRIPT] " .. getPlayerName(source):gsub("_", " ") .. " went in to the wearable positioning mode.")
...
addCommandHandler("aduzen", openClientGUIWindow)

--[[function openClientGUIWindowPD(thePlayer)
	local legitimatetable = {"Whoever", "is", "trying", 0101010101, "to", "fix", "this", "w-active", "check", 1337, "is", "a", "fucking", "faggot"}
	
	local team = getPlayerTeam(thePlayer)
	local teamName = getTeamName ( team ) 
	if teamName == "Los Santos Polis Departmanı" then
		triggerClientEvent(thePlayer, "wearable-system:openWindowPD", thePlayer, legitimatetable)
	end
end
addCommandHandler("pdduzen", openClientGUIWindowPD)]]
