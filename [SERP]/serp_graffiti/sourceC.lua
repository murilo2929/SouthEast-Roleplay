...
function graffitiCMD()
	if getElementData(localPlayer, "loggedin")~=1 then return end
	local gperm = getElementData(getLocalPlayer(), "gperm") or 0
	if gperm == 1 then
		showTheEditor()
	else
		outputChatBox("#00D5FF[Graffiti] #ffffffYou are not allowed to use /graffiti.", 255, 255, 255, true)
		outputChatBox("#00D5FF[Graffiti] #ffffffYou may report and ask an admin for permission, explaining why you need it.", 255, 255, 255, true)
	end
end
addCommandHandler("graffiti", graffitiCMD)
addCommandHandler("draw", graffitiCMD)

local maxsize = 8

addCommandHandler("gsize",
	function (command, size)
		if not tonumber(size) or tonumber(size) < 0.5 or tonumber(size) > maxsize then
			outputChatBox("#b0e56b[Usage]:#ffffff /" .. command .. " [Size (0.5 - "..maxsize..")]", 255, 255, 255, true)
		else
			graffitiSize = tonumber(size)
			outputChatBox("#b0e56b[Graffiti]:#ffffff Graffiti size sucessfully changed to #e56b6b" .. graffitiSize .. ".", 255, 255, 255, true)
		end
	end
)
...
addCommandHandler("gadmin",
	...
	if exports.integration:isPlayerTrialAdmin(getLocalPlayer()) then
		...
		if adminMode then
			outputChatBox("#5fd664[Graffiti] #ffffffGraffiti admin mode is now enabled.", 255, 255, 255, true)
		else
			outputChatBox("#ff4646[Graffiti] #ffffffGraffiti admin mode is now disabled.", 255, 255, 255, true)
		end
	end
end
)
...
addCommandHandler("gclean",
	function ()
	-- outputChatBox("#00D5FF[Graffiti] #ffffffGraffiti cleaning is currently disabled. Coming soon!", 255, 255, 255, true)
	...
	if job == 4 then
		...
		if graffitiCleanMode then
			outputChatBox("#5fd664[Graffiti] #ffffffGraffiti cleaning mode is now enabled.", 255, 255, 255, true)
			outputChatBox("#5fd664[Graffiti] #ffffffAbuse is a bannable offense.", 255, 255, 255, true)
		else
			outputChatBox("#ff4646[Graffiti] #ffffffGraffiti cleaning mode is now disabled.", 255, 255, 255, true)
		end
	end
	--]]
end
...
addEventHandler("onClientBrowserDocumentReady", getRootElement(),
	function ()
		if source == loadImagePanel.browserElement then
			...
			setTimer(
				function()
					if isElement(loadImagePanel.browserElement) then
				...
				if deltaTime >= 3000 then
					if utf8.len(path) > 0 then
						...
						if fileExtension and fileExtension == ".png" or fileExtension == ".jpg" or fileExtension == ".jpeg" or fileExtension == ".bmp" then
							...
							outputChatBox("#32b3ef[Graffiti]: #ffffffYou started downloading the selected image. Please wait..", 255, 255, 255, true)
						else
							outputChatBox("#e56b6b[Graffiti]: #ffffffThe file extension is not valid! Allowed: PNG, JPG, JPEG, BMP", 255, 255, 255, true)
						end
					else
						outputChatBox("#e56b6b[Graffiti]: #ffffffYou must add an image URL!", 255, 255, 255, true)
					end
				else
					outputChatBox("#e56b6b[Graffiti]: #ffffffPlease wait #e56b6b" .. 3 - math.floor(deltaTime / 1000) .. " second#ffffff for the next download.", 255, 255, 255, true)
				end
			else
...
addEventHandler("onClientReceiveDownloadedImage", getRootElement(),
	function (imagePixels, errno)
		if errno == 0 then
			...
			outputChatBox("#32b3ef[Graffiti]: #ffffffSelected image successfully saved.", 255, 255, 255, true)
			outputChatBox("#32b3ef[Graffiti]: #ffffffYou can find the image in the 'Saved graffitis' panel.", 255, 255, 255, true)
		else
			outputChatBox("#e56b6b[Graffiti]: #ffffffThe selected image is not available!", 255, 255, 255, true)
		end
	...
	function (fromClient, pixels, data)
		...
		if graffitiFile then
			...
			if fromClient == localPlayer then
				...
				outputChatBox("#b0e56b[Graffiti]: #ffffffGraffiti spraying successfully completed.", 255, 255, 255, true)

				local timer = setTimer(function()
					removeGraffitiTimer(data.fileName)
				end, 900000, 1, data.fileName)
				local playerName = getPlayerName(localPlayer)
				table.insert(graffitiTimers, {cleanTimer=timer, img=data.fileName, charName=playerName})
			end
		end
	end
)

function removeGraffitiTimer(imgName)
	for row, v in pairs(graffitiTimers) do
		if v.img == imgName then
			table.remove(graffitiTimers, row)
			break
		end
	end
end

function findPlayerGraffitiTimer(imgName, charName)
	for row, v in pairs(graffitiTimers) do
		if v.img == imgName and charName == v.charName then
			return true
		end
	end
end
	...
	function (weapon, _, _, hitX, hitY, hitZ)
		...
		if lastDrawedPixels and not startedGraffiti.isActive and not startedGraffiti.actionDisabled and #drawingSteps >= 2 and getTickCount() >= nextSprayingAfterTick then
			if getTickCount() >= tryToSprayGraffitiTick then
				...
				outputChatBox("#32b3ef[Graffiti]: #ffffffYou started spraying a new graffiti.", 255, 255, 255, true)
			end
...
addEventHandler("onClientRender", getRootElement(),
	function ()
		...
		elseif graffitiCleanMode or (not graffitiCleanMode and #graffitiTimers > 0) then
			local cameraX, cameraY, cameraZ = getCameraMatrix()
			local playerInterior, playerDimension = getElementInterior(localPlayer), getElementDimension(localPlayer)
			...
			for k in pairs(streamedGraffitis) do
				if loadedGraffitis[k] and loadedGraffitiTextures[loadedGraffitis[k].fileName] then
					...
					local found = findPlayerGraffitiTimer(k, getPlayerName(localPlayer)) -- returns table
					if not graffitiCleanMode and found ~= true then canContinue = false end -- AKA you don't have graffitiCleanMode and no graffiti of within 15min around.
...
addEventHandler("onClientClick", getRootElement(),
	function (button, state)
		if adminMode or graffitiCleanMode or (not graffitiCleanMode and #graffitiTimers > 0) then
			if selected3DButton then
				if button == "left" and state == "up" then
					if string.find(selected3DButton, "trash|") then
						...
						if serverGraffitis[fileName].isProtected then
							outputChatBox("#e56b6b[Graffiti]: #ffffffYou can't remove this graffiti because it's protected.", 255, 255, 255, true)
							return
						end

						triggerServerEvent("deleteGraffiti", localPlayer, fileName)
					elseif string.find(selected3DButton, "protect|") then
						if not exports.integration:isPlayerAdmin(getLocalPlayer()) then
							outputChatBox("#e56b6b[Graffiti]: #ffffffYou can't protect this graffiti. Admin+", 255, 255, 255, true)
							return
						end
					...
					elseif string.find(selected3DButton, "clean|") then
						...
						if not getPedOccupiedVehicle(localPlayer) then
							if not isTimer(cleaningGraffitiTimer) then
								if serverGraffitis[fileName].isProtected then
									outputChatBox("#e56b6b[Graffiti]: #ffffffYou can't clean this graffiti because it's protected.", 255, 255, 255, true)
									return
								end
								...
								-- Checks if the one trying to delete is part of your timer & ur not just generally cleaning.
								local playerName = getPlayerName(localPlayer)
								local foundTimer = findPlayerGraffitiTimer(fileName, playerName)
								if (not graffitiCleanMode and #graffitiTimers > 0) and foundTimer == true then
									cleaningGraffitiTimer = setTimer(cleanGraffiti, graffitiCleanTime, 1, fileName, false)
								else
									cleaningGraffitiTimer = setTimer(cleanGraffiti, graffitiCleanTime, 1, fileName, true)
								end
							else
								outputChatBox("#e56b6b[Graffiti]: #ffffffYou are already cleaning graffiti.", 255, 255, 255, true)
							end
						else
...
function cleanGraffiti(fileName, pay)
	if pay == nil then pay = true end

	triggerServerEvent("deleteGraffiti", localPlayer, fileName, pay, true)
...
function showTheEditor()
	if isElement(loadImagePanel.browserElement) then
		destroyElement(loadImagePanel.browserElement)
		loadImagePanel.browserElement = nil
	end
	...
	if isEventHandlerAdded("onClientRender", getRootElement(), renderTheEditor) then
		...
		if not drawingSurface or not isElement(drawingSurface) then
			...
			triggerEvent("checkScreenUpload", localPlayer)
...
function renderTheEditor()
	...
	if loadImagePanel.isActive then
		...
		if not isElement(loadImagePanel.browserElement) then
			dxDrawText("You have disabled CEF browsers via F10 settings.\nYou have to use /addgraffiti [URL] to load an image from URL.", paintAreaPosX, paintAreaPosY, paintAreaPosX + paintAreaWidth, paintAreaPosY + paintAreaHeight, tocolor(255, 255, 255), 1.0, mainFont, "center", "center", false, false, false, true)
		else
...
function processTheEditorButtons(button, state, cursorX, cursorY)
	...
	elseif button == "left" and state == "up" then
		panelIsMoving = false
		if loadImagePanel.browserElement then
...
addCommandHandler("addgraffiti",
	function (command, path)
		if getElementData(localPlayer, "loggedin")~=1 then return end

		if not path then
			outputChatBox("SYNTAX: /"..command.." [Image URL (imgur.com)]", 255,194,14)
			outputChatBox("Imports an image from URL to be used in /graffiti.", 255,126,14)
			return
		end
