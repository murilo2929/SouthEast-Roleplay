...
addEventHandler("neon->showList", root,
	function(vehID)
		...
		neonGUI = guiCreateWindow(0, 0, 355, 345, "Sistema de Neon", false)
		guiWindowSetSizable(neonGUI, false)
		exports.ed_global:centerWindow(neonGUI)
		gridlist = guiCreateGridList(9, 21, 640, 235, false, neonGUI)
		guiGridListAddColumn(gridlist, "Neon", 0.85)
		...
		vehicleID = guiCreateEdit(85, 267, 560, 28, "", false, neonGUI)
		if vehID then
			guiSetEnabled(vehicleID, false)
			guiSetText(vehicleID, vehID)
		end
		label = guiCreateLabel(4, 267, 81, 28, "Carro ID:", false, neonGUI)
		guiSetFont(label, "default-bold-small")
		guiLabelSetHorizontalAlign(label, "center", false)
		guiLabelSetVerticalAlign(label, "center")
		ok = guiCreateButton(9, 303, 342, 32, "Comprar ( $15 )", false, neonGUI)
		if vehID then
			guiSetText(ok, "Mudar ( 1,000$ )")
		end
		close = guiCreateButton(351, 303, 294, 31, "Fechar", false, neonGUI)
				...
				if (b == "left") then
					...
					elseif (source == ok) then
						if guiGetText(vehicleID) == "" or not tonumber(guiGetText(vehicleID)) then
							outputChatBox("#ffffff Você digitou o ID do seu veículo incorretamente..", 255, 0, 0, true)
							return
						end
						...
						if not selectedNeon or selectedNeon == -1 then
							outputChatBox("#ffffff Você não escolheu nenhuma cor neon.", 255, 0, 0, true)
							return
						end
						guiSetInputEnabled(false)
						showCursor(false)
...
function addNeon(vehicle, neon, setDefault)
	...
	if vehicleNeon[vehicle]["object.1"] or vehicleNeon[vehicle]["object.2"] then
		...
		setElementInterior(vehicleNeon[vehicle]["object.1"], getElementInterior(localPlayer))
		setElementDimension(vehicleNeon[vehicle]["object.1"], getElementDimension(localPlayer))
		setElementInterior(vehicleNeon[vehicle]["object.2"], getElementInterior(localPlayer))
		setElementDimension(vehicleNeon[vehicle]["object.2"], getElementDimension(localPlayer))
