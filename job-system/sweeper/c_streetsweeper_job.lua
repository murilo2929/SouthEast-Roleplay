...
function displaySweeperJob()
	if (jobstate==0) then
		...
		outputChatBox("#FF9933Aproxime-se do #FF66CCblip#FF9933 no seu radar e entre na varredora para começar seu trabalho.", 255, 194, 15, true)
		outputChatBox("#FF9933Digite /iniciartrabalho quando estiver no varredor.", 255, 194, 15, true)
	end
end
...
function startSweeperJob()
	if (jobstate==1) then
		...
		if not (vehicle) then
			outputChatBox("Você deve estar em um varredor.", 255, 0, 0)
		else
			local model = getElementModel(vehicle)
			if (model==574) then -- SWEEPER
				routescompleted = 0
			
				outputChatBox("#FF9933Dirija para o #FF66CCblip#FF9933 no radar e use /limparrua.", 255, 194, 15, true)
				destroyElement(blip)
				...
				outputChatBox("Você não está em um varredor.", 255, 0, 0)
			end
		end
	end
end
addCommandHandler("startjob", startSweeperJob)
addCommandHandler("iniciartrabalho", startSweeperJob)
...
function cleanRoad()
	if (jobstate==2 or jobstate==3) then
		...
		if not (vehicle) then
			outputChatBox("Você não está na van.", 255, 0, 0)
		else
			local model = getElementModel(vehicle)
			if (model==414) then -- MULE
				if (isElementWithinColShape(vehicle, colshape)) then
					...
					destroyElement(marker)
					destroyElement(blip)
					outputChatBox("Você completou sua corrida de caminhão.", 0, 255, 0)
					routescompleted = routescompleted + 1
					outputChatBox("#FF9933Agora você pode retornar ao #00CC00deposito #FF9933e obter o seu salário", 0, 0, 0, true)
					outputChatBox("#FF9933ou continue na próxima #FF66CCponto de entrega#FF9933 e aumentar o seu salário.", 0, 0, 0, true)
					...
					outputChatBox("#FF0066Você não está no seu #FF66CCponto de entrega#FF0066.", 255, 0, 0, true)
				end
			else
				outputChatBox("Você não está em uma van.", 255, 0, 0)
			end
		end
	end
end
	...
	if not (vehicle) then
		...
		if (model==414) then -- MULE
			if (jobstate==3) then
				...
				outputChatBox("Você ganhou $" .. wage .. " em suas viagens de caminhão.", 255, 194, 15)
				local vehicle = getPedOccupiedVehicle(localPlayer)
			...
			outputChatBox("Obrigado por seus serviços para RS Haul.", 0, 255, 0)
			destroyElement(colshape)
			destroyElement(marker)
			destroyElement(blip)
