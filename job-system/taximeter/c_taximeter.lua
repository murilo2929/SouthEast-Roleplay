...
function showTaximeterGui()
	...
	if isDriver then --buttons etc. go here
		...
		if meterRunning then
			gui_label_pax_running = guiCreateLabel(17,113,218,20,"Taxímetro em execução",false,gui_base)
			guiLabelSetColor(gui_label_pax_running,0,255,0)
		else
			gui_label_pax_running = guiCreateLabel(17,113,218,20,"Taxímetro não parado",false,gui_base)
...
function setFare()
	if(source == gui_btn3) then
		...
		gui_fareWin = guiCreateWindow(x,y,width,height,"Definir tarifa de taxímetro",false,nil)
			
			if(meterRunning and taxiDistance > 0) then
				local lbl = guiCreateLabel(20,20,260,80,"O medidor deve ser parado e redefinido antes de definir uma nova tarifa!",false,gui_fareWin)
			...
			elseif meterRunning then
				local lbl = guiCreateLabel(20,20,260,80,"O medidor deve ser interrompido antes de definir uma nova tarifa!",false,gui_fareWin)
					guiLabelSetColor(lbl,255,0,0)
					guiLabelSetHorizontalAlign(lbl,"center",true)
				local cnfm = guiCreateButton(110,100,80,40,"OK",false,gui_fareWin)
				addEventHandler("onClientGUIClick", cnfm, function()
			...
			elseif taxiDistance > 0 then
				local lbl = guiCreateLabel(20,20,260,80,"O medidor deve ser reiniciado antes de definir uma nova tarifa!",false,gui_fareWin)
					guiLabelSetColor(lbl,255,0,0)
					guiLabelSetHorizontalAlign(lbl,"center",true)
				local cnfm = guiCreateButton(110,100,80,40,"OK",false,gui_fareWin)
				addEventHandler("onClientGUIClick", cnfm, function()
			...
			local lbl = guiCreateLabel(20,20,150,20,"Defina uma tarifa por quilômetro",false,gui_fareWin)
			...
			local cnfm = guiCreateButton(110,100,80,40,"Definir Tarifa",false,gui_fareWin)
			addEventHandler("onClientGUIClick", cnfm, function()
				local percent = (guiScrollBarGetScrollPosition(scroll)/100)
...
function incomingTaximeterSync(realDistance, realRunning, realPos)
	if not isDriver then
		...
		if realRunning then
			meterRunning = true
			addEventHandler("onClientRender",root,monitoring)
			guiSetText(gui_label_pax_running, "Taxímetro em execução")
			guiLabelSetColor(gui_label_pax_running,0,255,0)
		else
			meterRunning = false
			guiSetText(gui_label_pax_running, "Taxímetro parado")
			guiLabelSetColor(gui_label_pax_running,255,0,0)
		end
	end
end
