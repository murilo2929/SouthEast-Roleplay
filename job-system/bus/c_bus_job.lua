...
function displayBusJob()
	...
	exports.hud:sendBottomNotification(localPlayer, "Motorista de Ônibus", "Aproxime-se do ponto amarelo em seu radar e entre em um ônibus para começar a trabalhar.")
end
...
function startBusJob()
	...
	if (job == 3) then
		...
		if busMarker then
			exports.hud:sendBottomNotification(localPlayer, "Motorista de Ônibus", "Você já iniciou uma rota de ônibus.")
		else
			...
			if vehicle and getVehicleController(vehicle) == getLocalPlayer() and bus[getElementModel(vehicle)] then
				...
				exports.hud:sendBottomNotification(localPlayer, "Motorista de Ônibus", "Siga a rota do ônibus, parando nos pontos de ônibus (marcador vermelho).")
			else
				exports.hud:sendBottomNotification(localPlayer, "Motorista de Ônibus", "Você deve estar em um ônibus para iniciar uma rota.")
			end
		end
	else
		exports.hud:sendBottomNotification(localPlayer, "Notificação", "Você não é um motorista de ônibus. Vá até a Prefeitura para conseguir este emprego.")
	end
end
addCommandHandler("startbus", startBusJob, false, false)
addCommandHandler("iniciarrota", startBusJob, false, false)
	...
	if thePlayer == getLocalPlayer() then
		...
		if vehicle and bus[getElementModel(vehicle)] then
			if curCpType == 3 then
				...
				triggerEvent("hudOverlay:drawOverlayTopRight", thePlayer, { {"Motorista de Ônibus"}, {""}, {"Espere no ponto de ônibus por um momento até que o marcador desapareça."}, }, false, false, false, 5)
			...
			exports.hud:sendBottomNotification(localPlayer, "Motorista de Ônibus", "Você deve estar em um ou ônibus para conduzir a rota.") -- Wrong car type.
		end
	end
end
...
function checkWaitAtStop(thePlayer)
	if thePlayer == getLocalPlayer() then
		if busStopTimer then
			exports.hud:sendBottomNotification(localPlayer, "Motorista de Ônibus", "Você não esperou no ponto de ônibus.")
...
function endOfTheLine()
	if busNextBlip then
		...
		setVehicleLocked(vehicle, false)
		exports.hud:sendBottomNotification(localPlayer, "Motorista de Ônibus", "Fim da linha. Use /startbus para começar a rota novamente.") --if line is finished
	end
end
			...
			if(seat~=0)then
				...
				if driver then -- you can only pay the driver if the bus has a driver
					if not exports.global:hasMoney(getLocalPlayer(), 5)then
						...
						outputChatBox("Você não pode pegar pela tarifa de $5 do ônibus.", 255, 0, 0)
					else
						triggerServerEvent("payBusFare", getLocalPlayer(), driver)
						outputChatBox("Você pagou $5 para andar no ônibus", 0, 255, 0)
					end
				end
			elseif not busMarker and getElementData(getLocalPlayer(), "job") == 3 then
				exports.hud:sendBottomNotification(localPlayer, "Motorista de Ônibus", "Use /iniciarrota para iniciar a rota do ônibus.")
				outputChatBox("Use /iniciarrota para iniciar a rota do ônibus.", 0, 255, 0)
			end
		end
	end
end
...
function startEnterBus(thePlayer, seat)
	if seat == 0 and bus[getElementModel(source)] then
		if getVehicleController(source) then -- if someone try to jack the driver stop him
			cancelEvent()
			if thePlayer == getLocalPlayer() then
				exports.hud:sendBottomNotification(localPlayer, "Notificação", "A porta do motorista está trancada.")
			end
		else
