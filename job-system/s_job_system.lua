...
function onEmploymentServer()
	exports.global:sendLocalText(source, "Jessie Smith diz: Olá, você está procurando um novo emprego?", nil, nil, nil, 10)
	exports.global:sendLocalText(source, " *Jessie Smith entrega uma lista de empregos para " .. getPlayerName(source):gsub("_", " ") .. ".", 226, 184, 255)
end
...
function givePlayerJob(jobID)
	...
	if (jobID==4) then -- CITY MAINTENANCE
		...
		outputChatBox("Use este spray para pintar sobre os graffites que você encontrar.", source, 255, 194, 14)
...
function printJobInfo(thePlayer)
	outputChatBox("~-~-~-~-~-~-~-~-~-~Informação de Carreira~-~-~-~-~-~-~-~-~-~-~", thePlayer, 255, 194, 14)
	outputChatBox("Emprego: "..(getJobTitleFromID(getElementData(thePlayer, "job")) or "Desempregado") , thePlayer, 255, 194, 14)
	outputChatBox("Titulo/Level: "..(tonumber(getElementData(thePlayer, "jobLevel")) or "0") , thePlayer, 255, 194, 14)
	outputChatBox("Progesso: "..(tonumber(getElementData(thePlayer, "jobProgress")) or "0") , thePlayer, 255, 194, 14)
	outputChatBox("~-~-~-~--~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-~-", thePlayer, 255, 194, 14)
end
addCommandHandler("myjob", printJobInfo)
addCommandHandler("meuemprego", printJobInfo)
...
function quitJob(source)
	...
	if logged == 1 then
		...
		if job == 0 then
			outputChatBox("Você está desempregado no momento.", source, 255, 0, 0)
		else
			local charID = getElementData(source, "dbid")
...
addCommandHandler("sairemprego", quitJob, false, false)
...
function startEnterVehJob(thePlayer, seat, jacked) 
	...
	if vjob>0 and job~=vjob and seat == 0 and not (getElementData(thePlayer, "duty_admin") == 1) and not (getElementData(thePlayer, "duty_supporter") == 1) then
		if (vjob==1) then
			outputChatBox("Você não é um motorista de entrega. Visite a prefeitura para conseguir este emprego.", thePlayer, 255, 0, 0)
		elseif (vjob==2) then
			outputChatBox("Você não é um motorista de táxi. Visite a prefeitura para conseguir este emprego.", thePlayer, 255, 0, 0)
		elseif (vjob==3) then
			outputChatBox("Você não é um motorista de ônibus. Visite a prefeitura para conseguir este emprego.", thePlayer, 255, 0, 0)
		end
