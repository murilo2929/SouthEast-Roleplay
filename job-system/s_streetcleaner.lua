...
function initiateCleanerJob(thePlayer)
	...
	outputChatBox("Bem-vindo à equipe, você pode retirar sua van de limpeza de ruas no Depo perto da Prefeitura. ", thePlayer, 255, 194, 14)
	outputChatBox("((Uma blip rosa foi adicionada ao seu mapa - Pressione F11))", thePlayer, 255, 194, 14)
...
function startCleaningMission(thePlayer)
	if (isElement(thePlayer)) then
		...
		outputChatBox("Dirija ao redor de Los Santos em seu caminho de limpeza, varrendo as ruas. ((Atravesse os checkpoints))", thePlayer, 255, 194, 14)
	end
	...
	if ( isPedInVehicle ( thePlayer ) ) then
		...
		if tonumber(vehid) == 574 then
			...
			if tonumber(max_number) == tonumber(m_number) then
				outputChatBox("Tudo bem, isso deve ser o suficiente. Dirija até o depósito de lixo. (( Blip adicionado. Verifique F11 ))", thePlayer, 255, 194, 14)
			...
			outputChatBox( "(( Você deve passar pelos postos de controle no varredor de rua. ))", thePlayer, 255, 194, 14)
		end
	else
		outputChatBox( "(( Você deve passar pelos postos de controle no varredor de rua. ))", thePlayer, 255, 194, 14)
	end
...
function FinalCheckpoints( thePlayer )
	if ( isPedInVehicle ( thePlayer ) ) then
		...
		if tonumber(vehid) == 574 then
			...
			outputChatBox("Dirija até o local de entrega para receber o pagamento. (( Blip adicionado. Verifique F11 ))", thePlayer, 255, 194, 14)
