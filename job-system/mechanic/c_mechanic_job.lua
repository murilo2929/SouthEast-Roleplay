...
function displayMechanicJob()
	outputChatBox("#FF9933Use o #FF0000menu do botão direito#FF9933 para ver os serviços que você pode fornecer.", 255, 194, 15, true)
end
...
function nearTrunk(player)

	for i, veh in ipairs(getElementsByType("vehicle", root, true)) do

		local px, py, pz = getElementPosition(localPlayer)

		local ax, ay, az = getVehicleComponentPosition(veh, "bonnet_dummy", "world")

		if(ax and ay and az and getDistanceBetweenPoints3D(px, py, pz, ax, ay, az) < 2)then
	...
	if factionType == 7 then -- Mechanic Faction / Adams]]
		if not vehicle then
			...
			if not (wMechanic) then
				-- Create the window
				wMechanic = guiCreateWindow(X, Y, Width, Height, "Opções Mecânicas", false )
				...
				if not isPedInVehicle( localPlayer ) and getPedSimplestTask(localPlayer) ~= "TASK_SIMPLE_CAR_DRIVE" then
					...
					if not noTyres[getVehicleType(vehicle)] then
						bMechanicThree = guiCreateButton( 0.05, y, 0.9, 0.1, "Troca de Pneu - $10", true, wMechanic )
					...
					if faction[4] or ( tonumber( getElementData(vehicle, "job") or 0 ) == 0 and ( getElementData(vehicle, "faction") == -1 or faction[getElementData(vehicle, "faction")] ) ) then
						bMechanicFour = guiCreateButton( 0.05, y, 0.9, 0.1, "Repintar - $100", true, wMechanic )
					...
					--[[Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Substituir faróis - $1,250", true, wMechanic )
				...
				-- Add/Remove Tint
				bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Insulfim", true, wMechanic )
				y = y + 0.1
				...
				-- Close
				bMechanicClose = guiCreateButton( 0.05, 0.85, 0.9, 0.1, "Fechar", true, wMechanic )
...
function tyreWindow()
	...
	if not (wTyre) then
		-- Create the window
		wTyre = guiCreateWindow(X+100, Y, Width, Height, "Selecione um pneu para trocar.", false )

		if getVehicleType(currentVehicle) ~= "Bike" then
			-- Front left
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.45, 0.35, "Dianteiro esquerdo", true, wTyre )
			...
			-- Back left
			bTyreTwo = guiCreateButton( 0.05, 0.5, 0.45, 0.35, "Traseiro Esquerdo", true, wTyre )
			...
			-- front right
			bTyreThree = guiCreateButton( 0.5, 0.1, 0.45, 0.35, "Dianteiro Direito", true, wTyre )
			...
			-- back right
			bTyreFour = guiCreateButton( 0.5, 0.5, 0.45, 0.35, "Traseiro Direito", true, wTyre )
			...
			-- Front
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.9, 0.35, "Dianteiro", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreOne, function(button, state)
				if(button == "left" and state == "up") then
			...
			-- back right
			bTyreThree = guiCreateButton( 0.05, 0.5, 0.9, 0.35, "Traseiro", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreThree, function(button, state)
				if(button == "left" and state == "up") then
		...
		-- Close
		bTyreClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Fechar", true, wTyre )
...
function paintColor( colorIndex )
	if exports.colorblender:isPickerOpened("paintjob") then
		outputChatBox("Você ainda está editando outra cor.", 255, 0 ,0)
		return
	end
	local r, g, b = 0,0 ,0
	...
	exports.colorblender:openPicker("paintjob", string.format("#%02X%02X%02X", r, g, b) , "Escolha uma cor para o seu veículo:")
end
...
function paintWindow()
	...
	if not (wPaint) then
		wPaint = guiCreateWindow(windowX, windowY, windowWidth, windowHeight, "Pintura de Veículo", false )
		bPaintColor1 = guiCreateButton( 0.05, 0.1, 0.45, 0.2, "Definir Cor 1", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor1,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 1 )
			end
		end, false)
		bPaintColor2 = guiCreateButton( 0.55, 0.1, 0.45, 0.2, "Definir Cor 2", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor2,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 2 )
			end
		end, false)
		bPaintColor3 = guiCreateButton( 0.05, 0.4, 0.45, 0.2, "Definir Cor 3", true, wPaint )
		addEventHandler( "onClientGUIClick", bPaintColor3,  function(button, state)
			if(button == "left" and state == "up") then
				paintColor( 3 )
			end
		end, false)
		bPaintColor4 = guiCreateButton( 0.55, 0.4, 0.45, 0.2, "Definir Cor 4", true, wPaint )
		...
		bPaintSubmit = guiCreateButton( 0.05, 0.75, 0.90, 0.1, "Salvar", true, wPaint )
		...
		bPaintClose = guiCreateButton( 0.05, 0.85, 0.90, 0.1, "Fechar", true, wPaint )
...
function replaceLightsWindow()
	...
	exports.colorblender:openPicker("headlights", string.format("#%02X%02X%02X", r, g, b) , "Escolha uma cor para as luzes:")
end
...
function paintjobWindow()
	...
	if not (wPaintjob) then
		...
		-- Create the window
		wPaintjob = guiCreateWindow(X+100, Y, Width, Height, "Selecione um novo PaintJob.", false )
		...
		-- Close
		bPaintjobClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Fechar", true, wPaintjob )
...
function serviceTrigger()
	local pertocapo = nearTrunk(localPlayer)
	if (pertocapo) then
		triggerServerEvent( "serviceVehicle", getLocalPlayer(), currentVehicle )
		closeMechanicWindow()
	else
		outputChatBox("Você deve estar perto do capô do veículo.", 255, 0 ,0)
	end
end
...
function somreparo(x,y,z)

	local x,y,z = getElementPosition(localPlayer);
	local soundEffect = playSound3D("mechanic/repair.mp3",x,y,z,false)
	setSoundVolume(soundEffect,0.2)

end
addEvent("somreparo", true)
addEventHandler("somreparo", getRootElement(), somreparo)
