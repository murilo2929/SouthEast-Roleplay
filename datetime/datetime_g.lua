...
function formatTimeInterval( timeInseconds )
	...
	if seconds < 60 then
		return formatTimeString( seconds, "second" ).." atrás", seconds
	end
	
	local minutes = math.floor( seconds / 60 )
	if minutes < 60 then
		return formatTimeString( minutes, "m" ) .. " " .. formatTimeString( seconds - minutes * 60, "s" ).." atrás" , seconds
	end
	
	local hours = math.floor( minutes / 60 )
	if hours < 48 then
		return formatTimeString( hours, "h" ) .. " " .. formatTimeString( minutes - hours * 60, "m" ).." atrás", seconds
	end
	
	local days = math.floor( hours / 24 )
	return formatTimeString( days, "dia(s)" ).." atrás", seconds
end
...
function formatTimeString( time, unit )
	...
	if unit == "dia" or unit == "hora" or unit == "minuto" or unit == "segundo" then
...
function formatSeconds(seconds)
	...
	local minutes = math.floor( seconds / 60 )
	if minutes < 60 then
		return formatTimeString( minutes, "minuto" ) .. " " .. formatTimeString( seconds - minutes * 60, "segundo" )
	end
	
	local hours = math.floor( minutes / 60 )
	if hours < 48 then
		return formatTimeString( hours, "hora" ) .. " " .. formatTimeString( minutes - hours * 60, "minuto" )
	end
	
	local days = math.floor( hours / 24 )
	return formatTimeString( days, "dia" )
end
