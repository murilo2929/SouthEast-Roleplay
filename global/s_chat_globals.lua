...
function changeWarnStyle(thePlayer, commandName)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		...
		if warnStyle == 1 then
			warnStyle = 0
			wrnStyleString = "Estilo alterado para caixa de bate-papo"
		else
			warnStyle = 1
			wrnStyleString = "Estilo alterado para barra lateral"
		end
		...
		if isElement( thePlayer ) then
			if not hideNotFound then
				if #candidates == 0 then
					outputChatBox("Player não encontrado.", thePlayer, 255, 0, 0)
				else
...
function sendLocalMeAction(thePlayer, message, fromPlayer, ignoreDeaths)
	...
	if masked then
		local name = "Mascarado (" ..playerID.. ")"
		local state, affectedPlayers = sendLocalText(thePlayer, (fromPlayer and "" or "").."" ..  string.gsub(name, "_", " ").. ( message:sub( 1, 1 ) == "'" and "" or " " ) .. message, 226, 184, 255, 30, {}, true, ignoreDeaths)
	else
...
function sendLocalDoAction(thePlayer, message, ignoreDeaths)
	...
	if masked then
		local state, affectedPlayers = sendLocalText(thePlayer, "" .. message .. " (( Mascarado (" ..playerID.. ") ))", 226, 184, 255, 30, {}, true, ignoreDeaths)
	else
		local state, affectedPlayers = sendLocalText(thePlayer, "" .. message .. " ((" .. getPlayerName(thePlayer):gsub("_", " ") .. "))", 226, 184, 255, 30, {}, true, ignoreDeaths)
	end
	return state, affectedPlayers
end