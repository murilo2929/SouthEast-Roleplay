...
function isEmail(str)
	if not str or str == "" or string.len(str) < 1 then
		return false, "O e-mail não deve estar vazio."
	end

	local _,nAt = str:gsub('@','@') -- Counts the number of '@' symbol
	
	if nAt ~=1 then 
		return false, "O e-mail deve conter um e apenas um '@'."
	end

	if str:len() > 100 then
		return false, "O e-mail não deve ter mais de 100 caracteres."
	end
	...
	if not localPart or not domainPart then 
		return false, "O local do e-mail ou parte do domínio está faltando." 
	end

	if hasSpecialChars(localPart) then
		return false, "A parte local do email é inválida." 
	end

	if hasSpecialChars(domainPart) then
		return false, "A parte do domínio do email é inválida." 
	end

	return true, "Endereço de email é válido!"
end
