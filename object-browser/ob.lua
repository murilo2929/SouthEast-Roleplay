...
function changeobj ()
	if guiGetVisible (objbrowser) == true then
		...
		outputChatBox("Raio: "..radius,0,255,0)
...
function openobjectsPre()
	if(preloaded==false) then
		outputChatBox("Itens de carregamento pela primeira vez. Isso pode levar VÁRIOS minutos ... SEJA PACIENTE!",255,0,0)
...
function startup ()
	objbrowser = guiCreateWindow (0.7, 0.2, 0.3, 0.8, "Objetos", true)
	objclose = guiCreateButton (0.25, 0.95, 0.5, 0.05, "Fechar", true, objbrowser)
