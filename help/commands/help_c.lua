...
function F1RPhelp( key, keyState )
	...
	if not myWindow then
		...
		myWindow = guiCreateWindow ( 0, 0, 800, 600, "Southeast Roleplay - Painel de Ajuda", false )
		...
		gui.reportWindow.main = guiCreateTab( "Centro de Reporte", tabPanel )
        -- draw the upper part of the tab.
        populateReportCenter()


		local tabCommands = guiCreateTab( "Ajuda de Comandos e Controles", tabPanel )
		...
		if canEditCmds() then
			...
			newCmdBtn = guiCreateButton(0, 0.95, 1 , 0.05, "Criar novo comando",true,tabCommands)
		...
		for category = 1, 7 do
			...
			col[category][4] = guiGridListAddColumn (grid[category], "Explicação", 0.5)
			col[category][5] = guiGridListAddColumn (grid[category], "Permissão", 0.1)
			if canEditCmds() then
...
local function checkReportLength()
	guiSetText(gui.reportWindow.memo_details_desc, "Comprimento " .. string.len(tostring(guiGetText(gui.reportWindow.memo_details)))-1 .. "/150")
...
local function checkNameExists()
	...
	if (count>1) then
		guiSetText(gui.reportWindow.label_player_desc, "Multiplios Encontrados.")
		guiLabelSetColor(gui.reportWindow.label_player_desc, 255, 255, 0)
	elseif (count==1) then
		guiSetText(gui.reportWindow.label_player_desc, "Jogador Encontrado: " .. getPlayerName(found) .. " (ID #" .. getElementData(found, "playerid") .. ")")
		guiLabelSetColor(gui.reportWindow.label_player_desc, 0, 255, 0)
		reportedPlayer = found
	elseif (count==0) then
		guiSetText(gui.reportWindow.label_player_desc, "Jogador não encontrado.")
...
function populateReportCenter( submited )
	...
	if submited then
		local l = guiCreateLabel ( 0, 0, 1, 1, "Obrigado por enviar seu relatório. Aguarde alguns minutos para entrar em contato com você.", true, gui.reportWindow.topPanel )
	...
	elseif pending_report then
		local l = guiCreateLabel ( 0, 0, 1, 1, "Seu reporte ID #" .. (pending_report[8] or "").. " ainda está pendente.\nAguarde ou digite /er para fechar este reporte antes de enviar outro.", true, gui.reportWindow.topPanel )
		guiLabelSetVerticalAlign( l, 'center' )
		guiLabelSetHorizontalAlign( l, 'center' )
	else
		gui.reportWindow.label_type = guiCreateLabel(23, 22, 351, 22, "Sobre o que você está reportando?", false, gui.reportWindow.topPanel)
        guiSetFont(gui.reportWindow.label_type, "default-bold-small")

        gui.reportWindow.combo_type = guiCreateComboBox(23, 44, 351, 27, "Tipo de Reporte", false, gui.reportWindow.topPanel)
        ...
        gui.reportWindow.label_details = guiCreateLabel(23, 81, 351, 22, "Adicione quaisquer detalhes que possam nos ajudar a ajudá-lo:", false, gui.reportWindow.topPanel)
        ...
        gui.reportWindow.label_player = guiCreateLabel(23, 265, 351, 22, "Jogador que você deseja denunciar (opcional):", false, gui.reportWindow.topPanel)
        guiSetFont(gui.reportWindow.label_player, "default-bold-small")

        gui.reportWindow.edit_player = guiCreateEdit(23, 287, 351, 34, "Nome Parcial Jogador / ID", false, gui.reportWindow.topPanel)
...
function openNewCommand(id, perm, cmd, key, ex)
	...
	local lineH2 = lineH
	local col1 = 100
	gui.l1 = guiCreateLabel(margin, margin, w-margin*2, lineH, "CRIAR NOVO COMANDO", false, gui.wNewStation)
	...
	gui.l6 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Permissão:", false, gui.wNewStation)
	...
	gui.l2 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Nome Comando:", false, gui.wNewStation)
	...
	gui.l3 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Hotkey (Se tiver):", false, gui.wNewStation)
	...
	gui.l4 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Explicação:", false, gui.wNewStation)
	...
	gui.bOk = guiCreateButton(margin, margin+lineH/2+lineH2, buttonW , lineH, id and "Salvar" or "Criar",false,gui.wNewStation)
	...
	gui.bDel = guiCreateButton(margin+buttonW, margin+lineH/2+lineH2, buttonW , lineH, "Deletar",false,gui.wNewStation)
