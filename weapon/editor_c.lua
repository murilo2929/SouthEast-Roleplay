...
function openEditor(item)
    ...
    GUIEditor.window[1] = guiCreateWindow(391, 431, 280+wo, 207, "Modificando '"..(item_values[3] or "").."'", false)
    guiWindowSetSizable(GUIEditor.window[1], false)
    exports.global:centerWindow(GUIEditor.window[1])

    GUIEditor.label[1] = guiCreateLabel(16, 32, 62, 24, "Nome:", false, GUIEditor.window[1])
    ...
    GUIEditor.label[2] = guiCreateLabel(16, 62, 62, 24, "Cartucho: ", false, GUIEditor.window[1])
    ...
    GUIEditor.label[3] = guiCreateLabel(16, 92, 62, 24, "Outros:", false, GUIEditor.window[1])
    ...
    if gun_source then
        ...
        local characterName = gun_creator and exports.cache:getCharacterNameFromID(gun_creator) or "Desconhecido"

        if gun_source == 1 then
            buffer = buffer.." - Source: Spawnado por admin.\n"
            buffer = buffer.." - Spawnado by: "..characterName.."\n"
        elseif gun_source == 2 then
            buffer = buffer.." - Source: Arma de Serviço\n"
            buffer = buffer.." - Spawnado para: "..characterName.."\n"
        elseif gun_source == 3 then
            buffer = buffer.." - Source: Ammunation\n"
            buffer = buffer.." - Autorizado sob licença de armas de fogo de: "..characterName.."\n"
        elseif gun_source == 4 then
            buffer = buffer.." - Source: Faction Drop NPC\n"
            buffer = buffer.." - Criado por: "..characterName..")\n"
        else
            buffer = buffer.." - Nenhuma informação pôde ser recuperada.\n"
        end
    end
    GUIEditor.memo[1] = guiCreateMemo(78, 92, 187+wo, 97, buffer, false, GUIEditor.window[1])
    guiMemoSetReadOnly(GUIEditor.memo[1], true)

    GUIEditor.button[1] = guiCreateButton(16, 121, 52, 33, "Salvar", false, GUIEditor.window[1])
    guiSetEnabled(GUIEditor.button[1], false)

    GUIEditor.button[2] = guiCreateButton(16, 156, 52, 33, "Fechar", false, GUIEditor.window[1])
