...
function canPlayerShoot()
	if getElementData(localPlayer, 'restrain') == 1 then
		return false, 'Algemado.'
	end

	if switching then
		return false, 'Mudando..'
	end
	...
	if ammo <= 0 then
		...
		return true, ammo..' munição carregada.'
	end
end
		...
		if source == localPlayer and not weapon_infinite_ammo[weapon] and getElementData(localPlayer, "paintball") ~= 2 then
			...
			if not can then
				if why == 'Sem munição!' then
					...
					outputChatBox(why.." Aperte 'R' para carregar.", 200,200,200)
				else
...
function weaponInteract(item)
	...
	if gun_source == 2 then
		outputChatBox("Você não pode modificar a arma de serviço.", 255, 0, 0)
		playSoundFrontEnd(4)
	else
		closeWeaponInteract()
        GUIEditor.window[1] = guiCreateWindow(677, 403, 177, 105, item_values[3] or "Arma Desconhecida" , false)
        guiWindowSetSizable(GUIEditor.window[1], false)
        exports.global:centerWindow(GUIEditor.window[1])

		GUIEditor.button[1] = guiCreateButton(9, 26, 158, 20, "Descarregar", false, GUIEditor.window[1])
        ...
        GUIEditor.button[3] = guiCreateButton(9, 76, 158, 20, "Fechar", false, GUIEditor.window[1])
        	...
        	elseif source == GUIEditor.button[1] then
				...
				if isGun(id) and not weapon_ammoless[id] then
					...
					if loaded_ammo and loaded_ammo > 0 then
						if reloading then
							playSoundFrontEnd(4)
							outputChatBox("Por favor espere...", 255,0,0)
						else
							reloading = true
							syncAmmo()
        					closeWeaponInteract()
        				...
        				playSoundFrontEnd(4)
						outputChatBox("Sua "..item_values[3].." esta vazia.", 255,0,0)
        			end
        		else
        			playSoundFrontEnd(4)
					outputChatBox("Esta arma não usa munição.", 255,0,0)
        		end
        	end
        end)
