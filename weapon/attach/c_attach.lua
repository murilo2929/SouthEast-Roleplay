...
local bones = {
	"Cabeça", "Pescoço", "Coluna", "Pelvis", "Clavícula esquerda", "Clavícula direita",
	"Ombro esquerdo", "Ombro direito", "Cotovelo esquerdo", "Cotovelo direito",
	"Mão esquerda", "Mão direita", "Quadril esquerdo", "Quadril direito", "Joelho esquerdo", "Joelho direito",
	"Tornozelo esquerdo", "Tornozelo direito", "Pé esquerdo", "Pé direito"
}
...
function createSelectionWindow()
	...
	sWin = guiCreateWindow(sx - 225, sy - 250, 200, 200, "Weapon Attachment", false)
	...
	local weaponName = guiGridListAddColumn(weaponsGridList, "Arma", 0.5)
	...
	local help = guiCreateLabel(12, 155, 200, 10, "Clique duas vezes para alternar.", false, sWin)
	guiSetFont(help, "default-small")

	btnSelectionEdit = guiCreateButton(0, 170, 90, 20, "Editar", false, sWin)
	addEventHandler("onClientGUIClick", btnSelectionEdit, createEditWindow, false)

	btnSelectionClose = guiCreateButton(100, 170, 90, 20, "Fechar", false, sWin)
	...
	for i = 1, 12 do
		...
		if weapon ~= 0 then
			...
			guiGridListSetItemText(weaponsGridList, row, 3, getElementData(localPlayer,
				"attachingWeapon" .. weapon) and "Sim" or "Não", false, false)
		end
	end
end
...
function createEditWindow()
	...
	guiCreateLabel(10, 26, 210, 15,
		"Editando " .. name .. " (" .. id .. ") para skin #" .. skin, false, eWin)

	guiCreateLabel(10, 52, 50, 15, "Bone:", false, eWin)
	bonesComboBox = guiCreateComboBox(65, 50, 150, 150, "Selecioanar", false, eWin)
	...
	btnEditSave = guiCreateButton(0, 295, 67, 20, "Salvar", false, eWin)
	...
	btnEditClose = guiCreateButton(146, 295, 67, 20, "Fechar", false, eWin)
	addEventHandler("onClientGUIClick", btnEditClose, closeEditWindow, false)

	local help = guiCreateLabel(10, 255, 210, 35, "X, Y and Z os deslocamentos devem estar entre -0.5 e 0.5.\n" ..
		"Pressione 'm' a qualquer momento para olhar ao redor.", false, eWin)
	guiSetFont(help, "default-small")
...
function saveWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		outputChatBox("Posição salva de (" .. editingWeapon .. ") " .. getWeaponNameFromID(editingWeapon) ..
			" para skin #" .. editingSkin .. ".", 0, 255, 0)
	end
end
...
function resetWeaponAttachment(button, state)
	if button == "left" and state == "up" then
		...
		outputChatBox("Posição redefinida de (" .. editingWeapon .. ") " .. getWeaponNameFromID(editingWeapon) ..
			" para skin #" .. editingSkin .. " para padrão.", 0, 255, 0)
	end
end
