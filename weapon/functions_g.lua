...
function canRetrieve(player, itemId, itemValue)
	...
	if serial_info and serial_info[2] and tonumber(serial_info[2]) then
		...
		if gun_source == 2 then
			return false , "Esta "..(itemId==115 and gun[3] or 'munição').." é uma arma de serviço. Você não tem permissão para recuperá-lo."
		elseif gun_source == 3 and gun_creator ~= getElementData(player, 'dbid') and (not exports.integration:isPlayerAdmin(player)) then
			return false, "Esta "..(itemId==115 and gun[3] or 'munição').." está autorizado sob "..(exports.cache:getCharacterNameFromID(gun_creator) or "alguem").."'s lei de licença de arma de fogo. Você não tem permissão para recuperá-lo."
		end
	end
	return true
end
