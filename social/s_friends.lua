...
function loadAccountData( accountID )
	if accountID then
		...
		if data then
			friends[ accountID ].name = exports.cache:getUsernameFromId(accountID) or "Desconhecido"
...
function new_addFriend( from, to )
	...
	if fromID and friends[ fromID ] and toID and friends[ toID ] then
		...
		if onFromList then
			outputChatBox( getPlayerName( to ):gsub("_", " ") .. " está na sua lista de amigos como " .. friends[ toID ].name .. ".", from, 255, 194, 14 )
		else
			...
			if onToList then -- the OTHER player has him on his friends list. shouldn't happen, but oh well.
				...
				outputChatBox( getPlayerName( to ):gsub("_", " ") .. " foi adicionado à sua lista de amigos como " .. friends[ toID ].name .. ".", from, 0, 255, 0 )
				...
				if getElementData(to,'social_invite_only') == '1' then
					outputChatBox(getPlayerName( to ):gsub("_", " ").."está ignorando todos os pedidos de amizade recebidos. Você deve ser convidado por ele para ser seu amigo.", from, 255, 0, 0 )
				else
...
function addFriendCmd( thePlayer, commandName, targetPlayer )
   ...
   elseif targetPlayer == thePlayer then
   		return not outputChatBox( "Você não pode se adicionar.", thePlayer, 255, 0, 0 )
   end
   for k, v in ipairs({fromID, toID}) do
      if not friends[ v ] then
         friends[ v ] = { }
   ...
   if fromID and friends[ fromID ] and toID and friends[ toID ] then
      ...
      if onFromList then
         outputChatBox( getPlayerName( targetPlayer ):gsub("_", " ") .. " está na sua lista de amigos como " .. friends[ toID ].name .. ".", thePlayer, 255, 194, 14 )
      else
         local onToList = false
         for k, v in ipairs( friends[ toID ] ) do
            if v == fromID then
         ...
         if onToList then -- the OTHER player has him on his friends list. shouldn't happen, but oh well.
            mysql:query_free("INSERT INTO friends VALUES(" .. mysql:escape_string(fromID) .. ", " .. mysql:escape_string(toID) .. ")" )
            outputChatBox( getPlayerName( targetPlayer ):gsub("_", " ") .. " foi adicionado à sua lista de amigos como " .. friends[ toID ].name .. ".", thePlayer, 0, 255, 0 )
            table.insert( friends[ fromID ], toID )
            sendFriends( thePlayer, fromID )
         else
            -- need permissiosn first
            if getElementData(targetPlayer,'social_invite_only') == '1' then
            	outputChatBox(getPlayerName( targetPlayer ):gsub("_", " ").." está ignorando todos os pedidos de amizade recebidos. Você deve ser convidado por ele para ser seu amigo.", thePlayer, 255, 0, 0 )
            else
...
addCommandHandler("adicionaramigo", addFriendCmd)
...
addEventHandler( "social:acceptFriend", root,
	function( )
		...
		if not from or not to then
			...
			sendFriends( to, toID )
			outputChatBox( getPlayerName( to ):gsub("_", " ") .. " foi adicionado à sua lista de amigos como " .. friends[ toID ].name .. ".", from, 0, 255, 0 )
			outputChatBox( getPlayerName( from ):gsub("_", " ") .. " foi adicionado à sua lista de amigos como " .. friends[ fromID ].name .. ".", to, 0, 255, 0 )
		end
	end
)
