...
function createLookWindow( name, description, attributes, picture, editFunction )
	...
	local column = guiGridListAddColumn( grid, "Objetos sendo usados (Ex: Distintivos, Capacetes, Ski Mask)", 0.95)
	...
	if editFunction then
		...
		local edit = guiCreateButton( x, 360, 130, 25, "Editar", false, advlook_window )
		addEventHandler( "onClientGUIClick", edit, editFunction, false )
		
		x = x + 140
	end
	local close = guiCreateButton( x, 360, 130, 25, "Fechar", false, advlook_window )
		...
		addEventHandler("onClientGUIClick", browser, 
			function()
				if largePictureWindow then
					...
					largePictureWindow = guiCreateWindow(x, y, width, height, "Foto de "..tostring(name), false)
					local close2 = guiCreateButton((width/2)-(130/2), height-30, 130, 25, "Fechar", false, largePictureWindow )
...
addEventHandler( "social:look", getRootElement( ),
	function( age, race, gender, weight, height, description )
		...
		local editButton = createLookWindow( currentLookData["name"],
			currentLookData["name"] .. " is a " .. ( race == 0 and "negro" or race == 1 and "branco" or "asiatico" ) .. " " .. ( gender == 1 and "mulher" or "homem" ) ..
			" com um peso de " .. math.floor(weight * 2.2 + 0.5) .. " lb (" .. weight .. "kg)" ..
			" e uma altura de " .. height .. "cm.\n\n" ..
			"Cor de Cabelo: " .. description[1] .. "\n" ..
			"Penteado: " .. description[2] .. "\n" ..
			"Características faciais: " .. description[3] .. "\n" ..
			"Características físicas: " .. description[4] .. "\n" ..
			"Roupa: " .. description[5] .. "\n" ..
			"Acessorios: " .. description[6],
			description[8] or {},
			description[7],
			editFunction
		)
...
function createEditablesWindow( )
	...
	advlook_edit_window = guiCreateWindow( screen_x - 191, 172, 181, 297, "Edite seus looks!", false )
	...
	local column = guiGridListAddColumn( grid, "Propriedades", 0.9 )
	...
	local close = guiCreateButton( 12, 266, 157, 22, "Fechar", false, advlook_edit_window )
...
function createEditValueWindow( key )
	...
	advlook_editvalue_window = guiCreateWindow( 349, 300, 285, 105, "Edite seus looks - " .. stuff.name, false )
	...
	guiCreateLabel(11, 26, 258, 16, "Agora você está editando seu " .. stuff.name .. ".", false, advlook_editvalue_window)
