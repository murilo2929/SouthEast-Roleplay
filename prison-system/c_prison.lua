...
function PrisonGUIF(result)
        ...
        PrisonGUI.window[1] = guiCreateWindow(x, y, w, h, "Sistema Prisional", false)
        ...
        PrisonGUI.column[1] = guiGridListAddColumn(PrisonGUI.gridlist[1], "ID Unico", 0.1)
        PrisonGUI.column[7] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Cela", 0.05)
        PrisonGUI.column[2] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Nome", 0.2)
        PrisonGUI.column[3] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Data de Condenação", 0.15)
        PrisonGUI.column[4] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Solto em dias", 0.1)
        PrisonGUI.column[8] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Solto em horas", 0.1)
        PrisonGUI.column[10] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Minutos online", 0.08)
        PrisonGUI.column[5] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Última atualização por", 0.1)
        PrisonGUI.column[9] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Fiança", 0.1)
        PrisonGUI.column[6] = guiGridListAddColumn(PrisonGUI.gridlist[1], "Motivo", 0.5)

        PrisonGUI.button[1] = guiCreateButton(13, 241, 100, 73, "Fechar", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[1], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[2] = guiCreateButton(129, 243, 119, 30, "Soltar prisioneiro", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[2], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[3] = guiCreateButton(130, 280, 119, 30, "Adicionar prisioneiro", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[3], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[4] = guiCreateButton(263, 243, 119, 30, "Atualizar prisioneiro", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[4], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[5] = guiCreateButton(588, 242, 134, 30, "Abri/Fech portão numero", false, PrisonGUI.window[1])
        guiSetProperty(PrisonGUI.button[5], "NormalTextColour", "FFAAAAAA")
        PrisonGUI.button[6] = guiCreateButton(588, 282, 134, 30, "Abri/Fech todos os portãos", false, PrisonGUI.window[1])
        ...
        if pd_offline_jail or exports.integration:isPlayerTrialAdmin(getLocalPlayer()) then
            PrisonGUI.button[7] = guiCreateButton(263, 280, 119, 30, "Adicionar prisioneiro offline", false, PrisonGUI.window[1])
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[2], function()
            ...
            if r>=0 and c>=0 then
                ...
                if not isCloseTo(getLocalPlayer(), targetPlayer) then
                    outputChatBox("Você deve estar perto deste prisioneiro para removê-los.", 255, 0, 0)
                    return
                end
                triggerServerEvent("removePrisoner", resourceRoot, removeid, true)
            else
                outputChatBox("Faça uma seleção primeiro.", 255, 0, 0)
            end
            end, false)
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[4], function()
            ...
            if r>=0 and c>=0 then
                local targetPlayer = getPlayerFromName(guiGridListGetItemText(PrisonGUI.gridlist[1], r, 3))
                if not isCloseTo(getLocalPlayer(), targetPlayer) then
                    outputChatBox("Você deve estar perto deste prisioneiro para atualizá-los.", 255, 0, 0)
                    return
                end
        ...
        addEventHandler("onClientGUIClick", PrisonGUI.button[6], function() -- Toggle all gates
                if getElementDimension(getLocalPlayer(  )) == gateDim then
                    ...
                    gui["_root"] = guiCreateWindow(left, top, windowWidth, windowHeight, "Abrir/Fechar todos os portões", false)
                    guiWindowSetSizable(gui["_root"], false)

                    gui["label"] = guiCreateLabel(10, 35, 231, 56, "Tem certeza de que deseja alternar todos os\n portões da prisão?", false, gui["_root"])
                    guiLabelSetHorizontalAlign(gui["label"], "center", false)
                    guiLabelSetVerticalAlign(gui["label"], "center")

                    gui["pushButton"] = guiCreateButton(30, 95, 81, 31, "Sim", false, gui["_root"])
                    ...
                    gui["pushButton_2"] = guiCreateButton(150, 95, 81, 31, "Não", false, gui["_root"])
                        addEventHandler("onClientGUIClick", gui["pushButton_2"], function()
                            destroyElement(gui["_root"])
                            local gui = nil
                        end, false)
                else
                    outputChatBox("Você deve estar no mesmo interior / dimensão dos portões.", 255, 0, 0)
                end
            end, false)
...
function addPrisonerGUI(source, name, cell, days, hours, charges, row, fines, jailID)
    ...
    if arrestLocation then
        ...
        newPrisonerGUI.window[1] = guiCreateWindow(x, y, w, h, "Adicionar prisioneiro", false)
        guiWindowSetSizable(newPrisonerGUI.window[1], false)

        newPrisonerGUI.button[1] = guiCreateButton(13, 241, 123, 28, "Cancelar", false, newPrisonerGUI.window[1])
        guiSetProperty(newPrisonerGUI.button[1], "NormalTextColour", "FFAAAAAA")
        newPrisonerGUI.button[2] = guiCreateButton(170, 241, 123, 28, "Adicionar prisioneiro", false, newPrisonerGUI.window[1])
        ...
        if source==PrisonGUI.button[3] then
            newPrisonerGUI.label[1] = guiCreateLabel(13, 22, 67, 15, "Nome/ID:", false, newPrisonerGUI.window[1])
            guiSetFont(newPrisonerGUI.label[1], "default-bold-small")
            newPrisonerGUI.label[4] = guiCreateLabel(226, 25, 72, 17, "Não encontrado", false, newPrisonerGUI.window[1])
        ...
        elseif source==PrisonGUI.button[7] then
            newPrisonerGUI.label[1] = guiCreateLabel(15, 22, 67, 15, "Nome Exato:", false, newPrisonerGUI.window[1])
            guiSetFont(newPrisonerGUI.label[1], "default-bold-small")

            guiSetEnabled(newPrisonerGUI.edit[5], false)
        end

        newPrisonerGUI.label[2] = guiCreateLabel(12, 147, 77, 15, "Motivo:", false, newPrisonerGUI.window[1])
        ...
        newPrisonerGUI.label[3] = guiCreateLabel(14, 48, 30, 15, "Cela:", false, newPrisonerGUI.window[1])
        ...
        newPrisonerGUI.label[8] = guiCreateLabel(120, 48, 30, 15, "Fiança:", false, newPrisonerGUI.window[1])
        ...
        if name then
            ...
            guiSetText(newPrisonerGUI.window[1], "Atualizar Prisioneiro") -- All about the cosmedics baby
            guiSetText(newPrisonerGUI.button[2], "Atualizar Prisioneiro")
        ...
        addEventHandler("onClientGUIClick", newPrisonerGUI.button[2], function()
            if overLimit(guiGetText(newPrisonerGUI.edit[3]), guiGetText(newPrisonerGUI.edit[4])) then -- Returns true if over hour limit
                outputChatBox("Você está tentando prender alguém por causa do "..tonumber(hourLimit).." limite de horas.", 255, 0, 0) return end

            local item = guiComboBoxGetSelected(newPrisonerGUI.edit[2])
            if item==-1 then
                outputChatBox("Faça uma seleção de células primeiro.", 255, 0, 0) return end
            ...
            if string.len(guiGetText(newPrisonerGUI.edit[1])) > 0 then
                ...
                if source==PrisonGUI.button[3] then -- Online add
                    if tonumber(guiGetText(newPrisonerGUI.edit[5])) <= 100000 then
                        --if tonumber(guiGetText(newPrisonerGUI.edit[4]))+tonumber(guiGetText(newPrisonerGUI.edit[3])) >= 1 then
                            ...
                            true)
                        --else
                            --outputChatBox("Insira um valor de condenação maior.", 255, 0, 0)
                        --end
                    else
                        outputChatBox("O valor máximo da multa é $100,000.", 255, 0, 0)
                    end
                ...
                outputChatBox("Você digitou um nome??", 255, 0, 0)
            end
        end, false)
        ...
        outputChatBox("Você deve estar em uma área de prisão.", 255, 0, 0)
    end
end
...
function toggleGate()
    if isElement(ToggleGateGUI.window[1]) then
        ...
        local w, h = 272, 200
        local x, y = (scr[1]/2)-(w/2), (scr[2]/2)-(h/2)
        ToggleGateGUI.window[1] = guiCreateWindow(x, y, w, h, "Abrir/Fechar um Portão", false)
        ...
        ToggleGateGUI.button[1] = guiCreateButton(11, 54, 120, 37, "Abrir/Fechar Portão", false, ToggleGateGUI.window[1])
        guiSetProperty(ToggleGateGUI.button[1], "NormalTextColour", "FFAAAAAA")
        ToggleGateGUI.button[2] = guiCreateButton(136, 54, 120, 37, "Cancelar", false, ToggleGateGUI.window[1])
        ...
        ToggleGateGUI.label[1] = guiCreateLabel(183, 120, 45, 20, "Aberto", false, ToggleGateGUI.window[1])
        ToggleGateGUI.label[2] = guiCreateLabel(183, 140, 40, 20, "Fechado", false, ToggleGateGUI.window[1])
        ...
        addEventHandler("onClientGUIClick", ToggleGateGUI.button[1], function()
            ...
            if item>-1 then
                ...
                outputChatBox("Selecione um portão primeiro.", 255, 0, 0)
            end
        end, false)
...
function checkNameExists()
    ...
    if (count>1) then
        guiSetText(newPrisonerGUI.label[4], "Múltiplos encontrados.")
    ...
    elseif (count==0) then
        guiSetText(newPrisonerGUI.label[4], "Não encontrado.")
        guiLabelSetColor(newPrisonerGUI.label[4], 255, 0, 0)
        guiSetEnabled(newPrisonerGUI.button[2], false)
    end
end
...
function updateGates()
    ...
    PrisonGUI.label[1] = guiCreateLabel(750, 287, 100, 30, open.." Portões Abertos", false, PrisonGUI.window[1])
    PrisonGUI.label[2] = guiCreateLabel(750, 247, 100, 30, closed.." Portões Fechados", false, PrisonGUI.window[1])
