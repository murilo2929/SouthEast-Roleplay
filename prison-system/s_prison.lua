...
addCommandHandler("prender", startGUI)
	...
	function( removeid, fromGUI )
			...
			if result then
				if not fromGUI then
					...
					sendPrisonMsg(getPlayerName(client).." removeu "..t[row][3].." da cadeia.")
					local players = exports.pool:getPoolElementsByType("player")
					for key, value in ipairs(players) do
						if getElementData(value, "dbid") == tonumber(t[row][2]) then
							outputChatBox("Você foi removido da prisão por ".. string.gsub(getPlayerName(client), "_", " "), value, 0, 255, 0)
                ...
                outputChatBox("Error, PS#01.", client, 255, 0, 0)
			end
		end
)
	...
	function( name, cell, days, hours, charges, fine, online )
		...
		if online then
			...
			if not arrestLocation then
				outputChatBox("O jogador alvo deve estar dentro da área de processamento.", client, 255, 0, 0)
				return
			elseif not (exports.factions:isInFactionType(client, 2) or exports.factions:isPlayerInFaction(client, 81)) or duty <= 0 then
				outputChatBox("Para prender um jogador online, você deve estar na facção legal e em serviço.", client, 255, 0, 0)
				return
			end
		else
            local arrestLocation = isInArrestColshape(client)
			if not arrestLocation then
				outputChatBox("Você deve estar dentro da área de processamento para adicionar um prisioneiro.", client, 255, 0, 0)
			end
		end

		if duplicateCheck(returnWhat(name, online)) then
			outputChatBox("Este jogador já está cumprindo uma pena, use atualizar prisioneiro em vez disso.", client, 255, 0, 0)
			return
		end
        ...
        if cells[cell].type == 1 then
            ...
            jailTime = ( r.timestamp + (tonumber(hours) + days) * 1 * 60  )
            onlineTime = 0
        end
		...
		if query then
			...
			if row then
				...
				if online then
					...
					triggerEvent( 'mdc-system:add_crime', client, charID, getPlayerName( name ), charges, 'Fiança: ' .. fine .. ' & Prisão: ' .. ( days + hours ) .. ' horas.', officer )

					triggerClientEvent(client, "PrisonGUI:Close", client, t) -- close the prison window as it will open MDC window
					-- END MDC CRAP --

					outputChatBox("Você foi colocado na prisão por "..string.gsub(getPlayerName(client), "_", " ")..".", name, 0, 255, 0)
					sendPrisonMsg(getPlayerName(client).." adicionou "..getPlayerName(name).." na cela: "..row.cell)
					-- FINE
					if tonumber(fine)>0 then -- Issue Fine
						...
						outputChatBox("Fiança de $"..amount.." emitida.", name, 0, 255, 0)
					end
					-- RESTRAINTS/WEAP
					...
					outputChatBox("[AVISo] Nenhum prisioneiro foi adicionado ao MDC automaticamente. Arquive isso manualmente.", client, 255, 0, 0)
				end
				mysql:free_result(result)
			else
				outputChatBox("Error, PS#02.", client, 255, 0, 0)
			end
		else
			outputChatBox("Nenhum personagem encontrado com esse nome.", client, 255, 0, 0)
		end
	end
)
	...
	function( name, cell, days, hours, charges, removeid, online )
		...
		if query then
			...
			if row then
				...
				if online then
					outputChatBox("Os detalhes do seu prisioneiro foram atualizados.", name, 0, 255, 0)
					activateJail(row.charid, name)
				end
			else
				outputChatBox("Error, PS#02", client, 255, 0, 0)
	...
	for key, value in ipairs(t) do
		if tonumber(value[2]) == tonumber(getElementData(client, "dbid")) then
    		...
    		if remainingtime<=0 and tonumber(onlineMinutes)<=0 then
    			...
    			assignSkin(client)
				outputChatBox("Seu tempo foi cumprido!", client, 0, 255, 0)
				return
			else
                ...
                if firstLogin then
                    outputChatBox("Você está atualmente na prisão PD. /tempocadeia para revisar sua sentença.", client, 255, 0, 0)
...
function timerCheck(thePlayer)
    ...
    if onlineMinutes and onlineMinutes > 0 then
        ...
        elseif not told[thePlayer] then
            outputChatBox("Você foi sinalizado como ausente do teclado, seu cronômetro online não está sendo reduzido.", thePlayer, 255, 0, 0)
            told[thePlayer] = true
        end
    elseif onlineMinutes and onlineMinutes == 0 then
        ...
        local remainingMinutes = (remainingtime / 60) -- Reduce by 2 minutes instead of 1
...
addCommandHandler("tempocadeia", function(thePlayer)
		local days, hours, remainingtime = cleanMath(getElementData(thePlayer, "jail_time"))
        local onlineMinutes = getElementData(thePlayer, "jail_time_online")
		if not remainingtime or not onlineMinutes then
			outputChatBox("Você não está cumprindo uma sentença de prisão.", thePlayer, 255, 0, 0)
		elseif remainingtime<=0 and tonumber(onlineMinutes)<=0 then
			for key, value in ipairs(t) do
				if tonumber(value[2]) == tonumber(getElementData(thePlayer, "dbid")) then
				    ...
				    outputChatBox("Seu tempo foi cumprido!", thePlayer, 0, 255, 0)
				end
			end
		else
			if tonumber(hours) and tonumber(days) and tonumber(onlineMinutes) then
				if cells[getElementData(thePlayer, "jail:cell")].type == 0 then
					...
					if tonumber(hours) < 1 and tonumber(days) <= 0 then
						...
						outputChatBox("Você tem atualmente ".. minutes .. " minutos restantes em sua sentença. Você é prisioneiro ID"..getElementData(thePlayer, "jail:id")..", na cela "..getElementData(thePlayer, "jail:cell"), thePlayer, 255, 255, 0)
					else
						outputChatBox("Você tem atualmente ".. days .. " dias e " .. hours .. " horas restantes em sua sentença. Você é prisioneiro ID ".. getElementData(thePlayer, "jail:id")..", na cela "..getElementData(thePlayer, "jail:cell"), thePlayer, 255, 255, 0)
					end
				else
					-- Show a mixture of online and offline time.
					if tonumber(hours) < 1 and tonumber(days) <= 0 then
						local minutes = ("%.1f"):format(remainingtime/60)
						outputChatBox("Você tem atualmente ".. minutes .. " minutos offline e ".. onlineMinutes .." online em sua sentença. Você é prisioneiro ID"..getElementData(thePlayer, "jail:id")..", na cela "..getElementData(thePlayer, "jail:cell"), thePlayer, 255, 255, 0)
					else
						outputChatBox("Você tem atualmente ".. days .. " dias offline, " .. hours .. " horas offline e ".. onlineMinutes .." online minutos restantes em sua sentença. Você é prisioneiro ID".. getElementData(thePlayer, "jail:id")..", na cela "..getElementData(thePlayer, "jail:cell"), thePlayer, 255, 255, 0)
					end
					if tonumber(onlineMinutes) <= 0 then
						outputChatBox("O seu tempo offline está diminuindo em dobro porque seus minutos online já estão servidos!", thePlayer, 255, 255, 0)
					end
				end
			elseif tostring(days) ~= "Awaiting" then
				outputChatBox("Você está cumprindo uma sentença de prisão perpétua. Você é prisioneiro ID "..getElementData(thePlayer, "jail:id")..", na cela "..getElementData(thePlayer, "jail:cell"), thePlayer, 255, 255, 0)
            elseif tostring(hours) == "Online Time" then
                outputChatBox("Você tem atualmente ".. onlineMinutes .." online minutos restantes em sua sentença. Você é prisioneiro ID"..getElementData(thePlayer, "jail:id")..", na cela "..getElementData(thePlayer, "jail:cell"), thePlayer, 255, 255, 0)
            end
		end
end )
		...
		if getElementData(value, "loggedin")==1 then
		    for _,res in ipairs(t) do
			    if tonumber(res[2]) == tonumber(getElementData(value, "dbid")) then
    			    ...
    			    if remainingtime<=0 and tonumber(onlineMinutes)<=0 then
    				    outputDebugString("PRISÂO: Timer removido " .. string.gsub(tostring(res[3]), "_", " ") .. " from jail.")
    				    outputChatBox("Seu tempo foi cumprido!", value, 0, 255, 0)
...
function processSpeakerMessage(thePlayer, commandName, ...)
	if not (...) then
		outputChatBox("SYNTAX: /"..commandName.." [mensagem] - Envia uma mensagem semelhante a um orador pela prisão.", thePlayer)
		outputChatBox("Você deve estar na prisão ou no pátio. Apenas uso de SASD.", thePlayer)
	else
			...
			for k, v in ipairs(exports.pool:getPoolElementsByType("player")) do
				...
				if arrayInt == speakerInt and speakerDimensions[arrayDim] then -- speakerInt and speakerDimensions are set in g_ config file
					if exports.factions:isPlayerInFaction(v, 1) then
						outputChatBox("PRISÂO INTERCOM ("..string.gsub(getPlayerName(thePlayer), "_", " ")..") o< "..message, v, 218, 165, 32)
					else
						outputChatBox("PRISÂO INTERCOM o< "..message, v, 218, 165, 32)
					end
