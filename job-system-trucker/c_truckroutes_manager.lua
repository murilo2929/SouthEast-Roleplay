...
function clearAllMarkers()
	if exports.integration:isPlayerTrialAdmin(getLocalPlayer()) then
		...
		bufferRoutes = nil
		outputChatBox("Limpado "..count.." marcadores de caminhão.")
	end
end
...
function showRouteInfo(player, dim)
	for i = 1, #staticMarkers1 do 
		if staticMarkers1[i] and (player == getLocalPlayer() and isElementWithinMarker ( player, staticMarkers1[i] ) )then
			outputChatBox("     Pedido ID #"..bufferRoutes[i][7].." - "..(bufferRoutes[i][6] or "Desconhecido").." (Int ID#"..bufferRoutes[i][8]..", "..(bufferRoutes[i][4] or "0").." kg)")
...
function delTruckerMarker( cm, id) 
	...
	if not exports.integration:isPlayerScripter(sourcePlayer) then
		outputChatBox("Somente administradores completos e superiores podem acessar /"..cm..".", 255,0 ,0 )
		return false  
	end     
	...
	if not markerIndex or not markerID then
		outputChatBox("Você não está no marcador. Use /showAllTruckMarkers primeiro.", 255, 0 ,0)
		return false
	end
