...
function addOrder(int, supplies, x, y, z, name)
	if int and hasOrder(int) then
		return false, "Você já tinha um pedido pendente. Aguarde até que seja entregue para colocar um novo."
	else
...
function checkActiveRoutes(thePlayer, commandName)
	if (exports.integration:isPlayerScripter(thePlayer)) then
		local count = 0
		outputChatBox("Todas as Rotas Ativas:", thePlayer)
		for i = 1, #routes do
			if routes[i] and routes[i][5] then
				outputChatBox("    "..getPlayerName(routes[i][5]):gsub("_", " ").." está trabalhando na rota #"..i.." - "..(routes[i][6] or "Desconhecido").." ("..(routes[i][4] or "0").." kg)", thePlayer)
				count = count + 1
			end
		end
		outputChatBox(count.." Rotas ativas.", thePlayer)
	else
		outputChatBox("Apenas Admin completo e superior podem acessar /"..commandName..".", thePlayer, 255,0,0)
	end
end
addCommandHandler("checkactiveroutes", checkActiveRoutes, false, false)
addCommandHandler("rotasativas", checkActiveRoutes, false, false)

function showActualOrders(thePlayer, commandName)
	if (exports.integration:isPlayerScripter(thePlayer)) then
		local count = 0
		local tempRoutes = {}
		outputChatBox("[TRUCKER] Todos os Pedidos Atuais:", thePlayer)
		for i = 1, #routes do
			if routes[i] and tonumber(routes[i][8]) and (tonumber(routes[i][8]) > 0) then
				table.insert(tempRoutes, routes[i])
				outputChatBox("     Order ID #"..routes[i][7].." - "..(routes[i][6] or "Desconhecido").." (Int ID#"..routes[i][8]..", "..(routes[i][4] or "0").." kg)", thePlayer)
				if debugmode then
					outputDebugString("     Order ID #"..routes[i][7].." - "..(routes[i][6] or "Desconhecido").." (Int ID#"..routes[i][8]..", "..(routes[i][4] or "0").." kg)")
				end
				count = count + 1
			end
		end
		outputChatBox(count.." pedidos atual", thePlayer)
		if debugmode then
		...
		outputChatBox("Apenas Administradores Plenos podem acessar /"..commandName..".", thePlayer, 255,0,0)
	end
end
addCommandHandler("showActualOrders", showActualOrders, false, false)
addCommandHandler("verrotasatuais", showActualOrders, false, false)
...
function showAllTruckMarkers(thePlayer, commandName, ...)
	if (exports.integration:isPlayerScripter(thePlayer)) then
		...
		outputChatBox("[TRUCKER] Todos os marcadores de caminhão:", thePlayer)
		for i = 1, #routes do
			if routes[i] then
				table.insert(tempRoutes, routes[i])
...
addCommandHandler("mostrarmarkerscaminhoneiro", showAllTruckMarkers, false, false)
...
addCommandHandler("criarrotacaminhoneiro", addOrderManually, false, false)
...
function selectAFreeSpot(thePlayer)
	...
	if route[8] and tonumber(route[8]) and tonumber(route[8]) > 0 then -- if it is an actual order from other players
		isActualOrder = "Atual (Para interior ID#"..route[8]..")"
		-- notify truckers
		local jobLevel = getElementData(thePlayer, 'jobLevel')
		notifyTruckers("RS Haul Operador: Pedido #"..routes[rand][7].." - Entrega para "..routes[rand][6].."foi atribuído a Trucker "..jobLevel.." - "..exports.global:getPlayerName(thePlayer)..".", true)
	else
		...
		isActualOrder = "Simulado (Suprimentos: "..routes[rand][4]..")"
	end
