...
function spawnRoute(thePlayer, spawnNewRoute)
	...
	if currentSpot and not spawnNewRoute then
		...
		if not selectedRoute then
			exports.hud:sendBottomNotification(thePlayer, "Operador de transporte RS:", "Não há pedidos do cliente para fazer no momento, aguarde...")
		else
...
function giveTruckingMoney(vehicle)
	...
	if takenWeight then
		exports.hud:sendBottomNotification(source, "Operador de transporte RS:", "RS Haul descarregou"..takenWeight.." Kg(s) de suprimentos restantes. ")
	end
	-- level up and reset runs/wage
	local vehicle = getPedOccupiedVehicle(source)
	if getElementData(vehicle, "job") ~= 1 then
		exports.hud:sendBottomNotification(source, "Operador de transporte RS:", "Cara.. você tem que usar o veículo RS Haul.")
	else
			...
			if truckruns > 0 then
				if truckrunsTilNextLevel then
					...
					if truckrunCarry >= 0 then -- level up
						...
						local info = {
							{string.upper("Nova conquista de trabalho de entrega!"), 255,194,14,255,1,"default-bold"},
							{""},
							{"Parabéns! Você acabou de obter um novo nível de certificado de motorista de entrega "..tostring(currentJobLevel+1)..".", 0,255,0,255,1,"default"},
						}
				...
				if not notified then
					if not truckrunsTilNextLevel then
						exports.hud:sendBottomNotification(source, "Nova Conquista do Trabalho de Entrega!", "Progresso: "..(getElementData(source, "jobProgress") or 0).." corridas de caminhão (você dominou este trabalho).")
					else
						exports.hud:sendBottomNotification(source, "Nova Conquista do Trabalho de Entrega!", "Progresso: "..math.floor((getElementData(source, "jobProgress") or 0)/truckrunsTilNextLevel*100).."%")
					end
				end
				playSoundFX(vehicle)
			end
	...
	if getElementData(source,"job") == 1 and truckerJobVehicleInfo[truckModel] then
		...
		if playerJobLevel < truckLevelRequire then
			...
			if truckLevelRequire == 1 then
				exports.hud:sendBottomNotification(thePlayer, "Operador de transporte RS:", "Você não é um funcionário RS Haul, inscreva-se para este trabalho na Prefeitura.")
			else
				exports.hud:sendBottomNotification(thePlayer, "Operador de transporte RS:", "Você precisa do nível de motorista de entrega "..truckLevelRequire.." para conduzir esse "..truckName..".")
			end
...
function checkTruckingEnterVehicle(thePlayer, seat)
	if seat == 0 and getElementData(source,"job") == 1 and getElementData(thePlayer,"job") == 1 then
		...
		if curentCrates <= 0 then
			exports.hud:sendBottomNotification(thePlayer, "RS Haul Operator: Seu caminhão está vazio!", "Retorne à estação RS Haul para recarregar seu caminhão com caixas de suprimentos!")
		else
			exports.hud:sendBottomNotification(thePlayer, "Operador de transporte RS:", "Seu caminhão tem "..exports.global:formatWeight(curentCrates).." de suprimentos nas costas. Entregue-os nas instalações de nossos clientes!")
...
function startLoadingUp()
	local vehicle = getPedOccupiedVehicle(source)
	if not vehicle then
		exports.hud:sendBottomNotification(source, "Operador de transporte RS:", "Cara, cadê o seu caminhão?")
		return false
	end
	if getElementData(vehicle, "job") ~= 1 then
		exports.hud:sendBottomNotification(source, "Operador de transporte RS:", "Cara .. você tem que usar o veículo RS Haul.")
	...
	if truck then
		...
		if crateWeight > 0 then
			...
			exports.hud:sendBottomNotification(source, "Operador de transporte RS:", "Seu caminhão está cheio e não pode carregar mais suprimentos! Dirija até o marker amarelo para concluir as entregas.")
			triggerClientEvent(source, "job-system:trucker:leaveStationLoadup", source, true)
		end
...
function checkIfPlayerTruckHasEnoughtShit()
	...
	local truck = truckerJobVehicleInfo[truckModel]
	if not truck then
		exports.hud:sendBottomNotification(source, "Operador de transporte RS:", "Man..RS Haul não permite este modelo de veículo.")
		return false
	end
	...
	if success then
		...
		exports.hud:sendBottomNotification(source, "Operador de transporte RS:", "O cliente pagou a você $"..formartedEarned.." por entregar "..exports.global:formatWeight(droppedWeight).." de suprimentos a uma distância de "..exports.global:formatLength(distance)..".")
	else
		exports.hud:sendBottomNotification(source, "Operador de transporte RS:", marker.." Retorne ao armazém da RS Haul primeiro.")
	end
...
function unloadCrates(thePlayer, veh)
	if getVehicleOccupant(veh) == thePlayer then
		...
		local truck = truckerJobVehicleInfo[truckModel]
		if not truck then
			return false, "RS Haul não permite este modelo de veículo."
		end
		local currentSpot = handledSpot(thePlayer)
		if currentSpot then
			...
			if whatPlayerHas > 0 then
				...
				return false, "Seu caminhão está vazio."
			end
		else
			return false, "((Script Error Code 407))"
		end
	else
		return false, "Você não pode usar o veículo de outras pessoas."
	end
...
addEventHandler("onResourceStart", resourceRoot, function ()
	...
	for key, thePlayer in pairs(getTruckers()) do
		exports.hud:sendBottomNotification(thePlayer, "Atualização do script de trabalho de entrega", "Um desenvolvedor atualizou o sistema de trabalho do caminhoneiro. Por favor, entre novamente no caminhão se você estiver em um.")
