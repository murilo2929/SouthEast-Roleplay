...
local function updateGUI(data)
	if show then	
		...
		if job == 1 and jobVeh == 1 then -- RS Haul
			...
			jobLevel = "Certificado: Nível de Motorista "..tempJobLevel
			if truckrunsTilNextLevel then
				jobProgress = "Progresso: "..tempProgress.."/"..truckrunsTilNextLevel.." ("..math.floor(tempProgress/truckrunsTilNextLevel*100).."%)"
			else
				jobProgress = "Progresso: "..tempProgress.." (Você dominou este trabalho)"
			end
			jobCurrentProgress = "Transportes: "..tempTruckRuns
			...
			if veh and jobVeh == 1 then
				...
				truckCap = "Capacidade do Caminhão: "..getVehicleNameFromModel(model).." - "..exports.global:formatWeight(tempTruckCap)
				local tempLoadedSupplies = data and data.carried or manualSupplies
				loadedSupplies = "Suprimentos Carregados: "..exports.global:formatWeight(tempLoadedSupplies).." ("..math.floor(tempLoadedSupplies/tempTruckCap*100).."%)"
				local currentRoute = getElementData(localPlayer, "job-system-trucker:currentRoute") or false
				if currentRoute then
					height = 175
					nextLocation = "Alvo: "..currentRoute[6] or "Desconhecido"
					if currentRoute[4] then
						...
						if tonumber(requiredSupplies) >= tempLoadedSupplies then
							nextDropStopRequires = "Necessitando de Suprimentos: "..exports.global:formatWeight(requiredSupplies).." - INSUFICIENTE"
							display_distance = currentRoute[9]
							r,b,g = 255, 0, 0
						else
							nextDropStopRequires = "Necessitando de Suprimentos: "..exports.global:formatWeight(requiredSupplies).." - SUFICIENTE"
							display_distance = currentRoute[9]
							r,b,g = 0, 255, 0
						end
					else
		...
		elseif job == 2 and jobVeh == 2 then -- Taxi
			...
			jobCurrentProgress = "Tarifas servidas: "..tempTruckRuns
		...
		elseif job == 4 and jobVeh == 4 then -- Citi Maintenance
			...
			jobLevel = "Certificado: Nível de Trabalhador "..tempJobLevel
			if truckrunsTilNextLevel then
				jobProgress = "Progresso: "..tempProgress.."/"..truckrunsTilNextLevel.." ("..math.floor(tempProgress/truckrunsTilNextLevel*100).."%)"
			else
				jobProgress = "Progresso: "..tempProgress.." (Você dominou este trabalho)"
			end
			jobCurrentProgress = "Turnos Limpos: "..tempTruckRuns
		elseif job == 5 and jobVeh == 5 then -- Mechanic
			...
			jobLevel = "Certificado: Nível "..tempJobLevel
			if truckrunsTilNextLevel then
				jobProgress = "Progresso: "..tempProgress.."/"..truckrunsTilNextLevel.." ("..math.floor(tempProgress/truckrunsTilNextLevel*100).."%)"
			else
				jobProgress = "Progresso: "..tempProgress.." (Você dominou este trabalho)"
			end
			jobCurrentProgress = "Veículos Atendidos: "..tempTruckRuns
		elseif job == 6 and jobVeh == 6 then -- Locksmith
			...
			jobCurrentProgress = "Chaves Manipuladas: "..tempTruckRuns
		else
			show = false
			return false
		end
	...
	if show and not getElementData(localPlayer, "integration:previewPMShowing") then
		if ( getPedWeapon( localPlayer ) ~= 43 or not getPedControlState( localPlayer, "aim_weapon" ) ) then
			...
			if tonumber(display_distance) then
				...
				dxDrawText( current_distance and ("Distancia: "..formatLength(current_distance).."/"..formatLength(display_distance)) or "" , sx-width+10, 145+yOffset, width-5, 15, tocolor ( r2, b2, g2, 255 ), 1, "default" )
			end
