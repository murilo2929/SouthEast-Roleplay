			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (jobID==4) then -- CITY MAINTENANCE
						...
						outputChatBox("Use este spray para pintar sobre o graffiti que você encontrar.", targetPlayer, 255, 194, 14)
					...
					if hiddenAdmin == 0 then
						outputChatBox("Seu trabalho foi definido para '" .. jobTitle .. "' por "..tostring(adminTitle) .. " " .. getPlayerName(thePlayer):gsub("_", " ") ..". ", targetPlayer, 0, 255,0)
					else
						outputChatBox("Seu trabalho foi definido para '" .. jobTitle .. "' por um administrador oculto. ", targetPlayer, 0, 255,0)
					end
					outputChatBox("Você definiu o emprego de " .. targetPlayerName .. " para '"..jobTitle.."'.", thePlayer)
				end
			end
		end
	end
...
function printSetJobSyntax(thePlayer, commandName)
	outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador / ID] [Job ID, 0 = Desempregado]", thePlayer, 255, 194, 14)
	outputChatBox("ID#1: Motorista de Entrega", thePlayer)
	outputChatBox("ID#2: Taxista", thePlayer)
	outputChatBox("ID#3: Motorista de Ônibus", thePlayer)
	outputChatBox("ID#4: Manutenção da Cidade", thePlayer)
	outputChatBox("ID#5: Mecanico", thePlayer)
	--outputChatBox("ID#6: Locksmith", thePlayer)
	outputChatBox("ID#7: Motorista de Caminhão de Longa Distância", thePlayer)
end
...
function setjobLevel(thePlayer, commandName, target, level, progress )
	if exports.integration:isPlayerLeadAdmin(thePlayer) then
		if not target or not tonumber(level) or (tonumber(level) < 1) then
			outputChatBox( "SYNTAX: /" .. commandName .. " [player ID ou Nome] [Level] [Progresso, opcional]", thePlayer, 255, 194, 14 )
			return false
		end
		...
		if not targetPlayer then
			outputChatBox("Player '"..target.."' não encontrado.", thePlayer, 255,0,0)
			return false
		end
		...
		if jobID <=0 then
			outputChatBox("O jogador está desempregado no momento, use /setjob primeiro.", thePlayer, 255,0,0)
			return false
		end
...
function setPlayerJobLevel(targetPlayer, jobID, level, progress)
	if mysql:query_free("UPDATE `jobs` SET `jobLevel`='"..level.."', `jobProgress`='"..progress.."' WHERE `jobCharID`='"..getElementData(targetPlayer, "dbid").."' AND `jobID`='"..jobID.."' " ) then
		exports["job-system"]:fetchJobInfoForOnePlayer(targetPlayer)
		return true, getPlayerName(targetPlayer):gsub("_", " ").." agora tem '"..getJobTitleFromID(jobID).."' emprego (Level: "..level..", Progresso: "..progress..")"
	else
			...
			if targetPlayer then
				if getElementData( targetPlayer, "loggedin" ) == 1 then
					...
					if result then
						...
						if hiddenAdmin == 0 then
							local adminTitle = exports.global:getPlayerAdminTitle(thePlayer)
							outputChatBox("Seu trabalho foi excluído por "..tostring(adminTitle) .. " " .. getPlayerName(thePlayer)..". Relog novamente (F10).", targetPlayer, 0, 255,0)
						else
							outputChatBox("Seu trabalho foi excluído por um administrador oculto.", targetPlayer, 0, 255,0)
						end
					else
						outputChatBox( "Falha ao excluir trabalho.", thePlayer, 255, 0, 0 )
					end
				else
					outputChatBox( "O jogador não está logado.", thePlayer, 255, 0, 0 )
				end
...
function adminRespawnAllTrucks(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		outputChatBox("Apenas Admin e superior podem acessar /"..commandName..".", thePlayer, 255,0,0)
	end
end
addCommandHandler("respawntrucks", adminRespawnAllTrucks, false, false)
addCommandHandler("respawnarcaminhoes", adminRespawnAllTrucks, false, false)

function scripterSkipRoute(thePlayer, commandName, target)
	if not exports.integration:isPlayerTrialAdmin(thePlayer) then
		outputChatBox("Apenas o superadministrador e superior podem acessar /"..commandName..".", thePlayer, 255,0,0)
		return false
	end
	if not target then
