...
--  Model   (1)Capacity (2)Level (3)Price/Crate (4)CrateWeight (5)Earning/Crate
	[440] = {700, 1, 15, 20, 100}, -- Rumpo
	[499] = {1120, 2, 35, 50, 150}, -- Benson
	[414] = {1400, 3, 55, 100, 200}, -- Mule
	[498] = {2100, 4, 75, 140, 250}, -- Boxville
	[456] = {2800, 5, 75, 140, 300}, -- Yankee
}
...
function displayTruckerJob(notext, spwan)
	-- if (jobstate==0) then
		...
		if not notext then
			exports.hud:sendBottomNotification(localPlayer, "Operador de Transporte RS:", "Aproxime-se do ícone do caminhão cinza em seu radar e entre no veículo do RS Haul para iniciar seu trabalho.")
		end
	-- end
end
...
function waitAtStationLoadup(thePlayer)
	...
	if thePlayer == localPlayer and vehicle and getVehicleController(vehicle) == localPlayer then
		if getElementHealth(vehicle) < 900 then
			exports.hud:sendBottomNotification(localPlayer, "Operador de Transporte RS:", "Você precisa consertar seu caminhão primeiro..")
		else
			if not timerLoadUp then
				exports.hud:sendBottomNotification(thePlayer, "Operador de Transporte RS:", "Agora, espere um momento enquanto seu caminhão está sendo carregado com caixas de suprimentos.")
...
function drawBuyLoadWindow(thePlayer)
	...
	lNumberOfCrates = guiCreateLabel(13,25,176,19,"Número de Caixas de Suprimentos: 0",false,wRSHaulLoadup)
	guiSetFont(lNumberOfCrates,"default-bold-small")
	lCost = guiCreateLabel(13,68,176,19,"Custo: $0",false,wRSHaulLoadup)
	guiSetFont(lCost,"default-bold-small")
	scrollbar = guiCreateScrollBar(13,44,176,20,true,false,wRSHaulLoadup)
	lMoney = guiCreateLabel(13,87,176,19,"Seu dinheiro: $0",false,wRSHaulLoadup)
	guiSetFont(lMoney,"default-bold-small")
	bBuyLoad = guiCreateButton(9,111,94,28,"Compre e Carregue",false,wRSHaulLoadup)
	bCancel = guiCreateButton(107,111,88,28,"Cancelar",false,wRSHaulLoadup)
end
...
function endDelivery(thePlayer)
	if thePlayer == localPlayer then
		...
		if not vehicle or getVehicleController(vehicle) ~= localPlayer then
			exports.hud:sendBottomNotification(thePlayer, "Operador de Transporte RS:", "Você deve estar em um veículo RS Haul para concluir as entregas.")
		else
			local health = getElementHealth(vehicle)
			if health <= 900 then
				exports.hud:sendBottomNotification(thePlayer, "Operador de Transporte RS:", "Este caminhão está danificado, conserte-o primeiro.")
			else
				if countInkeepableObjects(exports['item-system']:getItems(vehicle)) > 0 then
					outputChatBox('Remova todos os itens do veículo antes de concluir esta corrida RS Haul.', 255, 0, 0)
					return
				end
...
function waitAtDelivery(thePlayer)
	...
	if thePlayer == localPlayer and vehicle and getVehicleController(vehicle) == localPlayer then
		if getElementHealth(vehicle) < 900 then
			exports.hud:sendBottomNotification(thePlayer, "Operador de Transporte RS:", "Você precisa consertar seu caminhão.")
		else
			...
			exports.hud:sendBottomNotification(thePlayer, "Operador de Transporte RS:", "Espere um momento enquanto seu caminhão é processado.")
			addEventHandler("onClientMarkerLeave", marker, checkWaitAtDelivery)
		end 
	end
end
...
function checkIfPlayerTruckHasEnoughtShit()
	...
	if vehicle and getVehicleController(vehicle) == localPlayer  then
		if getElementData(vehicle, "job") ~= 1 then
			exports.hud:sendBottomNotification(localPlayer, "Operador de Transporte RS:", "Cara .. você tem que usar o veículo RS Haul.")
			return false
		end
		--spawnFinishMarkerTruckJob()
		triggerLatentServerEvent("truckerjob:checkIfPlayerTruckHasEnoughtShit", localPlayer)	
	else
		exports.hud:sendBottomNotification(localPlayer, "Operador de Transporte RS:", "Você deve estar no caminhão para concluir as entregas.")
	end
end