                        ...
                        if (getElementData(temp, "faction") ~= 4) then
                            if (impounded == 0) then
                                ...
                                if owner > 0 and account then
                                    exports.announcement:makePlayerNotification(account.id, factionName.."apreendeu o seu "..exports.global:getVehicleName(temp), "(propiedade de "..characterName..").")
                                end
                            end
                        end
                    end
                else
                    outputChatBox("O veículo desta facção não pode ser apreendido.", thePlayer, 255, 194, 14)
                end
            end
        end
    end
                ...
                if (owner > 0) then
                    if (faction > 3 or faction < 0) then
                        if (source == towSphere2) then
                            ...
                            outputChatBox("((Por favor, lembre-se de estacionar e trancar seu veículo em nosso estacionamento.))", player, 255, 194, 14)
                        else
                            if rank >= 5 then
                                if (faction ~= 4) then
                                    if (impounded == 0) then
                                        ...
                                        setVehicleEngineState(veh, false)
                                        outputChatBox("((A moto foi apreendida com sucesso.))", player, 50, 205, 50)
                                        outputChatBox("((Por favor, lembre-se de estacionar e trancar seu veículo em nosso estacionamento.))", player, 255, 194, 14)
                                        isin = false
                                ...
                                outputChatBox("Comando utilizável apenas por " .. factionRank .. " e acima.", player, 255, 194, 14)
                            end
                        end
                    else
                        outputChatBox("O veículo desta facção não pode ser apreendido.", player, 255, 194, 14)
                    end
                end
            else
                outputChatBox("Você só pode usar este comando para apreender motocicletas e bicicletas.", player, 255, 194, 14)
            end
        else
            outputChatBox("Você não está em um veículo.", player, 255, 0, 0)
        end
    end
end
addCommandHandler("impoundbike", setbikeimpound)
addCommandHandler("apreenderbike", setbikeimpound)
...
function payRelease(vehid)
    if exports.global:takeMoney(source, 1000) then
        ...
        outputChatBox("Seu veículo foi liberado, está na frente. ((Lembre-se de estacionar estacionar seu veículo para que ele não reapareça aqui.))", source, 255, 194, 14)
    else
        outputChatBox("Fundos insuficientes.", source, 255, 0, 0)
    end
end
...
function unimpoundVeh(thePlayer, commandName, vehid)
    if exports.integration:isPlayerTrialAdmin(thePlayer) then
        if not vehid then
            outputChatBox("SYNTAX: /" .. commandName .. " [Veículo ID]", thePlayer, 255, 194, 14)
        else
            local vehID = exports.pool:getElement("vehicle", tonumber(vehid))
            if not vehID then
                outputChatBox("Veículo invalido.", thePlayer, 255, 0, 0)
            else
                if getElementData(vehID, "Impounded") and getElementData(vehID, "Impounded") ~= 0 then
                    ...
                    outputChatBox("Veículo #" .. vehid .. " não está apreendido atualmente.", thePlayer, 255, 0, 0)
                end
            end
        end
    end
...
addCommandHandler("desapreenderveiculo", unimpoundVeh)

function disableEntryToTowedVehicles(thePlayer, seat, jacked, door)
    if (getVehicleTowingVehicle(source)) then
        if seat == 0 then
            outputChatBox("Você não pode entrar em um veículo que está sendo rebocado!", thePlayer, 255, 0, 0)
            cancelEvent()
        end
    end
end
    ...
    if (thePlayer) then
        if exports.factions:isPlayerInFaction(thePlayer, 4) or exports.factions:isPlayerInFaction(thePlayer, 42) then
            ...
            if owner < 0 and faction == -1 then
                outputChatBox("(( Esse " .. carName .. " é um veículo civil. ))", thePlayer, 255, 195, 14)
            elseif (faction==-1) and (owner>0) then
                local ownerName = exports["cache"]:getCharacterName(owner)
                outputChatBox("(( Esse " .. carName .. " pertence a " .. ownerName .. ". ))", thePlayer, 255, 195, 14)
            else
                ...
                if not (row == false) then
                    ...
                    outputChatBox("(( Esse " .. carName .. " pertence a facção " .. ownerName .. ". ))", thePlayer, 255, 195, 14)
                end
            end

            if (getElementData(source, "Impounded") > 0) then
                local output = getRealTime().yearday-getElementData(source, "Impounded")
                outputChatBox("(( Esse " .. carName .. " foi apreendido por: " .. output .. (output == 1 and " Dia." or " Dias.") .. " ))", thePlayer, 255, 195, 14)
            end
...
function updateCivilianVehicles(theTruck)
    if (isElementWithinColShape(theTruck, towSphere)) then
        ...
        if (dbid >= 0 and faction == -1 and owner < 0) then
            ...
            outputChatBox("O estado liberou o veículo que você estava rebocando.", getVehicleOccupant(theTruck), 255, 194, 14)
