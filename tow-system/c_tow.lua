...
function showImpoundUI(vehElementsret)
	if not wImpound then
		...
		wImpound = guiCreateWindow(x, y, width, height, "Apreensão: Liberar um veículo", false)
		guiWindowSetSizable(wImpound, false)
		
		bClose = guiCreateButton(0.6, 0.85, 0.2, 0.1, "Fechar", true, wImpound)
		bRelease = guiCreateButton(0.825, 0.85, 0.2, 0.1, "Soltar", true, wImpound)
		...
		local col = guiGridListAddColumn(gCars, "Veículos Apreendidos", 0.7)
		...
		lCost = guiCreateLabel(0.3, 0.85, 0.2, 0.1, "Custo: $1000.00", true, wImpound)
		...
		outputChatBox("Bem-vindo ao TTR Impound Lot.")
	end
end
...
function updateCar()
	...
	if (row~=-1) and (col~=-1) then
		guiSetText(lCost, "Custo: $1000.00")
