...
local pdReleasePed = createPed(280, 251.2568359375, 66.646484375, 1003.640625 )
setPedRotation( pdReleasePed, 91 )
setElementDimension( pdReleasePed, 8)
setElementInterior( pdReleasePed , 6 )
...
function openImpGui(dep, vehid, first, last, vehName, plate, vin, laneData)
	...
	gui.window[1] = guiCreateWindow(642, 140, 804, 405, dep.." Lote Apreendidos ((Veículo ID "..vehid.."))", false)
	...
	gui.label[3] = guiCreateLabel(31, 87, 82, 25, "Sobrenome:", false, gui.window[1])
	...
	gui.label[4] = guiCreateLabel(31, 117, 82, 25, "Distintivo: ", false, gui.window[1])
	...
	gui.label[6] = guiCreateLabel(14, 191, 365, 18, "Informação Veículo", false, gui.window[1])
	...
	gui.label[7] = guiCreateLabel(31, 219, 82, 25, "Modelo:", false, gui.window[1])
	...
	gui.label[8] = guiCreateLabel(31, 249, 82, 25, "Placa:", false, gui.window[1])
	...
	gui.label[10] = guiCreateLabel(406, 29, 365, 18, "Informações Apreenção", false, gui.window[1])
	...
	gui.label[11] = guiCreateLabel(423, 57, 82, 25, "Violações:", false, gui.window[1])
	...
	gui.label[12] = guiCreateLabel(423, 88, 82, 25, "Da localização:", false, gui.window[1])
	guiLabelSetVerticalAlign(gui.label[12], "center")
	gui.label[1] = guiCreateLabel(423, 118, 119, 25, "Número da faixa:", false, gui.window[1])
	...
	guiLabelSetVerticalAlign(gui.label[2], "center")
	gui.label[13] = guiCreateLabel(423, 148, 119, 25, "Está pronto para ser solto em:", false, gui.window[1])
	guiLabelSetVerticalAlign(gui.label[13], "center")
	gui.edit[11] = guiCreateEdit(542, 148, 142, 25, dep == "RT" and "Irrelevante" or "1", false, gui.window[1])
	guiEditSetMaxLength(gui.edit[11], 200)
	gui.label[14] = guiCreateLabel(694, 148, 85, 25, "0 dia = apreendido", false, gui.window[1])
	guiLabelSetVerticalAlign(gui.label[14], "center")
	gui.label[15] = guiCreateLabel(413, 217, 365, 18, "Informações adicionais", false, gui.window[1])
	guiSetFont(gui.label[15], "default-bold-small")
	gui.memo[1] = guiCreateMemo(423, 242, 348, 63, "", false, gui.window[1])
	gui.button[1] = guiCreateButton(21, 338, 368, 41, "Cancelar", false, gui.window[1])
	gui.button[2] = guiCreateButton(413, 338, 368, 41, "Apreender", false, gui.window[1])
	gui.label[3] = guiCreateLabel(423, 178, 119, 25, "Multa em dolares:", false, gui.window[1])
	guiLabelSetVerticalAlign(gui.label[3], "center")
	gui.edit[12] = guiCreateEdit(542, 178, 142, 25, dep == "RT" and "Irrelevante" or "0", false, gui.window[1])
	guiEditSetMaxLength(gui.edit[1], 200)
...
function openReleaseGUI(factionId, vehs)
	...
	rgui.window[1] = guiCreateWindow(587, 335, 749, 326, factionName.." Lote Apreendidos | Central de Ajuda", false)
	guiWindowSetSizable(rgui.window[1], false)
	exports.global:centerWindow(rgui.window[1])

	rgui.label[1] = guiCreateLabel(27, 40, 696, 35, "Para liberar seu veículo, você terá que pagar pelo reboque e taxas diárias de armazenamento de $"..exports.global:formatMoney(costPerDay).." por dia. Você também pode ter que pagar a mais se houver alguma multa aplicada ao seu veículo.", false, rgui.window[1])
	...
	rgui.col_mod = guiGridListAddColumn(rgui.gridlist[1], "Modelo", 0.4)
	rgui.col_days = guiGridListAddColumn(rgui.gridlist[1], "Apreendido", 0.15)
	rgui.col_fine = guiGridListAddColumn(rgui.gridlist[1], "Multa ($)", 0.1)
	rgui.col_date = guiGridListAddColumn(rgui.gridlist[1], "Data Saida", 0.2)
	...
	for vin, veh in pairs(vehs) do
		...
		local releasteDate = (veh.lane and veh.lane.release_date) and veh.lane.release_date or "Apreendido"
		if factionId == 4 then
			releasteDate = "Qualquer Hora"
		end
	...
	rgui.button[1] = guiCreateButton(621, 275, 102, 31, "Pagar e Liberar", false, rgui.window[1])
	rgui.button[2] = guiCreateButton(514, 275, 102, 31, "Fechar", false, rgui.window[1])
		...
		if source == rgui.button[1] then
			if selectedRow == -1 or selectedCol == -1 then
				...
				if exports.global:hasMoney(localPlayer, cost) then
					...
					triggerServerEvent("tow:pedSay", localPlayer, pedName, "Você precisa de $"..exports.global:formatMoney(cost)..".")
				end
			end
