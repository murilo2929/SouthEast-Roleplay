...
local convos = {
	...
	['one_car_at_once'] = {
		"Ops, algo está errado aqui, tente novamente por favor.",
		"Um carro por vez fazendo favor.",
	},
	['invalid_fine'] = {
		"A multa deve ser positiva e não pode ser superior a $ 50.000. Tente novamente.",
	},
	['no_car_to_release'] = {
		"Você não tem nenhum veículo em nosso estacionamento no momento.",
		"Desculpe, mas não consegui encontrar nenhum veículo seu no estacionamento aqui.",
		--"Nothing of yours here, sorry pal.",
	},
	['select_to_release'] = {
		"Qual você gostaria de soltar?",
		"Diga-me qual veículo você quer que eu libere?",
		"Selecione um por favor."
	},
}
...
function leoStartImpounding(dep, vehid, laneNumber, days, fine, first, last, badge, rank, vehName, plate, vin, volations, location, other)
	...
	local factionName = getTeamName(thefaction) or "Organização Desconhecida"
	report = report..factionName.." - Relatório de apreensão do soldado\n"
	report = report.."\nInformação do Oficial\n"
	report = report.."- Primeiro Nome: "..string.upper(first).."\n"
	report = report.."- Sobrenome: "..string.upper(last).."\n"
	report = report.."- Distintivo: "..string.upper(badge).."\n"
	report = report.."- Rank: "..string.upper(rank).."\n"
	report = report.."\nInformação do Veículo\n"
	report = report.."- Modelo: "..string.upper(vehName).."\n"
	report = report.."- Localização: "..string.upper(location).."\n"
	report = report.."- Área que esta apreendido: "..string.upper(laneNumber).."\n"
	report = report.."- Violação: "..string.upper(volations).."\n"
	report = report.."- Está pronto para ser solto em: "..string.upper(days).." dia(s)\n"
	...
	--notifications
	exports.global:sendLocalDoAction(source, "Veículo "..vehName.." foi apreendido no patio da "..factionName.." na área #"..freeLane.lane..".")
	...
	if owner > 0 and account then
		local state = "apreendido"
		if factionId ~= 4 and tonumber(days)==0 then
			state = "apreendido"
		end
		exports.announcement:makePlayerNotification(account.id, factionName.." apreendeu "..state.." seu "..vehName,"(propriedade de "..characterName..")"..(tonumber(days)>0 and (" por "..days.." dia(s).") or "."))
	end
...
function unimpVeh(vehid)
	...
	if veh then
		...
		if impounder == 0 then
			return false, "O veículo não está apreendido no momento."
		end
		...
		if impounder == 1 or impounder == 50 or impounder == 4 then
			...
			if source then
				...
				local factionName = thefaction and getTeamName(thefaction) or "Organização Desconhecida"
				report = report..factionName.." - Relatório de apreensão do soldado\n"
				report = report.."\nSolto por "..exports.global:getPlayerName(source)
            ...
            return true, "Você liberou o veículo #" .. vehid .. "."
		else
			return false, "Você não está autorizado a liberar o veículo deste patio."
		end
	else
...
function release(pedName, vehid, cost)
	...
	if thefaction then
		if exports.bank:giveBankMoney(thefaction, cost) then
			exports.bank:addBankTransactionLog(getElementData(source,"dbid"), -getElementData(thefaction, "id"), cost, 16, "Taxa de apreensão e multa para liberar VIN do veículo #"..vehid)
			exports.logs:dbLog(source, 25 , {thefaction}, "$"..exports.global:formatMoney(cost).." Taxa de apreensão e multa para liberar VIN do veículo #"..vehid)
		end
	end
