...
function newCharacter_init()
	...
	gui["lblCharName"] = guiCreateLabel(10, 25, 91, 16, "Nome:", false, gui["_root"])
...
--[[	gui["lblCharDesc"] = guiCreateLabel(10, 125, 230, 100, "When you first spawn type /editlook and describe your character in more detail.", false, gui["_root"])
	...
	gui["lblGender"] = guiCreateLabel(10, 160, 46, 13, "Gênero:", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["lblGender"], "left", false)
	guiLabelSetVerticalAlign(gui["lblGender"], "center")
	gui["rbMale"] = guiCreateRadioButton(90, 160, 51, 13, "Homem", false, gui["_root"])
	gui["rbFemale"] = guiCreateRadioButton(150, 160, 82, 13, "Mulher", false, gui["_root"])
	...
	gui["btnNextSkin"] = guiCreateButton(150, 180, 80, 16, "Proxima", false, gui["_root"])
	addEventHandler("onClientGUIClick", gui["btnNextSkin"], newCharacter_updateGender, false)

	gui["lblRace"] = guiCreateLabel(10, 140, 111, 16, "Etnia:", false, gui["_root"])
	...
	gui["chkWhite"] =  guiCreateCheckBox ( 120, 140, 55, 16, "Branco", false, false, gui["_root"] )
	...
	gui["lblAge"] = guiCreateLabel(10, 240, 111, 16, "Idade", false, gui["_root"])
	...
	gui["lblDay"] = guiCreateLabel(10, 282, 111, 16, "Data Nascimento:", false, gui["_root"])
	...
	gui["lblMonth"] = guiCreateLabel(10, 260, 111, 16, "Mês Nascimento", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["lblDay"], "left", false)
	guiLabelSetVerticalAlign(gui["lblDay"], "center")


	gui["drpMonth"] =  guiCreateComboBox ( 110, 260, 130, 16, "Janeiro", false, gui["_root"])
	guiComboBoxAdjustHeight(gui["drpMonth"], 15)
	guiComboBoxAddItem(gui["drpMonth"], "Janeiro")
	guiComboBoxAddItem(gui["drpMonth"], "Fevereiro")
	guiComboBoxAddItem(gui["drpMonth"], "Março")
	guiComboBoxAddItem(gui["drpMonth"], "Abril")
	guiComboBoxAddItem(gui["drpMonth"], "Maio")
	guiComboBoxAddItem(gui["drpMonth"], "Junho")
	guiComboBoxAddItem(gui["drpMonth"], "Julho")
	guiComboBoxAddItem(gui["drpMonth"], "Agosto")
	guiComboBoxAddItem(gui["drpMonth"], "Setembro")
	guiComboBoxAddItem(gui["drpMonth"], "Outubro")
	guiComboBoxAddItem(gui["drpMonth"], "Novembro")
	guiComboBoxAddItem(gui["drpMonth"], "Dezembro")
	...
	gui["lblLanguage"] = guiCreateLabel(10, 305, 111, 16, "Idioma:", false, gui["_root"])
	...
	gui["lblLanguageDisplay"] = guiCreateLabel(126, 305, 98, 16, "Inglês", false, gui["_root"])
	...
	gui["btnLangs"] = guiCreateButton(10, 330, 231, 41, "Todos os idiomas", false, gui["_root"])
	...
	gui["btnCreateChar"] = guiCreateButton(10,410+10, 231, 41, "Criar", false, gui["_root"])
...
function newCharacter_viewAllLangs(button)
	if (source == gui["btnLangs"] and button == "left") then
		...
		gui['langWindow'] = guiCreateWindow(720, 339, 339, 446, "Idiomas", false)
		...
		gui['langWindowCloseBtn'] = guiCreateButton(9, 411, 320, 25, "Fechar", false, gui['langWindow'])
...
function newCharacter_updateScrollBars()
	...
	--scrAge = math.floor( (scrAge * 0.8 ) + 16 )
	guiSetText(gui["lblAge"], "Idade: "..scrAge.." anos")
	...
	guiSetText(gui["lblDay"], "Dia Nascimento: "..(scrDay or "1"))
end
...
function newCharacter_changeSkin(diff)
	...
	if skin ~= nil then
		...
		if exports.newmodels:isCustomModID(skin) then
			setElementData(dummyPed, data_name, tonumber(skin)) -- sets the skin ID data
		else
			setElementModel( dummyPed, tonumber(skin) )
		end
	end
...
function newCharacters_getSkinArray()
	...
	if (guiCheckBoxGetSelected( gui["chkBlack"] )) then -- BLACK
		...
		elseif (guiRadioButtonGetSelected( gui["rbFemale"] )) then -- BLACK FEMALE
			...
			outputChatBox("Selecione um gênero primeiro!", 0, 255, 0)
		end
...
function newCharacter_attemptCreateCharacter()
	...
	if not nameCheckPassed then
		LoginScreen_showWarningMessage( "Erro ao processar o nome do seu personagem:\n".. nameCheckError )
		return
	end
	--[[
	...
	elseif (guiCheckBoxGetSelected(gui["chkAsian"])) then
		...
		LoginScreen_showWarningMessage( "Erro ao processar a etnia do seu personagem:\nNenhuma selecionada." )
		return
	end
	...
	elseif (guiRadioButtonGetSelected( gui["rbFemale"] )) then
		...
		LoginScreen_showWarningMessage( "Erro ao processar o gênero do seu personagem:\nNenhuma selecionada." )
		return
	end

	local skin = getElementModel( dummyPed )
	if not skin then
		LoginScreen_showWarningMessage( "Erro ao processar a skin do seu personagem:\nNenhuma selecionada." )
		return
	end
	...
	if languageselected == nil then
		LoginScreen_showWarningMessage( "Erro ao processar o idioma do seu personagem:\nNenhum selecionada." )
		return
	end
...
function newCharacter_response(statusID, statusSubID)
	if (statusID == 1) then
		LoginScreen_showWarningMessage( "Oops, algo deu errado. Tente novamente\nou contate um administrador.\nError ACC"..tostring(statusSubID) )
	elseif (statusID == 2) then
		if (statusSubID == 1) then
			LoginScreen_showWarningMessage( "Este nome de personagem já está em\nuso!" )
		else
...
function selectStartPointGUI(characterName, characterDescription, race, gender, skin, scrollHeight, scrWeight, scrAge, languageselected, selectedMonth, scrDay )
	...
	local locations = {
		...
		["default"] = {1168.6484375, -1412.576171875, 13.497941017151, 357.72854614258, 0, 0, "Um ponto de ônibus perto do shopping"},
		["igs"] = { 1922.9072265625, -1760.6982421875, 13.546875, 0,			0, 		0, 		"Próximo ponto de ônibus em Idlewood"},
		["bus"] = {1749.509765625, -1860.5087890625, 13.578649520874, 359.0744, 	0, 		0, 		"Estação Rodoviária Unity"},
		["metro"] = {808.88671875, -1354.6513671875, -0.5078125, 139.5092, 			0, 		0,		"Estação de metrô"},
		["air"] = {1691.6455078125, -2334.001953125, 13.546875, 0.10711, 			0, 		0,		"Aeroporto Internacional de Los Santos"},
		["boat"] = {2809.66015625, -2436.7236328125, 13.628322601318, 90.8995, 		0, 		0,		"Cais de Santa Maria"},
	}

	wSelectStartPoint = guiCreateWindow(0,0, 300, 250, "Como você chega em los santos?", false)
	exports.global:centerWindow(wSelectStartPoint)

	local busButton = guiCreateButton(40, 40, 100, 60, "Onibus", false, wSelectStartPoint)
	...
	local airButton = guiCreateButton(160, 40, 100, 60, "Avião", false, wSelectStartPoint)
	...
	local boatButton = guiCreateButton(160, 120, 100, 60, "Carco", false, wSelectStartPoint)
