...
function options_showmenu()
	...
	bChangeCharacter = guiCreateButton(margin, margin, 230, 30, "Mudar Personagem", false, wOptions)
	...
	bStatistics = guiCreateButton(margin, wHeight, 230, 30, "Estatísticas do personagem", false, wOptions)
	...
	bGraphicsSettings = guiCreateButton(margin, wHeight, 230, 30, "Configurações", false, wOptions)
	...
	if getResourceFromName("donators") then
		bStore = guiCreateButton(margin, wHeight, 230, 30, "Recursos Premium", false, wOptions)
		--addEventHandler("onClientGUIClick", bGraphicsSettings, options_opengraphicsmenu, false)
	...
	if getResourceFromName("vehicle") and getResourceFromName("vehicle_manager") and exports.vehicle_manager:canAccessVehicleManager( localPlayer ) then
		bVehicleManager = guiCreateButton(margin, wHeight, 230, 30, "Veículo Manager", false, wOptions)
	...
	if getResourceFromName('vehicle') and getResourceFromName("vehicle_manager") then
		...
		if exports.integration:isPlayerVCTMember(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerVehicleConsultant(thePlayer) then
			bVehicleLib = guiCreateButton(margin, wHeight, 230, 30, "Vehicle Biblioteca", false, wOptions)
	...
	bLogout = guiCreateButton(margin, wHeight, 230, 30, "Deslogar", false, wOptions)
	...
	bClose = guiCreateButton(margin, wHeight, 230, 30, "Fechar", false, wOptions)
...
function options_opengraphicsmenu()
	...
	wGraphicsMenu = guiCreateWindow(left, top, windowWidth, windowHeight, "Opções de jogo", false)
	guiWindowSetSizable(wGraphicsMenu, false)
	----------

	cMotionBlur = guiCreateCheckBox(10, 25, 180, 17, "Ativar motion blur", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cMotionBlur, options_updateGameConfig)
	-----------
	cSkyClouds = guiCreateCheckBox(10, 45, 180, 17, "Ativar Nuvens", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cSkyClouds, options_updateGameConfig)
	------------
	cStreamingAudio = guiCreateCheckBox(10, 65, 180, 17, "Enable audio streaming", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cStreamingAudio, options_updateGameConfig)

	bOverlayDescription = guiCreateButton ( 10, 85, 180, 17*2, "Configurações Overlay", false, wGraphicsMenu )
	...
	cLogsEnabled = guiCreateCheckBox(10, 160, 180, 17, "Log do Chat", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cLogsEnabled, options_updateGameConfig)

	cBubblesEnabled = guiCreateCheckBox(10, 180, 180, 17, "Ativar balões de bate-papo", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cBubblesEnabled, options_updateGameConfig)

	cIconsEnabled = guiCreateCheckBox(10, 200, 180, 17, "Habilitar ícones ao digitar", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cIconsEnabled, options_updateGameConfig)

	cEnableNametags = guiCreateCheckBox(10, 220, 180, 17, "Ativar nametags", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cEnableNametags, options_updateGameConfig)

	cEnableRShaders = guiCreateCheckBox(10, 240, 180, 17, "Ativar shader radar", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cEnableRShaders, options_updateGameConfig)

	cEnableWShaders = guiCreateCheckBox(10, 260, 180, 17, "Ativar shader agua", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cEnableWShaders, options_updateGameConfig)

	cEnableVShaders = guiCreateCheckBox(10, 280, 180, 17, "Ativar shader veículo", false, false, wGraphicsMenu)
	...
	bGraphicsMenuClose = guiCreateButton(10, 320, 490, 17*2, "Fechar", false, wGraphicsMenu)
...
function overlayDescSettings(button, state)
	if source == bOverlayDescription then
		if wOverlayDescSettings then
			...
			wOverlayDescSettings = guiCreateWindow(left, top, windowWidth, windowHeight, "Overlay Opções", false)
			...
			cEnableDescription = guiCreateCheckBox(10, 25+y, lane1w, 17, "Habilitar todas as descrições de Overlay", true, false, wOverlayDescSettings)
			...
			cEnableDescriptionVeh = guiCreateCheckBox(10, 25+y, lane1w, 17, "Habilitar descrição de Overlay (veículo)", true, false, wOverlayDescSettings)
			...
			bgVeh = guiCreateCheckBox ( 10, 25+y+3, 150, 17,  "Ativar Fundo", true, false, wOverlayDescSettings )
			enable = tonumber( loadSavedData("bgVeh", "1") or 1 )
			guiCheckBoxSetSelected ( bgVeh, enable == 1 and true or false)

			borderVeh = guiCreateCheckBox ( 10+150, 25+y+3, 150, 17, "Ativar Borda", true, false, wOverlayDescSettings )
			...
			cEnableOverlayDescriptionPro = guiCreateCheckBox(10, 25+y, lane1w, 17, "Habilitar descrição de Overlay (propiedade)", true, false, wOverlayDescSettings)
			...
			lFontPro = guiCreateLabel ( 10, 25+y+3, 40, 20,  "Fonte:", false, wOverlayDescSettings )
			...
			bgPro = guiCreateCheckBox ( 10, 25+y+3, 150, 17,  "Ativar Fundo", true, false, wOverlayDescSettings )
			enable = tonumber( loadSavedData("bgPro", "1") or 1 )
			guiCheckBoxSetSelected ( bgPro, enable == 1 and true or false)

			borderPro = guiCreateCheckBox ( 10+150, 25+y+3, 150, 17, "Ativar Borda", true,false,  wOverlayDescSettings )
			...
			bCloseOverlayDescSettings = guiCreateButton(10, 70+y, windowWidth+34, 17*2, "Fechar", false, wOverlayDescSettings)
