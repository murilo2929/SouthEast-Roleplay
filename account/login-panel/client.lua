...
local fade = { }
local logoScale = 0.2
local logoSize = { sw*logoScale, sw*2/5*logoScale }
local uFont
...
function open_log_reg_pannel()
	if not isElement ( panel.login.main ) then
		...
		panel.login.remember = guiCreateCheckBox(X + 230,Y + 275-120,100,20,"(Relembrar!)",false,false)
...
function startLoggingIn()
	if not getElementData(localPlayer, "clickedLogin") then
		...
		authen_msg("Login", "Enviando solicitação ao servidor..")
	else
		Error_msg("Login", "Calma..")
	end
end
...
function registerValidation(username, password, passwordConfirm, email)
	if not username or username == "" or not password or password == "" or not passwordConfirm or passwordConfirm == "" or not email or email == ""  then
		guiSetText(panel.login.toplabel, "Por favor, preencha todos os campos.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.len(username) < 3 then
		guiSetText(panel.login.toplabel, "O nome de usuário deve ter 3 caracteres ou mais.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.len(username) >= 19 then
		guiSetText(panel.login.toplabel, "O nome de usuário deve ter menos de 20 caracteres.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.find(username, ' ') then
		guiSetText(panel.login.toplabel, "Nome de usuário invalido.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.find(password, "'") or string.find(password, '"') then
		guiSetText(panel.login.toplabel, "A senha não deve conter ' ou "..'"')
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.len(password) < 8 then
		guiSetText(panel.login.toplabel, "A senha deve ter 8 caracteres ou mais.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.len(password) > 25 then
		guiSetText(panel.login.toplabel, "A senha deve ter menos de 25 caracteres.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif password ~= passwordConfirm then
		guiSetText(panel.login.toplabel, "Senhas incompatíveis!")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.match(username,"%W") then
		guiSetText(panel.login.toplabel, "\"!@#$\"%'^&*()\" não são permitidos no nome de usuário.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	else
		...
		if not validEmail then
			...
			authen_msg("Registro", "Enviando solicitação ao servidor.")
		end
	end
end
...
function displayRegisterConpleteText(username)
	...
	GUIEditor.window[1] = guiCreateWindow(667, 381, 357, 189+extend, "Parabéns! A conta foi criada com sucesso!", false)
    ...
    local temp = "Um e-mail contém instruções para ativar a sua conta foi enviada, por favor, verifique a caixa de entrada do seu e-mail.\n\nSe por algum motivo você não receber o e-mail, verifique sua caixa de lixo eletrônico."
    GUIEditor.label[1] = guiCreateLabel(8, 50, 339, 121+extend, "Sua conta para '"..username.."' está quase pronto para a ação!\n\n"..temp.."\n\nSinceramente, \nSoutheast Community Team\"", false, GUIEditor.window[1])
    guiLabelSetHorizontalAlign(GUIEditor.label[1], "left", true)

    GUIEditor.button[1] = guiCreateButton(10, 153+extend, 337, 26, "Copiar Link de Ativação", false, GUIEditor.window[1])
...
function screenStandBy(action, value) -- Maxime / 2015.3.25
	if action == "add" then
		...
		if screenStandByShowing then
			authen_msg("Login", "Carregando recursos de pré-requisito.."..screenStandBy("getPercentage").."%")
		end
	...
	elseif action == "getPercentage" then
		...
		if screenStandByShowing then
			authen_msg("Login", "Carregando recursos de pré-requisito.."..percentage.."%")
		end
		return percentage
	end
end
