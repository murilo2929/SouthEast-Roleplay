...
function validateCredentials(username,password,checksave)
	if not (username == "") then
		if not (password == "") then
			...
			triggerClientEvent(client,"set_warning_text",client,"Login","Por favor, insira sua senha!")
		end
	else
		triggerClientEvent(client,"set_warning_text",client,"Login","Por favor, insira seu usuário!")
	end
	return false
end
			...
			if result and client then
				if #result > 0 then
					...
					if exports.bans:checkAccountBan(accountData["id"]) then
						triggerClientEvent(client,"set_warning_text",client,"Login","Conta Banida")
					...
					if tonumber(accountData["activated"]) == 0 then
						triggerClientEvent(client,"set_warning_text",client,"Login","Conta '".. username .."' não esta ativada.")
						return false
					end

					--Validation is done, fetching some more details
					triggerClientEvent(client,"set_authen_text",client,"Login","Conta autenticada!")
					...
					for _, thePlayer in ipairs(exports.pool:getPoolElementsByType("player")) do
						...
						if (playerAccountID) then
							if (playerAccountID == tonumber(accountData["id"])) and (thePlayer ~= client) then
								kickPlayer(thePlayer, thePlayer, "Alguém mais acessou sua conta.")
								triggerClientEvent(client,"set_authen_text",client,"Login","A conta está online no momento. Desconectando outro usuário..")
								break
							end
						end
					end
					...
					triggerClientEvent(client,"set_warning_text",client,"Login","Nome da conta '".. username .."' não existe!")
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Login","Falha ao conectar ao servidor do jogo. Erro de banco de dados!")
				dbFree(qh)
			end
	...
	dbQuery(function(qh, username, password, confirmPassword, email, client)
		...
		if result then
			if #result == 0 then
				...
				if not Q2 then
					triggerClientEvent(client,"set_warning_text",client,"Registro","Ocorreu o código de erro 0003.")
					return false
				end

				local usernameExisted = mysql:fetch_assoc(Q2)
				if (mysql:num_rows(Q2) > 0) and usernameExisted["id"] ~= "1" then
					triggerClientEvent(client,"set_warning_text",client,"Registro","Múltiplas contas não são permitidas (Existente: "..tostring(usernameExisted["username"])..")")
					return false
				end
					...
					dbQuery(function(qh, client)
						...
						if result and #result == 1 then
							if dbExec(exports.mysql:getConn("mta"), "INSERT INTO account_details SET `account_id`=?, `mtaserial`=?", result[1].id, mtaSerial) then
								...
								triggerClientEvent(client,"set_warning_text",client,"Registro","Ocorreu o código de erro 0005.")
							end
						else
							triggerClientEvent(client,"set_warning_text",client,"Registro","Ocorreu o código de erro 0004.")
							dbFree(qh)
						end
					end, {client}, exports.mysql:getConn("core"), "SELECT id FROM accounts WHERE username=?", username)
				else
					triggerClientEvent(client,"set_warning_text",client,"Registro","Não foi possível criar uma nova conta.")
					return false
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Registro","Nome de usuário ou e-mail existente.")
			end
		else
			triggerClientEvent(client,"set_warning_text",client,"Registro","Ocorreu o código de erro 0002.")
			dbFree(qh)
		end
