...
function adminLoginToPlayerCharacter(thePlayer, commandName, ...)
    if exports.integration:isPlayerLeadAdmin(thePlayer) then
        if not (...) then
            outputChatBox("SYNTAX: /" .. commandName .. " [Nome Pesonagem Exato]", thePlayer, 255, 194, 14, false)
            outputChatBox("Loga no personagem do jogador.", thePlayer, 255, 194, 0, false)
        else
            ...
            if not fetchData then
                outputChatBox("Nenhum nome de personagem encontrado.", thePlayer, 255,0,0)
                return false
            end
            ...
            if result and #result == 1 then
                ...
                if targetCharID and  targetUserID then
                    ...
                    if targetAdminLevel > theAdminPower then
                        ...
                        outputChatBox("Você não pode entrar no personagem de um administrador de nível superior do que você.", thePlayer, 255,0,0)
                        exports.global:sendMessageToAdmins("[LOGINTO]: " .. tostring(adminTitle) .. " tentou entrar no personagem de administrador de alto escalão ("..targetUsername..").")
                        return false
                    end
                    ...
                    exports.global:sendMessageToAdmins("[LOGINTO]: " .. tostring(adminTitle) .. " fez login na conta '"..targetUsername.."'.")
                end
            end
        end
    end
...
function spawnCharacter(characterID, remoteAccountID, theAdmin, targetAccountName, location, freshSpawn)
    ...
    if characterData then
        ...
        -- FOME E SEDE
        setElementDataEx(client, "char.Hunger", tonumber(characterData["fome"]))
        setElementDataEx(client, "char.Thirst", tonumber(characterData["sede"]))
        ...
        if isElement(playerWithNick) and (playerWithNick~=client) then
            if theAdmin then
                ...
                kickPlayer(playerWithNick, getRootElement(), adminTitle.." "..adminUsername.." entrou na sua conta.")
                outputChatBox("Account "..targetAccountName.." ("..tostring(characterData["charactername"]):gsub("_"," ")..") foi expulso do jogo.", theAdmin, 0, 255, 0 )
            else
                kickPlayer(playerWithNick, getRootElement(), "Alguém mais se conectou ao seu personagem.")
            end
        end
        ...
        if result and num_affected_rows > 0 then
            ...
            if duty > 0 then
                ...
                if not foundPackage then
                    ...
                    outputChatBox("Você não tem mais acesso à tarefa que está usando - portanto, removido.", client, 255, 0, 0)
                end
            end
        ...
        if theAdmin then
            local adminTitle = exports.global:getPlayerAdminTitle(theAdmin)
            local adminUsername = getElementData(theAdmin, "account:username")
            outputChatBox("Você entrou no personagem do jogador com sucesso!", theAdmin, 0, 255, 0 )
            local hiddenAdmin = getElementData(theAdmin, "hiddenadmin")
            if hiddenAdmin == 0 then
                exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " "..adminUsername.." logado em outra conta ("..targetAccountName..") "..tostring(characterData["charactername"]):gsub("_"," ")..".")
            end
        else
        ...
        if (tonumber(characterData["blindfold"])==1) then
            ...
            outputChatBox("Seu personagem está vendado. Se esta foi uma ação OOC, entre em contato com um administrador via F2.", client, 255, 194, 15)
        ...
        if (tonumber(characterData["cuffed"])==1) then
            outputChatBox("Seu personagem está algemado.", client, 255, 0, 0)
        end
...
function Characters_onCharacterChange()
    ...
    exports.logs:dbLog("ac"..tostring(clientAccountID), 27, { "ac"..tostring(clientAccountID), client } , "Foi para a seleção de personagens" )
