...
local label = guiCreateLabel( 0, 0, screenX, 15, "SERP Versão Script "..exports.global:getScriptVersion().."", false )
...
--[[addEventHandler('onClientGUIClick', label, function()
	...
	setClipboard(url)
	outputChatBox("Copiado '".. url .. "' para a área de transferência!")
end, false)]]

function updateSoundLabel(setting)
	if setting == 0 then 
		guiSetText(toggleSoundLabel, "Desativar Musica Login")
	else 
		guiSetText(toggleSoundLabel, "Ativar Musica Login")
	end
end
...
function stopNameChange(oldNick, newNick)
	if (source==getLocalPlayer()) then
		...
		if (oldNick~=newNick) and (legitNameChange==0) then
			...
			outputChatBox("Clique em 'Alterar personagem' se desejar alterar sua identidade no roleplay.", 255, 0, 0)
		end
	end
end
...
addEventHandler("account:showMusicLabel", root, function()
	toggleSoundLabel = guiCreateLabel( 0, 0, screenX, 15, "Habilitar música de login", false )
