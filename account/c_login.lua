...
local loginTitleText = "Southeast Roleplay MTA"
local old_screen
...
function LoginScreen_Register()
	...
	if (string.len(username)<3) then
		LoginScreen_showWarningMessage( "Seu nome de usuário deve ter no mínimo 3 caracteres!" )
	elseif (string.find(username, ";", 0)) or (string.find(username, "'", 0)) or (string.find(username, "@", 0)) or (string.find(username, ",", 0)) or (string.find(username, " ", 0)) then
		LoginScreen_showWarningMessage("Seu nome de usuário não pode conter ;,@.' ou espaço!")
	elseif (string.len(password)<6) then
	    LoginScreen_showWarningMessage("Sua senha é muito curta. \n Você deve inserir 6 ou mais caracteres.", 255, 0, 0)
    elseif (string.len(password)>=30) then
        LoginScreen_showWarningMessage("Sua senha é muito longa. \n Você deve inserir menos de 30 caracteres.", 255, 0, 0)
    elseif (string.find(password, ";", 0)) or (string.find(password, "'", 0)) or (string.find(password, "@", 0)) or (string.find(password, ",", 0)) then
        LoginScreen_showWarningMessage("Sua senha não pode conter ;,@'.", 255, 0, 0)
	else
		showChat(true)
...
function checkCredentials()
	...
	if (string.len(username)<3) then
		outputChatBox("Seu nome de usuário é muito curto. Você deve inserir 3 ou mais caracteres.", 255, 0, 0)
	else
...
function LoginScreen_showWarningMessage( message )
	...
	warningBox = guiCreateWindow( x*.5-150, y*.5-65, 300, 120, "Atenção!", false )
...
function defaultLoginText()
	loginTitleText = "Southeast Roleplay"
end
...
function showPasswordUpdate()
	showCursor(true)
	Window[1] = guiCreateWindow(0.3562,0.3997,0.2891,0.2383,"AVISO DE SEGURANÇA:",true)
		guiSetInputEnabled ( true)
		Label[1] = guiCreateLabel(0.0378,0.153,0.9324,0.2404,"Percebemos uma possível falha de segurança em sua conta.\nPara ajudar a prevenir qualquer perda de dados, é altamente recomendável que\nvocê insira uma nova senha na caixa abaixo!",true,Window[1])
		...
		Label[2] = guiCreateLabel(0.1649,0.4754,0.2432,0.1038,"Nova senha:",true,Window[1])
		Label[3] = guiCreateLabel(0.1216,0.6284,0.2784,0.1038,"Confirmar senha:",true,Window[1])
		Button[1] = guiCreateButton(0.427,0.8087,0.527,0.1257,"Mudar Senha",true,Window[1])
