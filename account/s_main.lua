...
function resourceStart(resource)
	...
	setRuleValue("Versão Script", exports.global:getScriptVersion())
	setRuleValue("Author", "Southeast Roleplay - STAFF")
	setRuleValue("Website", "breve")
...
function resetNick(oldNick, newNick)
	...
	exports.global:sendMessageToAdmins("AdmWrn: " .. tostring(oldNick) .. " tentou mudar seu nome para " .. tostring(newNick) .. ".")
end
addEvent("resetName", true )
addEventHandler("resetName", getRootElement(), resetNick)

function makeOwlName(thePlayer)
	setPlayerName(thePlayer, "LY.Jogador."..tostring(math.random(0,9))..tostring(math.random(0,9))..tostring(math.random(0,9))..tostring(math.random(0,9)))
end
