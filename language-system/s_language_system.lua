...
function learnLanguage(player, lang, showmessages, skill)
	...
	if (hasLanguage) then
		if showmessages then
			outputChatBox("Você já sabe " .. languages[lang] .. ".", player, 255, 0, 0)
		end
		...
		return false, "Já sabe " .. languages[lang]
	else
		local freeslot = getNextEmptyLanguageSlot(player)

		if (freeslot==0) then
			if showmessages then
				outputChatBox("Você não tem espaço suficiente para aprender este idioma.", player, 255, 0, 0)
			end
...
function useLanguage(lang)
	...
	if (hasLanguage) then
		outputChatBox("Você agora está usando " .. getLanguageName(lang) .. " como seu idioma.", source, 255, 194, 14)
...
function unlearnLanguage(lang)
	...
	if (hasLanguage) then
		...
		outputChatBox("Você desaprendeu " .. getLanguageName(lang) .. ".", source, 255, 194, 14)
...
for key, value in pairs( languages ) do
	function speakLang( thePlayer, commandName, ... )
		if doesPlayerHaveLanguage( thePlayer, key ) then
			...
			outputChatBox( "Você não fala " .. value .. ".", thePlayer, 255, 0, 0 )
		end
	end
