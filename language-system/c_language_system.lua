...
function displayGUI(remotelanguages, rcurrslot)
	...
	if not (wLanguages) then
		...
		wLanguages = guiCreateWindow(x, y, width, height, "Idiomas: " .. string.gsub(getPlayerName(localPlayer), "_", " "), false)
		...
		for i = 1, 3 do
			...
			if L then
				...
				if languages[lang] then
					...
					if currslot == i then
						guiSetText(lLangName, guiGetText(lLangName) .. " (Atual)")
					else
						local bUse = guiCreateButton(0.83, 0.08+offset, 0.2, 0.05, "Usar", true, wLanguages)
				...
				if currslot ~= i then
					local bUnlearnLang = guiCreateButton(0.83, 0.14+offset, 0.2, 0.05, "Desaprender", true, wLanguages)
		...
		showCursor(true)
		local bClose = guiCreateButton(0.05, 0.92, 0.9, 0.07, "Fechar", true, wLanguages)
...
function unlearnLanguage(lang)
	if lang > 0 then
		if not languages[lang] then
			...
			wConfirmUnlearn = guiCreateWindow(sx/2 - 125,sy/2 - 50,250,100,"Saindo da Confirmação", false)
			local lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3,"Você realmente quer esquecer todo o seu conhecimento sobre " .. getLanguageName( lang ) .. "?",true,wConfirmUnlearn)
			guiLabelSetHorizontalAlign (lQuestion,"center",true)
			local bYes = guiCreateButton(0.1,0.65,0.37,0.23,"Sim",true,wConfirmUnlearn)
			local bNo = guiCreateButton(0.53,0.65,0.37,0.23,"Não",true,wConfirmUnlearn)
