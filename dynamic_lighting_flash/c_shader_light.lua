...
function toggleLight( state )
	if state == switch_key then
		...
		exports.hud:sendBottomNotification(localPlayer, "Flashlight", ( isLightOn and "ON" or "OFF" )..". Bateria: "..batt.."%", 255, 194, 14)
	end
end
		...
		if state then
			if not isHolding(localPlayer) then
				if getPedOccupiedVehicle ( localPlayer ) then
					playSoundFrontEnd(4)
					exports.hud:sendBottomNotification(localPlayer, "Flashlight", "Você não pode usar lanterna em veículos.", 255, 194, 14)
				elseif getPedWeaponSlot ( localPlayer ) ~= 0 then
					playSoundFrontEnd(4)
					exports.hud:sendBottomNotification(localPlayer, "Flashlight", "Suas mãos estão ocupadas.", 255, 194, 14)
				else
					...
					exports.hud:sendBottomNotification(localPlayer, "Flashlight", "Pressione L para ligar.", 255, 194, 14)
				end
			end
		else
...
function weaponSwitch ( prevSlot, newSlot )
	if isHolding(localPlayer) and newSlot ~= 0 then
		playSoundFrontEnd(4)
		exports.hud:sendBottomNotification(localPlayer, "Weapon", "Você não pode usar enquanto segura uma lanterna.", 255, 194, 14)
		cancelEvent()
	end
end
