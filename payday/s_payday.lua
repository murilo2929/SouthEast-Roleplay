		...
		if totalInsFee > 0 then
			if bankmoney >= totalInsFee then
				...
				for factionId, data in pairs(totalInsFeePerFactions) do
					if data.fee > 0 then
						...
						if exports.bank:giveBankMoney(theFaction, data.fee) then
							exports.bank:addBankTransactionLog(dbid, -factionId, data.fee, 14, "Taxas de seguro para "..(#(data.vehs)).." veículos: "..tostring(table.concat(data.vehs, ", "))..".")
						end
					end
				end
			else
				...
				if  exports.global:isResourceRunning("announcement") then
					...
					for factionId, data in pairs(totalInsFeePerFactions) do
						local details = 'Lista de veículos segurados do cliente '..customerName..":\n\n"
						for i, vehid in pairs(data.vehs) do
							details = details..'Veículo VIN #'..vehid.."\n"
						end
						details = details.."\nTotal: $"..exports.global:formatMoney(data.fee)
						exports.factions:sendNotiToAllFactionMembers(factionId, customerName.." falhou em pagar suas taxas totais de seguro de $"..exports.global:formatMoney(data.fee).." sobre seu veículo "..(#(data.vehs))..".", details)
					end
				end
			end
		end
		...
		for int_id, cost in pairs( rentalHouses[ dbid ] ) do
			if bankmoney >= cost then
				...
				if exports.global:isResourceRunning("interior_system") then
					...
					local characterName = exports.cache:getCharacterNameFromID(dbid) or "Ninguem"
					if account then
						exports.announcement:makePlayerNotification(account.id, characterName.." não pode pagar o aluguel do interior ID #"..int_id..".", "Você perdeu este interior porque o contrato de locação foi quebrado.\nNão pode pagar $"..exports.global:formatMoney(cost).." para o aluguel do interior.", "interior_inactivity_scanner")
					end
					-- leave some history for administrative purposes.
					exports["interior-manager"]:addInteriorLogs(int_id, "Quebrou o contrato de aluguel (Não foi possível pagar $"..exports.global:formatMoney(cost)..")", player)
				end
			end
		end
	end
...
function payAllWages(isForcePayday)
					...
					if not exports.vehicle:isProtected(veh) then
						if active then
							if reason and tonumber(reason) then
								...
								local characterName = exports.cache:getCharacterNameFromID(owner) or "Ninguem"
								...
								if owner > 0 and account then
									...
									exports.announcement:makePlayerNotification(account.id, "AVISO! Veículo ID #"..vehid.." ("..name..") está prestes a ser tirado da posse de "..characterName.." pelo scanner de inatividade do veículo", "Seu veículo foi marcado como inativo porque seu personagem não está logado no jogo há mais de 30 dias ou porque ninguem ligou o motor por mais de 14 dias enquanto estacionava ao ar livre. \n\nUm veículo inativo é um desperdício de recursos e, até agora, a propriedade do veículo foi removida ou retirada de sua posse para dar a outros jogadores a oportunidade de comprá-lo e usá-lo com mais eficiência.\n\nEste veículo não estava desprotegido. Para evitar que isso aconteça novamente com outros veículos seus, você pode gastar seu (s) GC (s) para protegê-lo do scanner de veículo inativo.\n\nVocê tem exatamente "..remainTime.." para reativar seu veículo antes que seja tarde demais!", "vehicle_inactivity_scanner")
								end
							end
						else
							local name = exports.global:getVehicleName(veh)
							if exports.vehicle_manager:systemDeleteVehicle(vehid, "Excluído pelo Scanner de Inatividade. Razão: "..reason) then
								inactiveVehicles.processed = inactiveVehicles.processed + 1
								local account = exports.cache:getAccountFromCharacterId(owner) or {id = 0, username="No-one"}
								local characterName = exports.cache:getCharacterNameFromID(owner) or "Ninguem"
								if owner > 0 and account then
									exports.announcement:makePlayerNotification(account.id, "Veículo ID #"..vehid.." ("..name..") foi tirado de "..characterName.."'s possession pelo scanner de inatividade do veículo.", "Razão: "..reason..". Seu veículo foi marcado como inativo porque seu personagem não está logado no jogo há mais de 30 dias ou porque ninguem ligou o motor por mais de 14 dias enquanto estacionava ao ar livre. \n\nUm veículo inativo é um desperdício de recursos e, até agora, a propriedade do veículo foi removida ou retirada de sua posse para dar a outros jogadores a oportunidade de comprá-lo e usá-lo com mais eficiência.\n\nEste veículo não estava desprotegido. Para evitar que isso aconteça novamente com outros veículos seus, você pode gastar seu (s) GC (s) para protegê-lo do scanner de veículo inativo.", "vehicle_inactivity_scanner")
								end
								--exports.global:sendMessageToAdmins("[VEÍCULO] Vehicle ID #"..vehid.." ("..name..", owner: "..characterName.." - "..account.username..") has been deleted pelo scanner de inatividade do veículo. "..reason)
	...
	if inactiveVehicles.processed > 0 then
		exports.global:sendMessageToAdmins("[VEÍCULO] O Inactivity Scanner acaba de excluir "..inactiveVehicles.processed.." veículos inativos e detectados "..inactiveVehicles.soon.." veículos possivelmente se tornando inativos nas próximas 12 horas.")
	end
			...
			if not exports.interior_system:isProtected(property) then
				if active then
					if reason and tonumber(reason) then
						...
						if owner > 0 and account then
							local remainTime = exports.datetime:formatSeconds(reason)
							exports.announcement:makePlayerNotification(account.id, "AVISO! Interior ID #"..intid.." ("..name..") está prestes a ser tirado da posse de "..characterName.." pelo scanner de inatividade interior", "Seu interior foi marcado como inativo porque nenhum corpo entrou nele nos últimos 14 dias ou seu personagem (quem o possui) não entrou no jogo por 30 dias.\n\nUm interior inativo é um desperdício de recursos e, até agora, a propriedade do interior foi retirada de sua posse para dar a outros jogadores a oportunidade de comprá-lo e usá-lo com mais eficiência.\n\nEste interior não estava desprotegido. Para evitar que isso aconteça novamente com outros interiores seus, você pode gastar seu (s) GC (s) para protegê-lo do scanner de interior inativo.\n\nVocê tem exatamente "..remainTime.." para reativar sua propriedade antes que seja tarde demais!", "interior_inactivity_scanner")
						end
					end
				else
					if exports.interior_system:unownProperty(intid, "Forcesold by Inactivity Scanner. Razão: "..reason) then
						local account = exports.cache:getAccountFromCharacterId(owner) or {id = 0, username="No-one"}
						local characterName = exports.cache:getCharacterNameFromID(owner) or "Ninguem"
						if owner > 0 and account then
							exports.announcement:makePlayerNotification(account.id, "Interior ID #"..intid.." ("..name..") foi tirado de "..characterName.." pelo scanner de inatividade interior.", "Razão: "..reason..". Seu interior foi marcado como inativo porque nenhum corpo entrou nele nos últimos 14 dias ou seu personagem (quem o possui) não entrou no jogo por 30 dias.\n\nUm interior inativo é um desperdício de recursos e, até agora, a propriedade do interior foi retirada de sua posse para dar a outros jogadores a oportunidade de comprá-lo e usá-lo com mais eficiência.\n\nEste interior não estava desprotegido. Para evitar que isso aconteça novamente com outros interiores seus, você pode gastar seu (s) GC (s) para protegê-lo do scanner de interior inativo.", "interior_inactivity_scanner")
						end
	...
	if inactiveInteriors.processed > 0 then
		exports.global:sendMessageToAdmins("[INTERIOR] O Inactivity Scanner acabou de forçar "..inactiveInteriors.processed.." interiores inativos e detectados "..inactiveInteriors.soon.." interiores possivelmente se tornando inativos nas próximas 12 horas.")
	end
...
function doPayDayPlayer(value, isForcePayday)
	...
	if ((logged==1) and (timeinserver>=58) and (getPlayerIdleTime(value) < 600000)) or isForcePayday then
		...
		if getElementData(value, "referrer") and getElementData(value, "hoursplayed") == 50 then
			...
			exports.global:sendMessageToAdmins("[ACHIEVEMENT] Player '"..exports.cache:getUsernameFromId(getElementData(value, "referrer")).."' foi recompensado com "..gc2Award.." GC(s) por indicar o amigo dele '"..getElementData(value, "account:username").."' que atingiu 50 horas jogadas no personagem '"..exports.global:getPlayerName(value).."'! ")
			exports.announcement:makePlayerNotification(getElementData(value, "referrer"), "Parabéns! Você foi recompensado com "..gc2Award.." GC(s) por indicar seu amigo", getElementData(value, "account:username").." has reached 50 hoursplayed on character "..exports.global:getPlayerName(value)..".")
			exports.announcement:makePlayerNotification(getElementData(value, "account:id"), exports.cache:getUsernameFromId(getElementData(value, "referrer")).." foi recompensado com "..gc2Award.." GC(s) por indicar você", "Você atingiu 50 horas jogadas no personagem "..exports.global:getPlayerName(value).."! Parabéns e obrigado por jogar.")
		end
	...
	elseif (logged==1) and (timeinserver) and (timeinserver<60) then
		outputChatBox("Você não jogou o tempo suficiente para receber um pagamento. (Você precisa de mais " .. 60-timeinserver .. " minutos.)", value, 255, 0, 0)
	end
end
			...
			if (logged==1) then
				...
				if targetPlayer then
					if getElementData(targetPlayer, "loggedin") == 1 then
						outputChatBox("Dia de pagamento forçado com sucesso para o jogador " .. getPlayerName(targetPlayer):gsub("_", " "), thePlayer, 0, 255, 0)
						exports.global:sendMessageToAdmins("[PAYDAY]: " .. exports.global:getPlayerFullIdentity(thePlayer) .. " forçou o dia de pagamento para o jogador " .. getPlayerName(targetPlayer))
					...
					outputChatBox("Falha ao forçar dia de pagamento.", thePlayer, 255, 0, 0)
					return
				end
			end
		end
