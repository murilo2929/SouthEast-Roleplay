...
addEventHandler("onResourceStart", getRootElement(), initStuff)
	...
	if playerVehicle then
		if (exports.vehicle:isVehicleWindowUp(playerVehicle)) then
			if masked then
				...
				outputChatBox("Mascarado (" ..playerID.. ") [Janelas Fechadas] diz: " .. message, source, unpack(color))
			else
				table.insert(affectedElements, playerVehicle)
				outputChatBox("" .. playerName .. " [Janelas Fechadas] diz: " .. message, source, unpack(color))
			end
		else
			if masked then
				outputChatBox("Mascarado (" ..playerID.. ") diz: " .. message, source, unpack(color))
			else
				outputChatBox("" .. playerName .. " diz: " .. message, source, unpack(color))
			end
		end
	else
			...
			if (nearbyPlayerDimension==dimension) and (nearbyPlayerInterior==interior) then
				...
				if not (isPedDead(nearbyPlayer)) and (logged==1) and (nearbyPlayer~=source) then
					...
					if nbpveh and exports.vehicle:isVehicleWindowUp(nbpveh) == true then
						...
						outputChatBox(" [" .. languagename .. "] " .. playerName .. " diz: " .. message2, nearbyPlayer, unpack(color))]]
						...
						if masked then
							outputChatBox("Mascarado (" ..playerID.. ") diz: " .. message2, nearbyPlayer, unpack(color))
							table.insert(affectedElements, nearbyPlayer)
						else
							outputChatBox("" .. playerName .. " diz: " .. message2, nearbyPlayer, unpack(color))
...
function meEmote(source, cmd, ...)
	...
	if logged == 1 then
		...
		if not (...) then
			outputChatBox("SYNTAX: /me [Ação]", source, 255, 194, 14)
		else
...
function outputChatBoxCar( vehicle, target, text1, text2, color )
	if vehicle and exports.vehicle:isVehicleWindowUp( vehicle ) then
		if getPedOccupiedVehicle( target ) == vehicle then
			outputChatBox( text1 .. " [Janelas Fechadas]" .. text2, target, unpack(color))
			return true
		else
			return false
		end
...
function radio(source, radioID, message, cmd)
	...
	if jailed > 0 then
		outputChatBox('Você não pode usar um rádio na prisão.', source, 255, 0, 0)
		return
	end
	...
	if isPedDead(source) then
		outputChatBox(" Você não pode usar seu rádio quando estiver morto.", source, 255, 0, 0)
		return
	end
	...
	if hasRadio or getElementType(source) == "ped" or radioID == -2 then
		...
		if theChannel == 1 or theChannel == 0 then
			outputChatBox("Sintonize seu rádio primeiro com /sintonizarradio [canal]", source, 255, 194, 14)
		elseif not hasChannelAccess(source, theChannel) then
			outputChatBox("Você não tem permissão para acessar este canal. Sintonize novamente o seu rádio.", source, 255, 194, 14)
		elseif (theChannel > 1 or radioID < 0) and itemValue > 0 then
			...
			if channelName == "COUNTY MED" then
				outputChatBoxCar(getPedOccupiedVehicle( source ), source, "[" .. channelName .. "] " .. username, " diz: " .. message, {60,179,113})
			elseif channelName == "DEPARTAMENTO" then
				outputChatBoxCar(getPedOccupiedVehicle( source ), source, "[" .. channelName .. "] " .. username, " diz: " .. message, {r,162,b})
			else
				if cmd == "rlow" then
					outputChatBoxCar(getPedOccupiedVehicle( source ), source, "[" .. channelName .. "] " .. username, " sussurros: " .. message, {r,g,b})
					triggerEvent("sendAme", source, "sussurra em seu rádio.")
				else
					outputChatBoxCar(getPedOccupiedVehicle( source ), source, "[" .. channelName .. "] " .. username, " diz: " .. message, {r,g,b})
				end
			end
			...
			for key, value in ipairs(affectedElements) do
				...
				if value ~= source then
					...
					if channelName == "COUNTY MED" then
						outputChatBoxCar( getPedOccupiedVehicle( value ), value, "[" .. channelName .. "] " .. username, " diz: " .. trunklateText( value, message2 ), {60,179,113} )
					elseif channelName == "DEPARTAMENTO" then
						outputChatBoxCar( getPedOccupiedVehicle( value ), value, "[" .. channelName .. "] " .. username, " diz: " .. trunklateText( value, message2 ), {r,162,b} )
					else
						outputChatBoxCar( getPedOccupiedVehicle( value ), value, "[" .. channelName .. "] " .. username, " diz: " .. trunklateText( value, message2 ), {r,g,b} )
					end
						...
						for k, v in ipairs(exports.global:getNearbyElements(value, "player",7)) do
							...
							if (logged2==1) then
								...
								if not found then
									...
									local text1 = "Radio de " .. getPlayerName(value) .. "."
			...
			for key, value in ipairs(getElementsByType("player")) do
				if cmd == "rlow" and getElementDistance(source, value) < 3 or getElementDistance(source, value) < 10 then
					if (value~=source) then
						local message2 = call(getResourceFromName("language-system"), "applyLanguage", source, value, message, language)
						local text1 = "[RADIO]" .. getPlayerName(source) .. ""
						if cmd == "rlow" then
							text2 = " sussurros: " .. trunklateText( value, message2 )
						else
							text2 = " diz: " .. trunklateText( value, message2 )
						end
			...
			outputChatBox("Seu radio esta desligado. ((/toggleradio))", source, 255, 0, 0)
		end
	else
		outputChatBox("Você não tem um radio.", source, 255, 0, 0)
	end
end
...
function scripterChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerScripter(thePlayer))  then
        if not (...) then
            outputChatBox("SYNTAX: /ss [Mensagem]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
...
function vctChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerVCTMember(thePlayer))  then
        if not (...) then
            outputChatBox("SYNTAX: /v [Mensagem]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
            local username = getElementData(thePlayer,"account:username")
...
function mappingTeamChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerMappingTeamMember(thePlayer))  then
        if not (...) then
            outputChatBox("SYNTAX: /mt [Mensagem]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
            local username = getElementData(thePlayer,"account:username")
...
function fmtChat(thePlayer, commandName, ...)
    ...
    if(logged==1) and (exports.integration:isPlayerFMTMember(thePlayer))  then
        if not (...) then
            outputChatBox("SYNTAX: /fmt [Mensagem]", thePlayer, 255, 194, 14)
        else
            local message = table.concat({...}, " ")
            local players = exports.pool:getPoolElementsByType("player")
            local username = getElementData(thePlayer,"account:username")
...
function pmPlayer(thePlayer, commandName, who, ...)
	...
	if tostring(commandName):lower() == "quickreply" and who then
		...
		if not target or not isElement(target) or not (getElementType(target) == "player") or not (getElementData(target, "loggedin") == 1) then
			outputChatBox("Ninguem te enviou PM.", thePlayer, 200,200,200)
			return false
		end
		...
		if not (who) or not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador] [Mensagem]", thePlayer, 255, 194, 14)
			outputChatBox("Pressione 'U' para responder um PM rapidamente.", thePlayer)
			return false
		end
		message = table.concat({...}, " ")
	end
	...
	if who and message then
		...
		if (targetPlayer) then
			...
			outputChatBox("PM enviado para (" .. targetid .. ") " .. targetPlayerName ..targetUsername.. ": " .. message, thePlayer, 255, 255, 0)
			...
			if senderPmPerk and tonumber(senderPmState) == 1 and not (getElementData(targetPlayer, "reportadmin") == thePlayer) then -- if sender has pms off.
				outputChatBox("Você está enviando mensagens privadas enquanto ignora as mensagens recebidas.", thePlayer, 200, 200, 200)
			end

			-- tells the PMing player if the target is afk.
			if getElementData(targetPlayer, "afk") then 
				outputChatBox("O jogador esta off.", thePlayer, 200, 200, 200)
			end
		end
	end
end
...
function localOOC(thePlayer, commandName, ...)
	...
	if (logged==1) and not (isPedDead(thePlayer)) then
		...
		if not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Mensagem]", thePlayer, 255, 194, 14)
		elseif (muted==1) then
			outputChatBox("Você esta mutado.", thePlayer, 255, 0, 0)
		else
			if(interior > 0 and dimension > 0) then
				...
				if blockOOC then
					if(exports.integration:isPlayerTrialAdmin(thePlayer) and exports.global:isStaffOnDuty(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) and exports.global:isStaffOnDuty(thePlayer)) then
						...
						exports.hud:sendBottomNotification(thePlayer, "OOC chat desativado", "O bate-papo OOC local é desabilitado neste interior, para promover a interpretação do personagem. Use /pm ao invés.")
						return false
					end
				end
			end
				...
				if (zonename==playerzone) and (dimension==playerdimension) and (interior==playerinterior) and getDistanceBetweenPoints2D(x, y, getElementPosition(value)) < 200 then
					...
					if (logged==1) then
						...
						if exports.integration:isPlayerTrialAdmin(value) then
							outputChatBox("Distrito: " .. message .. " ((".. playerName .."))", value, 226, 184, 255)
						else
							outputChatBox("Distrito: " .. message, value, 226, 184, 255)
						end
					end
				end
			end
...
function localDo(thePlayer, commandName, ...)
	...
	if logged==1 then
		if not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Ação/Evento]", thePlayer, 255, 194, 14)
		else
			local message = table.concat({...}, " ")
...
function localShout(thePlayer, commandName, ...)
	...
	if not (isPedDead(thePlayer)) and (logged==1) then
		if not (...) then
			...
			outputChatBox("" .. playerName .. " grita: " .. message .. "!", thePlayer, r, g, b)
				...
				if getElementDistance( thePlayer, nearbyPlayer ) < 40 then
					...
					if (nearbyPlayerDimension==dimension) and (nearbyPlayerInterior==interior) and (nearbyPlayer~=thePlayer) then
						...
						if (logged==1) and not (isPedDead(nearbyPlayer)) then
							...
							outputChatBox("" .. playerName .. " grita: " .. message2 .. "!", nearbyPlayer, r, g, b)
						end
					end
				end
			end
...
--addCommandHandler("gee", localShout, false, false)

function gritar(thePlayer, commandName, ...)
	if exports['freecam-tv']:isPlayerFreecamEnabled(thePlayer) then return end
					...
					if getElementDistance( thePlayer, nearbyPlayer ) < 40 then
						...
						if (nearbyPlayerDimension==dimension) and (nearbyPlayerInterior==interior) then
							...
							if (logged==1) and not (isPedDead(nearbyPlayer)) then
								...
								table.insert(affectedElements, nearbyPlayer)
								outputChatBox("" .. playerName .. " grita: " .. trunklateText(nearbyPlayer, message2), nearbyPlayer, 255, 255, 255)
							end
						end
					end
				end
...
addCommandHandler("g", gritar, false, false)
...
function megaphoneShout(thePlayer, commandName, ...)
	...
	if not (isPedDead(thePlayer)) and (logged==1) then
		...
		if types[2] or types[3] or types[4] or (exports.global:hasItem(thePlayer, 141)) or ( isElement(vehicle) and exports.global:hasItem(vehicle, 141) and (seat==1 or seat==0)) then
			...
			outputChatBox("Você não possui um megafone.", thePlayer, 255, 0 , 0)
		end
	end
end
	...
	if (logged==1) then
		if not (...) then
			...
			if not playerFaction then
				outputChatBox("Você não esta nessa facção.", thePlayer)
				return
			end
...
function sfpdHq(thePlayer, commandName, ...)
	...
	if factionID > 0 then
		...
		outputChatBox("Você deve estar em serviço.", thePlayer, 255, 0, 0)	
		return
	end
	...
	elseif #message == 0 then
		outputChatBox("SYNTAX: /hq [Mensagem]", thePlayer, 255, 194, 14)
	else
		...
		for key, value in ipairs(teamPlayers) do
				...
				outputChatBox("[#911] Dispatcher diz: ".. message .."", value, 255, 255, 144)
		end
	end
end
	...
	if (logged==1) then
		if not (...) then
			...
			if not exports.factions:hasMemberPermissionTo(thePlayer, playerFaction, "use_fl") then
				outputChatBox("Você não é um lider de facção.", thePlayer, 255, 0, 0)
			else
				local theTeam = exports.factions:getFactionFromID(playerFaction)
				local theTeamName = getTeamName(theTeam)
...
function govooc(thePlayer, commandName, ...)
	...
	if types[2] or types[3] or types[4] and (logged==1) then
		...
		if not (...) then
			outputChatBox("SYNTAX: /gooc [Mensagem]", thePlayer, 255, 194, 14)
		else
			local affectedElements = { }
			local message = table.concat({...}, " ")
			local players = exports.pool:getPoolElementsByType("player")
...
function setRadioChannel(thePlayer, commandName, slot, channel)
	...
	if not (channel) then
		outputChatBox("SYNTAX: /" .. commandName .. " [Radio Slot] [Numero Canal]", thePlayer, 255, 194, 14)
	else
		if (exports.global:hasItem(thePlayer, 6)) then
						...
						if tonumber(v[2]) > 0 then
							if channel > 1 and channel < 1000000000 and hasChannelAccess(thePlayer, channel) then
								if exports['item-system']:updateItemValue(thePlayer, k, channel) then
									outputChatBox("Você sintonizou novamente seu rádio para o canal #" .. channel .. ".", thePlayer)
									triggerEvent('sendAme', thePlayer, "sintoniza o radio.")
								end
							else
			...
			outputChatBox("Você não tem tantos rádios.", thePlayer, 255, 0, 0)
		else
			outputChatBox("Voce nao tem um radio!", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("tuneradio", setRadioChannel, false, false)
addCommandHandler("sintonizarradio", setRadioChannel, false, false)
...
function toggleRadio(thePlayer, commandName, slot)
	if (exports.global:hasItem(thePlayer, 6)) then
		...
		if titemValue < 0 then
			outputChatBox("Você ligou seu rádio.", thePlayer, 255, 194, 14)
			triggerEvent('sendAme', thePlayer, "liga " .. genderm .. " radio.")
		else
			outputChatBox("Você desligou seu rádio.", thePlayer, 255, 194, 14)
			triggerEvent('sendAme', thePlayer, "desliga " .. genderm .. " radio.")
		end
		...
		outputChatBox("Você não tem um radio!", thePlayer, 255, 0, 0)
	end
end
...
function adminChat(thePlayer, commandName, ...)
	...
	if(logged==1) and (exports.integration:isPlayerTrialAdmin(thePlayer))  then
		if not (...) then
			outputChatBox("SYNTAX: /a [Mensagem]", thePlayer, 255, 194, 14)
		else
			local affectedElements = { }
			local message = table.concat({...}, " ")
			local players = exports.pool:getPoolElementsByType("player")
...
function payPlayer(thePlayer, commandName, targetPlayerNick, amount)
	...
	if (logged==1) then
		if not (targetPlayerNick) or not (amount) or not tonumber(amount) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador] [Quantia]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
			...
			if targetPlayer then
				...
				if (distance<=10 and not getElementData(targetPlayer, "reconx")) then
					...
					if (targetPlayer==thePlayer) then
						outputChatBox("Você não pode pagar a si mesmo.", thePlayer, 255, 0, 0)
					...
					elseif (hoursplayed<5) and (amount>50) and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerTrialAdmin(targetPlayer) and not exports.integration:isPlayerSupporter(thePlayer) and not exports.integration:isPlayerSupporter(targetPlayer) then
						outputChatBox("Você deve jogar pelo menos 5 horas antes de transferir mais de $ 50.", thePlayer, 255, 0, 0)
					elseif exports.global:hasMoney(thePlayer, amount) then
						if hoursplayed < 5 and not exports.integration:isPlayerTrialAdmin(targetPlayer) and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerSupporter(targetPlayer) and not exports.integration:isPlayerSupporter(thePlayer) then
							...
							if totalAmount > 200 then
								outputChatBox( "Você só pode /pagar $200 por cinco minutos. /report para um administrador transferir uma grande quantidade de dinheiro.", thePlayer, 255, 0, 0 )
								return
							end
						...
						if (hoursplayed<5) then
							exports.global:sendMessageToAdmins("AdmWarn: Novo jogador '" .. getPlayerName(thePlayer) .. "' transferiu $" .. exports.global:formatMoney(amount) .. " para '" .. targetPlayerName .. "'.")
						end
						...
						triggerEvent('sendAme', thePlayer, "pega algumas notas de dólar de sua carteira e dá para " .. targetPlayerName .. ".")
						outputChatBox("Você deu $" .. exports.global:formatMoney(amount) .. " para " .. targetPlayerName .. ".", thePlayer)
						outputChatBox(getPlayerName(thePlayer) .. " deu a você $" .. exports.global:formatMoney(amount) .. ".", targetPlayer)
...
function localWhisper(thePlayer, commandName, targetPlayerNick, ...)
	...
	if (logged==1) then
		if not (targetPlayerNick) or not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador / ID] [Mensagem]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
			...
			if targetPlayer then
				...
				if (targetPlayerDimension==dimension) and (targetPlayerInterior==interior) then
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)<3) then
						...
						triggerEvent('sendAme', thePlayer, "susurra para " .. targetPlayerName .. ".")
						...
						outputChatBox("" .. name .. " sussurra: " .. message, thePlayer, r, g, b)
						local r, g, b = 255, 255, 0
						...
						outputChatBox("" .. name .. " sussurra: " .. message2, targetPlayer, r, g, b)
											...
											if playerVeh then
												if pVeh then
													if pVeh==playerVeh then
														outputChatBox("" .. name .. " sussurra para " .. getPlayerName(targetPlayer):gsub("_"," ") .. ": " .. message2, p, 255, 255, 255)
													end
												end
											else
												outputChatBox("" .. name .. " sussurra para " .. getPlayerName(targetPlayer):gsub("_"," ") .. ": " .. message2, p, 255, 255, 255)
							...
							if not exports.vehicle:isVehicleWindowUp(playerCar) then
								if pveh then
									if playerCar == pveh then
										...
										outputChatBox( " " .. name .. " sussurros: " .. message2, targetPlayers, r, g, b)
...
function businessOOC(thePlayer, commandName, business, ...)
	...
	if (logged==1) then
		if not business then
			outputChatBox("SYNTAX: /" .. commandName .. " [Mensagem]", thePlayer, 255, 194, 14)
			outputChatBox("OR SYNTAX: /" .. commandName .. " [Business] [Mensagem]", thePlayer, 255, 194, 14)
		else
			local playerName = getPlayerName( thePlayer ):gsub( "_", " ")
			local message = table.concat({...}, " ")
...
function gpsFunc(thePlayer, cmd, vehID)
	...
	if not vehID then
		outputChatBox("#d91e1eAviso: #ffffff /" .. cmd .. " [Veiculo ID]", thePlayer, 194, 0, 0, true)
		return false
	end
		...
		if vehicle then
			...
			if vehicleOwner == playerID then
				...
				if gpsBlips[playerID] then
					outputChatBox("#d91e1eAviso: #ffffffVocê pode seguir seu veículo a partir da marca da bandeira vermelha no mapa.", thePlayer, 0, 255, 0, true)
				end
			else
				outputChatBox("#d91e1eAviso: #ffffffO veículo não é seu.", thePlayer, 0, 255, 0, true)
			end
		end
	else
		outputChatBox("#d91e1eAviso: #ffffffVocê já está localizando um veiculo. Utilize /eloc antes de poder usá-lo novamente.", thePlayer, 0, 255, 0, true)
	end
end
addCommandHandler("vloc", gpsFunc)
...
function kgpsFunc(thePlayer, cmd)
	...
	if gpsBlips[playerID] then
		...
		outputChatBox("#d91e1eAviso: #ffffffVocê desligou o GPS com sucesso.", thePlayer, 0, 255, 0, true)
	end
end
addCommandHandler("eloc", kgpsFunc)
