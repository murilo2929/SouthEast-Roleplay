...
local maxbubbles = 5
local selfVisible = false -- Want to see your own message?
	...
	function(message, command)
		if source ~= localPlayer or selfVisible then
			if command == "ado" or command == "ame" then
				addText(source, message, { 226, 184, 255 }, "default-bold", false, command)
			else
...
function updateStatus(source, n)
	...
	if getElementData(source, "chat:status") then
		addText(source, getElementData(source, "chat:status"), {255, 255, 255}, "default-bold", true, n)
	end
end
