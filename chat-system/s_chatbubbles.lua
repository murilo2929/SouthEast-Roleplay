...
function sendStatus(thePlayer, commandName, ...)
	...
	outputChatBox("Você ativou um status permanente. Para removê-lo digite /status.", thePlayer, 255, 194, 14)
...
function writeChatbubbleText( thePlayer, commandName, ... )
	...
	if getElementData(thePlayer, "statusSpam") == message then
		outputChatBox("SPAM: Seu /" .. commandName .. " não foi mostrado para prevenir spam. Tente novamente em alguns segundos.", thePlayer, 255, 0, 0)
		return nil
	else
...
function sendChatBubble(source, command, message, inChatBox)
	...
	elseif command == "ame" then
		...
		outputDebugString("Incapaz de adicionar mensagem de status " .. command .. "?")
		return
	end
