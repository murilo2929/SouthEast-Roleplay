...
function openRestrictedFreqs(dataFromServer)
	...
	showCursor(true)
	local w, h = 600, 300
	gui.wMain = guiCreateWindow(0,0,w,h,"Painel de frequência restrita - by "..author.." v"..scriptversion,false)
	...
	gui.colFreq = guiGridListAddColumn(gui.grid, "Frequencia", 0.3)
	gui.colLimitedto = guiGridListAddColumn(gui.grid, "Limitada para", 0.3)
	gui.colAddedby = guiGridListAddColumn(gui.grid, "Criada por", 0.2)
	...
	local xOffset = 0.01
	gui.newFreq = guiCreateButton(0, 0.88, btnWidth , 0.18, "Nova", true, gui.wMain)
	addEventHandler("onClientGUIClick", gui.newFreq, function ()
		newFreqWindow()
		exports.global:playSoundSuccess()
	end, false)

	gui.bDelete = guiCreateButton(btnWidth+xOffset, 0.88, btnWidth , 0.18, "Deletar", true, gui.wMain)
	addEventHandler("onClientGUIClick", gui.bDelete, function ()
		...
		if row ~= -1 and col ~= -1 then
			...
			exports.hud:sendBottomNotification(localPlayer, "Frequência Restrita", "Selecione um item da lista primeiro.")
		end
	end, false)

	gui.closeButton = guiCreateButton(btnWidth*2+xOffset, 0.88, btnWidth , 0.18, "Fechar", true, gui.wMain)
...
function newFreqWindow()
	...
	local w, h = 300, 120
	gui.newFreq = guiCreateWindow(0,0,w,h,"Criar nova frequência",false)
	...
	gui.lText2 = guiCreateLabel(margin, margin+yOffset+lineH, col1W, lineH, "Limitado para: ", false, gui.newFreq)
	...
	local xOffset = 0.01
	gui.addNewFreq = guiCreateButton(margin, margin+yOffset+lineH*2, btnWidth, 35, "Criar", false, gui.newFreq)
	addEventHandler("onClientGUIClick", gui.addNewFreq, function()
		if source == gui.addNewFreq then
			...
			if string.len(freq) < 1 or string.len(faction) < 1 or not tonumber(faction) then	
				exports.global:playSoundError()
				exports.hud:sendBottomNotification(localPlayer, "Frequência Restrita", "O campo de frequência ou facção está vazio.")
			else
	...
	gui.closeNewWindow = guiCreateButton(margin+btnWidth, margin+yOffset+lineH*2, btnWidth, 35, "Fechar", false, gui.newFreq)
