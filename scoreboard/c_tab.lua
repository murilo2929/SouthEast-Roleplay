...
drawScoreboard = function ()
	...
	local strPlayerCount = "Jogadores: " .. tostring(#g_players) .. "/" .. tostring(maxPlayers) .. " (" .. math.floor(usagePercent + 0.5) .. "%)"
	...
	-- Draw the headers
	drawRowBounded ( "ID", "Nome", "Horas", "Ping",
	...
	local renderEntry = function ( player, seeUsername )
		...
		local adminvermascarado = false

		if masked and seeUsername then
			drawRowBounded ( playerID, "(Mascarado) " ..playerName, playerHours, playerPing, colors, "default-bold", top )
			adminvermascarado = true
		end

		-- Render it!
		if not adminvermascarado then
			if masked then
				drawRowBounded ( playerID, "Mascarado", playerHours, playerPing, colors, "default-bold", top )
			else
