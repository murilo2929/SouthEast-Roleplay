	...
	function(id)
		...
		if id < 0 then -- default skin
			...
			if is_active_biz and not exports.npc:hasSupplies(int, 16) then
				return outputChatBox("Este item está fora de estoque.", client, 255,0,0)
			end

			if exports.global:hasMoney(client, math.ceil(50*discount))  then
				if exports.global:giveItem(client, 16, -id) then
					outputChatBox('Você comprou uma roupa por $' .. exports.global:formatMoney(50) .. '.', client, 0, 255, 0)
					...
					outputChatBox('Você não tem espaço suficiente em seu inventário.', client, 255, 0, 0)
				end
			else
				outputChatBox('Você não tem o necessário $' .. exports.global:formatMoney(clothing.price) .. '.', client, 255, 0, 0)
			end
		else
			...
			if clothing and isForSale(clothing) then
				...
				if exports.global:hasMoney(client, math.ceil(clothing.price*discount)) then
					...
					if exports.global:giveItem(client, 16, clothing.skin .. ':' .. clothing.id) then
						outputChatBox('Você comprou uma roupa por $' .. exports.global:formatMoney(math.ceil(clothing.price*discount)) .. '.', client, 0, 255, 0)
						paid = clothing.price
						-- take & give some money
					...
					outputChatBox('Você não tem o necessário $' .. exports.global:formatMoney(math.ceil(clothing.price*discount)) .. '.', client, 255, 0, 0)
				end

			elseif clothing then
				outputChatBox("Você não tem permissão suficiente para comprar estas roupas.", client, 255,0,0)
			else
				outputChatBox("Verificamos o link para este tema e descobrimos que ele não está mais disponível. Removemos de nossos sistemas, desculpe por isso!", client, 255, 0, 0)
			end
		end
	end)
...
function saveClothes(values, player, no_output)
	...
	if not values.id then
		...
		if result then
			...
			if no_output then
				return false, "Seu design foi salvo"
			else
				outputChatBox('Seu design foi salvo', player, 0, 255, 0)
			end
			return last_insert_id
		else
			if no_output then
				return false, "Incapaz de adicionar roupas."
			else
				outputChatBox('Incapaz de adicionar roupas.', player, 255, 0, 0)
			end
		end
	else
		-- old clothing stuff
		...
		if existing then
			...
			if result then
				...
				if no_output then
					return false, "Seu design foi atualizado"
				else
					outputChatBox('Seu design foi atualizado', player, 0, 255, 0)
				end
				return values.id
			else
				if no_output then
					return false, "Incapaz de salvar roupas."
				else
					outputChatBox('Incapaz de salvar roupas.', player, 255, 0, 0)
				end
			end
		else
			if no_output then
				return false, "Incapaz de encontrar roupas?"
			else
				outputChatBox('Incapaz de encontrar roupas?', player, 255, 0, 0)
			end
		end
	end
end
...
addEventHandler('clothing:delete', resourceRoot, function(id, event)
	if type(id) == 'number' and savedClothing[id] then
		...
		if client and event == "deleteMyClothes" then
			outputChatBox('Seu design foi excluído.', client, 0, 255, 0)
		end
