...
function getSkinBasicInfo(id)
	...
	if gender and race then
		return (race == 0 and 'Negro' or (race == 1 and 'Branco' or 'Asiatico'))..' '..(gender == 0 and 'homem' or 'mulher')..(restricted and ' (Restrito)' or ''), gender, race, restricted
	end
	return ''
end
...
function getStatus(clothes)
	...
	elseif clothes.distribution == 1 then
		return "Projeto (Privado)"
	elseif clothes.distribution == 2 then
		if clothes.mdate and clothes.mdate > 0 and clothes.mdate > exports.datetime:now() then
			return 'Chegando em '..exports.datetime:formatFutureTimeInterval(clothes.mdate)
		else
			return "Pessoal (Privado)"
		end
	elseif clothes.distribution == 3 then
		return "Publico"
	elseif clothes.distribution == 4 then
		if clothes.for_sale_until > exports.datetime:now() then
			return "Distribuído globalmente por "..exports.datetime:formatFutureTimeInterval(clothes.for_sale_until)
		else
			return "Distribuição expirada "..exports.datetime:formatTimeInterval(clothes.for_sale_until).." (Private)"
		end
	elseif clothes.distribution == 5 then
		return "Uniforme de facção"
	else
		return "Arquivado"
	end
end
...
function formatManuDate(clothes)
	...
	elseif clothes.mdate and clothes.mdate > 0 then
		return clothes.mdate > exports.datetime:now() and ('Chegando em '..exports.datetime:formatFutureTimeInterval(clothes.mdate)) or exports.datetime:formatTimeInterval(clothes.mdate)
	end
	return 'Nunca'
end
