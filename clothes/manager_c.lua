...
function clothesManager(item, list)
 	...
 	if mGui.window and isElement(mGui.window) then
 		...
 		local margin = 30
 		mGui.window = guiCreateWindow(screen_width - width - 45, screen_height - height-110, width, height, "Coleção ", false)
 		guiSetEnabled(mGui.window, false)
 		guiSetAlpha(mGui.window, 0.95)
 		guiSetSizable(mGui.window, false)

 		loading_label = guiCreateLabel(10, 25, width - 20, height - 60, "Carregando.." ,false, mGui.window)
 		guiLabelSetHorizontalAlign(loading_label, 'center')
 		guiLabelSetVerticalAlign(loading_label, 'center')

 		local close = guiCreateButton(width - 110, height - 30, 100, 25, 'Fechar', false, mGui.window)
