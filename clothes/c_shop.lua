...
function updateGrid(dopont_npc)
	...
	if not dopont_npc then
		...
		guiGridListSetItemText(grid, row, 2, v.description.." projetado por "..v.creator_charname, false, false)
...
function listClothes(item, list)
	...
	if window and isElement(window) then
		...
		local margin = 30
		window = guiCreateWindow(screen_width - width - 45, screen_height - height-110, width, height, "Coleção #"..(item.itemValue or ""), false)
...
function listCreateGuiElements(dopont_npc)
	...
	guiGridListAddColumn(grid, 'Descrição', 0.5)
	guiGridListAddColumn(grid, 'Coleção', 0.07)
	guiGridListAddColumn(grid, 'Preço', 0.1)
	if dopont_npc then
		guiGridListAddColumn(grid, 'Status', 0.3)
		guiGridListAddColumn(grid, 'Data de fabricação', 0.2)
	end
	guiGridListAddColumn(grid, 'Data Criação', 0.2)
	--guiGridListSetSortingEnabled ( grid, false ) -- disable sorting cuz it will cause indexing issue.

	local buy, add, make = nil
	if dopont_npc then
		add = guiCreateButton(width - 220, height - 30, 100, 25, 'Criar', false, window)
		make = guiCreateButton(width - 330, height - 30, 100, 25, 'Fabricar', false, window)
	...
	if dopont_npc then
		...
		addEventHandler('onClientGUIClick', make,
			function(button)
				if button == 'left' then
					if guiGetText(source) == 'Fabricar' then
