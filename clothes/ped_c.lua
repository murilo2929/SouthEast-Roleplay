...
list_ = {}
dupont_ped = createPed(12, 2094.8876953125, -1147.1083984375, 26.588254928589)
...
function openClothesWizard(ped)
	...
	GUIEditor.window[1] = guiCreateWindow(1031, 453, 262, 147+y, "Bem-vindo ao Star Fashion!", false)
	exports.global:centerWindow(GUIEditor.window[1])
	guiWindowSetSizable(GUIEditor.window[1], false)

	GUIEditor.button[1] = guiCreateButton(9, 32, 243, 32, "Meus designs pessoais.", false, GUIEditor.window[1])
	if y > 0 then
		GUIEditor.button[4] = guiCreateButton(9, 68, 243, 32, "Meus designs de facção.", false, GUIEditor.window[1])
	end
	GUIEditor.button[2] = guiCreateButton(9, 68+y, 243, 32, "Envie uma nova proposta de design "..(isModerator(localPlayer) and '' or '($25)'), false, GUIEditor.window[1])
	GUIEditor.button[3] = guiCreateButton(9, 104+y, 243, 32, "Não, obrigado.", false, GUIEditor.window[1])
...
function callback_startWizard_1(ok)
	if ok then
		...
		guiSetText(GUIEditor.button[2], "Envie uma nova proposta de design "..(isModerator(localPlayer) and '' or '($25)'))
...
function startWizard_1()
	if (getElementData(localPlayer, 'hoursplayed') or 0) < 2 then
		playSoundFrontEnd(4)
		outputChatBox("Você deve ter jogado pelo menos 2 horas neste personagem ou ativar a vantagem'"..exports.donators:getPerks(32)[1].."' para enviar um novo design.", 255,0,0)
	elseif not exports.global:hasMoney(localPlayer, 25) then
		playSoundFrontEnd(4)
		outputChatBox("Você precisa de $ 25 para enviar uma nova proposta de design.", 255,0,0)
	else
		closeWizard_1()
		closeClothesWizard()
		closeWindow()
		...
		GUIEditor1.window[1] = guiCreateWindow(639, 397, 193, 300, "Nova proposta de design", false)
		guiWindowSetSizable(GUIEditor1.window[1], false)
		exports.global:centerWindow(GUIEditor1.window[1])

		GUIEditor1.label[1] = guiCreateLabel(20, 26, 154, 21, "Por favor, escolha uma coleção:", false, GUIEditor1.window[1])
		...
		GUIEditor1.button[1] = guiCreateButton(15, 250, 74, 32, "Cancelar", false, GUIEditor1.window[1])
		GUIEditor1.button[2] = guiCreateButton(101, 250, 74, 32, "Proximo", false, GUIEditor1.window[1])
...
function startWizard_2_faction(fid)
	...
	GUIEditor2.window[1] = guiCreateWindow(709, 366, 323, 135, "Nova proposta de design", false)
	guiWindowSetSizable(GUIEditor2.window[1], false)
	exports.global:centerWindow(GUIEditor2.window[1])

	GUIEditor2.button[1] = guiCreateButton(11, 98, 147, 27, "Anterior", false, GUIEditor2.window[1])
	GUIEditor2.button[2] = guiCreateButton(165, 98, 147, 27, "Proximo", false, GUIEditor2.window[1])

	local fname = exports.factions:getFactionName(fid)
	GUIEditor2.label[1] = guiCreateLabel(10, 27, 303, 18, "Estou enviando isso para benefícios em nome de...", false, GUIEditor2.window[1])
...
function startWizard_2()
	...
	GUIEditor2.label[1] = guiCreateLabel(10, 25, 105, 23, "URL Imagem:", false, GUIEditor2.window[1])
	...
	GUIEditor2.label[3] = guiCreateLabel(10, 50, 105, 23, "Descrição da roupa:", false, GUIEditor2.window[1])
	...
	GUIEditor2.label[2] = guiCreateLabel(10, 73, 302, 18, "Exemplo: http://i.imgur.com/MG9pkfl.png; Tamanho Maximo: 100KB", false, GUIEditor2.window[1])
	...
	addEventHandler('onClientGUIClick', GUIEditor2.window[1], function ()
		...
		elseif source == GUIEditor2.button[2] then
			...
			if string.len(url) < 1 then
				guiSetText(GUIEditor2.label[2], 'Insira um link direto para a imagem.')
				playSoundFrontEnd(4)
			elseif string.len(desc) < 1 then
				guiSetText(GUIEditor2.label[2], 'Descreva como é este design.')
				playSoundFrontEnd(4)
			elseif not exports.global:hasMoney(localPlayer, 25) then
				triggerServerEvent('clothes:pedSay', localPlayer, getElementData(dupont_ped, 'name'), "Você precisas de $25.")
				playSoundFrontEnd(4)
			else
				guiSetText(GUIEditor2.label[2], 'Recuperando imagem do URL. Por favor, espere..')
...
function listMyClothes(list, for_faction)
	...
	if window and isElement(window) then
		...
		local margin = 30
		window = guiCreateWindow(screen_width - width - 45, screen_height - height-110, width, height, for_faction and (exports.factions:getFactionName(for_faction).."'s coleção") or (exports.global:getPlayerName(localPlayer).."'s coleção pessoal"), false)
		guiSetEnabled(window, false)
		guiSetAlpha(window, 0.95)
		guiWindowSetSizable(window, false)

		loading_label = guiCreateLabel(10, 25, width - 20, height - 60, "Carregando.." ,false, window)
		guiLabelSetHorizontalAlign(loading_label, 'center')
		guiLabelSetVerticalAlign(loading_label, 'center')

		local close = guiCreateButton(width - 110, height - 30, 100, 25, 'Fechar', false, window)
...
function editMyClothes(index)
	...
	if clothes then
		...
		guiSetInputEnabled(true)
		GUIEditor3.window[1] = guiCreateWindow(1031, 640, 338, 235, "Editando design #"..index, false)
		...
		GUIEditor3.button[1] = guiCreateButton(175, 136, 149, 25, "Salvar", false, GUIEditor3.window[1])

		GUIEditor3.label[1] = guiCreateLabel(16, 36, 149, 18, "Por favor, escolha uma coleção:", false, GUIEditor3.window[1])
		...
		GUIEditor3.label[2] = guiCreateLabel(175, 36, 149, 18, "Preço: ($50~$10,000)", false, GUIEditor3.window[1])
		...
		GUIEditor3.label[3] = guiCreateLabel(175, 83, 149, 18, "Descrição da roupa:", false, GUIEditor3.window[1])
		GUIEditor3.edit[2] = guiCreateEdit(175, 101, 149, 25, clothes.description or "Um conjunto de roupas limpas", false, GUIEditor3.window[1])
		guiEditSetMaxLength(GUIEditor3.edit[2], 200)

		GUIEditor3.button[2] = guiCreateButton(175, 165, 149, 25, "Deletar", false, GUIEditor3.window[1])
		--guiSetEnabled(GUIEditor3.button[2],isDeletable(clothes))

		GUIEditor3.button[3] = guiCreateButton(175, 195, 149, 25, "Cancelar", false, GUIEditor3.window[1])
		...
		addEventHandler('onClientGUIClick', GUIEditor3.window[1], function ()
			...
			elseif source == GUIEditor3.button[1] then
				...
				if not price or price > 10000 or price < 50 then
					playSoundFrontEnd(4)
					guiSetText(GUIEditor3.window[1], "Erro! O preço deve variar de $50 a $10.000.")
				elseif string.len(desc) < 1 then
					playSoundFrontEnd(4)
					guiSetText(GUIEditor3.window[1], "Erro! Descrição é obrigatória.")
				else
			...
			elseif source == GUIEditor3.button[2] then
				if isDeletable( clothes ) then
					...
					guiSetText( GUIEditor3.button[2], 'Deletando...' )
...
function deleteMyClothes( index, ok, why )
	if ok then
		...
		exports.hud:sendBottomNotification( localPlayer, "Star Fashion", "Este design de roupa não pode ser removido. "..why )
...
function openManu(cid)
	...
	GUIEditor4.window[1] = guiCreateWindow(1002, 246, 500, 400, "Bem-vindo ao fabricante de roupas Star!", false)
	guiWindowSetSizable(GUIEditor4.window[1], false)
	exports.global:centerWindow(GUIEditor4.window[1])

	GUIEditor4.button[1] = guiCreateButton(388, 354, 87, 27, "Proximo", false, GUIEditor4.window[1])
	GUIEditor4.button[2] = guiCreateButton(291, 354, 87, 27, "Fechar", false, GUIEditor4.window[1])
    GUIEditor4.button[3] = guiCreateButton(20, 354, 250, 27, "Fabricar instantaneamente ((50 GCs))", false, GUIEditor4.window[1])
    guiSetVisible(GUIEditor4.button[3], false)

	local intro = "Bem-vindo ao fabricante de roupas Star!\n\nNosso objetivo é ajudar vocês, nossos clientes, a fabricar linhas de roupas novas ou existentes"
		.." de uma forma eticamente responsável.\n\nPodemos obter todos os materiais de que você precisa para fazer suas roupas, podemos enviar sua mercadoria para todo o mundo após a produção "
		.."estar completo. Podemos ser seu agente e fazer todo esse trabalho para você. Economizamos seu tempo e dinheiro eliminando o incômodo de ter que se comunicar com várias pessoas. Seremos"
		.." seu principal ponto de contato para tudo que você precisa.\n\nTemos uma rede bem estabelecida e uma equipe de fornecedores e fabricantes que construímos fortes relações de trabalho"
		.." com tudo o que será necessário para começar bem a sua marca. Quer você seja um designer profissional estabelecido ou alguém com uma ótima ideia que deseja"
		.." se jogar na moda, a equipe da Star Clothing Manufacturing pode ajudá-lo!"
	...
	addEventHandler('onClientGUIClick', GUIEditor4.window[1], function ()
		if source == GUIEditor4.button[1] then
			if guiGetText(GUIEditor4.button[1]) == 'Proximo' then
				local clothing = list_[cid]
				if clothing and not cloth_info then
					cloth_info = 	"Roupa ID: "..clothing.id.."\n"..
									"Coleção ((Skin base)): "..clothing.skin.."\n"..
									"Descrição: "..clothing.description.."\n"..
									"Designer: "..clothing.creator_charname.."\n"..
									"Data Criação: "..(clothing.fdate or exports.datetime:formatTimeInterval(clothing.date)).."\n"..
									"\n\n ((IMPORTANTE: Cada personagem só pode fabricar 3 designs de roupas. Uma vez fabricado, você"
									.." -não- sera capaz de editar ou remover as roupas fabricadas. Portanto, certifique-se de que o rascunho esteja perfeito antes de fabricar!))"
				end
				guiSetText(GUIEditor4.label[1], cloth_info or "Ocorreram erros ao buscar informações de design.")
				guiLabelSetHorizontalAlign(GUIEditor4.label[1], "center", true)
				guiSetText(GUIEditor4.button[2], 'Anterior')
		...
		elseif source == GUIEditor4.button[2] then
			if guiGetText(GUIEditor4.button[2]) == 'Fechar' then
				closeManu()
			elseif guiGetText(GUIEditor4.button[2]) == 'Anterior' then
				guiSetText(GUIEditor4.button[1], 'Proximo')
				guiSetText(GUIEditor4.button[2], 'Fechar')
...
function openDist(cid)
	...
	GUIEditor5.window[1] = guiCreateWindow(1002, 246, 500, 400, "Bem-vindo ao Sistema de Distribuição Star!", false)
	guiWindowSetSizable(GUIEditor5.window[1], false)
	exports.global:centerWindow(GUIEditor5.window[1])

	GUIEditor5.button[1] = guiCreateButton(388, 354, 87, 27, "Fechar", false, GUIEditor5.window[1])
	GUIEditor5.button[2] = guiCreateButton(291, 354, 87, 27, "Obter produto", false, GUIEditor5.window[1])
	GUIEditor5.button[3] = guiCreateButton(194, 354, 87, 27, "Distribuir globalmente", false, GUIEditor5.window[1])
	GUIEditor5.button[4] = guiCreateButton(97, 354, 87, 27, "Vender para Loja", false, GUIEditor5.window[1])
	...
	addEventHandler('onClientGUIClick', GUIEditor5.window[1], function ()
		if source == GUIEditor5.button[2] then
			if guiGetText(GUIEditor5.button[2]) == 'Obter produto' then
				-- need space for it.
				local clothing = list_[cid]
				if not exports.global:hasSpaceForItem(localPlayer, 16, clothing.skin) then
					guiSetText(GUIEditor5.label[1], "Seu inventário esta cheio.")
				...
				if not exports.global:hasMoney(localPlayer, price) then
					guiSetText(GUIEditor5.label[1], "Você não tem dinheiro para comprar este produto.")
					guiLabelSetHorizontalAlign(GUIEditor5.label[1], "center", true)
					playSoundFrontEnd(4)
					return
				end
				...
				if clothing and not cloth_info then
					cloth_info = 	"Roupa ID: "..clothing.id.."\n"..
									...
									"Designer: "..clothing.creator_charname.."\n"..
									"Data Criação: "..(clothing.fdate or exports.datetime:formatTimeInterval(clothing.date)).."\n"..
									"Esgotado: "..exports.global:formatMoney( clothing.sold ).."\n"..
									"Preço: $"..exports.global:formatMoney( price ).."\n"..
									"\n\nIMPORTANTE: Esta opção irá gerar um conjunto de roupas deste design.\n"
									.."O preço inicial é de $ 1 e você pode obter conjuntos ilimitados de roupas. No entanto, se este design for distribuído de forma privada (acessível apenas por você), toda vez que este design for vendido, o preço será dobrado conforme você obtenha o produto de seu próprio design."
				end
				guiSetText(GUIEditor5.label[1], cloth_info or "Ocorreram erros ao buscar informações de design.")
				guiLabelSetHorizontalAlign(GUIEditor5.label[1], "center", true)
				guiSetText(GUIEditor5.button[1], 'Anterior')
				guiSetText(GUIEditor5.button[2], 'Confirmar')
				guiSetVisible(GUIEditor5.button[3], false)
				guiSetVisible(GUIEditor5.button[4], false)
			elseif guiGetText(GUIEditor5.button[2]) == 'Confirmar' then
				playSoundFrontEnd(6)
				if triggerServerEvent('clothes:getProduct', resourceRoot, cid) then
					guiSetText(GUIEditor5.label[1], "Validando e Recuperando..")
					guiLabelSetHorizontalAlign(GUIEditor5.label[1], "center", true)
					guiSetEnabled(GUIEditor5.window[1], false)
				end
			elseif guiGetText(GUIEditor5.button[2]) == 'Vender' then
				playSoundFrontEnd(6)
				if triggerServerEvent('clothes:sellProduct', resourceRoot, cid) then
					guiSetText(GUIEditor5.label[1], "Validando e Vendendo..")
					guiLabelSetHorizontalAlign(GUIEditor5.label[1], "center", true)
					guiSetEnabled(GUIEditor5.window[1], false)
				end
			end
		elseif source == GUIEditor5.button[1] then
			if guiGetText(GUIEditor5.button[1]) == 'Fechar' then
				closeDist()
			elseif guiGetText(GUIEditor5.button[1]) == 'Anterior' then
				guiSetText(GUIEditor5.button[1], 'Fechar')
				guiSetText(GUIEditor5.button[2], 'Obter Produto')
		...
		elseif source == GUIEditor5.button[3] then
			playSoundFrontEnd(4)
			outputChatBox("Este recurso está atualmente em construção.", 255,0,0)
		elseif source == GUIEditor5.button[4] then
			...
			if clothing and not cloth_info then
				...
				cloth_info = 	"Roupa ID: "..clothing.id.."\n"..
								...
								"Data Criação: "..(clothing.fdate or exports.datetime:formatTimeInterval(clothing.date)).."\n"..
								"Esgotado: "..exports.global:formatMoney( clothing.sold ).."\n"..
								"Preço na loja: $"..exports.global:formatMoney( clothing.price ).."\n"..
								"\n\nIMPORTANTE: depois de vender seu design para a Dupont, seu design de roupas será distribuído publicamente e globalmente em todas as lojas de roupas.\n"..
								"Isso só pode render a você $"..exports.global:formatMoney( price ).." e você não recebe nenhum corte de lucro com a venda desse tipo de roupa em lojas de roupas pela cidade.\n"..
								"Ele ainda ocupará um espaço permanentemente em sua coleção."
			end
			guiSetText(GUIEditor5.label[1], cloth_info or "Ocorreram erros ao buscar informações de design.")
			guiLabelSetHorizontalAlign(GUIEditor5.label[1], "center", true)
