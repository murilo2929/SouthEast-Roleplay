function addUpgrade( source, vehicle, itemSlot, itemID, itemValue )
	...
	for key, value in ipairs(old_upgrades) do
		if value == itemValue then
			outputChatBox("Esta atualização já foi adicionada.", source, 255, 0, 0)
			return
	...
	if getElementData(vehicle, "job") ~= 0 or getElementData(vehicle, "owner") == -2 then
		outputChatBox("Você não pode instalar atualizações para veículos civis!", source, 255, 0, 0)
		return
	end
	...
	if takeItemFromSlot(source, itemSlot) then
		if addVehicleUpgrade(vehicle, itemValue) then
			...
			exports.global:sendLocalMeAction(source, "adiciona " .. getItemDescription(itemID, itemValue) .. " em seu " .. exports.global:getVehicleName(vehicle).."("..getVehicleName(vehicle) .. ").")
			...
			outputChatBox("Esta atualização não é compatível com este " .. exports.global:getVehicleName(vehicle) .. ".", source, 255, 194, 14)
			giveItem( source, itemID, itemValue )
		end
	else
		outputChatBox("Falha ao pegar o item.", source, 255, 194, 14)
	end
end
...
function moveUpgradeFromElement( upgrade )
	...
	if hasSpaceForItem( client, 114, upgrade ) then
		if removeVehicleUpgrade( source, upgrade ) then
			...
			outputChatBox( "A atualização do veículo não existe?", client, 255, 0, 0 )
		end
	else
		outputChatBox( "Seu estoque está cheio.", client, 255, 0, 0 )
	end
	triggerClientEvent( client, "finishItemMove", client )
end
