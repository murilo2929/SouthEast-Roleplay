...
function getItemDescription(id, value, metadata)
	...
	if i then
		...
		elseif id == 115 then
			...
			if exports.weapon:isWeapAmmoless( tonumber(values[1]) ) then
				...
				if values[4] and tonumber(values[4]) then
					return values[4]..' munição carregada.'
				else
					return '0 munições carregadas.'
				end
			end
		-- ammopack
		elseif id == 116 then
		...
		elseif id == 150 then--ATM card
			...
			if itemExploded and itemExploded[2] and tonumber(itemExploded[2]) then
				if tonumber( itemExploded[2] ) > 0 then
					return "Número Cartão: '"..itemExploded[1].."', Dono: '"..tostring(exports.cache:getCharacterNameFromID(itemExploded[2])):gsub("_", " ").."'"
				else
					return "Número Cartão: '"..itemExploded[1].."', Dono: '"..tostring(exports.cache:getBusinessNameFromID(math.abs(itemExploded[2]))).."'"
				end
			else
				return "O cartão parece estar quebrado e não pode mais ser usado."
			end
		elseif id == 152 then--ID card
			local itemExploded = explode(";", value)
			return "Um cartão de identificação de plástico elegante tem o nome de "..tostring(itemExploded[1]):gsub("_", " ")..", foto e algumas outras informações básicas."
		elseif id == 178 then
			local yup = split(value, ":")
		...
		elseif id == 273 then --Generic fish
			local FishValue = explode(":", value)
			return "Pesa muito: " .. FishValue[2] .. "lbs"
		else
			return desc:gsub("#v",value)
		end
	end
