...
armor = {
	[219]  = {49, "type2", "pega seu colete blindado e o coloca sobre a cabeça", "pega seu colete e o tira"},
	[220]  = {74, "type3", "pega seu colete blindado e o coloca sobre a cabeça", "pega seu colete e o tira"},
	[221]  = {100, "type4", "pega seu colete blindado e o coloca sobre a cabeça", "pega seu colete e o tira"},
}
...
function useItem(itemSlot, additional)
	...
	if itemID then
		if (itemID==1) then -- hotdog
			if getElementData(source, "char.Hunger") == 100 then 
					outputChatBox("Você não está com fome!!", source, 255, 255, 255, true)
				else
					exports.serp_fomesede:giveHunger( source, 5 )
					triggerEvent('sendAme', source, "come um hotdog.")
			...
			if veh and getElementData(veh, "dbid") == itemValue then
				...
				if value then
					...
					if getDistanceBetweenPoints3D(x, y, z, vx, vy, vz) <= 30 then -- car found
						...
						outputChatBox("Você está muito longe do veículo.", source, 255, 194, 14)
					end
				else
					outputChatBox("Veículo invalido.", source, 255, 194, 14)
				end
			end
		elseif (itemID==4) or (itemID==5) then -- house key or business key
			...
			if found then
				...
				if locked == 0 then
					triggerEvent('sendAme', source, "coloca a chave na porta para destrancá-la.")
				else
					newRealLockedValue = true
					triggerEvent('sendAme', source, "coloca a chave na porta para trancá-la.")
				end
				...
				outputChatBox("Você esta muito longe da porta.", source, 255, 194, 14)
			end
		...
		elseif (itemID==8) then -- sandwich
			if getElementData(source, "char.Hunger") == 100 then 
					...
					triggerEvent('sendAme', source, "come um sanduíche.")
					takeItemFromSlot(source, itemSlot)
			end
		elseif (itemID==9) then -- sprunk
			if getElementData(source, "char.Thirst") == 100 then 
					outputChatBox("Você não está com sede!!", source, 255, 255, 255, true)
				else	
					exports.serp_fomesede:giveThrink( source, 4 )
					toggleAllControls(source, true, true, true)
					triggerEvent('sendAme', source, "bebe uma sprunk.")
					takeItemFromSlot(source, itemSlot)
				end
		elseif (itemID==10) then -- red dice
			...
			exports.global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " joga um dado de " .. (max == 6 and "" or (max .. "-lados ")) .. "e obtem " .. math.random( 1, max ) ..".", 255, 51, 102)
		elseif (itemID==11) then -- taco
			if getElementData(source, "char.Hunger") == 100 then 
					outputChatBox("Você não está com fome!!", source, 255, 255, 255, true)
				else
					exports.serp_fomesede:giveHunger( source, 7 )
					triggerEvent('sendAme', source, "come um taco.")
					takeItemFromSlot(source, itemSlot)
				end
		elseif (itemID==12) then -- cheeseburger
			if getElementData(source, "char.Hunger") == 100 then 
					...
					exports.serp_fomesede:giveHunger( source, 10 )
					setTimer(removeAnimation, 4000, 1, source)
					triggerEvent('sendAme', source, "come um cheeseburger.")
					takeItemFromSlot(source, itemSlot)
				end
		elseif (itemID==13) then -- donut
			if getElementData(source, "char.Hunger") == 100 then 
					...
					exports.serp_fomesede:giveHunger( source, 3 )
					triggerEvent('sendAme', source, "come um donut.")
					takeItemFromSlot(source, itemSlot)
				end
		elseif (itemID==14) then -- cookie
			if getElementData(source, "char.Hunger") == 100 then 
					...
					exports.serp_fomesede:giveHunger( source, 3 )
					triggerEvent('sendAme', source, "come um cookie.")
					takeItemFromSlot(source, itemSlot)
				end
		elseif (itemID==15) then -- water
			if getElementData(source, "char.Thirst") == 100 then 
					...
					exports.serp_fomesede:giveThrink( source, 25 )
					triggerEvent('sendAme', source, "bebe uma garrafa de aguá.")
					takeItemFromSlot(source, itemSlot)
				end
		elseif (itemID==16) then -- clothes
			...
			local iscustom = exports.newmodels:isCustomModID(skin)
			local isdefault = exports.newmodels:isDefaultID("ped", skin)


			local fits = iscustom or isdefault
			if clothingid or fits then
				
				local data_name = exports.newmodels:getDataNameFromType("ped") -- gets the correct data name
		        if iscustom then
				  setElementData(source, data_name, skin) -- custom id
		        else
		          setElementModel(source, skin) -- default id
		        end
				...
				triggerEvent('sendAme', source, "muda de roupa.")
			else
				outputChatBox("Essas roupas não cabem em você.", source, 255, 0, 0)
			end
		elseif (itemID==17) then -- watch
			local metaName = getItemName(itemID, itemValue, metadata)
			triggerEvent('sendAme', source, "olha em seu " ..metaName .. ".")
			outputChatBox("O horário é " .. string.format("%02d:%02d", getTime()) .. ".", source, 255, 194, 14)
		...
		elseif (itemID==20) then -- STANDARD FIGHTING
			...
			outputChatBox("Você leu um livro sobre como lutar padrão.", source, 255, 194, 14)
		...
		elseif (itemID==21) then -- BOXING
			...
			outputChatBox("Você leu um livro sobre como fazer boxe.", source, 255, 194, 14)
		...
		elseif (itemID==22) then -- KUNG FU
			...
			outputChatBox("Você leu um livro sobre como fazer Kung Fu.", source, 255, 194, 14)
		...
		elseif (itemID==23) then -- KNEE HEAD
			...
			outputChatBox("Você abre o livro e percebe que ele está escrito em grego antigo.", source, 255, 194, 14)
		...
		elseif (itemID==24) then -- GRAB KICK
			...
			outputChatBox("Você leu um livro sobre como fazer Grab Kick Fighting.", source, 255, 194, 14)
		...
		elseif (itemID==25) then -- ELBOWS
			...
			outputChatBox("Você leu um livro sobre como fazer Elbow Fighting.", source, 255, 194, 14)
		...
		elseif (itemID==27) then -- FLASHBANG
			...
			triggerEvent('sendAme', source, "joga um flashbang.")
		...
		elseif (itemID==28) then -- GLOWSTICK
			...
			triggerEvent('sendAme', source, "joga um glowstick.")
			setTimer(destroyElement, 600000, 1, marker)
		elseif (itemID==29) then -- RAM
			outputChatBox("Por favor, informe para usar este item.", source, 255, 0, 0)
		elseif (itemID==34) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "cheira um pouco de cocaína.")
		elseif (itemID==35) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "engole uma pílula de morfina.")
		elseif (itemID==36) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "engole uma pílula de ecstasy.")
		elseif (itemID==37) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "injeta um pouco de heroína.")
                ...
                if sucess then
                    if (tonumber(itemvalue) >= 1) then
                        if (hasSpaceForItem(source, 182, 1)) then
                            ...
                            exports.global:sendLocalMeAction(source, "pega um pouco de maconha da sacola, começando a enrolar o baseado depois.")
                            giveItem(source, 181, itemvalue - 1)
                        else
                            outputChatBox( "Seu Inventario esta cheio.", source, 255, 0, 0 )
                        end
                    else
                        outputChatBox("Aquele pacote de seda acabou!", source, 255, 0, 0)
                    end
                else
                    outputChatBox("Você precisa de um pacote de seda para usar isto.", source, 255, 0, 0)
                end
            else
                triggerEvent('sendAme', source, "olha dentro de seu saco de maconha. Está vazio.")
            end
		elseif (itemID==39) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "cheira um pouco de metanfetamina.")
		elseif (itemID==40) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "injeta uma caneta de epinefrina.")
		elseif (itemID==41) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "derrama uma gota de LSD em sua boca.")
		elseif (itemID==42) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "come cogumelos secos")
		elseif (itemID==43) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "engole alguns comprimidos de PCP")
		...
		elseif (itemID==54) then -- GHETTOBLASTER
			triggerEvent('sendAme', source, "coloca um ghettoblaster no chão.")
		...
		elseif (itemID==57) then -- FUEL CAN
			...
			if found then
				...
				outputChatBox("Você está muito longe de um veículo.", source, 255, 194, 14)
			end
		elseif (itemID==58) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "bebe uma boa cerveja Ziebrand.")
			setElementHealth(source,getElementHealth(source)-5)
		elseif (itemID==59) then -- MUDKIP
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "come um mudkip.")
			--killPed(source)
		elseif (itemID==60) then
			...
			-- world map.
			if dim == 0 then
				exports.hud:sendBottomNotification( source, itemName, "Este cofre só pode ser colocado dentro de um interior." )
				playSoundFrontEnd( source, 4 )
			-- vehicle interiors
			elseif dim >= 20000 then
				local vid = dim - 20000
				if exports.vehicle:getSafe( vid ) then
					exports.hud:sendBottomNotification( source, itemName, "Já existe um cofre nesta propriedade. Digite /movercofre para movê-lo para onde você está." )
			...
			elseif dim >= 19000 then
				exports.hud:sendBottomNotification( source, itemName, "Você não pode colocar um cofre no interior de um veículo temporário." )
				playSoundFrontEnd( source, 4 )
			-- normal interiors.
			elseif hasItem( source, 5, dim ) or hasItem( source, 4, dim) then
				if exports.interior_system:getSafe( dim ) then
					exports.hud:sendBottomNotification( source, itemName, "Já existe um cofre nesta propriedade. Digite /movercofre para movê-lo." )
					playSoundFrontEnd( source, 4 )
				else
					if exports.interior_system:addSafe( dim, nil, pos, int, rot, true, false ) then
						exports.hud:sendBottomNotification( source, itemName, "Colocado." )
						takeItemFromSlot( source, itemSlot )
					end
				end
			end
		elseif (itemID==62) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "bebe um pouco de Vodka Bastradov pura.")
			setElementHealth(source,getElementHealth(source)-10)
		elseif (itemID==63) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "bebe um pouco de uísque escocês.")
			setElementHealth(source,getElementHealth(source)-10)
		elseif (itemID==68) then
			triggerEvent('sendAme', source, "olha o bilhete de loteria.")
			outputChatBox("Você olha para o bilhete de loteria e lê o número: " .. itemValue, source, 0, 255, 0)
		elseif (itemID==69) then -- Dictionary
			...
			if (learned) then
				...
				outputChatBox("Você aprendeu o básico de " .. lang .. ", Pressione F6 para gerenciar seus idiomas.", source, 0, 255, 0)
			end
		elseif (itemID==72) then -- Note
			triggerEvent('sendAme', source, "lê uma nota.")
		...
		elseif (itemID==83) then -- Coffee
			...
			triggerEvent('sendAme', source, "bebe uma xícara de café.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==88) then -- earpiece
			outputChatBox("Você pode usar este fone de ouvido com um rádio.", source, 255, 194, 14)
		elseif (itemID==89) then -- Generic Food
			...
			triggerEvent('sendAme', source, "come um " .. itemExploded[1] .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==91) then
			takeItemFromSlot(source, itemSlot)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "bebe uma boa gemada.")
			setElementHealth(source, 100)
		elseif (itemID==92) then
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come um pouco de peru.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==93) then
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come um pouco de pudim de natal.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==97) then -- SFES Procedures Manual (book)
			...
			triggerClientEvent( source, "showBook", source, bookName, bookTitle )]]
			outputChatBox("Este item foi desativado e não está mais em uso.", source, 255, 0, 0)
		...
		elseif (itemID==99) then --[ADDED: 2/22/10 by herbjr] tray
			...
			triggerEvent('sendAme', source, "come a comida da bandeja.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==100) then --[ADDED: 2/22/10 by herbjr] milk
			...
			triggerEvent('sendAme', source, "bebe uma pequena caixa de leite.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==101) then --[ADDED: 2/22/10 by herbjr] juice
			...
			triggerEvent('sendAme', source, "bebe uma pequena caixa de suco.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==102) then --[ADDED: 2/23/10 by herbjr] Cabbage (Asked for by Misha)
			...
			triggerEvent('sendAme', source, "come uma cabeça de repolho.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==104) then -- portable TV
			...
			if isTvUsed == nil or isTvUsed == false then
				triggerEvent('sendAme', source, "liga uma pequena TV portátil.")
		...
		elseif (itemID == 105) then -- Pack of cigarettes. Withdraw one if it still has one
			if (itemValue > 0) then
				if (hasSpaceForItem(source, 106, 1)) then
					...
					triggerEvent('sendAme', source, "olha dentro do maço de cigarros e tira um.")
				else
					outputChatBox( "Seu Inventario esta cheio.", source, 255, 0, 0 )
				end
			else
				triggerEvent('sendAme', source, "olha dentro de seu maço de cigarros. Está vazio.")
			end
		elseif (itemID == 106) then -- Cigarette
			if hasItem( source, 107 ) then
				triggerEvent('sendAme', source, "acende um cigarro.")
				outputChatBox( "(( /jogarfora para jogar fora, /mudarmao para mudar de mão ))", source )
				triggerEvent("realism:startsmoking", source, 0) -- 0 = left hand -- 1 = right hand
				takeItemFromSlot(source, itemSlot)
			else
				triggerEvent('sendAme', source, "mostra um cigarro a todos.")
				outputChatBox( "Você vai precisar de um isqueiro para usá-lo corretamente.", source, 255, 0, 0 )
			end
		elseif (itemID == 107) then
			triggerEvent('sendAme', source, "mostra a todos um isqueiro.")
		elseif (itemID==108) then -- donut
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come uma panqueca.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==109) or (itemID==110) then -- Fruit
			giveHealth(source, tonumber(getItemValue(itemID, itemValue)))
			exports.global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come um(a) " .. itemName .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID == 113) then -- Pack of glowsticks, Withdraw one if it still has one
			if tonumber(itemValue) and tonumber(itemValue) > 0 then
				if (hasSpaceForItem(source, 28, 1)) then
					...
					triggerEvent('sendAme', source, "olha dentro de seu pacote de bastões luminosos, deslizando um para fora.")
				else
					outputChatBox( "Seu Inventario esta cheio.", source, 255, 0, 0 )
				end
			else
				triggerEvent('sendAme', source, "olha dentro de seu pacote de bastões luminosos. Está vazio.")
			end
		elseif itemID == 114 then
			...
			if vehicle and not noUpgrades[getVehicleType(vehicle)] --[[and getElementDimension(vehicle) > 0]] and getItemDescription(itemID, itemValue) ~= "?" then
				...
				outputChatBox("Use isso em um veículo para adicioná-lo como uma atualização permanente.", source, 255, 194, 14)
			end
		elseif itemID == 126 then
			outputChatBox("Belo cinto tatico que você tem aí. ;)", source, 0, 255, 0)
		elseif itemID == 130 then
			outputChatBox("Coloque este sistema de alarme em um inventario de veículos para instalá-lo.", source, 0, 255, 0)
		elseif itemID == 132 then
			takeItemFromSlot(source, itemSlot)
			outputChatBox("Você pegou o seu " .. itemValue .. " prescrição.", source)
			triggerEvent('sendAme', source, "toma algum remédio prescrito.")
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
		elseif itemID == 134 then
			outputChatBox("$" .. exports.global:formatMoney(itemValue) .. " de moeda.", source)
		elseif itemID == 137 then -- New snake cam, Adams
			triggerEvent('snakecam:toggleSnakeCam', root, source)
		elseif itemID == 138 then
			outputChatBox("Coloque este dispositivo em um inventario de veículo para instalá-lo.", source, 0, 255, 0)
		...
		elseif masks[itemID] then
			...
			if mask then
				...
				exports.global:applyAnimation(source, "goggles", "goggles_put_on", 600, false, true, true)
			...
			elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
				outputChatBox("Você não pode usar isso em um veículo civil.", source, 255, 0, 0)
			else
		...
		elseif itemID == 139 then
			outputChatBox("((Peça a um administrador para instalar isso em seu veículo.))", source, 255, 0, 0)
		...
		elseif itemID == 173 then
			...
			if distance < 25 then
				...
				outputChatBox("Para vender seu veículo, você deve estar no estacionamento do DMV.", source, 0, 255, 0)
			end
    	...
    	elseif (itemID == 182) then
            if hasItem( source, 107 ) then -- If da player has a lighter, how else would he smoke it dawg
                ...
                outputChatBox( "(( /jogarfora para jogar fora, /mudarmao para mudar de mão, /passarbaseado para passar seu baseado ))", source )
                ...
                exports.global:sendLocalMeAction(source, "mostra a todos um baseado enrolado.")
                outputChatBox("Você vai precisar de um isqueiro para usá-lo corretamente.", source, 255, 0, 0)
            end
		...
		elseif (itemID==210) then -- Coca-Cola Christmas Edition
			...
			--exports.global:sendLocalMeAction(thePlayer, "bebe uma coke.")
			triggerEvent('sendAme', thePlayer, "bebe uma coca.")
		...
		elseif (itemID==213) then --pinnekjott
			...
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come um pinnekjott.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==214) then
			triggerEvent('sendAme', source, "leva um ".. itemName ..".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID >= 224 and itemID <= 228) then
			triggerEvent('sendAme', source, "leva algum ".. itemName ..".")
			takeItemFromSlot(source, itemSlot)
		elseif itemID == 232 then -- car battery charger
			local veh = additional
			local batt = getElementData( veh, 'battery' ) or 100
			if batt == 100 then
				exports.hud:sendBottomNotification( source, itemName, "A bateria deste "..exports.global:getVehicleName(veh).." já está cheia." )
			else
				if takeItemFromSlot( source, itemSlot) then
					exports.anticheat:setEld( veh, 'battery', math.min( 100, batt + 20 ) )
					exports.hud:sendBottomNotification( source, itemName, "A bateria deste "..exports.global:getVehicleName(veh).." foi carregado em 20%." )
					triggerEvent( 'fuel:sync', veh, source )
				end
			end
		elseif itemID == 260 then
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "bebe um pouco de amônia.")
		...
		elseif itemID == 270 then
			...
			if getElementData(source, "gloves") then
				triggerEvent('sendAme', source, "tira um par de " .. metaName .. " das mãos deles.")
				...
				triggerEvent('sendAme', source, "coloca um par de " .. metaName .. " nas mãos deles.")
		...
		elseif (armor[itemID]) then
			...
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==276) then -- MP7 (Skin)
...
function npcUseItem(element, itemID)
	...
	if itemID then
		...
		if(hasItem) then
			...
			if badges[itemID] then
				if ( getElementData( source, badges[itemID][1] ) ) then
					...
					if pedname then
						...
						exports.global:sendLocalText(source, "* "..tostring(pedname).." tira um(a) "..badges[itemID][2]..".", 255, 51, 102, 5)
					end
				else
						...
						if key ~= itemID then
							if ( getElementData ( source, badge[1] ) ) then
								...
								if pedname then
									...
									exports.global:sendLocalText(source, "* "..tostring(pedname).." tira um(a) "..badge[2]..".", 255, 51, 102, 5)
								end
							end
						end
					end
					...
					if pedname then
						...
						exports.global:sendLocalText(source, "* "..tostring(pedname).." coloca um(a) "..badges[itemID][2]..".", 255, 51, 102, 5)
					end
					return true
				end
			...
			outputDebugString("O ped não tem esse item")
			return false
		end
	end
end
...
function destroyItem(itemID)
	...
	outputChatBox("Você destruiu um(a) " .. itemName .. ".", source, 255, 194, 14)
	triggerEvent('sendAme', source, "destruiu um(a) "..itemName..".")
end
...
function showItem(itemName)
	if isPedDead(source) or getElementData(source, "injuriedanimation") then return end
	triggerEvent('sendAme', source, "mostra a todos "..(isVowel(itemName) and "um(a)" or "um(a)").." " .. removeOOC(itemName) .. ".")
end
...
addCommandHandler("verinventario", showInventoryRemote, false, false)
...
function sortMoney(thePlayer, commandName, moneyAmountCurrent, moneyAmountSort)
	if not (moneyAmountCurrent) or not (moneyAmountSort) or tonumber(moneyAmountCurrent) < 0 or tonumber(moneyAmountSort) < 0 then
		outputChatBox("SYNTAX: /" .. commandName .. " [Quantidade de dinheiro que você tem] [Quantidade de dinheiro para dividir]", thePlayer, 255, 194, 14)
	else
		...
		if tonumber(moneyAmountCurrent) <= tonumber(playerMoney) and tonumber(moneyAmountSort) <= tonumber(playerMoney) then
			if hasItem(thePlayer, 134, tonumber(moneyAmountCurrent)) then
				outputChatBox("Você classificou $" .. exports.global:formatMoney(moneyAmountSort) .. " de $" .. exports.global:formatMoney(moneyAmountCurrent) .. ".", thePlayer)
				...
				outputChatBox("Você não tem essa quantidade de dinheiro em um item classificado.", thePlayer, 255, 0, 0)
			end
		else
			outputChatBox("Você não tem essa quantidade de dinheiro.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("sortmoney", sortMoney)
addCommandHandler("dividirdinheiro", sortMoney)
...
function combineMoney(thePlayer, commandName)
		...
		if tonumber(playerMoney) > 0 then
			...
			outputChatBox("Você combinou $" .. exports.global:formatMoney(playerMoney) .. ".", thePlayer)
		else
			outputChatBox("Você não pode combinar dinheiro que você não tem!", thePlayer, 255, 0, 0)
		end
end
addCommandHandler("combinemoney", combineMoney)
addCommandHandler("juntardinheiro", combineMoney)
...
function toggleBadge(source, badges, itemID , itemValue)
	if ( getElementData( source, badges[itemID][1] ) ) then
		...
		if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
			triggerEvent('sendAme',  source, "remove um " .. badges[itemID][1] .. " de sua cabeça." )
		else
			triggerEvent('sendAme', source, "remove um " .. badges[itemID][1] .. ".")
		end
	else
		for key, badge in pairs ( badges ) do
			if key ~= itemID then
				if ( getElementData ( source, badge[1] ) ) then
					exports.anticheat:changeProtectedElementDataEx( source, badge[1], false, true )
					if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
					triggerEvent('sendAme',  source, "remove um " .. badge[1] .. " de sua cabeça." )
					else
					triggerEvent('sendAme',  source, "remove um" .. badge[1] .. "." )
					end
				end
			end
		end
		...
		triggerEvent('sendAme',  source, "coloca um " .. badges[itemID][1] .. " em torno de sua cabeça." )
		else
			exports.anticheat:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(itemValue), true )
			triggerEvent('sendAme',  source, "coloca um " .. badges[itemID][1] .. "." )
		end
	end
	exports.global:updateNametagColor(source)
end
