			...
			if result then
				...
				if itemID == 178 then
					...
					if not bID then
						...
						itemValue = itemValue .. ":" .. "Desconhecido" .. ":" .. tostring(bookIndex)
...
function takeItem(element, itemID, itemValue)
	...
	if success then
		...
		if success then
			...
			return false, "O elemento não tem este item"
		end
	else
		return false, "loadItems error: " .. error
	end
				...
				if hasSpaceForItem(to, saveditems[from][slot][1], saveditems[from][slot][2], saveditems[from][slot][5]) then
					...
					if itemIndex then
						...
						if itemID == 48 or itemID == 126 or itemID == 60 or itemID == 103 or itemID == 163 then
							return false, "Este item não pode ser movido"
						else
							...
							if query then
								...
								if itemID == 115 or itemID == 116 then
									...
									if isThisGunDuplicated(itemID, itemValue, target) then
										...
										outputChatBox("Esta arma foi duplicada por um abusador de bugs e não pode mais ser usada. Lamentamos excluí-lo agora.", target, 255,0,0)
										return false, "Arma ID#"..itemIndex.." duplicação detectada e excluída."
									end
								end
								...
								if ( (itemID >= 31) and (itemID <= 43) ) or itemBannedByAltAltChecker[itemID] then
									...
									if not exports.global:isStaffOnDuty(to) and not exports.global:isStaffOnDuty(from) then
										if hoursPlayedFrom < 10 then
											outputChatBox("Você precisa de 10 horas de tempo de jogo para mover um(a) "..getItemName( itemID ).." para "..getName(to)..".", from, 255, 0, 0)
											return false, "Mudança de item cancelada, <10 horas"
										end
										...
										if hoursPlayedTo < 10 then
											if not (itemID == 3 and getElementData(exports.pool:getElement("vehicle", itemValue), "owner") == getElementData(source, "dbid")) then -- Checks if they own the vehicle for the key
												outputChatBox("Você precisa de 10 horas de jogo para receber um(a) "..getItemName( itemID ).." de "..getName(from)..".", to, 255, 0, 0)
												return false, "Mudança de item cancelada, <10 horas"
											end
										end
									end
					...
					return false, "O alvo não tem espaço para o item."
				end
			else
				return false, "Slot não existe."
			end
