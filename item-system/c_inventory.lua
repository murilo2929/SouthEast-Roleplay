...
local actionIcons =
{
	[TAB_WALLET] = { -48, tocolor( 255, 255, 255, 50 ), "Carteira" },
	[TAB_ITEMS] = { 48, tocolor( 255, 255, 255, 50 ), "Itens" },
	[TAB_KEYS] = { -203, tocolor( 255, 255, 255, 50 ), "Chaves" },
	[TAB_WEAPONS] = { -204, tocolor( 255, 255, 255, 50 ), "Arma" },
...
local function getOverlayText( itemID, itemValue, metadata, isGrouped )
	...
	elseif itemID == 153 then
		text = "Carteira de Motocicleta"
	elseif itemID == 155 then
		text = "Carteira de Barco"
	elseif itemID == 154 then
		text = "Licença de Pesca"
...
local function getTooltipFor( itemID, itemValue, isGrouped, isProtected, metadata )
	...
	if itemID == 80 then
		if isGrouped then
			return isGrouped .. " Itens Genericos", ""
		else
	...
	if itemID == 223 and isGrouped then
		return isGrouped .. " Itens de Armazenamento"
	end
	...
	if isGrouped then
		return isGrouped .. "x " .. name, "Clique para mostrar todos esses itens."
	end
					...
					elseif elementType == "object" then
						...
						if getElementParent(getElementParent(hoverElement)) == getResourceRootElement(getResourceFromName("item-world")) then
							...
							if targetItemID == 166 then --Video System
								...
								name = getItemName(targetItemID, targetItemValue, targetItemMetadata) or "armazenamento"
							end
						end		
					end
					tooltip( boxx + sbox, boxy + ( box - 50 ) / 2, getItemName( item[1], item[2], item[6] ), "Mover para " .. name .. "." )
					...
					if state == "up" and clickItemSlot then
						...
						elseif not clickItemSlot.group then
									...
									if hoverElement == nil then
										if itemID > 0 then
											if itemID == 48 and countItems( localPlayer, 48 ) == 1 and getCarriedWeight( localPlayer ) - getItemWeight( 48, 1 ) > 10 then
												outputChatBox( "Você tem muitas coisas em seu inventário.", 255, 0, 0 )
											else
							...
							elseif hoverAction == ACTION_DROP then
								if isWatchingTV() then
									outputChatBox( "Você não está NA sua TV. Apenas sentado na frente dele.", 255, 0, 0 )
									clickItemSlot = nil
									return
								end
								local item = inventory[ clickItemSlot.invslot ]
							...
							elseif hoverAction == ACTION_SHOW then
								...
								if item[1] == 72 then
									itemName = itemName .. ", lendo " .. itemValue
								...
								elseif item[1] == 133 or item[1] == 153 or item[1] == 154 or item[1] == 155 or item[1] == 78 then--Cards
									itemName = itemName .. ", emitido para "..itemValue..""
							...
							elseif hoverAction == ACTION_DESTROY then
								...
								if itemID == 48 and countItems( localPlayer, 48 ) == 1 then -- backpack
									if getCarriedWeight( localPlayer ) - getItemWeight( 48, 1 ) > 10 then
										outputChatBox("Você tem muitas coisas em seu inventário.")
									else
										triggerServerEvent( "destroyItem", localPlayer, itemSlot )
									end
								elseif itemID == 134 then --Money
									outputChatBox("Você não pode destruir dinheiro.")
								else
									triggerServerEvent( "destroyItem", localPlayer, itemSlot )
								end
							elseif hoverAction == ACTION_SPLIT then
								...
								if splittableItems[itemID] then
									...
									outputChatBox(itemName.." não é divisível, digite '/splits' para obter uma lista de itens divisíveis.",255,0,0)
								end
							end
						end
						hoverItemSlot = false
...
function resetPicFrame(itemID, itemValue)
	if itemValue == 1 then
		outputChatBox("Esta textura já está vazia.", localPlayer, 255, 0, 0)
	else
...
function splitItem(itemID, itemName, itemValue, item)
	...
	elseif itemID == 115 then
		...
		local GUIEditor_Label = guiCreateLabel(12,20,54,24,"Quantia:",false,wSplitting)
			guiLabelSetVerticalAlign(GUIEditor_Label,"center")
			guiSetFont(GUIEditor_Label,"default-bold-small")
		local eAmount = guiCreateEdit(66,20,146,24,"um número inteiro",false,wSplitting)
		...
		addEventHandler("onClientGUIClick", bOK, function()
			...
			if not amount then
				guiSetText(wSplitting,"A quantidade deve ser um número!")
				return false
			end
			if amount%1 ~= 0 then
				guiSetText(wSplitting,"A quantidade deve ser inteira, como 1, 2, 3...")
				return false
			else
				if amount <= 0 then
					guiSetText(wSplitting,"A quantidade deve ser maior que 0!")
					return false
				end
			end
