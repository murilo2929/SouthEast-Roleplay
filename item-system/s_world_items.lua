	...
	if getElementData(source, "itemID") then
		...
		if type(faction) == "number" and exports.global:isStaffOnDuty(client) then
			...
			if protected then
				...
				outputChatBox("Definido para " .. faction .. " - se você quer para uma facção diferente, /itemprotect [facção ID ou -100]", client, 255, 0, 0)
				out = faction
			end
...
function dropItem(itemID, x, y, z, ammo, keepammo, cancelAnim)
	if disableCanDropPick then
		outputChatBox("O descarte de itens está desativado no momento. Enquanto nossos roteiristas investigam o problema.", source, 255, 0, 0)
	...
	if not ammo then
		...
		if not exports.global:isStaffOnDuty(source) then
			if ((itemID >= 31) and (itemID <= 43)) or itemBannedByAltAltChecker[itemID] then
				...
				if hoursPlayedFrom < 10 then
					outputChatBox("Você precisa de 10 horas de tempo de jogo para soltar um "..getItemName( itemID )..".", source, 255, 0, 0)
					triggerClientEvent( source, "finishItemDrop", source )
					return
				end
			end
		...
		if itemID == 220 or itemID == 221 or itemID == 219 or itemID == 162 then
			outputChatBox("Você não pode largar este item.", source, 255, 0, 0)
			triggerClientEvent( source, "finishItemDrop", source )
			return
		end
		...
		if itemID == 223 then
			...
			if not v or not v[1] or not v[2] or not v[3] then
				outputChatBox("Você quebrou este item! Faça com que ele seja gerado corretamente na próxima vez.", source, 255, 0, 0)
				return
			end
		end
		...
		if itemID == 115 or itemID == 116 then -- Weapons
			...
			if weaponDetails and weaponDetails[2] and tonumber(weaponDetails[2]) and tonumber(weaponDetails[2]) == 2  then -- /duty
				outputChatBox("Você não pode largar sua arma de serviço, desculpe.", source, 255, 0, 0)
				weaponBlock = true
			end

			if isThisGunDuplicated(itemID, itemValue, source) then
				takeItemFromSlot( source, itemSlot )
				outputChatBox("A arma caiu foi duplicada por um abusador de bugs. E não pode mais ser usado. Lamentamos excluí-lo agora.", source, 255,0,0)
				triggerClientEvent( source, "finishItemDrop", source )
				return false
			end
		end
		...
		if itemID == 60 or itemID == 137 or itemID == 138 or itemID == 175 and not exports.integration:isPlayerAdmin(source) then
			outputChatBox( "Este item não pode ser descartado.", source, 255, 0, 0 )
		elseif ( itemID == 81 or itemID == 103 or itemID == 223 ) and dimension == 0 then
			outputChatBox( "Você precisa soltar este item em um interior.", source )
		...
		elseif itemID == 139 or itemID == 262 or itemID == 263 then
			...
			elseif itemID == 134 then
				if not exports.global:takeMoney(source, itemValue) then
					outputDebugString(getPlayerName(source) .. ' (' .. getPlayerSerial(source) .. ') tentou jogar dinheiro que ele não tem')
			...
			elseif itemID == 169 then --Keypad Door Lock
				...
				local interiorName = "Desconhecido"
				...
				if not validInterior then
					exports.hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "O interior para o qual este dispositivo foi feito não existe mais. Até agora, este dispositivo não é mais utilizável.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end

				if not validDropper then
					exports.hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' não é mais sua propriedade. Até agora, este dispositivo não é mais utilizável.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if isIntLocked then
					exports.hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' está bloqueado, desbloqueie-o primeiro.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if not doorOutside or not doorInside then
					exports.hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' stá com defeito, notifique os criadores de scripts.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if interior == doorOutside.int and dimension == doorOutside.dim then
					if getDistanceBetweenPoints3D(x,y,z, doorOutside.x, doorOutside.y, doorOutside.z) > maxRange then
						exports.hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Este dispositivo só pode ser instalado em "..math.floor(maxRange/2).." metros de seu exterior.")
						triggerClientEvent( source, "finishItemDrop", source )
						return false
					end
				...
				elseif interior == doorInside.int and dimension == tonumber(itemValue) then
					if getDistanceBetweenPoints3D(x,y,z,doorInside.x, doorInside.y, doorInside.z) > maxRange then
						exports.hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Este dispositivo só pode ser instalado em "..math.floor(maxRange/2).." metros de seu interior.")
						triggerClientEvent( source, "finishItemDrop", source )
						return false
					end
				else
					exports.hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Este dispositivo só pode ser instalado em "..math.floor(maxRange/2).." metros de sua porta.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
			end
			...
			if getItemTexture(itemID, itemValue, metadata) then --if item is textured
				...
				if dimension ~= 0 or exceptions[itemID] or (exports.integration:isPlayerSupporter(source) and exports.global:isStaffOnDuty(source)) or (exports.integration:isPlayerTrialAdmin(source) and exports.global:isStaffOnDuty(source)) or exports.integration:isPlayerScripter(source) then
					...
					exports.hud:sendBottomNotification(source, getItemName(itemID, itemValue, metadata), "Apenas apoiadores e administradores podem descartar itens texturizados no mundo exterior. Use-o em seu interior.")
					triggerClientEvent(source, "finishItemDrop", source)
					return false
				end
			end
			...
			if insert then
				...
				if itemID == 134 then
					outputChatBox("Você largou $" .. exports.global:formatMoney(itemValue) .. ".", source, 255, 194, 14)
				elseif (not cancelAnim) then
					outputChatBox("Você largou um(a) " .. getItemName( itemID, itemValue, metadata ) .. ".", source, 255, 194, 14)
				end
				...
				if itemID == 134 then
					triggerEvent('sendAme', source, "largou $" .. exports.global:formatMoney(itemValue) .. ".")
				elseif (itemID==178) and tostring(itemValue):find( ":" ) then
					triggerEvent('sendAme', source, "largou um(a) " .. name .. ".")
				elseif (not cancelAnim) then
					triggerEvent('sendAme', source, "largou um(a) " .. getItemName( itemID, itemValue, metadata ) .. ".")
				end
			end
		end
	else
		if tonumber(getElementData(source, "duty")) > 0 then
			outputChatBox("Você não pode largar suas armas enquanto está em serviço.", source, 255, 0, 0)
		elseif tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
			outputChatBox("Você não pode deixar cair esta lata de spray.", source, 255, 0, 0)
		else
			...
			outputChatBox("Você largou um(a) " .. ( getWeaponNameFromID( itemID ) or "Colete" ) .. ".", source, 255, 194, 14)
			...
			if query then
				...
				triggerEvent('sendAme', source, "largou um(a) " .. getItemName( -itemID, nil, metadata ) .. ".")
...
function doItemGiveawayChecks(player, itemID, itemValue)
	...
	if itemID == 270 and getElementData(source, "gloves") and not hasItem(source, itemID) then
		triggerEvent('sendAme', source, "tira um par de luvas de suas mãos.")
...
local function moveItem(item, x, y, z)
	...
	if not item or not isElement(item) or getElementType(item) ~= 'object' then
		outputChatBox("Erro ao mover tem, entre em contato com um administrador", source, 255, 0, 0)
	...
	if not exports.global:isStaffOnDuty(source) then
		if ((itemID >= 31) and (itemID <= 43)) or itemBannedByAltAltChecker[itemID] then
			outputChatBox(getItemName( itemID ).." não pode ser movido desta forma.", source, 255, 0, 0)
			return false
		end
	end
	...
	if itemID == 138 then
		if not exports.integration:isPlayerAdmin(source) then
			outputChatBox("Apenas um líder + administrador pode mover este item.", source, 255, 0, 0)
			return
		end
	end

	if itemID == 139 and not exports.integration:isPlayerTrialAdmin(source) then
		outputChatBox("Apenas um Super + administrador pode mover este item.", source, 255, 0, 0)
		return
	end
	...
	for key, value in ipairs(getElementsByType("player")) do
		...
		if getDistanceBetweenPoints3D(oldx, oldy, oldz, getElementPosition(value)) < 1 then
			outputChatBox("Alguém está tentando mover o elemento em que você está (ou muito próximo), saia do caminho!", value, 255, 0, 0)
			return
		end
	end
	...
	function(x, y, z, rx, ry, rz)
		...
		if (dimension < 20000 and interior > 0 and hasItem(client, 4, dimension)) or (dimension < 20000 and interior > 0 and hasItem(client, 5, dimension)) or (dimension > 20000 and interior > 0 and hasItem(client, 3, dimension-20000)) or (exports.integration:isPlayerTrialAdmin(client) and exports.global:isAdminOnDuty(client)) or (exports.integration:isPlayerScripter(client) and exports.global:isStaffOnDuty(client)) or (dimension == 0) or (interior == 0) then
			if not canMove(client, source) then
				outputChatBox("Você não pode mover este item. Contate um administrador via F1.", client, 255, 0, 0)
				reset()
				return
			end
			...
			if itemID == 138 and not exports.integration:isPlayerAdmin(client) then
				outputChatBox("Apenas um líder + administrador pode mover este item.", client, 255, 0, 0)
				reset()
				return
			end

			if itemID == 139 and not exports.integration:isPlayerTrialAdmin(client) then
				outputChatBox("Apenas um Super + administrador pode mover este item.", client, 255, 0, 0)
				reset()
				return
			end
			...
			if mysql:query_free("UPDATE worlditems SET x = " .. x .. ", y = " .. y .. ", z = " .. z .. ", rx = " .. rx .. ", ry = " .. ry .. ", rz = " .. rz .. ", useExactValues = 1 WHERE id = " .. getElementData( source, "id" ) ) then
				...
				outputChatBox('Posição de item salva para o item #' .. getElementData( source, 'id' ) .. '.', client, 0, 255, 0)
			end
		else
			reset()
		end
...
function pickupItem(object, leftammo)
	if disableCanDropPick then
		outputChatBox("A coleta de itens está desativada no momento. Enquanto nossas escrituras investigam o assunto.", source, 255, 0, 0)
	...
	if not (getDistanceBetweenPoints3D(x, y, z, ox, oy, oz)<10) then
		outputDebugString("Distância entre o jogador e a retirada muito grande")
		return false
	end
	...
	if not id then
		outputChatBox("Error: Nenhum ID de item mundial. Notificar um criador de scripts. (s_world_items)",source,255,0,0)
	...
	if not canPickup(source, object) then
		outputChatBox("Você não pode pegar este item. Contate um administrador via F1.", source, 255, 0, 0)
	...
	if ((itemID >= 31) and itemID <= 43) or itemBannedByAltAltChecker[itemID] then
		...
		if hoursPlayedTo and hoursPlayedTo < 10 and not exports.global:isStaffOnDuty(source) then
			outputChatBox("Você precisa de 10 horas de tempo de jogo para pegar um(a) "..getItemName( itemID )..".", source, 255, 0, 0)
			exports.anticheat:setEld(object, "transfering", nil, 'all')
			return false
		end
		...
		if creator ~= picker then
			...
			if tonumber(accountCreator.id) == getElementData(source, "account:id") then
				outputChatBox("A transferência de bens entre personagens da mesma conta ou múltiplas contas de um jogador é estritamente proibida! ", source, 255, 0, 0)
				exports.global:sendMessageToAdmins("[ANTI-ALT->ALT]: Ativos ilegais detectados em transferência no nome da conta '"..getElementData(source, "account:username").."'.")
				exports.global:sendMessageToAdmins("[ANTI-ALT->ALT]: O suspeito está tentando transferir um(a) "..getItemName( itemID ).." entre personagens alternativos.")
	...
	if itemID == 115 or itemID == 116 then
		if isThisGunDuplicated(itemID, itemValue, source) then
			...
			outputChatBox("A arma que você tentou pegar foi duplicada por um abusador de insetos e não pode mais ser usada. Lamentamos excluí-lo agora.", source, 255,0,0)
			return false
		end
	...
	if itemID == 138 then
		if not exports.integration:isPlayerAdmin(source) then
			outputChatBox("Apenas um administrador completo pode pegar este item.", source, 255, 0, 0)
			exports.anticheat:setEld(object, "transfering", nil, 'all')
			return false
		end
	end
	...
	if itemID == 139 and not exports.integration:isPlayerTrialAdmin(source) then
		outputChatBox("Apenas o administrador pode retirar este item.", source, 255, 0, 0)
		exports.anticheat:setEld(object, "transfering", nil, 'all')
		return false
	end
	...
	if itemID > 0 then
		...
		while #getItems(object) > 0 do -- This one is suspicious / maxime
			...
			outputDebugString("Movi algo, pelo menos tentando")
		end
	else
		if itemID == -100 then
			mysql:query_free( "DELETE FROM worlditems WHERE id='" .. id .. "'")
	...
	if itemID == 134 then -- money
		...
		outputChatBox("Você pegou $" .. exports.global:formatMoney(itemValue) .. ".", source, 255, 194, 14)
		triggerEvent('sendAme', source, "se inclina e pega $" .. exports.global:formatMoney(itemValue) .. ".")
	elseif itemID == 178 then
		local yup = split(itemValue, ":")
		giveItem(source, itemID, itemValue)
		triggerEvent('sendAme', source, "se inclina e pega um(a) " .. getItemDescription( itemID, itemValue ) .. ".")
	else
		giveItem(source, itemID, itemValue, nil, nil, metadata)
		outputChatBox("Você pegou um(a) " .. getItemName( itemID, itemValue, metadata ) .. ".", source, 255, 194, 14)
		triggerEvent('sendAme', source, "se inclina e pega um(a) " .. getItemName( itemID, itemValue, metadata ) .. ".")
	end
