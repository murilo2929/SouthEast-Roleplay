...
function openMetadataEditor(item)
    ...
    elseif not canOpenMetadataEditor(localPlayer, item) then
        ...
        wEditor = guiCreateWindow(0, 0, main_width, main_height, "Propriedades do item", false)
        guiWindowSetSizable(wEditor, false)
        centerUnderCursor(wEditor)

        local bClose = guiCreateButton(10, main_height - 30, main_width, 25, "Fechar", false, wEditor)
        ...
        local lValue = guiCreateLabel(8, 37, main_width - 10, 13, "Valor Item: " .. getItemValue(item[1], item[2]), false, wEditor)
        if exports.integration:isPlayerTrialAdmin(localPlayer) or exports.integration:isPlayerScripter(localPlayer) then
            guiSetText(lValue, "Valor Item: " .. tostring(item[2]))
        end
        ...
        cKey = guiGridListAddColumn(grid, "Chave", 0.3)
...
function showEditingWindow()
    ...
    if row == -1 then
        ...
        if metadata_item.type == 'string' or metadata_item.type == 'integer' then
            ...
            wEditing = guiCreateWindow(0, 0, width, height, "Editando a Propriedade do Item", false)
            ...
            local bSave = guiCreateButton(width / 2, 60, width / 2 - 10, 25, "Salvar", false, wEditing)
            ...
            local commitChanges = function()
                ...
                outputChatBox("Incapaz de salvar, o item não está mais no seu inventario.", 255, 0, 0)
            end
            ...
            outputChatBox("Edição de " .. tostring(metadata_item.name) .. " ainda não suportado.", 255, 0, 0)
        end
    end
    -- editing_item
end
