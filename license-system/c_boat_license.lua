...
function createlicenseBoatTestIntroWindow()
	...
	guiIntroWindowBo = guiCreateWindow ( X , Y , Width , Height , "Teste de Teoria de Condução" , false )
	
	guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "banner.png", true, guiIntroWindowBo)
	
	guiIntroLabel1Bo = guiCreateLabel(0, 0.3,1, 0.5, [[Agora você vai prosseguir com o teste de teoria . Você irá
receber sete perguntas com base na teoria da navegação. Você deve pontuar
um mínimo de 80% para passar.

Boa sorte.]], true, guiIntroWindowBo)
	
	guiLabelSetHorizontalAlign ( guiIntroLabel1Bo, "center", true )
	guiSetFont ( guiIntroLabel1Bo,"default-bold-small")
	
	guiIntroProceedButtonBo = guiCreateButton ( 0.4 , 0.75 , 0.2, 0.1 , "Iniciar Teste" , true ,guiIntroWindowBo)
...
function createBoatLicenseQuestionWindow(number)
	...
	-- create the window
	guiQuestionWindowBo = guiCreateWindow ( X , Y , Width , Height , "Pergunta "..number.." de "..NoQuestionToAnswer , false )
	...
	if(number < NoQuestionToAnswer) then
		guiQuestionNextButtonBo = guiCreateButton ( 0.4 , 0.75 , 0.2, 0.1 , "Proxima Pergunta" , true ,guiQuestionWindowBo)
		...
		guiQuestionSumbitButtonBo = guiCreateButton ( 0.4 , 0.75 , 0.3, 0.1 , "Enviar Respostas" , true ,guiQuestionWindowBo)
...
function createBoatTestFinishWindow()
	...
	-- create the window
	guiFinishWindowBo = guiCreateWindow ( X , Y , Width , Height , "Fim do Teste.", false )
	
	if(score >= passPercent) then
	
		guiCreateStaticImage (0.35, 0.1, 0.3, 0.2, "pass.png", true, guiFinishWindowBo)
	
		guiFinalPassLabelBo = guiCreateLabel(0, 0.3, 1, 0.1, "Parabéns! Você passou no teste de teoria da navegação.", true, guiFinishWindowBo)
		...
		guiFinalPassTextLabelBo = guiCreateLabel(0, 0.4, 1, 0.4, "Você pontuou "..score.."%, e a marca de aprovação é "..passPercent.."%. Muito Bem!" ,true, guiFinishWindowBo)
		guiLabelSetHorizontalAlign ( guiFinalPassTextLabelBo, "center", true)
		
		guiFinalRegisterButtonBo = guiCreateButton ( 0.35 , 0.8 , 0.3, 0.1 , "Continuar" , true ,guiFinishWindowBo)
		...
		guiFinalFailLabelBo = guiCreateLabel(0, 0.3, 1, 0.1, "Desculpe, você não passou desta vez.", true, guiFinishWindowBo)
		...
		guiFinalFailTextLabelBo = guiCreateLabel(0, 0.4, 1, 0.4, "Você pontuou "..math.ceil(score).."%, e a marca de aprovação é "..passPercent.."%." ,true, guiFinishWindowBo)
		guiLabelSetHorizontalAlign ( guiFinalFailTextLabelBo, "center", true)
		
		guiFinalCloseButtonBo = guiCreateButton ( 0.2 , 0.8 , 0.25, 0.1 , "Fechar" , true ,guiFinishWindowBo)
