...
function mainGUI()
	...
	GUIEditor.window[1] = guiCreateWindow(532, 299, 542, 266, "Unidade de Licenciamento de Armas de Fogo", false)
	...
	GUIEditor.column[1] = guiGridListAddColumn(GUIEditor.gridlist[1], "Nome Completo", 0.3)
	...
	GUIEditor.button[1] = guiCreateButton(0.02, 0.81, 0.19, 0.14, "Procurar", true, GUIEditor.window[1])
	...
	GUIEditor.button[2] = guiCreateButton(0.24, 0.81, 0.19, 0.14, "Emitir", true, GUIEditor.window[1])
	...
	GUIEditor.button[3] = guiCreateButton(0.46, 0.81, 0.19, 0.14, "Revogar", true, GUIEditor.window[1])
	addEventHandler("onClientGUIClick", GUIEditor.button[3], function ()
			...
			if rindex ~= -1 then
				...
				outputChatBox("Você precisa selecionar um personagem da lista.")
			end
		end, false)

	GUIEditor.button[4] = guiCreateButton(0.79, 0.81, 0.19, 0.14, "Fechar", true, GUIEditor.window[1])
...
function fillTheGridlist()
	for k,v in ipairs(licenses_table) do
		...
		guiGridListSetItemText( GUIEditor.gridlist[1], row, GUIEditor.column[2], v["gun_license"] == "1" and "Sim" or "Não", false, false)
		guiGridListSetItemText( GUIEditor.gridlist[1], row, GUIEditor.column[3], v["gun2_license"] == "1" and "Sim" or "Não", false, false)
	end
end
...
function searchGUI()
	if isElement(search.window[1]) then destroyElement(search.window[1]) end
	search.window[1] = guiCreateWindow(596, 346, 376, 78, "Unidade de Licenciamento de Armas de Fogo - Search", false)
	guiWindowSetSizable(search.window[1], false)
	guiSetInputMode("no_binds_when_editing")

	search.label[1] = guiCreateLabel(0.04, 0.51, 0.23, 0.22, "Digite o nome completo", true, search.window[1])
	search.edit[1] = guiCreateEdit(0.29, 0.41, 0.37, 0.41, "", true, search.window[1])
	search.button[1] = guiCreateButton(0.68, 0.41, 0.13, 0.42, "Procurar", true, search.window[1])
	...
	search.button[2] = guiCreateButton(0.82, 0.41, 0.13, 0.42, "Fechar", true, search.window[1])
...
function issuelicenseGUI()
	if isElement(issue.window[1]) then destroyElement(issue.window[1]) end
	issue.window[1] = guiCreateWindow(628, 297, 322, 291, "Unidade de Licenciamento de Armas de Fogo - Emissão", false)
	...
	issue.label[1] = guiCreateLabel(0.04, 0.16, 0.40, 0.08, "Nome Completo", true, issue.window[1])
	issue.label[2] = guiCreateLabel(0.04, 0.34, 0.40, 0.08, "Nível de licença", true, issue.window[1])
	issue.progressbar[1] = guiCreateProgressBar(0.20, 0.57, 0.60, 0.11, true, issue.window[1])
	issue.button[1] = guiCreateButton(0.05, 0.76, 0.44, 0.18, "Emitir", true, issue.window[1])
	addEventHandler("onClientGUIClick", issue.button[1], function ()
		if guiGetText(issue.edit[1]) == "" then
			outputChatBox("Você não conseguiu inserir um nome.")
			return
		end
		if tonumber(guiGetText(issue.edit[1])) then
			outputChatBox("Você inseriu número(s) no nome.")
			return
		end
		...
		if licensetype == "Tier 2" then
			if exports.factions:hasMemberPermissionTo(getLocalPlayer(), 1, "add_member") or exports.factions:hasMemberPermissionTo(getLocalPlayer(), 50, "add_member") or exports.integration:isPlayerLeadAdmin(getLocalPlayer()) then
				...
				outputChatBox("Você não tem acesso para emitir uma licença Tier 2.")
				return false
			end
		end
		...
		timer3 = setTimer( function ()
				outputChatBox("Você deu "..targetname.." uma licença "..licensetype..".")
	...
	issue.button[2] = guiCreateButton(0.50, 0.76, 0.44, 0.18, "Fechar", true, issue.window[1])
...
function revokeGUI(tableid)
	...
	revoke.window[1] = guiCreateWindow(0.42, 0.33, 0.15, 0.14, "Unidade de Licenciamento de Armas de Fogo - Revogar licença", true)
	...
	revoke.button[1] = guiCreateButton(144, 51, 78, 28, "Revogar", false, revoke.window[1])
	addEventHandler("onClientGUIClick", revoke.button[1], function ()
			...
			if tier1 and not tier2 then
				...
				outputChatBox(name.." Teve sua licença de arma Tier 1 revogada.")
				triggerServerEvent("gunlicense:synctable", resourceRoot, licenses_table)
			elseif tier2 and not tier1 then
				table.remove(licenses_table, tableid)
				triggerServerEvent("gunlicense:revokemysql", resourceRoot, name)
				outputChatBox(name.." Teve sua licença de arma Tier 2 revogada.")
				triggerServerEvent("gunlicense:synctable", resourceRoot, licenses_table)
			elseif tier1 and tier2 then
				...
				elseif guiGetText(revoke.combobox[1]) == "Tier 2" then
					...
					outputChatBox("Você deve selecionar o tipo de licença que deseja revogar.")
				end
			end
	...
	revoke.button[2] = guiCreateButton(144, 84, 78, 28, "Fechar", false, revoke.window[1])
