...
function startTalkToPed ()
	...
	if (isPedInVehicle(thePlayer)) then
		...
		if (exports.vehicle:isVehicleWindowUp(theVehicle)) then
			outputChatBox("Você precisa abaixar a janela primeiro antes de falar com alguém fora do veículo.", thePlayer, 255,0,0)
			return
		end
	end
	...
	processMessage(thePed, "Ei, quer passar? Isso vai ser seis dólares por favor.")
	setConvoState(thePlayer, 1)
	local responseArray = { "Sim, por favor.", "Não, obrigado." }
	...
	if not tollElementsLocked[tollKey] or exports.global:hasItem(thePlayer, 64) or exports.global:hasItem(thePlayer, 65) or exports.global:hasItem(thePlayer, 112) then
		if payByBank then
			...
			if money >= 0 then
				...
				outputChatBox("[Pegadio] Sua conta bancária foi faturada $5.", thePlayer, 0, 255, 0)
			else
				return "Não há dinheiro suficiente em sua conta bancária."
			end
		else
			if not exports.global:takeMoney(thePlayer, 6) then
				return "Me desculpe, mas eu não posso deixar você passar então se você não puder pagar."
			end
		end
		...
		return "Desculpe, recebemos um pedido para não deixar ninguém passar."
	end
end
...
function talkToPed(answer, answerStr)
	...
	if (convState == 1) then --  "Hey, want to pass? That'll be six dollar please."
		...
		if (languageSkill < 60) or (currLang ~= 1) then
			processMessage(thePed, "Uh, desculpe, eu so falo ingles.")
		...
		elseif (answer == 2) then -- "No thanks."
			processMessage(thePed, "Certo, tudo bem...")
			setConvoState(thePlayer, 0)
		end
	end
end
...
function testToggle(thePlayer, commandName, gateID, silent)
	...
	if not gateID or not tonumber(gateID) then
		...
		for tollID, tollName in ipairs(tollElementsName) do
			...
			outputChatBox(" "..tostring(tollID).. " - "..tollName .." - Estado: "..state, thePlayer)
		end
		return
	end
	...
	if not tollElementsName[gateID] then
		outputChatBox("Não existe.", thePlayer)
	end

	tollElementsLocked[gateID] = not tollElementsLocked[gateID]
	if tollElementsLocked[gateID] then
		local first = true
		exports['chat-system']:departmentradio(thePlayer, "d", "Por favor, bloqueie o pedagio "..tollElementsName[gateID]..", over.")
		for _, thePed in ipairs(getElementsByType('ped')) do
			local tollKey = getElementData(thePed, "toll:key")
			if tollKey and tollKey == gateID then
				if first then
					exports['chat-system']:departmentradio(thePed, "d", "10-4, trancando "..tollElementsName[gateID]..", out.")
					first = false
				end
				processRadio(thePed, "Comms para as unidades em "..tollElementsName[gateID]..", não deixe ninguém passar, out!", thePed)
			end
		end
	else
		local first = true
		exports['chat-system']:departmentradio(thePlayer, "d", "Por favor abra o pedagio "..tollElementsName[gateID]..", over.")
		for _, thePed in ipairs(getElementsByType('ped')) do
			local tollKey = getElementData(thePed, "toll:key")
			if tollKey and tollKey == gateID then
				if first then
					exports['chat-system']:departmentradio(thePed, "d", "10-4, abrindo "..tollElementsName[gateID]..", out.")
					first = false
				end
				processRadio(thePed, "Comms para as unidades em "..tollElementsName[gateID]..", abra a cabine de pedágio novamente!", thePed)
			end
		end
	end
end
...
function processRadio(thePed, message, source)
	...
	exports['global']:sendLocalText(source, "[Ingles] " .. name .. "'s Radio: "..message, 255, 255, 255)
end
...
function tollCommand(thePlayer)
	...
	if duty > 0 and getElementDimension( thePlayer ) == 0 then
		...
		if not any then
			outputChatBox("Você não está perto de nenhum pedágio.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("toll", tollCommand)
addCommandHandler("pedagio", tollCommand)
