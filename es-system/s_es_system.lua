...
function playerDeath(totalAmmo, killer, killerWeapon)
	if getElementData(source, "dbid") then
		...
		elseif getElementData(source, "jailed") then
			...
			local killstr = ' faleceu'
			if (killer) then
				if getElementType(killer) == "player" then
					if (killerWeapon) then
						killstr = ' foi morto por '..getPlayerName(killer):gsub("_", " ").. ' ('..getWeaponNameFromID ( killerWeapon )..')'
					else
						killstr = ' faleceu'
					end
					table.insert(affected, killer)
				else
				killstr = ' foi morto por uma fonte desconhecida'
			...
			-- if killer and (getElementData(killer, "hoursplayed" ) >= 20) then
				-- victimDropItem = true
			-- end
			changeDeathViewTimer = setTimer(changeDeathView, 60000, 1, source, victimDropItem)

			outputChatBox("Se você foi morto devido ao DM ou algo semelhante, /report para que um administrador o reviva.", source)
			outputChatBox("Se você aceitar sua morte, poderá perder alguns de seus itens - a menos que seja revivido.", source)

			outputChatBox("Você podera reviver daqui 1 minuto.", source)
...
function acceptDeath(victimDropItem)
	if isPedDead(client) then
		...
		outputChatBox("Respawnando...", client)
		...
		outputChatBox("Você não está morto!", client, 255, 0, 0)
	end
end
...
addCommandHandler("mostrarmortes", readLog)
...
function respawnPlayer(thePlayer, victimDropItem)
	if (isElement(thePlayer)) then
		...
		if (getElementData(thePlayer, "loggedin") == 0) then
			exports.global:sendMessageToAdmins("AC0x0000004: "..getPlayerName(thePlayer):gsub("_", " ").." morreu enquanto não estava no personagem, desencadeando blackfade.")
			return
		end
		...
		outputChatBox("Você recebeu tratamento do Corpo de Bombeiros de Los Santos.", thePlayer, 255, 255, 0)
		...
		if count > 0 then
			outputChatBox("LSFD Funcionário: Nós entregamos seus medicamentos ao SFPD.", thePlayer, 255, 194, 14)
		end
					...
					if cart_id and gunlicense == 0 and gunlicense2 == 0 then
						if exports['item-system']:takeItemFromSlot(thePlayer, itemSlot-cor) then
							...
							if (removedWeapons == nil) then
								...
								removedWeapons = removedWeapons .. ", " .. ammountOfAmmo .. " munições de " .. pack.cartridge
								formatedWeapons = formatedWeapons .. "\n" .. ammountOfAmmo .. " munições de " .. pack.cartridge
							end
						end
					end
				end
		...
		if (removedWeapons~=nil) then
			if gunlicense == 0 and factiontype ~= 2 then
				outputChatBox("Funcionário LSFD: Retiramos armas para as quais você não tinha licença. (" .. removedWeapons .. ").", thePlayer, 255, 194, 14)
			else
				outputChatBox("Funcionário da LSFD: Retiramos armas que você não tem permissão para carregar. (" .. removedWeapons .. ").", thePlayer, 255, 194, 14)
			end
		end
		...
		if removedWeapons ~= nil then
			logMe(death)
			exports.global:sendMessageToAdmins("/mostrarmortes para ver armas perdidas.")
...
function recoveryPlayer(thePlayer, commandName, targetPlayer, duration)
	if not (targetPlayer) or not tonumber(duration) or (tonumber(duration) or 0) <= 0 then
		outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador / ID] [Horas]", thePlayer, 255, 194, 14)
	else
					...
					--if (targetPlayer == thePlayer) then
						...
						--if (dimesion==9) then
							...
							if totaltime < 12 then
								...
								outputChatBox("Você colocou com sucesso " .. targetPlayerName .. " em recuperação por " .. duration .. " hora(s) e cobrado $".. 100*totaltime ..".", thePlayer, 255, 0, 0)
								exports.global:sendMessageToAdmins("AdmWrn: " .. targetPlayerName .. " foi colocado em recuperação por " .. duration .. " hora(s) por " .. getPlayerName(thePlayer):gsub("_"," ") .. ".")
								outputChatBox("Você foi colocado em recuperação por " .. getPlayerName(thePlayer) .. " por " .. duration .. " hora(s) e cobrado $".. 100*totaltime ..".", targetPlayer, 255, 0, 0)
								exports.logs:dbLog(thePlayer, 4, targetPlayer, "RECOVERY " .. duration)
							else
								outputChatBox("Você não pode colocar alguém em recuperação por tanto tempo.", thePlayer, 255, 0, 0)
							end
						--[[else
...
addCommandHandler("recuperacao", recoveryPlayer)
		...
		if (logged==1) then -- Check all logged in players.
			...
			if (inRecovery == 1) then
				...
				if (recoveryEndsAt <= currentTime) then -- Is the time up? If yes:
					...
					outputChatBox("Você não está mais em recuperação!", value, 0, 255, 0) -- Let them know about it!
				else
					...
					if (player==value) and (eventname=="accounts:characters:spawn") then
						outputChatBox("Você ainda está em recuperação.", value, 255,0,0)
					end
				end
			end
		end
	...
	if (logged==1) then
        ...
        if (inRecovery == 1) then
			...
			if (recoveryEndsAt <= currentTime) then -- Is the time up? If yes:
				...
				mysql:query_free("UPDATE characters SET recovery='0', recoverytime=NULL WHERE id = " .. dbid) -- Allow them to move, and revert back to recovery type set to 0.
				outputChatBox("Você não está mais em recuperação!", thePlayer, 0, 255, 0) -- Let them know about it!
			else
				...
				local minutesLeft = math.floor(recoveryEndsAt / 60)
           		outputChatBox("Você tem " .. hoursLeft .. " hora(s) e " .. minutesLeft .. " minuto(s) de recuperação.", thePlayer)
           	end
        end
    end
end
	...
	if (logged==1) then
		...
		if (inRecovery == 1) then
			...
			if (recoveryEndsAt <= currentTime) then -- Is the time up? If yes:
				...
				mysql:query_free("UPDATE characters SET recovery='0', recoverytime=NULL WHERE id = " .. dbid) -- Allow them to move, and revert back to recovery type set to 0.
				outputChatBox("Você não está mais em recuperação!", player, 0, 255, 0) -- Let them know about it!
			else
				...
				outputChatBox("Você ainda está em recuperação.", player, 255,0,0)
			end
		end
	end
end
...
function prescribe(thePlayer, commandName, ...)
	if exports.factions:isInFactionType(thePlayer, 4) then
		if not (...) then
			outputChatBox("SYNTAX /" .. commandName .. " [valor de prescrição]", thePlayer, 255, 184, 22)
		else
			local itemValue = table.concat({...}, " ")
			itemValue = tonumber(itemValue) or itemValue
			if not(itemValue=="") then
				exports.global:giveItem( thePlayer, 132, itemValue )
				outputChatBox("A prescrição '" .. itemValue .. "' foi processado.", thePlayer, 0, 255, 0)
...
addCommandHandler("prescrever", prescribe)
		...
		if not (targetPlayer) then
			...
			if targetPlayer then
				if isPedDead  ( targetPlayer ) then
					...
					outputChatBox("Você foi revivido por "..tostring(exports.global:getPlayerAdminTitle(thePlayer)).." "..tostring(getPlayerName(thePlayer):gsub("_"," "))..".", targetPlayer, 0, 255, 0)
					outputChatBox("Você reviveu "..tostring(getPlayerName(targetPlayer):gsub("_"," "))..".", thePlayer, 0, 255, 0)
					...
					outputChatBox(tostring(getPlayerName(targetPlayer):gsub("_"," ")).." não esta morto.", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function medicalBill(thePlayer, commandName, targetPlayer, amount)
	...
	if medicalBillFactions[dutyFaction] then
		if not (targetPlayer) or not tonumber(amount) or (tonumber(amount) or 0) <= 0 then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador / ID] [Quantia]", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				if (logged==1) then
					...
					outputChatBox("Conta médica enviada para "..tostring(targetPlayerName)..".", thePlayer, 245, 217, 7)
				else
					outputChatBox("O jogador não está logado.", thePlayer, 255,0,0)
				end
			else
...
addCommandHandler("contamedica", medicalBill)
	...
	if action == 1 then --pay by cash
		if exports.global:takeMoney(client, toPay) then
			...
			if exports.global:giveMoney(theTeam, amount) then
				...
				outputChatBox("Você pagou sua conta médica ($"..tostring(exports.global:formatMoney(amount))..").", client, 0, 250, 0)
				outputChatBox(name.." pagou sua conta médica ($"..tostring(exports.global:formatMoney(amount))..").", doctor, 0, 250, 0)
			end
		else
			outputChatBox("Você não tem dinheiro suficiente para pagar a conta. Pegando da conta bancária.", client, 255, 0, 0)
			payMedicalBill(client, doctor, faction, amount, insurance, 2)

			local name = exports.global:getPlayerName(client):gsub("_", " ")
			outputChatBox(name.." não tem dinheiro para pagar a conta médica.", doctor, 255, 0, 0)
		end
	elseif action == 2 then --pay by bank
		if exports.bank:hasBankMoney(client, toPay) then
			if exports.bank:takeBankMoney(client, toPay) then
				...
				outputChatBox("Ocorreu um erro de transação. Conta médica não paga.", client, 255, 0, 0)
				local name = exports.global:getPlayerName(client):gsub("_", " ")
				outputChatBox(name.." não pôde pagar a conta médica devido a um erro de transação.", doctor, 255, 0, 0)
			end
		else
			outputChatBox("Você não pode pagar a conta médica.", client, 255, 0, 0)
			local name = exports.global:getPlayerName(client):gsub("_", " ")
			outputChatBox(name.." não pode pagar a conta médica.", doctor, 255, 0, 0)
		end
	elseif action == 3 then --refuse to pay
		local name = exports.global:getPlayerName(client):gsub("_", " ")
		outputChatBox(name.." recusou-se a pagar a conta médica.", doctor, 255, 0, 0)
	end
end
