...
function updateUnansweredReports()
	local info = {}
	table.insert(info, {string.upper(" Reportes não respondidos"), 255,194,14,255,1,"default-bold" })
				...
				if staff then
					for k,v in ipairs(staff) do
						if string.find(adminTeams, v) and not seenReport[i] then
							table.insert(info, {abrv.." Reporte #" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' reportando '" .. tostring(getPlayerName(reported)) .. "' as " .. timestring .. ". Responsavel: " .. handler .. "", r, g, b})
							seenReport[i] = true
						end
					end
					count = count + 1
...
function updateUnansweredReportsGMs()
	local info = {}
	table.insert(info, {string.upper(" Reportes GameMaster não respondidos"), 255,194,14,255,1,"default-bold" })
				...
				if staff then
					for k,v in ipairs(staff) do
						if SUPPORTER == v and not seenReport[i] then
							table.insert(info, {abrv.." Report #" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' reportando '" .. tostring(getPlayerName(reported)) .. "' as " .. timestring .. ". Responsavel: " .. handler .. "", r, g, b})
							seenReport[i] = true
						end
					end
					count = count + 1
