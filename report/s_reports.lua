...
function sortReports(showMessage)
	...
	-- reports[slot][2] = reportedPlayer -- Jogador Reportado
	...
	for key , report in pairs(reports) do
		if report[8] ~= key then
			if isSupporterReport(report[7]) then
				...
				if showMessage then
					outputChatBox("Seu reporte ID#"..report[8].." foi deslocado para ID#"..key.." devido aos relatórios na frente terem sido resolvidos.", report[1], 70, 200, 30)
				end
			else -- Admin report
				adminNotice = adminNotice.."#"..report[8]..", "
				gmNotice = gmNotice.."#"..report[8]..", "
				if showMessage then
					outputChatBox("Seu reporte ID#"..report[8].." foi deslocado para ID#"..key.." devido aos relatórios na frente terem sido resolvidos.", report[1], 255, 195, 15)
				end
			end
	...
	if showMessage then
		if adminNotice ~= "" then
			...
			for key, value in ipairs(admins) do
				...
				if (adminduty==1) then
					outputChatBox(" Reportes com ID "..adminNotice.." foram deslocados para cima.", value, 255, 195, 15)
				end
			end
		end
		if gmNotice ~= "" then
			...
			for key, value in ipairs(gms) do
				...
				if (gmDuty == 1) then
					outputChatBox(" Reportes com ID "..gmNotice.." foram deslocados para cima.", value, 70, 200, 30)
				end
			end
		end
		...
		for key, value in ipairs(players) do
			...
			if (logged==1) then
				...
				if requested=="requested" then
					ckcount = 1
					outputChatBox("Solicitação Self-CK de '" .. pname .. "' ("..playerID..") pela razão '" .. reason .. "'.", thePlayer, 255, 195, 15)
				end
			end
		end

		if ckcount == 0 then
			outputChatBox("Nenhum.", thePlayer, 255, 194, 15)
		else
			outputChatBox("Use /cka [id] ou /ckd [id] para responder ao pedido.", thePlayer, 255, 194, 15)
		end
	end
end
	...
	if exports.integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			if reports[id] then
				...
				local playerID = getElementData(reporter, "playerid") or "Desconhecido"
				local reportedID = getElementData(reported, "playerid") or "Desconhecido"


				if staff then
					outputChatBox(" ["..abrv.." #" .. id .."] (" .. playerID .. ") " .. tostring(getPlayerName(reporter)) .. " reportou (" .. reportedID .. ") " .. tostring(getPlayerName(reported)) .. " at " .. timestring .. ".", thePlayer, r, g, b)
					outputChatBox("Razão: " .. reason, thePlayer, 70, 200, 30)
					...
					if (isElement(admin)) then
						...
						outputChatBox(" [#" .. id .."] Este relatório está sendo tratado por " .. getPlayerName(admin) .. " ("..adminName..").", thePlayer, 70, 200, 30)
					else
				...
				outputChatBox("ID de Reporte Invalido.", thePlayer, 255, 0, 0)
			end
		end
	end
end
...
function changeReportType(thePlayer, commandName, id, rID)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerScripter(thePlayer) then
		if not (id) or not (rID) then
			outputChatBox("SYNTAX: " .. commandName .. " [Report ID] [Tipo Reporte ID]", thePlayer, 255, 194, 15)
			...
			if reportID > #reportTypes or reportID < 1 then
				outputChatBox("Error: Nenhum tipo de relatório com esse ID encontrado.", thePlayer, 255, 0, 0)
				return
			end
			if reports[id] then
				local reporter = reports[id][1]
				...
				if oldReportType == reportID then
					outputChatBox("Error: O relatório já era desse tipo.", thePlayer, 255, 0, 0)
					return
				end
				...
				if not staff then
					outputChatBox("Nenhum membro da equipe auxiliar desse tipo online.", thePlayer, 255, 0, 0)
				...
				outputChatBox("Seu reporte foi mudado de '"..oname.."'' para '"..name.."' by "..adminUser, reporter, 255, 126, 0)
				for k, v in ipairs(staff) do
					if string.find(auxiliaryTeams, v) then
						outputChatBox("Reporte #"..id.." - foi mudado para "..name..".", thePlayer, 255, 126, 0)
						for key, value in pairs(players) do
							if getElementData(value, "loggedin") == 1 then
								if string.find(getElementData(value, "forum_perms"), v) then
									...
									outputChatBox("Razão: " .. reason, value, 200, 240, 120)
								end
							end
						end
					else
						if isSupporterReport(oldReportType) then
							for key, value in pairs(GMs) do
								local gmDuty = getElementData(value, "duty_supporter")
								if (gmDuty == 1) then
									outputChatBox("Report #"..id.." - foi mudade de '"..oname.. "'' para '"..name.."'", value, 255, 126, 0)
								end
							end
	...
	if originalReportID then
		...
		if report and reports[report] then
			...
			if (isElement(theAdmin)) then
				outputChatBox(" ["..abrv.." #" .. report .."] Player " .. getPlayerName(source) .. " saiu do jogo.", theAdmin, 255, 126, 0)--200, 240, 120)
				outputChatBox("Você foi creditado por lidar com este reporte.", theAdmin, 255, 126, 0)
							...
							for key, value in ipairs(getElementsByType("players")) do
								if getElementData(value, "loggedin") == 1 then
									if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
										outputChatBox(" ["..abrv.." #" .. report .."] Player " .. getPlayerName(source) .. " saiu do jogo.", value, 255, 126, 0)
	...
	for i = 1, #reports do
						...
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
									...
									outputChatBox(" ["..abrv.." #" .. i .."] Reporte não atribuído (" .. adminName .. " saiu do jogo)", value, 255, 126, 0)
									alreadyTold[value] = true
									update = true
								end
							end
						...
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
									...
									if adminduty == 1 or gmduty == 1 then
										local adminName = getElementData(source, "account:username")
										outputChatBox(" ["..abrv.." #" .. i .."] Reporte não atribuído (" .. adminName .. " saiu do jogo)", value, 255, 126, 0)--200, 240, 120)
										alreadyTold[value] = true
										update = true
									end
								end
		...
		elseif reports[i] and reports[i][2] == source then
					...
					if string.find(auxiliaryTeams, usergroup) then
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
									local adminName = getElementData(source, "account:username")
									outputChatBox(" ["..abrv.." #" .. i .."] Jogador Reportado " .. getPlayerName(source) .. " saiu do jogo.", value, 255, 126, 0)
									alreadyTold[value] = true
									update = true
								end
							end
						...
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
									...
									local adminduty = getElementData(value, "duty_admin")
									if adminduty == 1 or gmduty == 1 then
										outputChatBox(" ["..abrv.." #" .. i .."] Jogador Reportado " .. getPlayerName(source) .. " saiu do jogo.", value, 255, 126, 0)--200, 240, 120)
										update = true
										alreadyTold[value] = true
									end
								end
			...
			if reporter ~= source then
				local adminName = getElementData(source, "account:username")
				outputChatBox("Seu reporte "..abrv.."#" .. i .. " foi fechado (" .. adminName .. " saiu do jogo)", reporter, 255, 126, 0)--200, 240, 120)
				...
				if reports[i][5] then
					...
					outputChatBox(" ["..abrv.." #" .. report .."] Player " .. adminName .. " saiu do jogo.", reports[i][5], 255, 126, 0)
					outputChatBox("Você foi creditado por lidar com este reporte.", reports[i][5], 255, 126, 0)
				end
			else
				exports.anticheat:setEld( source, "reportNum", nil, 'one' )
			end
		...
		for i = 1, #reports do
			if reports[i] then
				...
				if not staff then
					...
					outputChatBox("Seu reporte foi fechado automaticamente.", reports[i][1], 255, 0, 0)
					reports[i] = nil
					update = true
					exports.anticheat:changeProtectedElementDataEx(reporter, "adminreport", false, true)
					exports.anticheat:changeProtectedElementDataEx(reporter, "gmreport", false, true)
...
function handleReport(reportedPlayer, reportedReason, reportType)
	...
	if getElementData(reportedPlayer, "loggedin") ~= 1 then
		outputChatBox("O jogador que você está denunciando não está logado.", source, 255, 0, 0)
		return
	end
	if getElementData(source, "loggedin") ~= 1 then
		outputChatBox("você não está logado.", source, 255, 0, 0)
		return
	end
	...
	for _, usergroup in ipairs(staff) do
		if string.find(SUPPORTER, usergroup) then -- Supporters
			...
			for key, value in ipairs(GMs) do
				...
				if (gmDuty == 1) then
					nigger = nigger + 1
					outputChatBox(" ["..abrv.." #" .. slot .."] (" .. playerID .. ") " .. tostring(getPlayerName(source)) .. " pediu ajuda.", value, r, g, b)
					outputChatBox("Duvida: " .. reportedReason, value, 200, 240, 120)
					outputChatBox("Use /ar [id] para ajuda-lo.", value, 255, 194, 15)
			...
			if not skipadmin then
				...
				-- Show to GMs
				--local reason1 = reportedRazão:sub( 0, 70 )
				--local reason2 = reportedRazão:sub( 71 )
				for key, value in ipairs(GMs) do
					...
					if (gmDuty == 1) then
						...
						outputChatBox(" ["..abrv.." #" .. slot .."] (" .. playerID .. ") " .. tostring(getPlayerName(source)) .. " pediu ajuda.", value, r, g, b)--200, 240, 120)
						outputChatBox("Duvida: " .. reportedReason, value, 200, 240, 120)
						outputChatBox("Use /ar [id] para ajuda-lo.", value, 255, 194, 15)
						skipadmin = true
						-- if reason2 and #reason2 > 0 then
			...
			outputChatBox("Obrigado por enviar seu reporte. (Report ID: #" .. tostring(slot) .. ").", source, 70, 200, 30)

			outputChatBox("Você pode fechar este reporte a qualquer momento digitando /er.", source, 70, 200, 30)
		elseif string.find(auxiliaryTeams, usergroup) then -- Auxiliary Teams
			for key, value in ipairs(getElementsByType("player")) do
				if getElementData(value, "loggedin") == 1 then
					if string.find(getElementData(value, "forum_perms"), usergroup) then -- Opens up functionality to have reports ONLY go to leaders or only members
						...
						outputChatBox("Razão: " .. reportedReason, value, 200, 240, 120)
					end
				end
			end
			outputChatBox("Obrigado por enviar seu "..name.." report. (Report ID: #" .. tostring(slot) .. ").", source, 200, 240, 120)
			outputChatBox("Você reportou (" .. reportedID .. ") " .. tostring(getPlayerName(reportedPlayer)) .. ". Razão: ", source, 237, 145, 33 )
			outputChatBox(reportedReason, source, 200, 240, 120)
			outputChatBox("Você pode fechar este reporte a qualquer momento digitando /er.", source, 200, 240, 120)
			break
		else -- Admins
			local admins = exports.global:getAdmins()
			local count = 0
			...
			if not skipadmin then
				...
				if not playergotit then
					...
					-- end
					outputChatBox("Um admin responderá ao seu reporte o mais rápido possível. Atualmente existem " .. count .. " admin" .. ( count == 1 and "" or "s" ) .. " disponiveis.", source, 200, 240, 120)
					outputChatBox("Você pode fechar este reporte a qualquer momento digitando /er.", source, 200, 240, 120)
					playergotit = true
				end
			end
...
function subscribeToAdminsReports(thePlayer)
	if exports.integration:isPlayerSupporter(thePlayer) then
		if getElementData(thePlayer, "report-system:subcribeToAdminReports") then
			...
			outputChatBox("Você cancelou a assinatura de reportes de administração.",thePlayer, 255,0,0)
		else
			setElementData(thePlayer, "report-system:subcribeToAdminReports", true)
			outputChatBox("Você se inscreveu para reportes de administração.",thePlayer, 0,255,0)
		end
	end
end
addCommandHandler("showadminreports", subscribeToAdminsReports)
addCommandHandler("verreportes", subscribeToAdminsReports)
			...
			for k, usergroup in ipairs(staff) do
				if string.find(auxiliaryTeams, usergroup) then
					for key, value in ipairs(getElementsByType("player")) do
						if getElementData(value, "loggedin") == 1 then
							if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
								outputChatBox(" [#" .. id .. "] ainda não foi respondido: (" .. playerID .. ") " .. tostring(getPlayerName(reportingPlayer)) .. " reportado (" .. reportedID .. ") " .. tostring(getPlayerName(reportedPlayer)) .. " as " .. timestring .. ".", value, 200, 240, 120)
								alreadyTold[value] = true
							end
						end
					end
...
function pendingReportTimeout(id)
	if (reports[id]) then
		...
		outputChatBox("[" .. timestring .. "] Seu reporte (#" .. id .. ") has expired.", reportingPlayer, 200, 240, 120)
		outputChatBox("[" .. timestring .. "] If you still require assistance, please resubmit Seu reporte or visit our forums (http://forums.owlgaming.net).", reportingPlayer, 200, 240, 120)
		sortReports(false)
		updateReportCount()
	end
end]]
		...
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					...
					outputChatBox("Report #" .. id .. " já está sendo tratado por " .. getPlayerName(reportHandler) .. " ("..getElementData(reportHandler,"account:username")..")", thePlayer, 255, 0, 0)
				else
					local reportingPlayer = reports[id][1]
					local reportedPlayer = reports[id][2]
					...
					if not found and not exports.integration:isPlayerLeadAdmin(thePlayer) then
						outputChatBox("Você não pode falsificar um reporte que não tenha a ver com a sua divisão de pessoal.", thePlayer, 255, 0, 0)
						return
					end
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
											outputChatBox(" [#" .. id .. "] - "..adminTitle.." ".. getPlayerName(thePlayer) .. " ("..adminUsername..") marcou reporte #" .. id .. " como falso. -", value, r, g, b)
											alreadyTold[value] = true
										end
									end
								end
								...
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox(" [#" .. id .. "] - "..adminTitle.." ".. getPlayerName(thePlayer) .. " ("..adminUsername..") marcou reporte #" .. id .. " como falso. -", value, r, g, b)--200, 240, 120)
												alreadyTold[value] = true
											end
										end
									end
					...
					outputChatBox("[" .. timestring .. "] Seu reporte (#" .. id .. ") foi marcado como falso by "..adminTitle.." ".. getPlayerName(thePlayer) .. " ("..adminUsername..").", reportingPlayer, r, g, b)--200, 240, 120)
					triggerClientEvent ( reportingPlayer, "playNudgeSound", reportingPlayer, "Seu reporte foi marcado false by "..adminTitle.." "..adminUsername, "warning")
					--local accountID = getElementData(thePlayer, "account:id")
					--exports.logs:dbLog({"ac"..tostring(accountID), thePlayer }, 38, {reportingPlayer, reportedPlayer}, getPlayerName(thePlayer) .. " maked a report como falso. Report: " .. reason )
					sortReports(true)
					updateReportCount()
				end
			end
...
function arBind()
	if exports.integration:isPlayerTrialAdmin(client) then
		exports.global:sendMessageToAdmins("AdmWarn: ".. getPlayerName(client) .. " tem relatório de aceitação vinculado às chaves.")
	end
end
	...
	if exports.integration:isPlayerStaff(thePlayer) then
		if not (id) then
			...
			if not (reports[id]) then
				...
				if (reportHandler) then
					outputChatBox("Report #" .. id .. " já está sendo tratado por " .. getPlayerName(reportHandler) .. ".", thePlayer, 255, 0, 0)
				else
					...
					if reportingPlayer == thePlayer and not exports.integration:isPlayerScripter(thePlayer) then
						outputChatBox("Você não pode aceitar seu próprio relatório.",thePlayer, 255,0,0)
						return false
					--[[
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
											outputChatBox(" ["..abrv.." #" .. id .. "] - "..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") aceitou o reporte #" .. id .. " -", value, r, g, b)
											alreadyTold[value] = true
										end
									end
								end
								...
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox(" ["..abrv.." #" .. id .. "] - "..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") aceitou o reporte #" .. id .. " -", value, r, g, b)--200, 240, 120)
												alreadyTold[value] = true
											end
										end
									end
					...
					local reportedID = getElementData(reportedPlayer, "playerid") or "Desconhecido"
					outputChatBox(adminTitle.." " .. getPlayerName(thePlayer) .. " ("..adminName..") aceitou seu reporte (#" .. id .. ") at "..timestring..", Por favor, espere que ele entre em contato com você.", reportingPlayer, 255,126, 0)--200, 240, 120)
					outputChatBox("Você estava reportando (" .. reportedID .. ") " .. tostring(getPlayerName(reportedPlayer)) .. ". Razão: ", reportingPlayer, 255, 126, 0)
					outputChatBox(reason, reportingPlayer, 255, 126, 0)


					triggerClientEvent ( reportingPlayer, "playNudgeSound", reportingPlayer, adminTitle .. " " .. adminName .. " aceitou seu reporte!", "info")

					outputChatBox("Você aceitou o reporte #" .. id .. ". Contate o jogador ID #" .. playerID .. " (" .. getPlayerName(reportingPlayer) .. ").", thePlayer, r, g, b)--200, 240, 120)
...
function acceptAdminReport(thePlayer, commandName, id, ...)
	...
	if (exports.integration:isPlayerLeadAdmin(thePlayer)) then
		if not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Reporte ID] [NomeAdmin]", thePlayer, 255, 194, 14)
		else
			...
			if targetAdmin then
				...
				if not (reports[id]) then
					...
					if (reportHandler) then
						outputChatBox("Reporte #" .. id .. " já está sendo tratado por " .. getPlayerName(reportHandler) .. ".", thePlayer, 255, 0, 0)
					else
						local reportingPlayer = reports[id][1]
						local reportedPlayer = reports[id][2]
						local reason = reports[id][3]
						...
						outputChatBox("[" .. timestring .. "] "..adminTitle.." " .. getPlayerName(targetAdmin) .. " aceitou seu reporte (#" .. id .. "), Por favor, espere que eles entrem em contato com você.", reportingPlayer, 200, 240, 120)
						outputChatBox("Um reporte atribuído pelo administrador principal #" .. id .. " para você. Por favor, prossiga para contatar o jogador ( (" .. playerID .. ") " .. getPlayerName(reportingPlayer) .. ").", targetAdmin, 200, 240, 120)
						local alreadyTold = { }
							...
							for k, usergroup in ipairs(staff) do
								if string.find(auxiliaryTeams, usergroup) then
									for key, value in ipairs(getElementsByType("player")) do
										if getElementData(value, "loggedin") == 1 then
											if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
												outputChatBox(" ["..abrv.." #" .. id .. "] - " .. getPlayerName(theAdmin) .. " aceitou o reporte #" .. id .. " (Assigned) -", value, r, g, b)
												alreadyTold[value] = true
											end
										end
									end
									...
									for key, value in ipairs(getElementsByType("player")) do
										if getElementData(value, "loggedin") == 1 then
											if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
												...
												local gmduty = getElementData(value, "duty_supporter")
												if (adminduty==1) or (gmduty==1) then
													outputChatBox(" ["..abrv.." #" .. id .. "] - " .. getPlayerName(theAdmin) .. " aceitou o reporte #" .. id .. " (Assigned) -", value, r, g, b)--200, 240, 120)
													alreadyTold[value] = true
												end
											end
										end
...
function transferReport(thePlayer, commandName, id, ...)
	...
	if (exports.integration:isPlayerLeadAdmin(thePlayer)) then
		if not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Report ID] [NomeAdmin]", thePlayer, 200, 240, 120)
		else
			local targetAdmin, username = exports.global:findPlayerByPartialNick(thePlayer, adminName)
			if targetAdmin then
				local id = tonumber(id)
				...
				elseif (reports[id][5] ~= thePlayer) and not (exports.integration:isPlayerAdmin(thePlayer)) then
					outputChatBox("Esse não é o seu reporte, pal.", thePlayer, 255, 0, 0)
				else
					local reportingPlayer = reports[id][1]
					local reportedPlayer = reports[id][2]
					...
					outputChatBox("[" .. timestring .. "] " .. getPlayerName(thePlayer) .. " entregou seu reporte para ".. getPlayerName(targetAdmin) .." (#" .. id .. "), Por favor, espere que ele entre em contato com você.", reportingPlayer, 200, 240, 120)
					outputChatBox(getPlayerName(thePlayer) .. " entregou o reporte #" .. id .. " para você. Por favor, prossiga para contatar o jogador ( (" .. playerID .. ") " .. getPlayerName(reportingPlayer) .. ").", targetAdmin, 200, 240, 120)
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
											outputChatBox(" [#" .. id .. "] - " .. getPlayerName(thePlayer) .. " entregou o reporte #" .. id .. " para  ".. getPlayerName(targetAdmin) , value, r, g, b)
											alreadyTold[value] = true
										end
									end
								end
								...
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox(" [#" .. id .. "] - " .. getPlayerName(thePlayer) .. " entregou o reporte #" .. id .. " para  ".. getPlayerName(targetAdmin) , value, r, g, b)--200, 240, 120)
												alreadyTold[value] = true
											end
										end
									end
		...
		if not (id) then
			...
			elseif (reports[id][5] ~= thePlayer) then
				...
				if (isElement(reporter)) then
					...
					exports.anticheat:changeProtectedElementDataEx(reporter, "reportadmin", false, false)
					exports.anticheat:setEld( reporter, "reportNum", nil, 'one' )
					outputChatBox(adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") Fechou seu reporte.", reporter, r, g, b)
					...
					for k, usergroup in ipairs(staff) do
						if string.find(auxiliaryTeams, usergroup) then
							for key, value in ipairs(getElementsByType("player")) do
								if getElementData(value, "loggedin") == 1 then
									if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
										outputChatBox(" ["..abrv.." #" .. id .. "] - "..adminTitle.." " .. getPlayerName(thePlayer) .. " ("..adminName..") fechou o reporte #" .. id .. ". -", value, r, g, b)
										alreadyTold[value] = true
									end
								end
							end
							...
							for key, value in ipairs(getElementsByType("player")) do
								if getElementData(value, "loggedin") == 1 then
									if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
										...
										local gmduty = getElementData(value, "duty_supporter")
										if (adminduty==1) or (gmduty==1) then
											outputChatBox(" ["..abrv.." #" .. id .. "] - "..adminTitle.." " .. getPlayerName(thePlayer) .. " ("..adminName..") fechou o reporte #" .. id .. ". -", value, r, g, b)--200, 240, 120)
											alreadyTold[value] = true
										end
									end
								end
...
function closeAllReports(thePlayer)
	if exports.integration:isPlayerStaff(thePlayer) then
		...
		if count == 0 then
			outputChatBox(" Nenhum foi fechado.", thePlayer, 255, 126, 0)--255, 194, 15)
		else
			outputChatBox(" Você terminou de atender "..count.." de seus reportes.", thePlayer, 255, 126, 0)--255, 194, 15)
		end
	end
end
		...
		if not (id) then
			...
			if (reports[id] == nil) then
				...
				if (reports[id][5] ~= thePlayer) then
					outputChatBox("Você não está lidando com este reporte.", thePlayer, 255, 0, 0)
				else
					...
					if (isElement(reporter)) then
						...
						exports.anticheat:changeProtectedElementDataEx(reporter, "reportadmin", false, false)
						outputChatBox(adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") lançou o seu reporte. Aguarde até que outro funcionário aceite o reporte.", reporter, r, g, b)
					end
						...
						for k, usergroup in ipairs(staff) do
							if string.find(auxiliaryTeams, usergroup) then
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
											outputChatBox(" ["..abrv.." #" .. id .. "] - "..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") largou o reporte #" .. id .. ". -", value, r, g, b)
											alreadyTold[value] = true
										end
									end
								end
								...
								for key, value in ipairs(getElementsByType("player")) do
									if getElementData(value, "loggedin") == 1 then
										if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
											...
											local gmduty = getElementData(value, "duty_supporter")
											if (adminduty==1) or (gmduty==1) then
												outputChatBox(" ["..abrv.." #" .. id .. "] - "..adminTitle.." "..getPlayerName(thePlayer) .. " ("..adminName..") largou o reporte #" .. id .. ". -", value, r, g, b)--200, 240, 120)
												alreadyTold[value] = true
											end
										end
									end
...
function endReport(thePlayer, commandName)
	...
	if not adminreport or not report then
		outputChatBox("Você não tem reportes pendentes. Pressione F1 para criar um.", thePlayer, 255, 0, 0)
		...
		outputChatBox("[" .. timestring .. "] Você fechou seu reporte enviado ID #"..report, thePlayer, 200, 240, 120)
		local otherAccountID = nil
		if (isElement(reportHandler)) then
			outputChatBox(getPlayerName(thePlayer) .. " fechou o pedido de reporte (#" .. report .. ").", reportHandler, 255, 126, 0)--200, 240, 120)
...
function showUnansweredReports(thePlayer)
	if exports.integration:isPlayerStaff(thePlayer) then
		if showTopRightReportBox(thePlayer) then
			...
			outputChatBox("~~~~~~~~~ Reportes não respondidos ~~~~~~~~~", thePlayer, 0, 255, 15)
			--reports = sortReportsByTime(reports)
			local count = 0
			local seenReport = { }
			for i = 1, #reports do
						...
						if staff then
							for k,v in ipairs(staff) do
								if string.find(getElementData(thePlayer, "forum_perms"), v) and not seenReport[i] then
									outputChatBox("Reporte "..abrv.."#" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' reportando '" .. tostring(getPlayerName(reported)) .. "' as " .. timestring .. ".", thePlayer, r, g, b)
									count = count + 1
									seenReport[i] = true
								end
							end
			...
			if count == 0 then
				...
				outputChatBox("Digite /ri [id] para obter mais informações sobre o reporte.", thePlayer, 255, 194, 15)
			end
		end
	end
end
					...
					if staff then
						for k,v in ipairs(staff) do
							if (string.find(getElementData(thePlayer, "forum_perms"), v) or exports.integration:isPlayerTrialAdmin(thePlayer)) and not seenReport[i] then
								outputChatBox("Report "..abrv.."#" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' reportando '" .. tostring(getPlayerName(reported)) .. "' as " .. timestring .. ". Responsavel: " .. handler .. "", thePlayer, r, g, b)
								count = count + 1
								seenReport[i] = true
							end
						end
...
function getSavedReports(thePlayer)
	...
	outputChatBox("Você ajudou "..adminreports_saved.." reportes. "..reportsToAward-adminreports_saved.." para recompença!", thePlayer, 255, 126, 0)
end
...
function setSavedReports(thePlayer, cmd, reports)
	...
	outputChatBox(" Você definiu a contagem de relatórios salvos para "..reports..".", thePlayer, 255, 126, 0)
end
