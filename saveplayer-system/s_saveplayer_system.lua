...
function savePlayer(reason, player)
	...
	if isElement(player) then
		...
		if (logged==1 or reason=="Change Character") then
			...
			local fome = getElementData(player, "char.Hunger") or 0
			local sede = getElementData(player, "char.Thirst") or 0
			...
			dbExec(exports.mysql:getConn("mta"), "UPDATE characters SET fome = ?, sede = ?, x = ?, y = ?, z = ?, rotation = ?, health = ?, armor = ?, dimension_id = ?, interior_id = ?, lastlogin=NOW(), lastarea = ?, timeinserver = ?, alcohollevel = ? WHERE id = ?", fome, sede, x, y, z, rot, health, armor, dimension, interior, zone, timeinserver, tostring(alcohollevel), getElementData(player, "dbid"))
