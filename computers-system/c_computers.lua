...
function createComputerGUI(computerName, websiteToStartOn)
	...
	wComputer = guiCreateWindow(computerX, computerY, computerWidth, computerHeight + 29, computerName or "Computador", false)
	...
	local toggleInput = guiCreateButton(0, computerHeight - 24, 90, 24, "Alternar entrada", false, desktopImage)
	...
	if key == "left" and state == "down" then
		if(source == chatButton) then
			if (guiGetInputEnabled( )) then
				...
				showCursor(false)
				outputChatBox("Caixa de bate-papo ativa", 0, 255, 0, true)
				outputChatBox("Pressione M no teclado para voltar e usar o computador")
			else
				guiSetInputEnabled(true)
				showCursor(false)
				outputChatBox("Janela do computador ativa", 0, 255, 0, true)
			end
		end
	end
end
