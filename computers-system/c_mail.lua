...
function openEmailWindow()
    ...
    if not (wEmail) then
        ...
        -- Username
        username_label = guiCreateLabel(13,54,63,16,"Usuario:",false,wEmail)
        username_input = guiCreateEdit(78,52,200,22,"J.Smith",false,wEmail)

        address_label = guiCreateLabel(13,88,63,16,"Fornecedor:",false,wEmail)
        ...
        password_label = guiCreateLabel(13,154,63,16,"Senha:",false,wEmail)
        ...
        addEventHandler("onClientGUIClick",email_login_button,function()
            ...
            guiSetText(email_error_label,"Digite um nome de usuário!")
            ...
            guiSetText(email_error_label,"Você precisa selecionar um provedor!")
            elseif (tostring(guiGetText(password_input))=="") then
                guiSetText(email_error_label,"Insira uma senha!")
            else
        ...
        -- Register
        email_register_button = guiCreateButton(164,184,70,26,"Registrar",false,wEmail)
...
function name_error()
    guiSetText(email_error_label, "Nome de usuário já em uso!")
end
addEvent("name_in_use",true)
addEventHandler("name_in_use", getLocalPlayer(),name_error)

function login_error()
    guiSetText(email_error_label, "Nome de usuário ou senha inválidos!")
end
addEvent("loginError",true)
addEventHandler("loginError", getLocalPlayer(),login_error)

function not_leader()
    guiSetText(email_error_label, "Você não pode se registrar neste provedor!")
end
...
function show_inbox(inbox_table, accountName)
    if not (wEmail) then
        ...
        send_tab = guiCreateTab("Criar Mensagem",email_tab_panel)
        ...
        ------------------
        -- To:
        new_message_to_label = guiCreateLabel(0.1,0.1,0.2,0.05,"Para:",true,send_tab)
        new_message_to_input = guiCreateEdit(0.2,0.1,0.3,0.05,"",true,send_tab)
        -- Subject:
        new_message_subject_label = guiCreateLabel(0.1,0.2,0.2,0.05,"Assunto:",true,send_tab)
        new_message_subject_input = guiCreateEdit(0.2,0.2,0.5,0.05,"",true,send_tab)
        -- Message:
        new_message_content_label = guiCreateLabel(0.1,0.3,0.3,0.05,"Mensagem:",true,send_tab)
        new_message_content = guiCreateMemo(0.1,0.4,0.8,0.45,"",true,send_tab)
        -- Send Button
        send_button = guiCreateButton(0.4,0.9,0.2,0.08,"Enviar",true,send_tab)
    ...
    inbox_grid_date = guiGridListAddColumn(inbox_grid,"Data",0.16)
    inbox_grid_sender = guiGridListAddColumn(inbox_grid,"De",0.3)
    inbox_grid_subject = guiGridListAddColumn(inbox_grid,"Assunto",0.5)
    ...
    if not(inbox_message_datetitle_label) then
        ...
        inbox_message_datetitle_label = guiCreateLabel(0.02,0.4,0.96,0.1,"Data recebida:",true,inbox_tab)
        ...
        inbox_message_fromtitle_label = guiCreateLabel(0.5,0.4,0.96,0.1,"De:",true,inbox_tab)
        ...
        inbox_message_subjecttitle_label = guiCreateLabel(0.02,0.45,0.96,0.1,"Assunto:",true,inbox_tab)
    ...
    if not (check_mail_button) then
        check_mail_button = guiCreateButton(0.2,0.9,0.2,0.08,"Checar Email",true,inbox_tab)
        ...
        inbox_delete_mail_button = guiCreateButton(0.4,0.9,0.2,0.08,"Deletar",true,inbox_tab)
        ...
        --REPLY BUTTON
        inbox_reply_button = guiCreateButton(0.6,0.9,0.2,0.08,"Responder",true,inbox_tab)
...
function show_outbox(outbox_table,accountName)
    ...
    outbox_grid_date = guiGridListAddColumn(outbox_grid,"Data",0.16)
    outbox_grid_sender = guiGridListAddColumn(outbox_grid,"De",0.3)
    outbox_grid_subject = guiGridListAddColumn(outbox_grid,"Assunto",0.5)
    ...
    if not(outbox_message_datetitle_label) then
        -- Static labels showing date and sender
        outbox_message_datetitle_label = guiCreateLabel(0.02,0.4,0.96,0.1,"Data recebida:",true,outbox_tab)
        ...
        outbox_message_totitle_label = guiCreateLabel(0.5,0.4,0.96,0.1,"Para:",true,outbox_tab)
        ...
        outbox_message_subjecttitle_label = guiCreateLabel(0.02,0.45,0.96,0.1,"Assunto:",true,outbox_tab)
    ...
    if not (outbox_delete_mail_button) then
        ...
        outbox_delete_mail_button = guiCreateButton(0.4,0.9,0.2,0.08,"Deletar",true,outbox_tab)
...
function invalid_address()
    guiSetText(new_message_to_input, "Endereço não encontrado!")
end
