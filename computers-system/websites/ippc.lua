...
local function www_ippc_sa_themes_main(desiredLength)
	...
	local headertxt2 = guiCreateLabel(140,27,170,25,"Centro de planejamento piloto da Internet",false,header)

	local usertxt1 = guiCreateLabel(wrapper_width-100,5,100,15,"Logado como",false,header)
	...
	local menu_link2_txt = guiCreateLabel(10,30,80,20,"Voos",false,nav)
	...
	local menu_link3_txt = guiCreateLabel(10,55,80,20,"Novo Plano de Voo",false,nav)
	...
	local menu_link5_txt = guiCreateLabel(10,105,80,20,"Graficos",false,nav)
	...
	local menu_link6_txt = guiCreateLabel(10,130,80,20,"Meu Perfil",false,nav)
	...
	menu_link8_txt = guiCreateLabel(10,180,80,20,"Portões",false,nav)
	...
	local footer_txt = guiCreateLabel(0,0,wrapper_width,20,"Serviço IPPC fornecido por FAA.",false,footer)
...
function www_ippc_sa()
	...
	local para1 = guiCreateLabel(10,30,400,240, "O IPPC é uma ferramenta online para pilotos, oferecida pelae FAA.\n\nEste site oferece aos pilotos acesso fácil a informações importantes e úteisn.\n\nOs pilotos podem usar o IPCC para criar, editar e visualizar planos de vôo que são enviados diretamente para a FAA e controladores de tráfego aéreo.",false,content)
...
function www_ippc_sa_flights()
	...
	-- Content --
	-------------	
	local content_title = guiCreateLabel(10,5,400,25,"Voos",false,content)
		guiSetFont(content_title, guiCreateFont(":fonts/helveticastrong.ttf", 20))
		guiLabelSetColor(content_title,0,0,0)

	local para1 = guiCreateLabel(10,30,50,20, "Mostrar: ",false,content)
		guiSetFont(para1, "clear-normal")
		guiLabelSetColor(para1,0,0,0)
		guiLabelSetHorizontalAlign(para1, "left", true)	

	selectFlights = guiCreateComboBox(60,30,250,80,"",false,content)
		local item = guiComboBoxAddItem(selectFlights, "Tudo")
			guiComboBoxSetSelected(selectFlights, item)
		guiComboBoxAddItem(selectFlights, "Meus Voos")

	selectFlightsTime = guiCreateComboBox(320,30,100,67,"",false,content)
		local item = guiComboBoxAddItem(selectFlightsTime, "proximos")
			guiComboBoxSetSelected(selectFlightsTime, item)
		guiComboBoxAddItem(selectFlightsTime, "passado")

	flightsGrid = guiCreateGridList(10, 70, 540, 250, false, content)
		guiGridListAddColumn(flightsGrid, "Voo", 0.155)
		guiGridListAddColumn(flightsGrid, "ETD", 0.2)
		guiGridListAddColumn(flightsGrid, "De", 0.16)
		guiGridListAddColumn(flightsGrid, "Para", 0.16)
...
function www_ippc_sa_fpl()
	...
	-- Content --
	-------------	
	local content_title = guiCreateLabel(10,5,400,25,"Plano de vôo",false,content)
		guiSetFont(content_title, guiCreateFont(":fonts/helveticastrong.ttf", 20))
		guiLabelSetColor(content_title,0,0,0)
