...
function loadCEFURLDirectly(url)
    ...
    if not loaded then
        ...
        requestBrowserDomains(urls, is_url, function(wasAccepted)
            if wasAccepted then
                outputDebugString("Solicitação de domínio aceita pelo usuário, carregando " .. url)
                setTimer(function() loadBrowserURL(cefBrowser, url) end, 1000, 1)
            else
                outputDebugString("Solicitação de domínio recusada pelo usuário para " .. url)
                updateDisplayedInfo()
            end
        end)
    end
...
function openBrowser(home_url)
    ...
    -- CEF
    blockedResources = {}
    bUnblockResources = guiCreateButton(computerWidth - 160, 25, 120, 25, "Recursos de desbloqueio", false, wBrowser)
    ...
    addEventHandler("onClientBrowserCreated", zBrowser, function()
        ...
        addEventHandler("onClientBrowserLoadingFailed", source, function(...) outputDebugString("Falha ao carregar: " .. toJSON({...})) end)
