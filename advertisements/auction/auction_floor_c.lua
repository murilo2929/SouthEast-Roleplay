...
local function timeRemaining(expiry)
    ...
    if daysLeft > 0 then
        str = daysLeft .. (daysLeft > 1 and " Dias" or " Dia")
    elseif hoursLeft > 0 then
        str = hoursLeft .. (hoursLeft > 1 and " Horas" or " Hora")
    elseif minutesLeft > 0 then
        str = minutesLeft .. (minutesLeft > 1 and " Minutos" or " Minuto")
    else
        str = secondsLeft .. (secondsLeft > 1 and " Segundos" or " Segundo")
    end
...
local function renderVehicle (vehicle)
    ...
    local text = vehicle:getData("year") .. " " .. vehicle:getData("brand") .. " " .. vehicle:getData("maximemodel")
        .. "\n Descrição: " .. vehicle:getData("auction_vehicle:description")
        .. "\n\n" .. timeRemaining(tonumber(vehicle:getData("auction_vehicle:expiry"))) .. " Restante"
        .. "\nOferta Atual $" .. formatNumberAsCommaSeparated(vehicle:getData("auction_vehicle:current_bid"))
        .. "\nCompra $" .. formatNumberAsCommaSeparated(vehicle:getData("auction_vehicle:buyout"))
