function openDutyWindow()
	...
	if #availablePackages > 0 then
		...
		outputChatBox("Não há nenhum local de trabalho disponível para você neste local!")
	end
end
...
function selectPackageGUI_open(availablePackages, allowList)
	...
	gui["_root"] = guiCreateWindow(left, top, windowWidth, windowHeight, "Armario", false)
	...
	for pIndex, packageDetails in ipairs(availablePackages) do
		...
		gui[guiTabName.."-cancel"] = guiCreateButton(10, 400, 200, 35, "Cancelar", false, gui[guiTabName])
