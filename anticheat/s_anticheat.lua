...
function showSpeedToAdmins(velocity)
	...
	exports.global:sendMessageToAdmins("[Possível Speedhack/HandlingHack] " .. getPlayerName(client) .. ": " .. velocity .. "Mph/".. kph .." Kph")
end
...
function showDMToAdmins(kills)
	exports.global:sendMessageToAdmins("[Possível DeathMatching] " .. getPlayerName(client) .. ": " .. kills .. " mortes em <=2 Minutos.")
end
...
function scanMoneyHacks()
	...
	for key, value in ipairs(hackers) do
		...
		outputChatBox("AntiCheat: " .. targetPlayerName .. " foi banido automaticamente por Money Hacks. (" .. tostring(money) .. "$)", getRootElement(), 255, 0, 51)
	end
end
...
function outputModInfo(thePlayer, command, targetPlayer)
	...
	if not targetPlayer then
		outputChatBox("SYNTAX: /" .. command .. " [Nome Parcial Jogador/ID]", thePlayer, 255, 194, 14)
		return
	else
			...
			if mods[targetPlayer] then
				for filename, modList in pairs(mods[targetPlayer]) do
					...
					for idx,item in ipairs(modList) do
						outputChatBox( idx .. ") id: " .. item.id .. " nome: " .. item.name, thePlayer, 255, 194, 14)
						if item.sizeX then
							outputChatBox( "tamanho: " .. item.sizeX .. "," .. item.sizeY .. "," .. item.sizeZ, thePlayer, 255, 194, 14)
							outputChatBox( "TamanhoOriginal: " .. item.originalSizeX .. "," .. item.originalSizeY .. "," .. item.originalSizeZ, thePlayer, 255, 194, 14)
						end
						if item.length then
							outputChatBox( "comprimento: " .. item.length .. " md5: " .. item.md5, thePlayer, 255, 194, 14)
						end
					end
				end
			else
				outputChatBox("Nenhuma modificação encontrada.", thePlayer, 255, 194, 14)
			end
		else
