...
function createElevator(thePlayer, commandName, oneway)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		if not getElementData(thePlayer, "adm:addelevator") then
			...
			outputChatBox("[ADDELEVATOR] Ponto de origem salvo. Por favor /"..commandName.." mais uma vez no ponto de destino.", thePlayer, 0, 255, 0)
			return false
		else
		...
		if id then
			...
			if (query) then
				...
				loadOneElevator(id)
				outputChatBox("[ADDELEVATOR] Elevador e controle remoto de elevador criado com ID #" .. id .. ". Verifique seu inventário!", thePlayer, 0, 255, 0)
				exports.global:giveItem(thePlayer, 73, id)
				removeElementData(thePlayer, "adm:addelevator")
			end
		else
			outputChatBox("[ADDELEVATOR] Ocorreu um erro ao criar um elevador. Tente novamente.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("addelevator", createElevator, false, false)
addCommandHandler("adde", createElevator, false, false)
addCommandHandler("criarelevador", createElevator, false, false)
...
function createElevatorWithFriend(thePlayer, commandName, targetPlayer)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		if not (targetPlayer) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nome Parcial Jogador ou ID]", thePlayer, 255, 194, 14)
		else
			...
			if id then
				...
				if (query) then
					loadOneElevator(id)
					outputChatBox("Elevador criado com ID #" .. id .. "!", thePlayer, 0, 255, 0)
					outputChatBox(getPlayerName(thePlayer):gsub("_"," ") .. " criou um elevador com um ID de " .. id .. "!", targetPlayer, 0, 255, 0)
				end
			else
				outputChatBox("Ocorreu um erro ao criar um elevador. Tente novamente.", thePlayer, 255, 0, 0)
			end
		end
	end
end
...
addCommandHandler("criarelevador2", createElevatorWithFriend, false, false)
...
function enterElevator(goingin)
	...
	if elevatorStatus[INTERIOR_TYPE] == 3 then
		outputChatBox("Você tenta a maçaneta da porta, mas parece estar trancada.", player, 255, 0,0, true)
		return false
	end
	...
	if ( ( vehicle and elevatorStatus[INTERIOR_TYPE]  ~= 0 and getVehicleOccupant( vehicle ) == player ) or not vehicle ) then
		if not vehicle and elevatorStatus[INTERIOR_TYPE]  == 2 then
			outputChatBox( "Esta entrada é apenas para veículos.", player, 255, 0, 0 )
			return false
		end

		if elevatorStatus[INTERIOR_DISABLED] then
			outputChatBox( "Este interior está atualmente desativado.", player, 255, 0, 0 )
			return false
		end
		...
		if oneway then
			if goingin then
				outputChatBox("Parece que esta porta só pode ser aberta do outro lado.", player, 255, 0,0, true)
				return false
			end
		end
		...
		elseif isElement(player) then
			...
			outputChatBox("Este elevador está trancado.", player, 255, 0,0, true)
			return false
		end
	end
			...
			if elevatorElement then
				...
				if query then
					...
					if commandName ~= "PROPERTYCLEANUP" then
						outputChatBox("Elevador #" .. id .. " Deletado!", thePlayer, 0, 255, 0)
					end
				else
					if commandName ~= "PROPERTYCLEANUP" then
				...
				if commandName ~= "PROPERTYCLEANUP" then
					outputChatBox("O ID do elevador não existe!", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
addCommandHandler("removerelevador", deleteElevator, false, false)
...
function getNearbyElevators(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerScripter(thePlayer)) then
		...
		outputChatBox("Elevadores proximos:", thePlayer, 255, 126, 0)
		local found = false
				...
				if (point[INTERIOR_DIM] == dimension) then
					...
					if (distance <= 11) then
						...
						if point == elevatorEntrance then
							outputChatBox(" ID " .. dbid ..", levando à dimensão "..elevatorExit[INTERIOR_DIM], thePlayer, 255, 126, 0)
						else
							outputChatBox(" ID " .. dbid ..", levando à dimensão "..elevatorEntrance[INTERIOR_DIM], thePlayer, 255, 126, 0)
						end
...
addCommandHandler("elevadoresproximos", getNearbyElevators, false, false)

--TEMP FIX ELEVATOR FOR PLAYERS / MAXIME
function fixNearbyElevator(thePlayer)
	local posX, posY, posZ = getElementPosition(thePlayer)
	local dimension = getElementDimension(thePlayer)
	outputChatBox("Consertando elevadores próximos:", thePlayer, 255, 126, 0)
	local found = false
	local possibleElevators = getElementsByType("elevator")
	for _, elevator in ipairs(possibleElevators) do
		local elevatorEntrance = getElementData(elevator, "entrance")
			...
			if (point[INTERIOR_DIM] == dimension) then
				...
				if (distance <= 11) then
					if elevator then
						...
						if point == elevatorEntrance then
							reloadOneElevator(dbid)
							outputChatBox(" ID de elevador arrumado " .. dbid ..", levando a int ID #"..elevatorExit[INTERIOR_DIM], thePlayer, 255, 126, 0)
						else
							reloadOneElevator(dbid)
							outputChatBox(" ID de elevador arrumado " .. dbid ..", levando a int ID #"..elevatorEntrance[INTERIOR_DIM], thePlayer, 255, 126, 0)
						end
					end
	...
	if not found then
		outputChatBox("   Não há elevadores por aqui, peça a um administrador para criar um novo.", thePlayer, 255, 126, 0)
	end
end
...
addCommandHandler("arrumarelevador", fixNearbyElevator, false, false)
...
function delNearbyElevators(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		local posX, posY, posZ = getElementPosition(thePlayer)
		local dimension = getElementDimension(thePlayer)
		outputChatBox("Excluindo elevadores próximos:", thePlayer, 255, 126, 0)
		local found = false
					...
					if (distance <= 11) then
						...
						if point == elevatorEntrance then
							if deleteElevator(thePlayer, "dele", dbid) then
								outputChatBox(" Elevador ID #" .. dbid .." foi deletado.", thePlayer, 255, 126, 0)
							end
						else
							if deleteElevator(thePlayer, "dele", dbid) then
								outputChatBox(" Elevador ID #" .. dbid .." foi deletado.", thePlayer, 255, 126, 0)
		...
		if not found then
			outputChatBox("   nenhum foi deletado.", thePlayer, 255, 126, 0)
		end
	end
end
...
addCommandHandler("removerelevadoresproximos", delNearbyElevators, false, false)
...
addEventHandler( "toggleCarTeleportMode", getRootElement(),
	function( player )
		...
		if query then
			...
			if mode == 0 then
				outputChatBox( "Você mudou o modo para 'jogadores apenas'.", player, 0, 255, 0 )
			elseif mode == 1 then
				outputChatBox( "Você mudou o modo para 'jogadores e veículos'.", player, 0, 255, 0 )
			elseif mode == 2 then
				outputChatBox( "Você mudou o modo para 'veículos apenas'.", player, 0, 255, 0 )
			else
				outputChatBox( "Você mudou o modo para 'sem entrada'.", player, 0, 255, 0 )
			end
		else
	...
	if exports.integration:isPlayerTrialAdmin( thePlayer ) or exports.integration:isPlayerSupporter(thePlayer) then
		...
		if not id then
			...
			if elevatorElement then
				...
				outputChatBox( "Elevador não encontrado.", thePlayer, 255, 194, 14 )
			end
		end
	end
end
