...
function createLift(thePlayer, commandName, lift, floor, ...)
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer)) then
		...
		if(not lift or not floor or not name) then
			outputChatBox("SYNTAX: /" .. commandName .. " [elevador ID] [andar] [nome]", thePlayer, 255, 194, 14)
			outputChatBox("Defina o ID do elevador como 0 para criar um novo elevador. Use o ID para adicionar um andar ao elevador existente.", thePlayer, 255, 194, 14)
			return false
		end
		if(string.len(floor) > 3) then
			outputChatBox("AddLift: o andar deve ter de 1 a 3 caracteres. E.g. '1', '203' or 'U1'.", thePlayer, 255, 0, 0)
			return false
		end
		...
		elseif(lift > 0) then
			...
			if id then
				...
				if (query) then
					loadOneLiftFloor(id)
					outputChatBox("Andar com ID #"..id.." adicionado ao elevador #".. lift ..".", thePlayer, 0, 255, 0)
				end
			else
				outputChatBox("Ocorreu um erro ao criar um elevador. Tente novamente.", thePlayer, 255, 0, 0)
			end
...
addCommandHandler("acriarelevador", createLift, false, false)
...
function enterLiftFloor(otherCP, movingInSameInt, pickup, floorNum)
	...
	if isElement(player) then
		...
		if(not isInteriorLocked(dimension)) then
			local meText = "pega o elevador até o andar "..tostring(floorNum).."."
	...
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		if not (tonumber(id)) then
			...
			if element then
				...
				if query then
					reloadOneLift(dbid)
					outputChatBox("Elevador #" .. id .. " Deletador!", thePlayer, 0, 255, 0)
				else
					outputChatBox("ELE0015 Error, please report to a scripter.", thePlayer, 255, 0, 0)
				end
			else
				outputChatBox("Este ID não existe!", thePlayer, 255, 0, 0)
			end
		end
	end
end
addCommandHandler("dellift", deleteLift, false, false)
addCommandHandler("aremoverelevador", deleteLift, false, false)
...
addCommandHandler("aelevadoresproximos", getNearbyLifts, false, false)
	...
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		...
		if not id then
			...
			if element then
				...
				outputChatBox( "elevaodr não encontrado.", thePlayer, 255, 194, 14 )
			end
		end
	end
end
addCommandHandler( "togglelift", toggleLift )
addCommandHandler( "ativarelevador", toggleLift )
