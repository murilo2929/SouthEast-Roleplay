	...
	if(thePlayer == getLocalPlayer()) then
		if(getElementDimension(thePlayer) == getElementDimension(source)) then --matchingDimension
			if(getElementData(source, "rpp.lift.floor.id")) then
				...
				gOwnerName = guiCreateLabel(0.0, 0.90, 1.0, 0.3, "Pressione F para usar.", true)
...
function triggerLiftGUI(key, keyState, pickup)
	...
	if(keyState == "down" and not wElevator) then
		...
		local columnFloor = guiGridListAddColumn(floorList, "Andar", 0.2)
		local columnName = guiGridListAddColumn(floorList, "Nome", 0.7)
		
		bGoto = guiCreateButton(0.05, 0.84, 0.45, 0.1, "IR PARA", true, wElevator)
		...
		lblCurrent = guiCreateLabel(0.05, 0.05, 0.9, 0.05, "Esse andar: "..tostring(getElementData(pickup, "rpp.lift.floor.floor")).." - "..tostring(getElementData(pickup, "rpp.lift.floor.name")), true, wElevator)
		...
		if floors then
			...
			outputChatBox("Você aperta o botão do elevador, mas nada acontece.", 255, 0, 0)
			hideElevatorGUI()
		end
	end
end
...
function gotoFloor(button, state)
	if (button=="left") then
		...
		if (row==-1) or (col==-1) then
			...
			exports.hud:sendBottomNotification(localPlayer, "Elevador", "Selecione um andar.")
		else
			...
			if not pickup then
				hideElevatorGUI()
				outputChatBox("ERROR: Andar não encontrado.", 255, 0, 0)
				return false
			end
	...
	if (button=="right") then
		if(exports.integration:isPlayerTrialAdmin(localPlayer)) then
			...
			if (row==-1) or (col==-1) then
				...
				wElevatorEdit = guiCreateWindow(x, y, width, height, "Editar andar elevador", false)

				local y = 0.05
				lbl1 = guiCreateLabel(0.05, 0.1, 0.9, 0.112, "Elevador ID: "..tostring(liftID)..", Andar ID: "..tostring(floorID), true, wElevatorEdit)
				y = y + 0.225
				lbl2 = guiCreateLabel(0.05, y, 0.3, 0.112, "Numero andar:", true, wElevatorEdit)
				editFloorNum = guiCreateEdit(0.35, y, 0.6, 0.112, tostring(floorNum), true, wElevatorEdit)
				y = y + 0.225
				lbl3 = guiCreateLabel(0.05, y, 0.3, 0.112, "Nome andar:", true, wElevatorEdit)
				editFloorName = guiCreateEdit(0.35, y, 0.6, 0.112, tostring(floorName), true, wElevatorEdit)
				y = y + 0.225
				btnSave = guiCreateButton(0.05, y, 0.27, 0.225, "Salvar", true, wElevatorEdit)
				btnDelete = guiCreateButton(0.3625, y, 0.27, 0.225, "Deletar", true, wElevatorEdit)
...
function confirmFloorDeletion()
	if wElevatorEdit then
		...
		lbl1 = guiCreateLabel(0.05, 0.1, 0.9, 0.723, "Tem certeza que deseja excluir este andar?", true, wElevatorEdit)
			guiLabelSetHorizontalAlign(lbl1, "center", true)
			guiLabelSetVerticalAlign(lbl1, "center")

		btnYes = guiCreateButton(0.17, 0.725, 0.27, 0.225, "Sim", true, wElevatorEdit)
		btnNo = guiCreateButton(0.56, 0.725, 0.27, 0.225, "Não", true, wElevatorEdit)
