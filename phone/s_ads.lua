...
function SAN_newAdvert(message, sourceName, sourceElement, theCost, outboundPhoneNumber)
	...
	exports.global:sendMessageToSupporters("[ADVERT] ID#"..advertID .. " ("..advertMsg..") criado por "..sourceName..".")
	...
	for adIndex, adTable in ipairs (ads) do
		...
		elseif adTable[ AD_FROZEN ] then
			...
			if (currentTime - adTable [ AD_EDITTIME ]) > 600 then
				table.insert(removeIDs, adIndex)
				exports.global:sendMessageToSupporters("[ADS] Removendo ad "..adIndex.." devido estar 10 minutos congelado. (`"..adTable[AD_MESSAGE].."`, `"..adTable[AD_SENDERNAME].."`)")
			end
				...
				if getElementData(value, "loggedin")==1 then
					...
					if hasTogAd and tonumber(togAdState) == 1 then
						...
						if exports.integration:isPlayerSupporter(value) or exports.integration:isPlayerTrialAdmin(value) then
							outputChatBox("   ANUNCIO: " .. adTable[AD_MESSAGE] .. " (( ".. adTable[AD_SENDERNAME].." ))", value, 0, 255, 64)
						else
							outputChatBox("   ANUNCIO: " .. adTable[AD_MESSAGE], value, 0, 255, 64)
						end
					end
				end
			end
...
function SAN_listAdverts(thePlayer)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) then
		outputChatBox("Lista de anúncios atuais", thePlayer)
		for adIndex, adTable in ipairs (ads) do
			local msg = " "..adIndex..". "..adTable[AD_MESSAGE].." /-/ "..adTable[AD_SENDERNAME].." /-/ "

			if (adTable[AD_AIRED]) then
				msg = msg.."Já foi ao ar"
			elseif (adTable[AD_FROZEN]) then
				msg = msg.."Congelado por "..adTable[AD_FROZEN]
			else
				msg = msg.."Na fila"
			end
		...
		outputChatBox("--- Comandos: /congelarad /descongelarad /removerad", thePlayer)
	end
end
...
function SAN_freezeAdvert(thePlayer, commandHandler, advertID)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) then
		...
		if not ads[advertID] then
			outputChatBox("AD Invalido", thePlayer, 255, 0 ,0)
			return
		end

		if ads[advertID][AD_FROZEN] then
			outputChatBox("Anúncio já está congelado por `"..ads[advertID][AD_FROZEN].."`.", thePlayer, 255, 0 ,0)
			return
		end

		if ads[advertID][AD_AIRED] then
			outputChatBox("Anúncio já foi ao ar", thePlayer, 255, 0 ,0)
			return
		end
...
addCommandHandler("congelarad", SAN_freezeAdvert)
...
function SAN_unfreezeAdvert(thePlayer, commandHandler, advertID)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) then
		...
		if not ads[advertID][AD_FROZEN] then
			outputChatBox("Anúncio não está congelado.", thePlayer, 255, 0 ,0)
			return
		end
		...
		outputChatBox("Anúncio descongelado "..advertID, thePlayer, 0, 255 ,0)
	end
end
addCommandHandler("unfreezead", SAN_unfreezeAdvert)
addCommandHandler("descongelarad", SAN_unfreezeAdvert)
...
function SAN_deleteAdvert(thePlayer, commandHandler, advertID)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) then
		...
		exports.global:sendMessageToSupporters("[AD] "..getPlayerName(thePlayer).." anúncio marcado "..advertID .. " como foi ao ar.")
	end
end
addCommandHandler("deletead", SAN_deleteAdvert)
addCommandHandler("delad", SAN_deleteAdvert)
addCommandHandler("removerad", SAN_deleteAdvert)
