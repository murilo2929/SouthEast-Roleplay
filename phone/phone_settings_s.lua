...
function activatePrivateNumber(fromPhone, cost)
	...
	if fromPhone and cost then
		if cost > 0 then
			...
			if not took then
				outputChatBox("Você não conseguiu ativar este Recurso Premium. Razão: "..reason, client, 255, 0, 0)
				return false
			end

			exports.donators:addPurchaseHistory(client, "Número privado do celular ativado #"..fromPhone, -cost)
		end
