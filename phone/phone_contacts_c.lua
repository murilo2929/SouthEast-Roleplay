...
function openPhoneContacts(contacts, contactListLimitFromServer)
	...
	if isPhoneGUICreated() then
		...
		local colName = guiGridListAddColumn(gPhoneBook,"Nome",0.5)
		local colNumber = guiGridListAddColumn(gPhoneBook,"Numero",0.4)
		
		local row = guiGridListAddRow(gPhoneBook)
		guiGridListSetItemText(gPhoneBook, row, colName, "Novo contato", false, false)
		...
		if (contactList[phone]) then
			...
			if (#favorites > 0) then
				guiGridListSetItemText( gPhoneBook, guiGridListAddRow(gPhoneBook), colName, "Favoritos", true, false )
			...
			if (#acquaintances > 0) then
				if (#favorites > 0) then 
					guiGridListSetItemText( gPhoneBook, guiGridListAddRow(gPhoneBook), colName, "Conhecidos", true, false )
				end
