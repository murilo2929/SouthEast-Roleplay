...
function sendSMS(from, to, content, private, showChatNotification)
    ...
    if showChatNotification then
        outputChatBox("SMS para #" .. to .. ": " .. content, source, 120, 255, 80)
    end
    ...
    if not isNumberAHotline(to) then
        ...
        if not t_powerOn then --not existed
            local notExisted = "A entrega falhou para estes destinatários: #"..to..". Número não existe."
                ...
                if v[1] == 2 then -- it's a phone
                    ...
                    if count == i then
                        ...
                        if power and power ~= 0 then 
                            ...
                            outputChatBox("Seu telefone precisa estar ligado para poder enviar SMS.", thePlayer, 255, 0, 0)
                        end
                    end
                end
            end
        else
            outputChatBox("SYNTAX: /" .. theCommand .. " [Numero Telefone] [Mensagem]", thePlayer, 255, 194, 14)
        end
    end)
end
