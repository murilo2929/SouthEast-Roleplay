...
function drawPhoneGUI()
	...
	bPowerOn = guiCreateButton(btnM,btnPosY,btnW,btnH,"Ligar",false,wPhoneMenu)
	guiSetFont ( bPowerOn, font1 )

	bCancel = guiCreateButton(btnM+btnW+5,btnPosY,btnW,btnH,"Fechar",false,wPhoneMenu)
...
function triggerSlidingPhoneIn(thePhoneNumber, popOutOnPhoneCall, callingNumberFromCommand, openToSMSThread)
	...
	if not canPlayerSlidePhoneIn(localPlayer) then
		outputChatBox("Você não pode usar o celular no momento.")
		return false
	end
...
function drawHomeMidBlock()
	if isPhoneGUICreated() then
		cHotlines = guiCreateComboBox ( 30,155+40+5,204,20, "Linhas Diretas mais Comuns", false, wPhoneMenu )
		guiComboBoxAddItem(cHotlines, "Linhas Diretas mais Comuns")
		selectedHotline = ""
		...
		addEventHandler ( "onClientGUIComboBoxAccepted", guiRoot,
		    function ( comboBox )
		        if ( comboBox == cHotlines ) and ePhoneNumber and isElement(ePhoneNumber) then
		            ...
		            if ( text ~= "" and text ~= "Linhas Diretas mais Comuns") then
...
function guiConfirmDelete(name, number)
	...
	wDelete = guiCreateWindow(sx/2 - 150,sy/2 - 50,300,100,"Excluir entrada", false)
	lQuestion = guiCreateLabel(0.05,0.25,0.9,0.3, "Tem certeza de que deseja excluir '"..name.."' da sua lista de contatos?",true,wDelete)
	guiLabelSetHorizontalAlign (lQuestion,"center",true)
	bButtonDeleteYes = guiCreateButton(0.1,0.65,0.37,0.23,"Sim",true,wDelete)
	bButtonDeleteNo = guiCreateButton(0.53,0.65,0.37,0.23,"Não",true,wDelete)
	saveDeleteName = name
...
function showSettingsGui(itemValue)
	...
	wPhoneMenu = guiCreateWindow(sx/2 - 125,sy/2 - 175,250,310,"Menu de Configurações",false)
	gRingtones = guiCreateGridList(0.0381,0.1977,0.9153,0.6706,true,wPhoneMenu)
	guiGridListAddColumn(gRingtones,"ringtones",0.85)
	guiGridListSetItemText(gRingtones, guiGridListAddRow(gRingtones), 1, "modo de vibração", false, false)
	...
	bCancel = guiCreateButton(0.5212,0.8821,0.4322,0.0742,"Cancelar",true,wPhoneMenu)
end
...
function powerOnPhone()
	if isPhoneGUICreated() then
		...
		powerOnPhone_text = guiCreateLabel(80, 180+rawH+5, rawW, rawH, "Iniciando...0%", false, wPhoneMenu)
	...
	if isPhoneGUICreated() then
		if powerOnPhone_logo and isElement(powerOnPhone_logo) then
			...
			if alphaTmp <= 1 then
				...
				guiSetText(powerOnPhone_text, (turningOn == 1 and "Ligando" or "Desligando").."..."..(alphaTmp*100).."%")
			else
	...
	if isPhoneGUICreated() then
		if turnedOn == 1 then
			if success then
				...
				guiSetText(powerOnPhone_text, "Ligando...Falha!")
			end
		else
			if success then
				destroyElement(powerOnPhone_logo)
				...
				guiSetText(powerOnPhone_text, "Desligando...Falha!")
			end
		end
		guiSetEnabled(wPhoneMenu, true)
	end
...
function validateContactNameAndNumber(name, number, id)
	...
	if (string.len(name) > 1 and name ~= "Novo contato") and (string.len(number) > 1 and tonumber(number)) then
