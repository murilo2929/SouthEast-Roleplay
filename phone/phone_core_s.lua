...
function getPhoneSettings(phone, doNotCreateNew) --powerOn, ringtone, isSecret, isInPhonebook, boughtById, boughtByName, boughtDate, sms_tone, tone_volume
	...
	if not phoneSettings then
		...
		callerphoneBoughtByName = phoneSettings["charactername"] or "Desconhecido"
		callerphoneBoughtDate = phoneSettings["bought_date"] or "Desconhecido"
...
function applyPhone(string, popOutOnPhoneCall, itemName)
	...
	if string == "phone_in" then
		...
		triggerEvent('sendAme', source, "pega " .. (getElementData(source, "gender") == 0 and "seu" or "seu") .. " " .. itemName .. ".")
	...
	elseif string == "phone_out" then
		...
		if getElementData(source, "cellphoneGUIStateSynced") then
			if not popOutOnPhoneCall then
				triggerEvent('sendAme', source, "guarda "..(getElementData(source, "gender") == 0 and "seu" or "seu").." " .. itemName .. ".")
			end
			if getElementData(source, "phone_anim") ~= "0" then
				setPedAnimation(source, "ped", string, 1, false)
			end
...
function callSomeone(thePlayer, commandName, phoneNumber, withNumber)
		...
		if isElementWithinColShape( thePlayer, v ) then
			for kx, vx in pairs( getElementsByType( "player" ) ) do
				if getElementData( vx, "call.col" ) == v then
					outputChatBox( "Alguém já está usando este telefone.", thePlayer, 255, 0, 0 )
					return
				end
			end
			publicphone = v
	...
	if not outboundPhoneNumber or outboundPhoneNumber == -1 then
		outputChatBox("Acredite ou não, é difícil discar em um celular que você não tem.", thePlayer, 255, 0, 0)
		return
	end

	if not (phoneNumber) then
		outputChatBox("Pressione 'i' e clique no telefone que deseja usar, por favor.", thePlayer)
		return
	...
	if not tonumber(phoneNumber) then
		...
		if result then
			...
			outputChatBox("Não foi possível encontrar um número para o nome de contato especificado.", thePlayer, 255, 0, 0)
			return
		end
	end

	if not tonumber(phoneNumber) then
		outputChatBox("Número de telefone inválido.", thePlayer)
		return
	end
	...
	if callerphoneIsTurnedOn == 0 then
		outputChatBox("Seu telefone está desligado.", thePlayer, 255, 0, 0)
	else
		local calling = getElementData(thePlayer, "calling")

		if (calling) then -- Using phone already
			outputChatBox("Você já está usando um telefone.", thePlayer, 255, 0, 0)
		elseif getElementData(thePlayer, "injuriedanimation") then
			...
			-- /me it
			if publicphone then
				triggerEvent('sendAme', thePlayer, "pega o telefone público.")
			end
			...
			-- Otherwise find a fool to answer it
				...
				if bankMoney >= 1 then
					if not exports.donators:hasPlayerPerk(thePlayer, 6) and not exports.anticheat:changeProtectedElementDataEx(thePlayer, "bankmoney", tonumber(bankMoney) - 1, false) then
						outputChatBox("Você não pode pagar uma ligação.", thePlayer, 255, 0, 0)
						return
					end
				else
					outputChatBox("Você não pode pagar uma ligação.", thePlayer, 255, 0, 0)
				...
				if not found then
					outputChatBox("Você consegue um tom morto...", thePlayer, 255, 194, 14)
					return
				end
				...
				if not t_powerOn then --not existed
					outputChatBox("Você liga mas a ligação cai...", thePlayer, 255, 194, 14)
					...
					if not foundInGame then
						...
						if t_ringtone > 1 and tone_volume > 0 then
							...
							if t_ringtone > 2 then
								triggerEvent('sendAme', targetPlayer, " celular começa a tocar.")
							end
						end
	...
	if (logged==1) then
		if (exports.global:hasItem(thePlayer, 7)) then
			if not (partialNick) then
				outputChatBox("SYNTAX: /listatelefonica [Nome Parcial Jogador / Numero]", thePlayer, 255, 194, 14)
			else
				...
				elseif (mysql:num_rows(result)>0) then
					...
					outputChatBox("Você não encontra ninguém com esse nome.", thePlayer, 255, 194, 14)
				end
				mysql:free_result(result)
			end
		else
			outputChatBox("Acredite ou não, é difícil usar uma agenda que você não tem.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("phonebook", phoneBook)
addCommandHandler("listatelefonica", phoneBook)
	...
	if logged == 1 then
		...
		elseif tonumber(phoneNumber) < 10 then
			...
			if not (exports.global:hasItem(thePlayer, 2, tonumber(phoneNumber))) then
				outputChatBox("Você não possui este número de telefone", thePlayer, 255, 0, 0)
				return
			end
		end
		...
		if getElementData( thePlayer, "calling" ) then
			outputChatBox("Você está usando seu telefone!", thePlayer, 255, 0, 0)
		else
			if calledphoneIsTurnedOn == 0 then
				outputChatBox("Você ligou seu telefone com número '"..tostring(phoneNumber)..".", thePlayer, 0, 255, 0)
			else
				outputChatBox("Você desligou seu telefone com o número '"..tostring(phoneNumber)..".", thePlayer, 255, 0, 0)
			end
...
addCommandHandler("ativartelefone", togglePhone)
...
function setPhoneBook(thePlayer, commandName, phoneNumber, ...)
	...
	if logged == 1 then
		if not phoneNumber then
			outputChatBox("Usage: /" .. commandName .. " [celular nu.] [texto a ser encontrado em /listatelefonica]", thePlayer, 255, 194, 14)
			return
		end
		...
		if tonumber(phoneNumber) < 10 then
			...
			if not (exports.global:hasItem(thePlayer, 2, tonumber(phoneNumber))) then
				outputChatBox("Você não possui este número de telefoner", thePlayer, 255, 0, 0)
				return
			end
		end
		...
		if name then
			...
			outputChatBox("Você definiu seu numero da agenda para '" .. name .. "'.", thePlayer, 0, 255, 0)
		else
			success = mysql:query_free( "UPDATE `phones` SET `phonebook`=NULL WHERE `phonenumber`='".. mysql:escape_string(tostring(phoneNumber)) .."'")
			outputChatBox("Você removeu seu numero da lista telefônica.", thePlayer, 0, 255, 0)
		end
	end
end
...
addCommandHandler("definirlistatelefonica", setPhoneBook)
