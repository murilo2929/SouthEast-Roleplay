...
function startDialing(to, from)
	...
	elseif not exports.global:hasItem(source, 2, tonumber(from)) then
		outputDebugString("[Telefone] "..getPlayerName(source).." started calling from "..from.." to "..to.." without a phone.")
		outputChatBox("Error Code x0EDSCVF23. Reporte no discord", source, 255,0,0)
	...
	if powerOn == 0 then
		outputChatBox("Seu telefone está desligado.", source, 255,0,0)
		triggerClientEvent(source, "phone:slidePhoneOut", source)
		return false
	end

	if not canPlayerCall(source) then
		outputChatBox("Você não pode usar o celular no momento.", source, 255,0,0)
		triggerClientEvent(source, "phone:slidePhoneOut", source)
		return false
	end
	...
	elseif t_powerOn ~= 1 then --turned off
		...
		if not foundInGame then
			...
			if t_ringtone > 1 and tone_volume > 0 then
				...
				if isSecret == 1 then
					displayedNumber = "Numero Privado"
				else
				...
				if t_ringtone > 2 then
					triggerEvent('sendAme', targetPlayer, " celular começa a vibrar.")
				end
			end
...
function makeCall(thePlayer, commandName, phoneNumber)
	if not (phoneNumber) then
		outputChatBox("SYNTAX /" .. commandName .. " [Numero Telefone / Nome Contato]", thePlayer, 255, 194, 14)
	else
		if not canPlayerCall(thePlayer) then
			outputChatBox("Você não pode fazer uma ligação no momento.", thePlayer, 255,0,0)
			return false
		end
		...
		if itemValue then
			...
			outputChatBox("Voce nao tem um telefone.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("call", makeCall)
addCommandHandler("ligar", makeCall)
...
function makeFCall(thePlayer, commandName, from, to)
	...
	if from and to then
		...
		if owningPlayer == thePlayer then
			...
			outputChatBox("SYNTAX /" .. commandName .. " [Seu Numero] [Numero Telefone]", thePlayer, 255, 194, 14)
		end
	else
		outputChatBox("SYNTAX /" .. commandName .. " [Seu Numero] [Numero Telefone]", thePlayer, 255, 194, 14)
	end
...
function cancelPhoneCall(reason)
	...
	outputDebugString("[Telefone] "..getPlayerName(source).." triggered cancelPhoneCall / "..(reason and reason or "").." / "..phonestate)
	...
	if col then
		outputChatBox("Você desligou.", caller1, 155, 155, 255)
	...
	if caller2col then
		outputChatBox("Ele desligou a chamada.", caller2, 155, 155, 255)
	...
	if caller1Called then
		...
		elseif caller1Phonestate == 5 then
			triggerClientEvent(caller1, "phone:updateDialingScreen", caller1, isQuitType(reason) and reason or "ligou, respondeu mas cancelou")
		end
		...
		if caller2 then
			...
			if caller2Phonestate == 1 then
				if reason then
					...
					triggerClientEvent(caller2, "phone:updateDialingScreen", caller2, isQuitType(reason) and reason or "chamador, começou a discar, mas cancelou")
				end
			elseif caller2Phonestate == 2 then
				triggerClientEvent(caller2, "phone:updateDialingScreen", caller2, isQuitType(reason) and reason or "chamador, começou a discar e o destino está tocando. mas cancelado")
			elseif caller2Phonestate == 4 then
				triggerClientEvent(caller2, "phone:updateDialingScreen", caller2, isQuitType(reason) and reason or "ligou, respondeu mas cancelou")
			end
		end
	else
		writeCellphoneLogToClient(caller1)
		...
		if caller1Phonestate == 1 then
			if reason then
				...
				triggerClientEvent(caller1, "phone:updateDialingScreen", caller1, isQuitType(reason) and reason or "chamador, começou a discar, mas cancelou")
			end
		elseif caller1Phonestate == 2 then
			triggerClientEvent(caller1, "phone:updateDialingScreen", caller1, isQuitType(reason) and reason or "chamador, começou a discar e o destino está tocando. mas cancelado")
...
function acceptPhoneCall()
	...
	if phonestate ~= 3 then
		outputChatBox("Seu telefone não está tocando.", source, 255,0,0)
		triggerClientEvent("stopRinging", source)
		return false
	end
	...
	if not getElementData(caller, "call.col") then
		...
		outputChatBox("Eles atenderam o telefone.", caller, 155, 155, 255)
	end
	...
	if cost > 0 then
		if exports.bank:takeBankMoney(source, cost) then
			...
			if exports.global:giveMoney(foundFaction, cost) then
				return exports.bank:addBankTransactionLog(getElementData(source, "dbid"), -20, cost, 2, "Taxa de chamada de telefone celular", "Chamada feita de #"..fromNumber..", duração: "..duration, nil, nil)
			end
		end
	end
	return false
		...
		if (exports.global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then -- 2 = Cell phone item
			...
			if phonestate == 4 or phonestate == 5 then
				...
				if (not loudspeaker) then
					triggerEvent('sendAme', thePlayer, "coloca o telefone em modo viva voz.")
					outputChatBox("Você coloca seu telefone em modo viva voz.", thePlayer)
					exports.anticheat:changeProtectedElementDataEx(thePlayer, "call.loudspeaker", true, false)
				else
					triggerEvent('sendAme', thePlayer, "tira o viva voz do telefone.")
					outputChatBox("Você tira seu telefone do modo viva voz.", thePlayer)
...
function talkPhone(thePlayer, commandName, ...)
	...
	if not exports.global:hasItem(thePlayer, 2) and not getElementData(thePlayer, "call.col") then
		outputChatBox("Acredite ou não, é difícil usar um celular que você não tem.", thePlayer, 255, 0, 0)
		return
	end

	if not (...) then
		outputChatBox("SYNTAX: /p [Mensagem]", thePlayer, 255, 194, 14)
		return
	end

	if getElementData(thePlayer, "injuriedanimation")  then
		outputChatBox("Você não pode usar o telefone enquanto estiver desmaiado.", thePlayer, 255, 0, 0)
		return
	end
		
	local phoneState = getElementData(thePlayer, "phonestate")
	if not (phoneState == 4 or phoneState == 5) then
		outputChatBox("Você não está em uma chamada.", thePlayer, 255, 0, 0)
		return
	end
	...
	if (callprogress) then
		...
		if commandName == "plow" then
			...
			if not theVehicle or not exports.vehicle:isVehicleWindowUp(theVehicle) then
				exports.global:sendLocalText(thePlayer, "" .. username .. " sussurros [Telefone]: " .. message, nil, nil, nil, 3, {[thePlayer] = true})
			end	
			outputChatBox("" .. username .. " sussurros [Telefone]: " .. message, thePlayer, 200, 255, 200)
			triggerEvent("sendAme", thePlayer, "sussurra em seu telefone.") 
		else
			if not theVehicle or not exports.vehicle:isVehicleWindowUp(theVehicle) then
				exports.global:sendLocalText(thePlayer, username .. " [Telefone]: " .. message, nil, nil, nil, 10, {[thePlayer] = true})
			end	
			outputChatBox("" .. username .. " [Telefone]: " ..message, thePlayer, 200, 255, 200)
		end
	...
	if commandName == "plow" then
		outputChatBox("" .. username .. " sussurra [Telefone]: " .. translatedMessage, target)
		outputChatBox("" .. username .. " sussurra [Telefone]: " ..message, thePlayer)
		if not theVehicle or not exports.vehicle:isVehicleWindowUp(theVehicle) then
			sendLocalText(thePlayer, "" .. username .. " sussurra [Telefone]: " .. translatedMessage, 255, 255, 255, 3, {[thePlayer] = true})
		end	
		triggerEvent("sendAme", thePlayer, "sussurra em seu telefone.")
	else
		outputChatBox("" .. username .. " [Telefone]: " .. translatedMessage, target, 200, 255, 200)
		outputChatBox("" .. username .. " [Telefone]: " ..message, thePlayer, 200, 255, 200)
		if not theVehicle or not exports.vehicle:isVehicleWindowUp(theVehicle) then
			sendLocalText(thePlayer, "" .. username .. " [Telefone]: " .. translatedMessage, 255, 255, 255, 10, {[thePlayer] = true})
		end	
	end
					...
					if (exports.vehicle:isVehicleWindowUp(pveh)) then
						...
						for i = 0, getVehicleMaxPassengers(pveh) do
							...
							if (lp) and (lp~=source) then
								outputChatBox("" .. username .. " ((No carro)) diz: " .. translatedMessage, lp)
						...
						outputChatBox("" .. username .. "'s Telefone Viva-Voz: " .. translatedMessage, nearbyPlayer)
...
function outputChange(dataName,oldValue)
	if getElementType(source) == "player" and dataName == "phonestate"  then -- check if the element is a player
		...
		outputDebugString("[Telefone] "..getPlayerName(source).."'s "..tostring(dataName).."' has changed from '"..tostring(oldValue).."' to '"..tostring(newValue).."'") -- output the change for the affected player
	end
end
