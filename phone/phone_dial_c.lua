...
function drawPhoneDial()
	if isPhoneGUICreated() then
		...
		bCall = guiCreateButton(30,155,102,30,"Ligar",false,wPhoneMenu)
...
function startDialing(from, to, popOutOnPhoneCall)
	if from and to and string.len(to) > 0 then
		...
		if not callingTo then
			if not tonumber(to) and to ~= "Privado" then
				return false
			end
			callingTo = {to, nil}
		end
		...
		lCallingText = guiCreateLabel(margin,posY-10,lineW,100, "Ligando..." , false, wPhoneMenu)
		guiLabelSetHorizontalAlign(lCallingText, "center")
		posY = posY + lineH*4+10

		local bW,bH = 200, 30

		bSpeaker = guiCreateButton(margin+2,posY,bW,bH, "Viva-voz", false, wPhoneMenu)
		posY = posY + bH + 2
		...
		bEndCall = guiCreateButton(margin+2,posY,bW,bH, "Desligar", false, wPhoneMenu)
...
function finishPhoneCall(reason)
	...
	if reason == "out_of_service" then
		reasonText = "O assinante que você discou\n esta fora de serviço"
	elseif reason == "cant_afford" then
		reasonText = "Você não pode pagar esta ligação.\nPor favor deposite dinheiro"
	elseif reason == "declined" then
		reasonText = "O assinante rejeitou sua chamada"
	elseif reason == "not_existed" then
		reasonText = "O número que você discou\n não existe"
	elseif isQuitType(reason) then
		reasonText = "O sinal foi perdido, desconectado! \n(("..reason.."))"
	else
		reasonText = "End"
	end
	local callingCost = getPhoneCallCost()
	if lCallingText and isElement(lCallingText) then
		guiSetText(lCallingText, reasonText..".\n\nDuração: "..actualCountedTime.."\nCusto: $"..exports.global:formatMoney(callingCost))
		if isPhoneGUICreated() then
			guiSetEnabled(wPhoneMenu, true)
		end
	end
...
function updateDialingScreen(action, data, calledHotline1)
	...
	if isPhoneGUICreated() and lCallingTo and isElement(lCallingTo) then
		if action then
			if action == "chamador, começou a discar, mas cancelou" then
				finishPhoneCall()
			elseif action == "chamador, começou a discar e o destino está tocando. mas cancelou" then
				finishPhoneCall()
			elseif action == "ligou, começou a tocar. mas eles cancelaram" then
			...
			elseif action == "ligou, começou a tocar, mas expirou" then
				finishPhoneCall()
			...
			elseif action == "connected" then
				if data then
					...
					guiSetText(lCallingTo, data.entryName or data.entryNumber)
					guiSetText(lCallingText, "Conectado!")
					startCounting()
					guiSetEnabled(bEndCall, true)
					guiSetEnabled(bSpeaker, true)
				end
			...
			elseif action == "ligou, respondeu mas cancelou" then
				finishPhoneCall()
			elseif action == "chamador, conectado, cancelado" then
				finishPhoneCall()
...
function drawRinging(phoneToDisplay, numberToDisplay)
	...
	wRing = guiCreateWindow(ringOffSetOut,y,w,h, "Seu telefone #"..phoneToDisplay.." esta tocando!", false)
    guiWindowSetMovable(wRing, false)
    guiWindowSetSizable(wRing, false)

	local lFrom = guiCreateLabel(10, 25, w  - 20, 20, "Ligação de " .. numberToDisplay .. ".", false, wRing)
	guiLabelSetHorizontalAlign(lFrom, "center", false)

    bRingingAnswer = guiCreateButton(10, 48, 117, 35, "Atender", false, wRing)
    bRingingDecline = guiCreateButton(129, 48, 118, 35, "Recusar", false, wRing)
...
function answerToPhoneCall()
	if canPlayerAnswerCall(localPlayer) then
		...
		outputDebugString("Você não pode usar o celular no momento.", 255,0,0)
	end
end
