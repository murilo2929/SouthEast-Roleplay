...
function drawAllSMSThreads(xoffset, yoffset)
	...
	if sortedThreads[phone] and smsThreadInitiated[phone] then
		...
		if #sortedThreads[phone] > 0 then
			...
			for i = 1, #sortedThreads[phone] do
				...
				if thread then
					...
					guiCreateLabel(0.5, 0.5, 1, 0.5, "Sem mensagens.", true, wSMS)
					isEmpty = true
					break
				end
			end
		...
		guiCreateLabel(0.5, 0.5, 1, 0.5, "Carregando...", true, wSMS)
	end
end
...
function drawNewSMSWindow(fromNumber, toNumber, message)
	...
	smsWindow = guiCreateWindow(x,y,w,h, "Seu telefone #" .. toNumber .. " recebeu um SMS!", false)
	guiWindowSetMovable(smsWindow, false)
	guiWindowSetSizable(smsWindow, false)

	local lFrom = guiCreateLabel(10, 25, w  - 20, 20, "SMS de " .. fromNumber .. ".", false, smsWindow)
	guiLabelSetHorizontalAlign(lFrom, "center", false)

	local lMessage = guiCreateLabel(10, 45, w - 20, 20, message, false, smsWindow)

	bReply = guiCreateButton(10, 68, 117, 35, "Responder", false, smsWindow)
	bClose = guiCreateButton(129, 68, 118, 35, "Fechar", false, smsWindow)
