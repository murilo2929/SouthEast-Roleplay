...
function writeCellphoneLog(thePlayer, theOtherPlayer, type, message, starting )
	if starting or (message and string.len(message) > 0) and type then
		...
		if tonumber(charId1) and tonumber(number1) and tonumber(number2Test) then
			...
			local message1 = ""
			if starting then
				message1 = "\nSAIDA "..(type == "Ligações" and "CHAMADA" or "SMS").." DE #"..number1.." ("..string.upper(exports.global:getPlayerName(thePlayer))..") PARA #"..number2Test.." ("..(theOtherPlayer and string.upper(exports.global:getPlayerName(theOtherPlayer)) or string.upper(getHotlineName(number2Test)))..") AS ".. getCurrentDateTimeText()..":"
			else
				message1 = "-> "..message
			end
			...
			if tonumber(charId2) and tonumber(number2) and tonumber(number1Test) and tonumber(number1Test) == tonumber(number1) and tonumber(number2Test) == tonumber(number2) then
				...
				local message2 = ""
				if starting then
					message2 = "\nENTRADA "..(type == "Ligações" and "CHAMADA" or "SMS").." DE #"..number1.." ("..string.upper(exports.global:getPlayerName(thePlayer))..") PARA #"..number2Test.." ("..string.upper(exports.global:getPlayerName(theOtherPlayer))..") AS ".. getCurrentDateTimeText()..":"
				else
					message2 = "<- "..message
				end
