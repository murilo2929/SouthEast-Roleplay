...
function startStep2(questsFromServer)
	...
	guiStep2.back = guiCreateButton ( bX, bY, bW, bH, "< Regras Servidor", false)
	guiStep2.next = guiCreateButton ( bX+bW, bY, bW, bH, "Proximo >", false)
...
function validateApp()
	...
	if (quest1>1) and (quest2>1) and (quest3>1) and (quest4>1) then
		guiSetText(guiStep2.intro2, "Enviando aplicação para o servidor..\nProcessando aplicação..Aguarde!")
		guiSetText(guiStep2.intro, "Enviando aplicação para o servidor..\nProcessando aplicação..Aguarde!")
		...
		guiSetText(guiStep2.intro2, "Você não respondeu a todas as perguntas.")
		guiSetText(guiStep2.intro, "Você não respondeu a todas as perguntas.")
...
function resetNoti()
	if guiStep2.intro and isElement(guiStep2.intro) and guiGetVisible(guiStep2.intro) then
		guiSetText(guiStep2.intro2, "Antes de se tornar elegível para jogar neste servidor, você primeiro terá que terminar uma pequena aplicação que irá testar seu portugues, conhecimento geral de roleplay e compreensão geral das regras de servidores que você acabou de ver. (Seção 2/2)")
		guiSetText(guiStep2.intro, "Antes de se tornar elegível para jogar neste servidor, você primeiro terá que terminar uma pequena aplicação que irá testar seu portugues, conhecimento geral de roleplay e compreensão geral das regras de servidores que você acabou de ver. (Seção 2/2)")
