...
function switchModex()
	if isPD() and (getPedWeapon(localPlayer)==25) and (getPedTotalAmmo(localPlayer)>0) then -- has an un-empty Shotgun
		...
		if mode == 0 then -- bean bag
			...
			outputChatBox( "Você mudou sua munição de espingarda para o modo letal", 0, 255, 0 )
		elseif mode == 1 then -- lethal gun mode
			outputChatBox( "Você mudou sua munição de espingarda para Borracha", 0, 255, 0 )
			triggerServerEvent("shotgunmode", localPlayer, 0)
		end
		triggerServerEvent("sendLocalMeAction", localPlayer, localPlayer, "muda a munição de sua espingarda.")
	end
end
