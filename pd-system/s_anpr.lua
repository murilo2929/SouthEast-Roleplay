...
function getColorName(c1, c2)
	local color1 = COLORS[c1] or "Desconhecido"
	local color2 = COLORS[c2] or "Desconhecido"
        ...
        if (fID == 1) or (fID == 50) then
            if exports.global:hasItem(vehicle, 284) then
                if getElementData(vehicle, "anpr:state") then
                    ...
                    outputChatBox("O sistema ANPR foi desativado neste cruiser.", player, 255, 180, 20, false)
                    ...
                    if not isANPRExisting(player) then
                        ...
                        outputChatBox("Você ativou o sistema ANPR.", player, 255, 180, 20, false)
                ...
                outputChatBox("Você deve estar em um veícul com um sistema ANPR.", player, 255, 0, 0, false)    
            end    
        else
            outputChatBox("Você deve estar em um veículo para ativar seu radar de velocidade.", player, 255, 0, 0, false)
        end
    end
end, false, false )
...
addCommandHandler({"resetanpr", "resetanprcam", "resetanprcamera"},
	function(player, cmd)
		if isANPRExisting(player) then
			...
			setElementData(player, "anpr:vehicleColor", "No info", true )
			setElementData(player, "anpr:vehicleName", "No info", true )
			outputChatBox("Todos os seus radares estão agora desativados.", player, 255, 180, 20, false)
		else
			outputChatBox("Não há sistema ANPR ativo.", player, 255, 0, 0, false)
		end
	end, false, false
)
                        ...
                        if isVerifiedElement(radar, "vehicle") and getElementData(radar, "anpr:state") then
                            ...
                            if hasVehicleANPR(hitElement) then
                                for seat,player in pairs(getVehicleOccupants(radar)) do
                                    ...
                                    outputChatBox("[RADAR] Um hit ANPR foi gravado em um " .. getColorName(c1, c2) .. " " .. exports.global:getVehicleName(hitElement).." with plates "..getElementData(hitElement, "plate")..".", player, 255, 0, 0, false)
	...
	function()
		for _,v in ipairs(getElementsByType("vehicle")) do
			if getElementData(v, "anpr:state") then
				...
				setElementData(source, "anpr:vehicleColor", "No info", true )
				setElementData(source, "anpr:vehicleName", "No info", true )
				outputChatBox("Sistema ANPR foi desativado.", getVehicleController(v), 255, 180, 20, false)
			end
		end
...
addEventHandler("onPlayerWasted", root,
	function(ammo, killer, weapon, bodypart, stealth)
		if isANPRExisting(source) then
			...
			outputChatBox("Sistema ANPR foi desativado.", source, 255, 180, 20, false)
		end
	end
)