...
function dispatchGUI()
	...
	if count > 1 then
		outputChatBox("Você só pode estar em uma facção legal por vez para usar /dispatch.", 255, 0, 0)
		return 
	end
	...
	guiGridListAddColumn(DispatchUI.gridlist[1], "Nome Completo", 0.3)
	guiGridListAddColumn(DispatchUI.gridlist[1], "Disponibilidade", 0.3)
	...
	DispatchUI.label[3] = guiCreateLabel(11, 261, 61, 19, "Disponibilidade:", false, DispatchUI.window[1])
	DispatchUI.button[1] = guiCreateButton(302, 206, 78, 45, "Fechar", false, DispatchUI.window[1])
	DispatchUI.button[2] = guiCreateButton(214, 206, 78, 45, "Atualizar", false, DispatchUI.window[1])
	DispatchUI.combobox[1] = guiCreateComboBox(74, 205, 134, 100, "", false, DispatchUI.window[1])
	guiComboBoxAddItem(DispatchUI.combobox[1], "Em serviço")
	guiComboBoxAddItem(DispatchUI.combobox[1], "De folga")
	...
	DispatchUI.checkbox[1] = guiCreateCheckBox( 239, 253, 150, 16, "Operações Conjuntas", getElementData(root, "dispatch:"..fID), false, DispatchUI.window[1] )
	...
	if dispatchTable then
		for k, v in pairs(dispatchTable) do
			if v["faction"] == fID then
				...
				guiGridListSetItemText(DispatchUI.gridlist[1], row, 3, v["Disponibilidade"], false, false)
			end
		end
	end
	...
	if type(dispatchTable[processedName]) == "table" then
		...
		guiSetText(DispatchUI.edit[2], dispatchTable[processedName]["Disponibilidade"])
	else
	...
	addEventHandler("onClientGUIClick", DispatchUI.button[2], function ()
			...
			if status == "De folga" then
				...
				dispatchTable[processedName]["Disponibilidade"] = guiGetText(DispatchUI.edit[2])
...
function canUseDispatch()
    if exports.global:hasItem(localPlayer, 177) and (exports.factions:isPlayerInFaction(localPlayer, 1) or exports.factions:isPlayerInFaction(localPlayer, 50)) then
		...
		outputChatBox("Dispatch através do ar não parece funcionar bem...", 255, 0, 0)
	end
end
