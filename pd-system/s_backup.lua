	...
	--if getPedOccupiedVehicle(thePlayer) then
		...
		if veh then
			...
			if veh and vehiclesDashboardRadio[model] then
				triggerEvent('sendAme', thePlayer, "liga o rádio do painel.")
			else
				triggerEvent('sendAme', thePlayer, "liga o seu rádio.")
			end
		else
			triggerEvent('sendAme', thePlayer, "liga o seu rádio.")
		end
				...
				if pTheTeam then
					...
					if allowedFactionTypes[pFactionType]  then
						if (faction > 0) then
							...
							outputChatBox("A '".. backupColours[availableColourIndex][4]  .."' unidade (" .. factionShortName ..") não requer mais assistência. Retomar a patrulha normal.", theArrayPlayer, 255, 194, 14)
						end
					end	
				end
			end	
				...
				if pTheTeam then
					...
					if allowedFactionTypes[pFactionType]  then
						if (faction > 0) then
							...
							outputChatBox("A '".. backupColours[availableColourIndex][4]  .."' unidade (" .. factionShortName ..") habilitou seu farol de backup de rádio!", theArrayPlayer, 255, 194, 14)
						end
					end	
				end
			end		
		else
			outputChatBox("Todos os beacons de backup já estão em uso.", thePlayer, 255, 194, 14)
		end
	--end
	end
end
addCommandHandler("backup", startBackup)
addCommandHandler("reforco", startBackup)
