...
addCommandHandler("k9status", function (p)
	...
	if currentStatus == 1 then
		...
		setElementData(ped[p], "k9:sits", false)
		outputChatBox("O cachorro agora está esperando.", p)
	elseif currentStatus == 0 then
		setElementData(ped[p], "k9:status", 1)
		outputChatBox("O cachorro agora está te seguindo.", p)
	else
		setElementData(ped[p], "k9:status", 1)
		setElementData(ped[p], "k9:sits", false)
		outputChatBox("O cachorro agora está te seguindo.", p)
...
addCommandHandler("k9ataque", function (p, cmd, target)
	if not ped[p] then return false end
	...
	if not target then
		outputChatBox("SYNTAX: /"..cmd.." [alvo]", p)
		return
	end
	...
	if isElement(targetPlayer) then
		...
		if getDistanceBetweenPoints3D(x,y,z,tx,ty,tz) > 500 then
			outputChatBox("Seu cachorro não consegue localizar um alvo.",p)
			triggerEvent("sendAme", ped[p], "olha em volta obviamente parecendo confuso.")
			return false
		end

		setElementData(ped[p], "k9:status", 2)
		setElementData(ped[p], "k9:target", targetPlayer)
		outputChatBox("O cachorro agora esta contra "..targetPlayerName, p)
	else
		outputChatBox("Nenhum jogador encontrado.", p, 255, 0, 0)
	end
end)
