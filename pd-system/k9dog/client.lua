...
function start_GUI(thePlayer, commandName)
	if exports.integration:isPlayerHeadadmin(getLocalPlayer()) or exports.integration:isPlayerScripter(getLocalPlayer()) then
		if isElement(GUIEditor.window[1]) then destroyElement(GUIEditor.window[1]) end
		GUIEditor.window[1] = guiCreateWindow(659, 293, 254, 309, "Gerenciar Usuários de Cães", false)
		...
		GUIEditor.column[1] = guiGridListAddColumn(GUIEditor.gridlist[1], "Nome", 0.5)
		GUIEditor.column[2] = guiGridListAddColumn(GUIEditor.gridlist[1], "Ataque", 0.5)
		...
		GUIEditor.button[1] = guiCreateButton(0.36, 0.82, 0.29, 0.13, "Revogar", true, GUIEditor.window[1])
		addEventHandler("onClientGUIClick", GUIEditor.button[1], function ()
				...
				if rindex ~= -1 then
					...
					outputChatBox("Você deve selecionar uma pessoa para revogar.")
				end
			end, false)

		GUIEditor.button[2] = guiCreateButton(0.67, 0.82, 0.29, 0.13, "Fechar", true, GUIEditor.window[1])
...
addCommandHandler("dogadd", function (commandName, character, attack)
	if exports.integration:isPlayerHeadAdmin(getLocalPlayer()) or exports.integration:isPlayerScripter(getLocalPlayer()) then
		if character and tonumber(attack) then
			...
			outputChatBox("O usuário foi adicionado! "..character.." - Ataque: "..attack)
		else
			outputChatBox("SYNTAX: /"..commandName.." [nome exato] [ataque: 1 or 0]")
		end
	end
end)