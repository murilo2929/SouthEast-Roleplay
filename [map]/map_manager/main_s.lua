...
addEventHandler( 'maps:approveRequest', resourceRoot, function( map_id, note, accepting )
	note = getCurrentTimeString().." "..exports.global:getPlayerFullIdentity( client, 1 )..": "..(accepting and "ACEITA" or "RECUSADA")..". "..note.."\n"
	dbQuery( function( qh, client, map_id, note )
		local res, nums, id = dbPoll( qh, 0 )
		if res and nums > 0 then
			triggerClientEvent( client, 'maps:approveRequest', resourceRoot, 'ok', map_id, accepting )
			notifyPlayer( map_id, "A adição do seu mapa externo solicita atualizações de status.", "Olá <Username>!\n\nSua solicitação de adição de mapa externo #" .. map_id .. " foi "..(accepting and "ACEITA" or "RECUSADA")..".\n\n"..note.."\nSERP,\nSERP Community\nSERP Mapping Team" )
		else
			dbFree( qh )
...
addEventHandler( 'maps:implement', resourceRoot, function( map_id, implementing )
	local note = getCurrentTimeString().." "..exports.global:getPlayerFullIdentity( client, 1 )..": "..(implementing and "Implementou mapa." or "Desativou o mapa.").."\n"
	dbQuery( function( qh, client, map_id, note )
		local res, nums, id = dbPoll( qh, 0 )
		if res and nums > 0 then
			if implementing and exports.map_load:loadMap( map_id ) or exports.map_load:unloadMap( map_id ) then
				...
				notifyPlayer( map_id, "O status do seu mapa foi atualizado.", "Olá <Username>!\n\nSeu mapa #" .. map_id .. " foi "..(implementing and "IMPLEMENTADO" or "DESATIVADO")..".\n\n"..note.."\nSERP,\nSERP Community\nSERP Mapping Team" )
			else
				triggerClientEvent( client, 'maps:implement', resourceRoot, 'Error ao '..(implementing and 'implementar' or 'desativar')..' the map. Code 122.' )
			end
		else
			dbFree( qh )
			triggerClientEvent( client, 'maps:implement', resourceRoot, 'Error ao '..(implementing and 'implementar' or 'desativar')..' o mapa. Codigo 124.' )
		end
