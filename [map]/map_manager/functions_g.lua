...
function getReqStatus( v )
	if v.approved == 0 then
		return 'Pendente', 255, 255, 255, 200
	elseif v.approved == 2 then
		return 'Recusado', 255, 0, 0, 255
	else
		if v.enabled == 1 then 
			return 'Aceito e Implementado', 0, 255, 0, 255
		else
			return 'Aceito e Desativado', 255, 0, 0, 200
		end
	end
end
...
function canAccessMgmtTab( player )
	return exports.integration:isPlayerScripter( player, true )
...
function processMapContent( content, max_objects, content_is_filepath )
	...
	result, message = false, 'Error 341'
	if map then   
	    ...
	    if root then
	    	...
	    	if objects then
	    		if #objects < 1 or #objects > settings.external_map_max_objects then
	    			result, message = false, "Seu mapa ("..#objects.." objs) deve conter pelo menos um objeto e no máximo "..max_objects.." objetos (incluindo objetos removidos)."
	    		else
					        ...
					        if name == 'interior' and value then
					        	if not int then
					        		...
					        		if int ~= value then
		    							...
		    							return false, 'Todos os objetos do mapa devem estar no mesmo interior.'
		    						end
		    					end
		    				elseif name == 'dimension' and value then
		    					if not dim then
		    						...
		    						if dim ~= value then
		    							...
		    							return false, 'Todos os objetos do mapa devem estar no mesma dimensão.'
		    						end
		    					end
		    				end
					    end
	    		...
	    		result, message = false, 'Error 136.'
	    	end
	    	xmlUnloadFile( root )
	    else
	    	result, message = false, 'Error 133.'
	    end
	    if not content_is_filepath then
	    	fileDelete( settings.client_file )
	    end
