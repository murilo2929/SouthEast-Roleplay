...
function animationList(thePlayer)
	outputChatBox("/mijar /wank /slapass /fixcar /handsup /hailtaxi /scratch /fu /carchat /tired /cover", thePlayer, 255, 194, 14)
	outputChatBox("/strip 1-2 /lightup /drink /beg /mourn /cheer 1-3 /dance 1-3 /crack 1-4 /walk(2) 1-37", thePlayer, 255, 194, 14)
	outputChatBox("/gsign 1-5 /puke /rap 1-3 /sentar 1-5 /smoke 1-3 /smokelean /laugh /startrace /bat 1-3", thePlayer, 255, 194, 14)
	...
	outputChatBox("/cumprimentar /beijar", thePlayer, 255, 194, 14)
	outputChatBox("/stopanim ou pressione barra de espaço para cancelar a animação.", thePlayer, 255, 194, 14)
end
...
addCommandHandler ( "mijar", pedPiss, false, false )
		...
		if otherGuy then
			...
			if otherPlayer then
				if (getPedOccupiedVehicle(thePlayer) == false and getPedOccupiedVehicle(otherPlayer) == false)  then
					...
					outputChatBox("Você está muito longe para cumprimentar este jogador.", thePlayer, 255, 0, 0)
					end
				else
				outputChatBox("Você não pode cumprimentar se você ou o outro jogador estiver em um veículo.", thePlayer, 255, 0, 0)
				end
			end
		else
			exports.global:applyAnimation( thePlayer, "GANGS", "hndshkfa", -1, false, false, false)
...
function kissingAnimation( thePlayer, commandName, target )
	...
	if not target or target == "" then
		outputChatBox( "SYNTAX: /" .. commandName .. " [Nome Pacial / ID]", thePlayer, 255, 194, 14 )
		return false
	end
...
addCommandHandler( "beijar", kissingAnimation, false, false )
			...
			if otherPlayer then
				if (getPedOccupiedVehicle(thePlayer) == false and getPedOccupiedVehicle(otherPlayer) == false)  then
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)<1.5) then -- Are they standing next to each other?
						...
						outputChatBox("Você está muito longe para apertar a mão deste jogador.", thePlayer, 255, 0, 0)
					end
				else
					outputChatBox("Você não pode apertar a mão se você ou o outro jogador estiver em um veículo.", thePlayer, 255, 0, 0)
				end
			end
		else
			exports.global:applyAnimation( thePlayer, "GANGS", "prtial_hndshk_biz_01", -1, false, false, false)
...
addCommandHandler("cumprimentar", realHandshakeAnimation, false, false)