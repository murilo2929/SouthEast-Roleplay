...
function ani_start()
	...
	ani_tab_all = guiCreateTab("All", ani_tab_panel)
		...
		ani_loop = guiCreateCheckBox(10, 221, 104, 15, "Repetir", false, false, ani_tab_all)

		ani_button = guiCreateButton(119, 221, 93, 21, "Iniciar", false, ani_tab_all)
			addEventHandler("onClientGUIClick", ani_button, ani_animation, false)
			
		ani_add_favourites = guiCreateButton(222, 221, 93, 21, "Favorito", false, ani_tab_all)
			addEventHandler("onClientGUIClick", ani_add_favourites, ani_add, false)
		
		ani_close = guiCreateButton(325, 221, 93, 21, "Fechar", false, ani_tab_all)
	...
	ani_tab_favourites = guiCreateTab("Favourites", ani_tab_panel)
		...
		ani_loop_fav = guiCreateCheckBox(10, 221, 104, 15, "Repetir", false, false, ani_tab_favourites)

		ani_button_fav = guiCreateButton(119, 221, 93, 21, "Iniciar", false, ani_tab_favourites)
			addEventHandler("onClientGUIClick", ani_button_fav, ani_animation, false)
			
		ani_add_favourites_fav = guiCreateButton(222, 221, 93, 21, "Deletar", false, ani_tab_favourites)
			addEventHandler("onClientGUIClick", ani_add_favourites_fav, ani_del, false)
		
		ani_close_fav = guiCreateButton(325, 221, 93, 21, "Fechar", false, ani_tab_favourites)
		...
		if tekst ~= "..." then
			if actual_block and tekst then
				if isAnimation(actual_block, tekst) then
					local add_confirm_window = guiCreateWindow(648, 361, 296, 125, "Animações", false)
					guiWindowSetSizable(add_confirm_window, false)
					guiSetInputEnabled(true)

					local add_confirm_label = guiCreateLabel(11, 34, 270, 20, "Atalho de comando (deixe em branco para nenhum):", false, add_confirm_window)
					local add_confirm_edit = guiCreateEdit(10, 60, 276, 25, "", false, add_confirm_window)
					local add_confirm_back = guiCreateButton(11, 95, 108, 20, "Voltar", false, add_confirm_window)
					local add_confirm_save = guiCreateButton(178, 95, 108, 20, "Salvar", false, add_confirm_window)
						...
						if source == add_confirm_save then
							if string.match(guiGetText(add_confirm_edit), "%w") then
								...
								if guiGetText(add_confirm_edit) ~= "" then
									...
									outputChatBox("Você pode usar sua animação digitando /anim " .. guiGetText(add_confirm_edit) .. ".", 0, 255, 0)
								end
								...
								guiSetText(add_confirm_label, "Carateres inválidos detectados!")
							end
...
function commandAnimation(command, shortcut)
	if not shortcut then
		outputChatBox("Syntax: /" .. command .. " [atalho de animação]", 255, 215, 0)
		outputChatBox("Para atribuir atalhos de animação, use /animselect.", 255, 215, 0)
	else
		if xmlFile then
			for key, node in pairs(xmlNodeGetChildren(xmlFile)) do
...
function loadCustomAnimations()
	for customAnimationBlockIndex, customAnimationBlock in ipairs ( IFP ) do 
		...
		if not ifp then
			outputDebugString ("Falha ao carregar '"..customAnimationBlock.path.."'")
		end
	end
