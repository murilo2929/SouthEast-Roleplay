...
function openMotdManager(motdList1)
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
        ...
        showCursor(true)
        GUIEditor.window[1] = guiCreateWindow(412, 210, 800, 354, "MOTD - Mensagem do Dia", false)
        ...
        GUIEditor.gridcol.audiences = guiGridListAddColumn(GUIEditor.gridlist[1], "Audiências", 0.3)
        GUIEditor.gridcol.expiration_date = guiGridListAddColumn(GUIEditor.gridlist[1], "Data de validade", 0.25)
        GUIEditor.gridcol.dismissable = guiGridListAddColumn(GUIEditor.gridlist[1], "Dispensável", 0.05)
        GUIEditor.gridcol.author =  guiGridListAddColumn(GUIEditor.gridlist[1], "Autor", 0.1)
        GUIEditor.gridcol.creation_date = guiGridListAddColumn(GUIEditor.gridlist[1], "Data de criação", 0.2)
        GUIEditor.gridcol.id = guiGridListAddColumn(GUIEditor.gridlist[1], "ID", 0.05)

        GUIEditor.button[1] = guiCreateButton(10, 314, 388, 30, "Criar nova MOTD", false, GUIEditor.window[1])
        ...
        GUIEditor.button[2] = guiCreateButton(402, 314, 388, 30, "Fechar", false, GUIEditor.window[1])
        ...
        if not motdList then
            ...
            GUIEditor.label["loading"] = guiCreateLabel(9, 28, 781, 276, "Carregando..\n\nDICAS: Clique duas vezes para editar, clique duas vezes com o botão direito para excluir.", false, GUIEditor.window[1])
...
function deleteMOTD(motdId)
    ...
    GUIEditor.window[3] = guiCreateWindow(684, 373, 339, 137, "Confirmação", false)
    ...
    GUIEditor.label[100] = guiCreateLabel(14, 25, 315, 63, "Você está prestes a excluir MOTD ID #"..motdId.." feito por "..motd.author.." em "..motd.creation_date..".\nEsta ação não pode ser desfeita, você tem certeza que deseja continuar?", false, GUIEditor.window[3])
    guiLabelSetHorizontalAlign(GUIEditor.label[100], "left", true)
    GUIEditor.button[101] = guiCreateButton(15, 97, 153, 25, "Sim", false, GUIEditor.window[3])
    GUIEditor.button[102] = guiCreateButton(168, 97, 153, 25, "Não", false, GUIEditor.window[3])
...
function createNewMotd(motdId)
    ...
    GUIEditor.window[2] = guiCreateWindow(611, 279, 800, 354+yExtend, motdId and ("Editar MOTD ID #"..motdId) or "Criar nova MOTD", false)
    ...
    GUIEditor.label[2] = guiCreateLabel(17, 63, 81, 29, "Conteudo:", false, GUIEditor.window[2])
    ...
    GUIEditor.label[4] = guiCreateLabel(554, 34, 65, 29, "Expira em:", false, GUIEditor.window[2])
    guiSetFont(GUIEditor.label[4], "default-bold-small")
    guiLabelSetVerticalAlign(GUIEditor.label[4], "center")
    GUIEditor.combobox[1] = guiCreateComboBox(615, 37, 165, 22, "Nunca", false, GUIEditor.window[2])
    guiComboBoxAddItem(GUIEditor.combobox[1], "Nunca")
    ...
    guiComboBoxAddItem(GUIEditor.combobox[1], "1 ano")
    exports.global:guiComboBoxAdjustHeight(GUIEditor.combobox[1], 9)

    GUIEditor.checkbox[1] = guiCreateCheckBox(689, 73, 91, 13, "Dispensável", false, false, GUIEditor.window[2])
    ...
    GUIEditor.label[5] = guiCreateLabel(554, 63, 65, 29, "Público:", false, GUIEditor.window[2])
	...
	GUIEditor.button[5] = guiCreateButton(669, 288+yExtend, 111, 22, "Deletar", false, GUIEditor.window[2])
    guiSetFont(GUIEditor.button[5], "default-bold-small")
    GUIEditor.button[4] = guiCreateButton(554, 315+yExtend, 111, 22, "Fechar", false, GUIEditor.window[2])
    guiSetFont(GUIEditor.button[4], "default-bold-small")
    GUIEditor.button[3] = guiCreateButton(669, 315+yExtend, 111, 22, "Salvar", false, GUIEditor.window[2])
    ...
    addEventHandler("onClientGUIClick", GUIEditor.window[2], function()
        ...
        elseif source == GUIEditor.button[3] then
            ...
            if string.len(title) > 0 and string.len(content) > 1 then
                ...
                if #audiences > 0 then
                    ...
                    guiSetText(GUIEditor.label[3], "Selecione pelo menos um grupo de público-alvo.")
                end
            else
                guiSetText(GUIEditor.label[3], "Por favor, insira o título e o conteúdo.")
            end
		elseif source == GUIEditor.button[5] then
			if confirm then
				...
				guiSetText(GUIEditor.button[5], "Tem certeza?")
				confirm = true
			end
        end
    end)
