...
function showOpmTip()
	closeOpmTip()
	GUIEditor.window[2] = guiCreateWindow(607, 304, 449, 280, "Mensagem Privada Offline", false)
	...
	GUIEditor.label[1] = guiCreateLabel(17, 31, 414, 67, "Mensagem privada offline é um recurso premium que permite enviar uma massagem privada para qualquer jogador, não importa se ele está online ou offline.\n\nO jogador alvo receberá seu PM como uma notificação demonstrada abaixo:", false, GUIEditor.window[2])
	guiLabelSetHorizontalAlign(GUIEditor.label[1], "left", true)
	local perk = exports.donators:getPerks(37)
	GUIEditor.label[2] = guiCreateLabel(17, 197, 414, 30, "Isso custa "..perk[2].." GC(s) por mensagem.\nPara enviar mensagens privadas offline, use /opm", false, GUIEditor.window[2])
...
function startOpm(cmd, username, ...)
	if not username or not (...) then
		...
		GUIEditor.window[1] = guiCreateWindow(664, 283, 438, 295, "Mensagem privada offline", false)
		...
		GUIEditor.label[2] = guiCreateLabel(20, 59, 60, 26, "Mensagem:", false, GUIEditor.window[1])
		...
		addEventHandler("onClientGUIChanged", GUIEditor.edit[1], function()
        	if source == GUIEditor.edit[1] then
        		...
        		if string.len(text) >= 3 then
	        		local count = 0
	        		guiSetText(GUIEditor.label[3], "Procurando..")
	        		...
	        		timer.username = setTimer(function()
	        			...
	        			if username then
	        				...
	        				if count > 4 then
	        					guiSetText(GUIEditor.label[3], "Nome de usuário não encontrado.")
