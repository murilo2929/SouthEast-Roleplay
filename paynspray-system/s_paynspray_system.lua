	...
	if (isElement(element)) and (getElementType(element)=="vehicle") and (matchingDimension) then
		...
		if (thePlayer) then
			...
			if (vehicleHealth >= 1000) then
				outputChatBox("Bem-vindo à oficina mecânica. Seu " .. exports.global:getVehicleName(element) .. " esta bem. Se você tiver algum problema com isso no futuro, volte!", thePlayer, 255, 194, 14)
			else
				...
				if playerMoney == 0 and not factionPlayer[factionVehicle] and not doesPayForRepair(factionPlayer) and not PAYING_FACTIONS[factionVehicle] then
					outputChatBox("Você não pode dar ao luxo de consertar seu carro, desculpe.", thePlayer, 255, 0, 0)
				else
					outputChatBox("Bem-vindo à oficina mecânica. Por favor, aguarde enquanto avaliamos seu " .. exports.global:getVehicleName(element) .. ".", thePlayer, 255, 194, 14)
