...
function sendWarningToCops(theVehicle, thePlayer, speed, x, y, z)
	local direction = "em uma direção desconhecida"
	...
	if dy > math.abs(dx) then
		direction = "Rumo ao norte"
	elseif dy < -math.abs(dx) then
		direction = "Rumo ao sul"
	elseif dx > math.abs(dy) then
		direction = "Rumo ao leste"
	elseif dx < -math.abs(dy) then
		direction = "Rumo ao oeste"
	end
		...
		for key, value in ipairs(teamPlayers) do
			...
			if vehicleOfficerIsIn then
					...
					--if vehicleFaction and vehicleFaction == 1 or vehicleFaction == 2 then
							...
							outputChatBox("[RADIO] Todas as unidades, temos uma infração de trânsito no " .. areaName .. " radar.", value, 0, 210, 255)
							outputChatBox("[RADIO] O veículo era um " .. vehicleColor(color1, color2) .. " " .. vehicleName .. " viajando em " .. tostring(math.ceil(speed)) .. " KM/H.", value, 0, 210, 255)
						if getElementData(theVehicle, "registered") == 1 and getElementData(theVehicle, "show_plate") == 1 and exports.vehicle:hasVehiclePlates( theVehicle ) then
							outputChatBox("[RADIO] As placas são '"..  getVehiclePlateText ( theVehicle ) .."' e o veículo estava indo " .. direction .. ".", value, 0, 210, 255)
						elseif getElementData(theVehicle, "registered") == 0 and getElementData(theVehicle, "show_plate") == 1 and exports.vehicle:hasVehiclePlates( theVehicle ) then
							outputChatBox("[RADIO] O veículo não está registrado e estava indo " .. direction .. ".", value, 0, 210, 255)
						else
							outputChatBox("[RADIO] O veículo não tem placas e estava indo " .. direction .. ".", value, 0, 210, 255)
						end
					--end
				end
			end
