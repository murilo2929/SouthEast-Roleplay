...
function createTrafficCam(thePlayer, commandName, maxSpeed)
	if (exports.integration:isPlayerAdmin(thePlayer)) then
		if not (maxSpeed) then
			outputChatBox("SYNTAX: /" .. commandName .. " [velocidade que irá ativar o radar em KPH]", thePlayer, 255, 194, 14)
		else
			if (tonumber(maxSpeed) > 35) then
				...
				outputChatBox("Ok, armazenado. Agora defina a parte externa da câmera de velocidade com /setradius.", thePlayer, 255, 0, 0)
			else
				outputChatBox("A velocidade para ativar o radar precisa ser superior 35 KPH.", thePlayer, 255, 0, 0)
			end
		end
	end
end
addCommandHandler("addspeedcam", createTrafficCam, false, false)
addCommandHandler("criarradar", createTrafficCam, false, false)
	...
	if (exports.integration:isPlayerAdmin(thePlayer)) then
		...
		if (checkPoint) then
			...
			if (radius > 99) then
				...
				if (id) then
					outputChatBox("Radar criado.", thePlayer, 0, 255, 0)
					loadOneTrafficCam(id)
				else
					outputChatBox("Error ao criar radar.", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function delTrafficCam(thePlayer, commandName)
	if (exports.integration:isPlayerAdmin(thePlayer)) then
		...
		if (colShape) then
			...
			outputChatBox("Você não está em um radar de velocidade.", thePlayer, 255, 0, 0)
		end
	end
end
addCommandHandler("delspeedcam", delTrafficCam, false, false)
addCommandHandler("removerradar", delTrafficCam, false, false)

function getNearbyTrafficCams(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		local posX, posY, posZ = getElementPosition(thePlayer)
		outputChatBox("Radares Proximos:", thePlayer, 255, 126, 0)
		local found = false
		...
		for k, theColshape in ipairs(exports.pool:getPoolElementsByType("colshape")) do
			...
			if (isSpeedcam) then
				...
				if (distance<=20) then
					...
					if (enabled) then
						outputChatBox("   ID " .. dbid .. " - Ativado.", thePlayer, 255, 126, 0)
					else
						outputChatBox("   ID " .. dbid .. " - Desativado.", thePlayer, 255, 126, 0)
					end
					found = true
				end
			end
...
addCommandHandler("radaresproximos", getNearbyTrafficCams, false, false)
