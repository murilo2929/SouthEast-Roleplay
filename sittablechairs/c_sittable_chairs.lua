local chairData =
{
	...
	[1810] = { offset = { -0.25, 0, 1.3 }, rotation = 0 },
	[2121] = { offset = { 0, -0.1, 0.9 }, rotation = 0 },
}
...
function useChair(chair)
	...
	if getElementData(localPlayer, "recovery") then
		outputChatBox("Você não pode sentar em uma cadeira durante a recuperação.", 255, 0, 0)
		return false
	end
	...
	outputChatBox("Agora você está sentado em uma cadeira. Pressione M e clique em você mesmo para se levantar.", 255, 195, 14)
end

function attemptToSitOnChair(chair)
	if ( canISitOnChair(chair) ) then
		if ( chairPerson[chair] ~= nil ) then
			outputChatBox("Esse assento já está ocupado!", 255, 0, 0)
		else
			useChair(chair)
		end
	else
		outputChatBox("Você está muito longe desta cadeira, chegue mais perto!", 255, 0, 0)
	end
end
...
addCommandHandler("levantar", attemptToStandUp)
