...
function lotteryDraw()
	...
	for key, value in ipairs(exports.pool:getPoolElementsByType("player")) do
		if (getElementData(value, "loggedin")==1) then
			outputChatBox("[NEWS] A loteria foi sorteada! Número: " .. lotteryNumber .. ". Não se esqueça de comprar seus bilhetes de loteria!", value, 200, 100, 200)
...
function lotteryCheckJackpot(thePlayer, commandName)
	if getLotteryJackpot() == -1 then
		outputChatBox( "Desculpe alguem ja ganhou na loteria.", thePlayer, 255, 0, 0 )
	else
		outputChatBox( "Jackpot de loteria atual: $" .. getLotteryJackpot(), thePlayer, 0, 255, 0 )
	end
end
addCommandHandler("checkjackpot", lotteryCheckJackpot, false, false)
addCommandHandler("checarjackpot", lotteryCheckJackpot, false, false)

function lotteryCheckNumber(thePlayer, commandName)
	if getLotteryJackpot() == -1 then
		outputChatBox( "Sorry, someone already won the lottery.", thePlayer, 255, 0, 0 )
	else
		outputChatBox("Número da loteria atual: " .. lotteryNumber, thePlayer, 0, 255, 0)
	end
end
addCommandHandler("checknumber", lotteryCheckNumber, false, false)
addCommandHandler("checarnumero", lotteryCheckNumber, false, false)
