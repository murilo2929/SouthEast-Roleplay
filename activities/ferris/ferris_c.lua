...
local tstgs = {
  text = "Pressione '"..bdKey[ 1 ].."' para entrar na cabine como passageiro",       -- Text that will show up when cabin gets closer to a player (Default, change it below also, lines 33 and 65)
  ...
  for i, col in ipairs( t ) do
    addEventHandler( 'onClientColShapeHit', col, function( player, dim )
      if player == localPlayer and dim then
        ...
        tstgs.text = "Pressione '"..bdKey[ 1 ].."' para entrar na cabine";
...
function getInside( key, state, col )
  ...
  tstgs.text = "Pressione '"..bdKey[ 2 ].."' para sair da cabine";
  unbindKey( bdKey[ 1 ], 'down', getInside );
