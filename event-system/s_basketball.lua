...
function checkDunkThing(thePlayer)
    if isElement(thePlayer) and getElementType(thePlayer) == "player" and isPedOnGround(thePlayer) then
		...
		if isHoldingBall then
			...
			exports.global:sendLocalMeAction(thePlayer, "pula e agarra o aro, enterrando a bola na rede.")
	...
	local reachedBoundries=checkBoundries(thePlayer)
	    if isHoldingBall then
	        if reachedBoundries then
				outputChatBox("Não jogue a bola fora da quadra.", thePlayer, 255,0,0)
	        else
				local px, py, pz = getElementPosition(thePlayer)
...
addCommandHandler("passarbola",passBall)
...
function dropBall(thePlayer)
	...
	local isHoldingBall, ballElement = isPlayerHoldingBall(thePlayer)
		...
		exports.global:sendLocalMeAction(thePlayer, "deixa a bola cair.")	
        end
    end
end
addCommandHandler("dropball",dropBall)
addCommandHandler("soltarbola",dropBall)
...
function shootBall(thePlayer,command)
    ...
    if isPedOnGround(thePlayer) then
	    ...
	    if isHoldingBall then
			...
			exports.global:sendLocalMeAction(thePlayer, "tenta arremessar enquanto joga a bola em direção ao aro.")		
			...
			if luck == 1 then
				...
				setTimer(function()
					...
					if scoremessage == 1 then
						exports.global:sendLocalDoAction(thePlayer, "A bola cai no aro e desliza pela rede.")
					elseif scoremessage == 2 then
						exports.global:sendLocalDoAction(thePlayer, "A bola quica na tabela de volta e cai na rede.")
					elseif scoremessage == 3 then
						exports.global:sendLocalDoAction(thePlayer, "A bola rola pela borda do aro e rola para dentro da rede.")
					end
				end, 1300, 1)
			elseif luck > 1 then
				...
				setTimer(function()
					...
					if failmessage == 1 then
						exports.global:sendLocalDoAction(thePlayer, "A bola atinge a borda do aro e quica.")
					elseif failmessage == 2 then
						exports.global:sendLocalDoAction(thePlayer, "A bola quica no quadro e erra o aro.")
					elseif failmessage == 3 then
						exports.global:sendLocalDoAction(thePlayer, "A bola rola pela borda do aro e cai para fora do aro.")
					end
				end, 1300, 1)
		    end
		end
...
addCommandHandler("arremessarbola",shootBall)
...
function basketHelp(thePlayer, commandName)
	for _,boundryShape in pairs(boundries) do
        if isElementWithinColShape(thePlayer,boundryShape) then
			outputChatBox("/pegarbola: Para pegar a bola. Você tem que ficar perto da bola.", thePlayer)
			outputChatBox("/arremessarbola: Para atirar a bola para a cesta mais próxima.", thePlayer)
			outputChatBox("/passarbola: Para passar a bola na direção para a qual você está olhando.", thePlayer)
			outputChatBox("Para enterrar: basta correr para a cesta enquanto segura a bola.", thePlayer)
			outputChatBox("Você não é obrigado a interpretar suas ações, você pode fazer binds para ficar mais facil.", thePlayer)
		end
	end
end
addCommandHandler("baskethelp", basketHelp)
addCommandHandler("basqueteajuda", basketHelp)
...
function enterBoundries(thePlayer, matchingDimension)
	if getElementType(thePlayer) == "player" and matchingDimension and not getPedOccupiedVehicle(thePlayer) and not getElementData(thePlayer, "superman:flying") then 
		outputChatBox("Bem vindo a quadra de basquete. Digite /basqueteajuda para ver os comandos.", thePlayer)
	end
end
