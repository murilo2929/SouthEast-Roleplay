...
function createATM(thePlayer, commandName, withdraw, deposit)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		...
		elseif not (withdraw) or not (deposit) then
			outputChatBox("SYNTAX: /" .. commandName .. " [LIMITE SAQUE / 0 = Infinito] [DEPOSITO - 1 = verdadeiro, 0 = falso]", thePlayer, 255, 194, 14)
		...
		elseif (tonumber(withdraw)) and (tonumber(deposit)) then
			...
			if (tonumber(deposit) > 1) or (tonumber(deposit) < 0) then
				...
				if (id) then
					...
					outputChatBox("ATM criado com ID #" .. id .. "!", thePlayer, 0, 255, 0)
				else
					outputChatBox("Ocorreu um erro ao criar um caixa eletrônico. Tente novamente.", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
addCommandHandler("criaratm", createATM, false, false)
...
addCommandHandler("criaratmrapido", createFastATM, false, false)
	...
	if (exports.integration:isPlayerAdmin(thePlayer)) then
		if not (id) then
			...
			if (counter>0) then -- ID Exists
				...
				outputChatBox("ATM #" .. id .. " Deletado!", thePlayer, 0, 255, 0)
			else
				outputChatBox("ATM com essa ID não existe!", thePlayer, 255, 0, 0)
			end
		end
	end
end
addCommandHandler("delatm", deleteATM, false, false)
addCommandHandler("removeratm", deleteATM, false, false)
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		for k, theObject in ipairs(getElementsByType("object", resourceRoot)) do
			...
			if (distance<=10) then
				...
				outputChatBox("   ATM com ID " .. dbid .. ".", thePlayer)
				count = count + 1
			end
		end
...
addCommandHandler("atmsproximos", getNearbyATMs, false, false)

function getATMName(theAtm)
	return "Maquina ATM ID#"..getElementData(theAtm, "dbid").." em "..exports.global:getElementZoneName(theAtm)
end
