...
function withdrawMoneyPersonal(amount)
	...
	if money >= 0 then
		...
		outputChatBox("Você sacou $" .. exports.global:formatMoney(amount) .. " de sua conta pessoal.", client, 255, 194, 14)
		exports.logs:dbLog(client, 25, client, "WITHDRAW " .. amount.. " - REMAINING $"..money)
	else
		outputChatBox( "Não.", client, 255, 0, 0 )
	end
end
...
function depositMoneyPersonal(amount)
	...
	if exports.global:takeMoney(client, amount, nil, true) then
			...
			outputChatBox("Você depositou $" .. exports.global:formatMoney(amount) .. " em sua conta pessoal.", client, 255, 194, 14)
			exports.logs:dbLog(client, 25, client, "DEPOSIT " .. amount.. " - REMAINING $"..money+amount)
	else
		outputChatBox("Você não tem esse valor em um item de dinheiro classificado.", client, 255, 194, 14)
	end
end
...
function withdrawMoneyBusiness(amount, factionID)
	...
	if exports.global:takeMoney(theTeam, amount) then
		if exports.global:giveMoney(client, amount) then
			...
			outputChatBox("Você sacou $" .. exports.global:formatMoney(amount) .. " da sua conta comercial.", client, 255, 194, 14)
...
function depositMoneyBusiness(amount, factionID)
	...
	if exports.global:takeMoney(client, amount) then
		...
		if exports.global:giveMoney(theTeam, amount) then
			...
			outputChatBox("Você depositou $" .. exports.global:formatMoney(amount) .. " em sua conta comercial.", client, 255, 194, 14)
...
function transferMoneyToPersonal( fromFactionId, name, amount, reason )
	if getElementData( client, "loggedin" ) == 1 then
		...
		if not amount or not tonumber(amount) or tonumber(amount) <=0 then
			return not outputChatBox( "Quantia inválida para transferir.", client, 255, 0, 0 )
		else
		...
		if fromFactionId then
			...
			if not sender then 
				return not outputChatBox( "Falha na transação. Erro de código: 147", client, 255, 0, 0 )
			end
			...
			if res then
				if nums > 0 then
					...
					if tonumber(res[1]['account']) == tonumber(senderAccountID) then -- Uh oh... bad boy!
						exports.global:sendWrnToStaff(exports.global:getPlayerFullIdentity(sender).." ("..getPlayerName(sender).." para "..receiver_name..") tentou um ALT para ALT por meio de transferência bancária, mas foi interrompido por script.", "ALT>ALT")
						return not outputChatBox( "Você não pode transferir dinheiro para um de seus personagens alternativos.", client, 255, 0, 0 )
					end
				else
					return not outputChatBox( "Destinatários não encontrados com '"..name.."'. Tente com um nome de facção ou um nome de personagem.", client, 255, 0, 0 )
				end
			else
				dbFree( qh )
		...
		if sender == receiver then
			return not outputChatBox( "Você não pode transferir dinheiro para si mesmo. ", client, 255, 0, 0 )
		end
		...
		if hasBankMoney( sender, amount ) then
			if takeBankMoney( sender, amount ) then
				...
				return outputChatBox("Você transferiu $" .. famount .. " de sua conta "..(fromFactionId and "de negócio" or "pessoal").." para "..receiver_name..".", client, 255, 194, 14)
			else
				return not outputChatBox("Interal Error! Code 197", client, 255, 0, 0 )
			end
		else
			return not outputChatBox("Falha na transação. Saldo insuficiente.", client, 255,0, 0)
		end
	end
end
