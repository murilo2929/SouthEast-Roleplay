...
function copyATMCardNumber(text)
	if setClipboard(text) then
		exports.hud:sendBottomNotification(getLocalPlayer(), "Maquina ATM", "Copiado '"..text.."' para a área de transferência.")
	end
end
...
function isThisTransactionWithinLimitation(cardNumber,amount, limitType)
	...
	if total > limits[limitType] then
		return false, "Você só pode sacar/transferir $"..exports.global:formatMoney(limits[limitType]).." deste cartão ATM por dia ((Cada "..math.floor(limitedInterval/60/60/1000).." hora(s)))\n Você já fez um total de $"..exports.global:formatMoney(total-amount).." valor de transações recentemente."
	end
