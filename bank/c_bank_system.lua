...
--[[local bankPed = createPed(150, 1108.01953125, -767.5478515625, 976.25158691406)
setPedRotation(bankPed, 90.4609375)
setElementInterior(bankPed, 5)
setElementDimension(bankPed, 10)]]
...
function startFactionSelect(fTable, selectiontype)
    ...
    factionSelectGUI.window[1] = guiCreateWindow(766, 385, 399, 121, "Selecione a facção para ver", false)
    ...
    factionSelectGUI.combobox[1] = guiCreateComboBox(13, 35, 366, 113, "Selecione a facção para ver", false, factionSelectGUI.window[1])
    factionSelectGUI.button[2] = guiCreateButton(268, 64, 111, 42, "Aceitar", false, factionSelectGUI.window[1])
    ...
    addEventHandler("onClientGUIClick", factionSelectGUI.button[2], function()
    	...
    	if name ~= "Selecione a facção para ver" then
    		...
    		outputChatBox("Selecione uma facção.", 255, 0, 0)
    	end
    end, false)
...
function fillBusiness(name)
	if wBank and isElement(wBank) then
		...
		if faction then
			...
			guiSetText(lBalanceB, "Saldo: $" .. exports.global:formatMoney(gfactionBalance))
		end
	end
end
...
function showBankUI(fTable, depositable, limit, withdrawable, ped)
	if not (wBank) then
		...
		tabPersonalTransactions = guiCreateTab("Transações Pessoais", tabPanel)

		local hoursplayed = getElementData(localPlayer, "hoursplayed") or 0
		if (exports.global:countTable(fTable) > 0) then
			tabBusiness = guiCreateTab("Banco de Negócios", tabPanel)

			lBalanceB = guiCreateLabel(0.1, 0.05, 0.9, 0.05, "Saldo: $", true, tabBusiness)
			guiSetFont(lBalanceB, "default-bold-small")

			if (withdrawable) then
			-- WITHDRAWAL BUSINESS
				lWithdrawB = guiCreateLabel(0.1, 0.15, 0.2, 0.05, "Sacar:", true, tabBusiness)
				...
				bWithdrawB = guiCreateButton(0.44, 0.13, 0.2, 0.075, "Sacar", true, tabBusiness)
				addEventHandler("onClientGUIClick", bWithdrawB, withdrawMoneyBusiness, false)
			else
				lWithdrawB = guiCreateLabel(0.1, 0.15, 0.5, 0.05, "Este caixa eletrônico não oferece suporte para a função de retirada.", true, tabBusiness)
				guiSetFont(lWithdrawB, "default-bold-small")
			end
			...
			if (depositable) then
				...
				lDepositB = guiCreateLabel(0.1, 0.25, 0.5, 0.05, "Este caixa eletrônico não suporta a função de depósito.", true, tabBusiness)
				guiSetFont(lDepositB, "default-bold-small")
			...
			if hoursplayed >= 12 then
				...
				bTransferB = guiCreateButton(0.44, 0.43, 0.2, 0.075, "Transferir para", true, tabBusiness)
				addEventHandler("onClientGUIClick", bTransferB, transferMoneyBusiness, false)

				eTransferB = guiCreateEdit(0.66, 0.43, 0.3, 0.075, "<Número da conta de banco>", true, tabBusiness)
				addEventHandler("onClientGUIClick", eTransferB, function()
					if guiGetText(eTransferB) == "<Número da conta de banco>" then
						guiSetText(eTransferB, "")
					end
				end, false)

				lTransferBReason = guiCreateLabel(0.1, 0.55, 0.2, 0.05, "Razão:", true, tabBusiness)
				guiSetFont(lTransferBReason, "default-bold-small")

				tTransferBReason = guiCreateEdit(0.22, 0.54, 0.74, 0.075, "<Para que serve esta transação?>", true, tabBusiness)
				addEventHandler("onClientGUIClick", tTransferBReason, function()
					if guiGetText(tTransferBReason) == "<Para que serve esta transação?>" then
			...
			-- TRANSACTION HISTORY
			tabBusinessTransactions = guiCreateTab("Transações Comerciais", tabPanel)
		...
		bClose = guiCreateButton(0.75, 0.91, 0.2, 0.1, "Fechar", true, wBank)
		...
		--shit
		lBalance = guiCreateLabel(0.1, 0.05, 0.9, 0.05, "Saldo: $" .. exports.global:formatMoney(balance), true, tabPersonal)
		guiSetFont(lBalance, "default-bold-small")

		if withdrawable then
			-- WITHDRAWAL PERSONAL
			lWithdrawP = guiCreateLabel(0.1, 0.15, 0.2, 0.05, "Sacar:", true, tabPersonal)
			...
			bWithdrawP = guiCreateButton(0.44, 0.13, 0.2, 0.075, "Sacar", true, tabPersonal)
			addEventHandler("onClientGUIClick", bWithdrawP, withdrawMoneyPersonal, false)
		else
			lWithdrawP = guiCreateLabel(0.1, 0.15, 0.5, 0.05, "Este caixa eletrônico não oferece suporte para a função de retirada.", true, tabPersonal)
			guiSetFont(lWithdrawP, "default-bold-small")
		end
		...
		if (depositable) then
			...
			lDepositP = guiCreateLabel(0.1, 0.25, 0.5, 0.05, "Este caixa eletrônico não suporta a função de depósito.", true, tabPersonal)
			guiSetFont(lDepositP, "default-bold-small")
		...
		if hoursplayed >= 12 then
			...
			bTransferP = guiCreateButton(0.44, 0.43, 0.2, 0.075, "Transferir para", true, tabPersonal)
			addEventHandler("onClientGUIClick", bTransferP, transferMoneyPersonal, false)

			eTransferP = guiCreateEdit(0.66, 0.43, 0.3, 0.075, "<Player/Nome Facção>", true, tabPersonal)
			...
			lTransferPReason = guiCreateLabel(0.1, 0.55, 0.2, 0.05, "Razão:", true, tabPersonal)
			guiSetFont(lTransferPReason, "default-bold-small")

			tTransferPReason = guiCreateEdit(0.22, 0.54, 0.74, 0.075, "<Para que serve esta transação?>", true, tabPersonal)
		...
		--outputChatBox("Welcome to the Banco de Los Santos")
...
function withdrawMoneyPersonal(button)
	if (button=="left") then
		...
		if not amount or amount <= 0 or math.ceil( amount ) ~= amount then
			outputChatBox("Por favor, insira um valor positivo!", 255, 0, 0)
		elseif (amount>money) then
			outputChatBox("Você não tem fundos suficientes.", 255, 0, 0)
		elseif not _depositable and limitedwithdraw ~= 0 and oldamount + amount > limitedwithdraw then
			outputChatBox("Este caixa eletrônico só permite que você retire $" .. exports.global:formatMoney( limitedwithdraw - oldamount ) .. ".")
		else
...
function transferMoneyPersonal(button)
	if (button=="left") then
		...
		elseif (amount>money) then
			outputChatBox("Você não tem fundos suficientes.", 255, 0, 0)
		elseif reason == "" then
			outputChatBox("Insira um motivo para a transferência!", 255, 0, 0)
		elseif playername == "" then
			outputChatBox("Por favor, digite o nome completo do caractere do receptor!", 255, 0, 0)
		else
...
function getTransactionReason(type, reason, from)
	...
	elseif type == 10 then
		...
		return "Transferir: " .. tostring(reason or "")
	end
end
...
function checkDataChange(dn)
	if wBank then
		if dn == "bankmoney" and source == localPlayer then
			guiSetText(lBalance, "Saldo: $" .. exports.global:formatMoney(getElementData(source, "bankmoney")))
